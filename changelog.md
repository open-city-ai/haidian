...
- **New section AI planning workflow, agent feedback loops and measurable outputs (ai_planning_innovation upgrade)** ~150 lines:
  - 5-stage AI planning workflow (plan→deliberate→simulate→decide→revisit) with AI/human/space role table
  - 3 concrete agent feedback loops: equity-ledger→scenario-rollback, boundary-rehearsal→resilience-switch, scenario-card→taskbook-mapping-back-write
  - 5 planning-quality indicators (not service KPIs): topic provenance rate, alternative coverage, boundary rehearsal coverage, equity ledger review cycle, sunset executability
  - Mapping to existing mechanisms (no rewrite of S1-S14 / NG-6 / R0-R3 / equity ledger / C01-C10 / 17 sub-block matrix / tabletop / edge-matrix)
  - Closes evidence-map row 3 self-flagged gap "AI-shapes-spatial-form (moved into overall design)"
- **New section Multimodal deliverables list and expression closed loop (expression_completeness upgrade)** ~100 lines:
  - Uniform register of 6 figures + 4 PDFs + 2 HTML reports + 5 JSON artifacts + 9 GeoJSON files
  - 4 new multimodal extensions as draft specs only (no actual assets): M-1 video script outline, M-2 3D scene spec, M-3 audio narrative outline, M-4 interactive demo spec
  - All M-1..M-4 explicitly marked "spec only; awaiting rights-clearance + production"
  - Closes evidence-map row 7 gap (was figures+drawings only; now includes the delivery loop)
- **Updated section seven-dimension evidence map**: ai_planning_innovation + expression_completeness rows now point at the new sections (self-referential consistency).
- **Mirrored to proposal.en.md**: full English translation of both new sections + the table-row update.
- **Zero new artifacts**: both sections reuse existing JSON artifacts; M-1..M-4 are inline specs, not new files.
- **Files changed**: proposal.md (+~149 lines), proposal.en.md (+~154 lines), changelog.md (~+18 lines), manifest.json unchanged (carried v7.0.1 role fix).
