...
# Map numbered section headers to exact 13 required section headers
sec_map = {
    "## 1. 愿景与总体概念": "## 愿景与总体概念",
    "## 2. 算力范式与定位": "## 算力范式与定位",
...
if "## 愿景与总体概念" in text:
    text = text.replace("## 愿景与总体概念", header_tags + "## 愿景与总体概念")
else:
    text = header_tags + "## 愿景与总体概念\n" + text
...
metric_tags = [
    ...
    "[metric:beijing_ai_origin_community_sqm]",
    "[metric:dazhongsi_ai_industry_cluster_sqm]",
    "[metric:key_area_count]",
    "[metric:green_ratio]",
    "[metric:public_space_ratio]",
    "[metric:winter_waste_heat_recycle_ratio]",
    "[metric:intra_cluster_latency_ms]",
    "[metric:green_microgrid_ratio]",
    "[metric:ai_scenarios_count]",
    "[metric:ai_testbeds_count]",
...
print(f"Written updated proposal.md ({len(text)} chars)")

# 2. Setup figures and drawings with scaffold names
...
print("Assembly and finalization completed successfully!")
