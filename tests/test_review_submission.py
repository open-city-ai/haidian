...
""",
            ...
            boundary = review_input["source_registry_summary"]["participant_source_boundary"]
            self.assertIn("package's own sources.json", boundary)
            self.assertIn("must not by itself reduce risk_compliance", boundary)
            self.assertIn("create a required action or gate failure", boundary)
            self.assertEqual(review_input["source_registry_summary"]["counts"]["total"], 0)
            self.assertEqual(
            ...
            self.assertIn("cause an adverse recommendation", prompt)
