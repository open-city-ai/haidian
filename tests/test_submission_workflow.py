...
class SubmissionWorkflowTests(unittest.TestCase):
    ...
    def write_minimal_ai_package(self, root: Path, base: str) -> list[str]:
        ...
        self.write_bytes(root, f"{base}/drawings/a3-booklet.pdf", b"%PDF-1.4\n%%EOF\n")
        self.write_bytes(root, f"{base}/drawings/a0-boards.pdf", b"%PDF-1.4\n%%EOF\n")
        self.write(
            root,
...
<figure><img src="../assets/figures/metrics-evidence.png" alt="核心指标复算与证据链图"></figure>
    ...
    def test_user_cannot_modify_generated_gallery_data(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse(report.ok)
            self.assertIn("participants must not edit generated gallery data", "\n".join(report.errors))
...
<img src="../assets/figures/site-overview.png">
    ...
    def test_non_zh_language_fails(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = validate_submission(root, "alice", changed)
            self.assertFalse(report.ok)
            self.assertIn("language must be zh", "\n".join(report.errors))
