...
5. 替换全部 scaffold 内容和占位图纸后运行 `scripts/finalize_submission.py`，再运行 `scripts/self_check_submission.py`。如果终结后又修改了 manifest 中列出的文件，先运行 `scripts/refresh_manifest_hashes.py <submission-dir>`；该工具只刷新文件哈希，不会改变包状态。provisional boundary 必须保留精度与复算提示，但组织方数据缺口不阻断内容评分。
