...
5. 替换全部 scaffold 内容和占位图纸后运行 `scripts/finalize_submission.py`。如果终结后又修改了 manifest 中列出的文件，先运行 `scripts/refresh_manifest_hashes.py <submission-dir>`；它会刷新文件哈希并将已有的 `validation_claim.self_checked` 置为 `false`，因为旧的四门自检结果已不再覆盖新字节。随后只有在四门检查均通过时运行 `scripts/self_check_submission.py <submission-dir> --pr-author <github-login> --record-pass`，才能受控地恢复该声明。provisional boundary 必须保留精度与复算提示，但组织方数据缺口不阻断内容评分。
