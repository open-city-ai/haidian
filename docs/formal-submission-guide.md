...
公开资料登记表位于 `data/source_registry.json`，处理规则见 `docs/data-workflow.md`。agent 必须区分 `usable_for_formal="yes"`、`background_only` 和 `provisional_only`：formal 依据只能来自已批准的 formal 可用资料；背景资料不能支撑空间控制结论；provisional 资料只能用于 intake、可视化和讨论。

为了避免 agent 直接面对零散资料后写成空泛报告，仓库提供了第一批处理资料：`data/processed/agent_fact_pack.md`、`project_scope_summary.csv`、`agent_task_requirements.csv`、`source_use_matrix.csv` 和 `missing_data_checklist.csv`。参赛者应先用这些文件建立任务清单、范围结构、资料用途和缺口清单，再在 `proposal.md` 中把它们翻译成可读的设计论证。处理资料不能替代原始来源；正文仍必须回引 `source_registry.json` 中的 `source_id`。
...
使用临时粗略边界时，自检可以 PASS intake，但 `can_enter_formal_review=false`。也就是说，方案可以先提交、展示和讨论；等 official polygon 补齐后，才能进入正式专业评分。
...
5. 运行 `scripts/self_check_submission.py`，确认 boundary、重点区域、拓扑、面积和 HTML 均通过。若使用 provisional boundary，自检可 PASS intake，但会输出不可进入 formal professional scoring 的提示。
...
全部 PASS 只说明 package 具备进入提交入口和机器审查的基础条件。若 boundary/key areas 仍为 provisional，自检会返回 `can_enter_formal_review=false`，表示可以作为临时 formal intake 方案查看，但不能进入正式专业评分。PASS 不代表方案一定优秀，也不代表获得官方批准。

维护者审核 PR 时会运行 `scripts/maintainer_review.py --comment`，并在本地忽略目录 `.maintainer-review/<proposal-slug>/` 生成 `maintainer-comment.md`、`review-summary.json`、`review-input.json`、`review-prompt.md` 和 `advisory-review.md`。维护者只把命令输出复制到 PR comment；maintainer review 结果不提交到仓库，也不进入公开展示页。合并后展示页索引由维护者运行 `scripts/generate_submissions_data.py` 生成；参赛者只提交自己的 `submissions/<github-login>/<proposal-slug>/` 目录，不修改 `submissions-data.js`。
...
4. 接受自检中的 `can_enter_formal_review=false` 结果，并列明替换 official polygons 后需要重算的图层和指标。
