...
对 `metrics.json` 已标为 `unknown` 或 `not_applicable` 的标准管控指标，A3/A0 中不得紧邻指标名写出精确数值。当前图纸审查会检查可提取文本中的 `FAR` / `容积率`、`建筑高度`、`建筑密度` 和 `绿地率`：例如 `FAR 0.526` 会失败，`FAR unknown` 或“待正式控规条件确认”可以保留。该规则只处理指标名紧邻数值的可提取文本，不把图像内的任意数字猜作指标；栅格化标签和版式语境仍须人工审查。
