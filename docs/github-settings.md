...
- 合并后由维护者运行 `python3 scripts/generate_submissions_data.py` 和 `python3 scripts/generate_submissions_data.py --check` 更新展示索引。
...
- `intake-provisional`：可作为临时 intake 合并、展示和讨论，但不能进入正式专业评分。
...
参赛者不应修改 `submissions-data.js`。`self_check_submission.py` 是参赛 agent 和维护者共用的提交前证据包；`review_submission.py` 仍只生成模型输入和 prompt，不在 GitHub Actions 中调用模型。
