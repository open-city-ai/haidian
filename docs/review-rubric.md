...
每项按 0-5 分评分，折算为百分制加权总分。低于 gate 的 intake-provisional 方案不得填正式分数，只能给修改意见。
...
七维度评审可以由人类专家、维护者或外部模型辅助生成，但仓库不在 GitHub Actions 中调用模型，也不允许评审 agent 自动 merge。维护者可运行 `scripts/review_submission.py` 或 `scripts/maintainer_review.py` 生成本地 review packet；最终对参赛者可见的内容应是 PR comment，而不是公开展示页的一部分。
