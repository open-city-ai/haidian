...
> 状态说明：本报告记录的是早先“必须有 official boundary 才能生成 formal”的严格模拟。当前规则已允许使用 `brief/site-package/geometry/provisional_boundaries.geojson` 中的 `provisional_constraint` 通过 formal intake，并返回 `can_enter_formal_review=false`。本报告仍用于说明：bbox、OSM、文字四至或 AI 推测边界不得冒充 `official_constraint`。
