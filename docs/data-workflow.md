...
- `data/source_registry.json`：公开资料登记表。它记录资料是否可作为 formal 依据、背景资料或 provisional intake 线索。
...
- `provisional_only`：只可用于 intake、自检、可视化或设计讨论，不能进入正式专业评分。
...
4. 读取 `data/source_registry.json`，只选择 `review_status` 为 `approved` 或 `provisional` 的资料。
