...
The package passes intake validation, spatial review, visual review, and professional evidence review. It is correctly classified as `intake-provisional`, not `formal-review-ready`, because the site boundary and three key-area polygons are still provisional.
...
Until these are available, the correct behavior is exactly what this rehearsal shows: allow a readable, topology-clean, self-checked formal intake package, but keep `can_enter_formal_review=false`.
