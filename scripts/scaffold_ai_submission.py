...
def make_proposal(
    ...
    if boundary_mode == "official" and key_area_mode == "official":
        ...
        summary_phrase = "基于 provisional boundary 和结构化自检要求生成的 formal intake AI 城市设计方案包；可通过提交入口，但不能进入正式专业评分。"
    ...
    boundary_readiness_zh = "官方边界就绪" if boundary_mode == "official" and key_area_mode == "official" else "临时边界 intake，暂不可正式评分"
...
边界和重点区域的可读解释对应 [data:geometry/site_boundary.geojson#SITE-001]、[data:geometry/key_areas.geojson#{key_ref_1}] 和 [metric:site_area_sqm]、[metric:key_area_count]。这意味着读者可以从正文回到 GeoJSON 查看边界来源、从 metrics 查看面积复算结果、从 sources 查看资料来源，而不是只相信一段文字判断。
