...
def can_enter_formal_review(stage: str, report: dict[str, Any]) -> bool:
    if stage != "formal" or not report.get("ok"):
        return False
    deterministic = report.get("deterministic_validation", {}).get("stdout", {})
    if isinstance(deterministic, dict):
        warnings = "\n".join(str(item) for item in deterministic.get("warnings", []) or [])
        if "cannot enter formal" in warnings or "non-official" in warnings:
            return False
    return True
...
def format_markdown(report: dict[str, Any]) -> str:
    ...
    lines.append(f"Visual review: {'PASS' if visual.get('ok') else 'FAIL'}")
