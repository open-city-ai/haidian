...
def validate_geojson_file(
    ...
    if geometry_name == "site_boundary.geojson":
        if stage == "formal" and official_boundary_features == 0 and provisional_boundary_features == 0:
            ...
            report.add_warning(
                f"{display_path}: uses provisional boundary; submission may pass intake, but cannot enter formal professional scoring until official SITE_BOUNDARY is supplied"
            )
    return len(features)
...
def validate_manifest_file(report: ValidationReport, repo_root: Path, proposal_dir: str) -> tuple[dict | None, str]:
    ...
    if not isinstance(files, list) or not files:
        ...
        for index, item in enumerate(files):
            ...
            if not (repo_root / proposal_dir / safe_path).exists():
                report.add_error(
...
def validate_proposal_file(
    ...
    if language and language != "zh":
        report.add_error(f"{proposal_path}: language must be zh; English may only appear as an appendix")
    ...
    for path in normalized_files:
        ...
        if not report.maintainer_bypass:
            if path in PARTICIPANT_PROTECTED_GLOBAL_FILES:
                report.add_error(
                    f"{path}: participants must not edit generated gallery data; maintainers run scripts/generate_submissions_data.py after merge"
                )
                continue
