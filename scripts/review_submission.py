...
def build_prompt(review_input: dict) -> str:
    return "\n".join(
        [
            ...
            "",
            "Important: deterministic validation and spatial review results are evidence. Treat blocking self-checks and participant-controlled known blockers as serious readiness limits.",
            "Missing organizer-supplied official geometry is advisory only: preserve provisional warnings and recalculation duties, but do not block content scoring or lower any rubric score for that data gap itself.",
