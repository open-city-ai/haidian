...
- **AI 场景**：合流测试场、低速配送共路、上游评测场（SCENE-01/02/03）；
...
- **AI 场景**：合流公议站、AI+文化导览（SCENE-04/06）；
...
- **AI 场景**：企业服务助手、年度合流日演练（SCENE-08/12）；
...
十二张场景卡 [metric:scenario_card_count] 均写入 [data:geometry/constraints.geojson#SCENE-01] 至 SCENE-12，逐卡标注空间位置、服务对象、类型与数据隐私边界：
...
| 04 | 合流公议站（市民提交合并请求，人工评审） | 原点社区 [SCENE-04] | 市民/共创者 | 公共服务 | 仅公开议题与意见摘要，无个人追踪；采纳人工决定 |
...
| 08 | 企业服务助手 | 大钟寺发行区 [SCENE-08] | 企业 | 产业服务 | 实名信息双向授权；争议交易人工复核 |
...
每个合流(场景准入)须通过"**合流五可**"检验,否则不进入开放运营 [depth:municipal_new_infrastructure]：①**可见**——现场说明牌公开用途、输入、输出、风险、责任人、运行状态与日落日期；②**可用**——等价非数字通道与无障碍路径与主线同时到位，即"支流不降级"；③**可议**——异议入口、合流公议与**四类公共回执**（已采纳/部分采纳/不采纳/待核实，每条附理由、负责人、复核节点与申诉渠道）；④**可责**——人工终审、申诉、更正、急停与日落条款（试点不自动续期）；⑤**可回**——预约令牌到期归还、撤场时间窗、数据删除证据与年度 Undo 演练。每张卡除上述五要素外，补充"准入闸门—暂停阈值—退出条件"三字段，统一走合流五可与回程预算。

临时 AI 占用公共空间实行"**合流令牌**"预约互锁（借鉴单线铁路令牌闭塞制度）：令牌授予具名运营者一个区段、一个时段，附数据边界与人工复核点，到期强制归还；同一区段同时仅一枚令牌，不得圈占整座站前广场或连续全天运行；异议挂起预约待人工审查，防止"开放区"悄然变永久占用。
