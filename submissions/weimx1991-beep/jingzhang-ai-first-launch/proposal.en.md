...
- **Copyright authorization (registered scope)**: AI-generated images are recorded in `report/copyright_statement.md` and `sources.json` with their generation tools; fonts, images, trademarks and basemaps are cleared item by item before use, third-party assets are registered in the rights ledger, and any asset whose rights cannot be proven will be replaced or removed.
