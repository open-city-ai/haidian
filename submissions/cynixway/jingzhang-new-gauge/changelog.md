...
边界条件矩阵 + 渐进式暴露 + 可核验证据列深化（针对 intake 66 的结构差距）

- **新增「评审首屏摘要」**（首屏 300 字）：4 句概括核心命题 + 3 差异化 + 诚实约束 + 七维度自评
- **新增「12 边界条件 × 7 应急路径」矩阵**：EDGE-01..EDGE-12（参照 87pt edgecase）+ 7 应急路径（对应 NG-6 步骤），每行带 [data:]/[source:]/[metric:] 可核验证据列
- **新增「渐进式暴露梯度 T1→T2→T3」**：T1 Controlled Edge Court 众智园 / T2 Human Override Commons AI原点 / T3 Market Edge Forum 大钟寺——与 v5.3 S0/S1/S2/S3 韧性态形成测试×降级两个正交轴
- **重写三重点区与 9 子片区**：每个重点区有明确主场定位 + 主导 2-3 条边界条件 + 锚定到具体 sub-precinct
- **SC-04 试点升级**：从 4 张合成工单（无法验证退路）升级为 3 条边界 + 1 控制样本（可验证退路），试点目标从"工单可复跑"升级为"3 条边界的人工退路可复跑"
- **新增「自评七维度」**：内部 7 维度评分卡，自评 4.30/5 ≈ 86/100（与 intake 评分不直接对应）
