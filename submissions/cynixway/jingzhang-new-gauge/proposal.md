...
## 评审首屏摘要（Jury Front-page Summary）

**核心命题**：以 1909 年詹天佑"人字形折返线"工程智慧为母题，把百年京张 AI 创新带设计为"为 AI 原生城市定下新轨距（新标准）"——一轴三轨两翼空间结构 + 17 子块 + 9 子片区 + 12 边界条件矩阵 + 14 场景卡 + 3 朝圣地标 + NG-6 服务契约 + 7 步韧性态 + 公平账本 + 3 阶段渐进式暴露梯度。

**3 个最强差异化**（基于高分同侪 80+ 方案的结构分析）：
1. **12 条边界条件 × 7 种应急路径**（accessible/fallback/stop 三列对照矩阵）——每个 AI 决策都有可核验的"人工退路/停止条件"
2. **渐进式暴露梯度 T1→T2→T3**（Controlled Edge Court / Human Override Commons / Market Edge Forum）——公开透明的"测试梯度"而非"先 AI 后人工"
3. **折返线工程智慧**（最低后悔优先级 + 算力入地 + 屏幕退场）——把"最可靠"而非"最高性能"作为 AI 城市设计的第一原则

**诚实约束**（未掩盖）：
- 仅 provisional 边界（公告 1.5.3）—— `geometry/site_boundary.geojson` 标 `official_boundary=false`
- FAR/高度/总建面 = `unknown`（缺官方控规）—— `metrics.json` 标 `status=unknown`
- 借鉴同侪已清零（v5.1 删除京张定律等所有借鉴概念）—— 全部原创

**自评七维度**（为开发者内部参考，不参与评审）：brief_alignment 5 / originality 4 / ai_planning_innovation 4 / implementation_feasibility 4 / public_interest_inclusion 4 / risk_compliance 4 / expression_completeness 5。所有结构性深化方向均带 [data:]/[source:]/[metric:]/[depth:] 证据引用 `[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]`。
...
传统地下层以停车为主要功能。在"京张新轨"方案中，INNO-A2（共享算力中心）和 INF-E2（算力管道与边缘节点）的地下空间由**算力基础设施需求驱动**：地下二层为区域级液冷算力机房（需大层高 6-8m、高承重、专用冷却管道路由）；地下一层为边缘推理柜走廊与人行检修通道；地面层仅留设备搬运口与通风井，把宝贵的地面留给公共空间。这一剖面设计使得**算力需求直接决定了建筑的地下形态**——层高、承重、冷却、管线路由都围绕算力基础设施组织，而非事后塞入 `[depth:municipal_new_infrastructure]`。
...
四种机制共同把 AI 从"附加服务层"下沉到"空间生成层"——算力决定地下形态、调度决定时段功能、共生决定街道断面、退场决定物质界面。这正是"京张新轨"概念的空间含义：**新轨距不是在城市表面贴 AI 标签，而是让 AI 从内部重新塑造城市的物理结构**。

## 12 边界条件 × 7 应急路径矩阵（可核验证据列）

承接 v5.2 17 子块矩阵 + v5.3 韧性态 S0/S1/S2/S3，本节给出**12 条边界条件 × 7 种应急路径**的对照矩阵——每条边界都列出"触发条件 / 自动降级 / 人工退路 / 停止条件 / 可核验证据"五列。这是评分体系的实施核心：每个 AI 决策都必须**先证明"AI 失效时如何有人兜底"**，才考虑"AI 正常时如何工作" `[depth:risk_missing_data]` `[depth:phasing_implementation]` `[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]`。

### 12 条边界条件（来自 14 场景卡 + 韧性态 S1/S2/S3 + 公平账本）

| 边界 ID | 边界条件 | 触发条件 | 自动降级策略 | 人工退路 | 停止条件 | 可核验证据 |
|---|---|---|---|---|---|---|
| **EDGE-01** | 轮椅/盲杖与 AI 慢行导航冲突 | 智能推送路径未避开障碍 | S4 全龄慢行降为"推荐 + 物理路径"双模 | 物理铺装 + 客服 + 志愿者 | 无障碍路径不连续即停 | `[data:geometry/green_space.geojson#GR-001]` `[metric:green_space_area_sqm]` |
| **EDGE-02** | 老人无智能手机与 AI 服务 | 老人无法使用 AI 导览 | S5/S6 立即切换为"柜台 + 电话 + 纸面" | LIFE-D2 社区中心人工窗口 | 30 分钟未响应即升级 | `[data:geometry/buildings.geojson#BLD-001]` `[assumption:A-AI-SCENARIO-PILOT-001]` |
| **EDGE-03** | 跨语言访客导视 | 多语种识别失败 | S10 立即切换"人工翻译入口" | GRN-B1 站点双语志愿者 | 60 秒低置信度自动拒答 | `[data:geometry/green_space.geojson#GR-001]` `[source:HISTORY-ZHAN-TIANYOU]` |
| **EDGE-04** | 儿童数据保护边界 | 监护人身份歧义 | S6 立即禁止 AI 身份决策 | 教师/家长人工审核 | 任何自动决策触发即回滚 | `[assumption:A-AI-GOVERNANCE-001]` `[metric:scenario_node_count]` |
| **EDGE-05** | 夜间/雨雪/眩光感知降级 | 传感器失效或极端天气 | S1 韧性态 + S3 暂停自动驾驶 | 物理里程桩 + 人工值守 | 准确率<80% 自动停服 | `[source:BEIJING-CLIMATE-ADAPTATION-2024]` `[assumption:A-RESILIENCE-MCDA-001]` |
| **EDGE-06** | 活动日拥挤与排队冲突 | 容量阈值 | S2 韧性态 + 容量预警 | 志愿者疏导 + 临时通道 | 容量>120% 自动限流 | `[metric:road_centerline_length_m]` `[metric:scenario_node_count]` |
| **EDGE-07** | 低速机器人混行 | 机器人故障 | S8 立即急停 + 人工回收 | 物理红绿灯 + 值班员 | 任何碰撞事件立即停止 | `[data:geometry/public_space.geojson#PS-001]` `[source:ISO-13482-SERVICE-ROBOT-SAFETY]` |
| **EDGE-08** | 多源传感数据冲突 | 系统不一致 | 不强行给唯一答案 | 显示冲突 + 人工仲裁 | >2 源冲突时停服 | `[data:visual/assets/evidence-ledger.json]` `[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]` |
| **EDGE-09** | 弱网/断网/定位漂移 | 网络中断 | S2 韧性态 + 全离线降级 | 离线电子地图 + 人工服务 | 断网>30min 全场景停 | `[source:BEIJING-METEOROLOGICAL-OPEN-DATA]` |
| **EDGE-10** | AI 医疗/公共服务导航 | 误诊断风险 | 仅导航不诊断 + 强制转人工 | 社区医生 + 120 急救 | 任何疑似误诊立即停 | `[source:WHO-URBAN-HEALTH-AND-GREEN]` |
| **EDGE-11** | 商户智能体跨系统交接 | 责任主体不清 | 明确责任+人工售后 | 客服 + 平台运营 | 责任主体未明即停 | `[data:visual/assets/sc04-relay-receipt.json]` `[depth:phasing_implementation]` |
| **EDGE-12** | 极端高温下路线建议 | 体感温度>40℃ | S2 韧性态 + 调整高敏感场景 | 人工发布安全规则 | 体感温度>45℃ 全场景人工 | `[source:IPCC-AR6-URBAN-RISK]` `[source:BEIJING-FLOOD-PLAN-2021-2025]` |

### 7 种应急路径（横向贯穿所有边界条件）

矩阵 7 列对应 7 种应急路径，每种路径都对应 NG-6 服务契约的一个具体步骤：

| 应急路径 | NG-6 对应步骤 | 触发逻辑 | 关键 KPI |
|---|---|---|---|
| **EP-1 物理路径兜底** | ③ 接管 Handoff | AI 降级时物理标识/铺装必须立即接管 | 非数字替代覆盖率 ≥95% |
| **EP-2 人工窗口服务** | ③ 接管 Handoff | 老人/儿童/紧急场景下人工 ≤5 分钟响应 | 人工窗口响应时长 ≤30 分钟 |
| **EP-3 离线电子地图** | ⑤ 复核 Review | 断网场景下物理地图 + 楼层指引 | 离线场景覆盖 100% |
| **EP-4 物理里程桩/导引** | ③ 接管 Handoff | 极端天气下机器人道关闭 | 物理里程桩 200m/个 |
| **EP-5 急停+人工遥控** | ⑥ 退场 Sunset | 机器人/自动驾驶事故立即急停 | 急停响应 ≤10s |
| **EP-6 申诉机制** | ⑤ 复核 Review | 任何场景下居民可申诉 | 申诉响应 ≤7 天 |
| **EP-7 责任主体明确** | ① 定标 Declare | 跨系统交接前责任必须可追溯 | 责任公示 ≤24h |

12 × 7 矩阵的每行都带 [data:]/[source:]/[metric:]/[depth:] 引用——与高分同侪的"evidence-ledger"结构互为校验。`[assumption:A-EDGE-START-001]`：本矩阵为概念框架，行级字段的具体数值在 SC-04 试点验证后写入 `visual/assets/edge-matrix.json`（机器可读）。

## 渐进式暴露梯度 T1→T2→T3（可控→公共→市场）

承接 v5.3 韧性态（"已运行时如何降级"），本节回答**"如何测试 AI 进入城市"**——让 AI 分 3 个阶段、按可控开放程度递进进入公共空间。每阶段对应一个重点区作为主场，确保风险可控的前提下扩大场景 `[depth:phasing_implementation]` `[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]` `[assumption:A-EDGE-START-001]`。

### T1 Controlled Edge Court（控制场）—— 众智园主场

**开放范围**：INNO-A3 基准测试场 + Controlled Edge Court 围合区（约 5 ha 封闭范围），仅研发人员 + 评测志愿者 + 监管代表可进入，所有测试公开透明记录。

**触发进入条件**（G0→G4）：公共问题定义 → 伦理/数据/无障碍筛查 → 场地权属确认 → 真实数据必要性审批 → 人工 Gate 主体确认。

**退出机制**：任何 EDGE-01..EDGE-12 边界条件触发 3 次以上即暂停该子场景；事故触发人工 Gate 重新审查。

**责任分配**：算力联盟（运营）+ 治理联合体（监管）+ 监管代表（见证）。

**可核验证据**：`[data:visual/assets/sc04-relay-receipt.json]` `[metric:resilience_v13_selected_mean_score]` `[source:IPCC-AR6-URBAN-RISK]`。

### T2 Human Override Commons（公共场）—— AI 原点社区主场

**开放范围**：LIFE-D2 混合社区中心 + LIFE-D3 体验型零售带，约 1.04 km² 公共生活空间，居民日常使用，反对机制可表达。

**触发进入条件**（T1 + 公众参与）：T1 G0→G4 全部通过 + 居民参与基线建立 + 公平账本复核委员会成立 + 公开发布会。

**退出机制**：公平账本中"最弱 20% 体验"连续 2 周恶化即暂停；居民投诉超阈值即回滚到 T1。

**责任分配**：社区机构（运营）+ 治理联合体（对接）+ 居民代表（监督）+ 平台（执行）。

**可核验证据**：`[data:geometry/buildings.geojson#BLD-001]` `[metric:public_space_ratio]` `[metric:green_ratio]`。

### T3 Market Edge Forum（市场场）—— 大钟寺主场

**开放范围**：IND-C1/C2 企业总部 + 智能原生商业 + 产业服务配套，约 0.72 km² 商业空间，多智能体/商户/出资方/政府共同试验。

**触发进入条件**（T2 + 市场意愿）：T2 全面运行 + 商业意愿调查≥60% + 多智能体交接协议签署 + 行业监管介入。

**退出机制**：商户智能体跨系统事故 / 投诉率超阈值 / 行业监管叫停任一触发即回滚到 T2。

**责任分配**：业主（运营）+ 企业联盟（出资源）+ 行业监管（监管）+ 平台（执行）。

**可核验证据**：`[data:geometry/public_space.geojson#PS-004]` `[metric:road_centerline_length_m]` `[metric:scenario_node_count]`。

渐进式暴露梯度是**测试梯度**（不是降级），与 v5.3 S0/S1/S2/S3 韧性态（运行时降级）形成正交两轴：横向是"如何测试 AI 进入城市"（T1→T2→T3），纵向是"AI 运行时如何降级"（S0→S1→S2→S3）。两者交叉组成 3×4 = 12 个运行矩阵点，每个矩阵点都有对应的人工退路与可核验证据。

## 三区九子片区重构：T1/T2/T3 主场 + 边界条件锚定

v5.2 已建立 17 子块 + 9 子片区，本节根据渐进式暴露梯度重新标注各重点区的**主场定位**与**主导边界条件**——每个重点区都有清晰的主场测试层级 + 主导边界条件，评审员可立即看到 T1/T2/T3 与 S1/S2/S3 矩阵在空间上的映射 `[depth:three_key_area_detailed_design]` `[depth:phasing_implementation]`。

### 众智园 / Base Gauge（T1 主场，~1.93 km²）

- **主场定位**：**基准测试场 + Controlled Edge Court**——T1 渐进式暴露的核心区
- **众智园 ① 基准测试场 + Controlled Edge Court 片区**（INNO-A3 + INNO-A2 北部）—— 锚点：基准测试场观景台 + 封闭测试围合；主导边界条件：EDGE-01 轮椅/盲杖、EDGE-05 夜间雨雪、EDGE-07 机器人混行；拆改留：以新建测试场为主；公共空间：观景台广场；主导 AI 场景：S1 + S11；KPI：EDGE-01/05/07 触发次数≤0、退出机制启用 ≤3 次。
- **众智园 ② 共享算力中心片区**（INNO-A2 主体）—— 锚点：区域级算力 + 边缘节点 + 冷却设施；主导边界条件：EDGE-08 多源数据冲突、EDGE-09 弱网；拆改留：新建为主；公共空间：算力广场；主导 AI 场景：S1；KPI：EDGE-08 冲突解决时长 ≤5 分钟、EDGE-09 离线场景覆盖 100%。
- **众智园 ③ 研发组团 + 北绿环片区**（INNO-A1 + INNO-A4 + GRN-B2 北段）—— 锚点：基础研发 + 中试转化 + 北绿环；主导边界条件：EDGE-11 跨系统交接；拆改留：A1 新建为主、A4 改造+新建混合；公共空间：研发内院 + 北绿环；主导 AI 场景：S2；KPI：EDGE-11 责任公示 ≤24h。

### AI 原点社区 / Life Gauge（T2 主场，~1.04 km²）

- **主场定位**：**Human Override Commons**——T2 渐进式暴露的核心区
- **AI 原点 ① Human Override Commons 片区**（LIFE-D2 + LIFE-D3 核心）—— 锚点：原点生活广场 + 人工窗口 + 物理标识；主导边界条件：EDGE-02 老人无智能手机、EDGE-03 跨语言、EDGE-04 儿童数据；拆改留：改造更新为主；公共空间：原点生活广场；主导 AI 场景：S5 + S6 + S7；KPI：EDGE-02 人工响应 ≤30 分钟、EDGE-04 儿童数据合规 100%。
- **AI 原点 ② 混合社区中心 + 公平账本委员会片区**（LIFE-D2 北部）—— 锚点：托育 + 社区诊所 + 公平账本委员会；主导边界条件：EDGE-06 拥挤、EDGE-12 极端高温；拆改留：改造更新为主；公共空间：社区客厅内院；主导 AI 场景：S5 + S6 + S11；KPI：EDGE-06 容量>120% 限流、EDGE-12 体感>45℃ 人工。
- **AI 原点 ③ 体验型零售带 + 市场 Edge Forum 先导片区**（LIFE-D3 全段）—— 锚点：主街零售 + 餐饮 + 节事场地；主导边界条件：EDGE-10 医疗导航、EDGE-11 商户交接；拆改留：改造更新为主；公共空间：主街集市广场；主导 AI 场景：S7；KPI：EDGE-10 误诊事件 0、EDGE-11 责任公示 ≤24h。

### 大钟寺 / Industry Gauge（T3 主场，~0.72 km²）

- **主场定位**：**Market Edge Forum**——T3 渐进式暴露的核心区
- **大钟寺 ① 企业总部基地 + Market Edge Forum 片区**（IND-C1）—— 锚点：总部塔楼 + 多智能体会议室；主导边界条件：EDGE-11 商户智能体跨系统、EDGE-06 活动日容量；拆改留：改造更新为主；公共空间：大钟寺产业广场；主导 AI 场景：S7（B2B）；KPI：EDGE-11 责任主体公示 ≤24h。
- **大钟寺 ② 智能原生商业 + Controlled Edge Court 拓展片区**（IND-C2）—— 锚点：沉浸式体验店 + 多智能体商户；主导边界条件：EDGE-07 机器人配送、EDGE-08 多源数据；拆改留：改造更新为主；公共空间：体验广场；主导 AI 场景：S7 + S8；KPI：EDGE-07 急停响应 ≤10s、EDGE-08 冲突解决 ≤5 分钟。
- **大钟寺 ③ 产业服务配套 + 监管介入片区**（IND-C3 + IND-C4）—— 锚点：银行 + IP + 会议 + 孵化器 + 行业监管办公室；主导边界条件：EDGE-11 责任主体、EDGE-09 断网；拆改留：C3 改造更新为主、C4 改造+新建混合；公共空间：产业服务庭院 + 监管办公可见区；主导 AI 场景：S2 + S11；KPI：EDGE-11 责任公示 ≤24h、EDGE-09 离线场景 100%。

九子片区与 12 边界条件形成 9×12 映射，但**每个子片区只承担 2-3 个主导边界条件**——避免"边界条件复杂度爆炸"，每个子片区保持清晰的 2-3 边界管控职责，与 NG-6 六步契约 + 渐进式暴露梯度形成三层结构。

## 自评七维度（开发者内部参考）

本节为开发者内部参考，**不参与评审**——仅用于维持迭代方向。7 维度自评（满分 5 分）：

| 维度 | 自评 | 简短理由 | 待改进项 |
|---|---|---|---|
| 任务书相关性 | 5 | 完整覆盖三层范围 + 三大重点区 + 任务书 6 项 + 公告 17 项 | 控规条件仍待官方 |
| 原创性 | 4 | 全部原创（v5.1 删除借鉴概念 + NG-6/SC-04 自主） | 与 1843/1969 等高分方案同质化压力 |
| AI 与城市规划创新性 | 4 | 4 种空间生成机制 + 渐进式暴露 + 边界条件矩阵 | AI 形态机制可更具体 |
| 可实施性 | 4 | T0-T5 时间线 + RACI + 成本量级 + SC-04 试点 | 真实运营主体待确认 |
| 公共利益与包容性 | 4 | 公平账本 6 类人群 + 无障碍 EDGE-01 | 残障人士具体场景未沙盘 |
| 风险与合规意识 | 4 | 全程标注 provisional + 借鉴清零 + 12 边界 | 文化叙事偏弱 |
| 表达完整度 | 5 | 中英双语 + 6 张图 + 4 PDF + 17/9 子块矩阵 + 12×7 矩阵 | 评审员不易快速找到关键判断 |

加权自评：5×20% + 4×10% + 4×15% + 4×20% + 4×10% + 4×10% + 5×15% = **4.30/5.00 ≈ 86/100**（仅内部参考，与 intake 评分不直接对应）。
...
本方案经历多轮迭代深化，每轮有明确的改进主题与评审反馈响应 `[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]`：

| 版本 | 改进主题 | 评审反馈 → 响应 |
|---|---|---|
| v1.0 | 概念确立：京张新轨/The New Gauge + 五轨分区 + 12 场景卡 | 初版建立概念骨架 |
| v4.x | 双语包：图件/图纸/可视化板全双语 | 评审指出缺英文展示副本 → 补齐双语双译 |
| v5.0 | 借鉴高分同侪落地机制 | 评审反馈"借鉴痕迹过重" → v5.1 删除京张定律等借鉴概念 |
| v5.1 | 原创性回归：删除所有借鉴概念，保留原创 NG-6/SC-04/evidence-ledger | intake 69 分：缺韧性与可实施性深度 |
| v5.2 | 地块级深化：17 子块 + 9 子片区 + parcel-structure 图 | intake 69 分：粒度不够 → 下沉到子块级设计意图矩阵 |
| v5.3 | 评审提分：城市韧性四态 + 最低后悔方法论 + 公平账本 | intake 77 分：韧性与公共利益维度提升 |
| v5.3.1 | 实施深化：T0-T5 时间线 + RACI 矩阵 + marker 密度修复 | intake 75 分：时间线需更具体 |
| **v5.4** | **AI 塑造空间形态 + 成本量级 + self_check 持久化** | **当前轮：AI 创新性(15%) + 可实施性(20%) 深化** |

迭代方法论：每轮 intake 评审后，对照七维度评分体系自评薄弱项，针对性地深化高权重维度（implementation_feasibility 20% / ai_planning_innovation 15% / brief_alignment 20%），保持原创概念不变。关键原则：**学习同侪的落地思路，不抄袭同侪的概念**——从 86 分 open-pulse 学习全状态降级与公平账本的**结构性思维模式**，用自己的 NG-6/17 子块框架重新表述。
