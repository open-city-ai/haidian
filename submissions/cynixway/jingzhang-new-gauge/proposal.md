...
上述契约回答了"系统怎么运作"；本节回答"**住在这里是什么感觉**" `[depth:three_key_area_detailed_design]` `[depth:blue_green_public_space]` `[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]`。

**清晨 7:00 — 研发工程师小李从 AI 原点社区出发**。住在 LIFE-D1 人才公寓，骑车 8 分钟到 INNO-A1 研发组团。沿 GRN-B1 遗产公园绿带骑行，经过故事段②"标准奠基"。到达后把基准测试任务提交到 INNO-A2 共享算力中心，在基准测试场观景台（朝圣地标③）一边喝咖啡一边看排行榜 `[data:geometry/buildings.geojson#BLD-001]`。**这就是AI攻克生产力陡坡的日常形态。**

**午间 12:30 — 退休教师王奶奶到 AI 原点社区取药**。72 岁，不用智能手机。走到 LIFE-D2 社区中心**人工窗口**（NG-6 ③接管的空间实现），社区工作人员帮她完成健康预筛。取药后在原点生活广场按了一个 AI 导览桩的物理按钮，听到京张铁路 1909 年通车的故事。**整个过程没碰任何屏幕——这就是AI攻克生活陡坡时"不留屏幕痕迹"的设计哲学。** `[source:WHO-URBAN-HEALTH-AND-GREEN]`

**夜间 22:00 — 配送员老张在体验型零售带完成最后一单**。22:00 后阻尼桩自动升起，分隔出机器人配送道。23:30 突降暴雨——韧性态自动切换到 **R1 雨天态**：机器人配送暂停，收到短信"暴雨预警，每 200m 有避雨点"。**急停优先于性能——这就是AI运行时的安全底线。** `[source:ISO-13482-SERVICE-ROBOT-SAFETY]`

三个人物代表三种 AI 使用方式：**主动攻克（研发）、被动受益（健康）、职业依赖（配送）**。他们都受益于同一个 NG-6 创新契约——随时可转人工、物理标识随时可读、数据最小化随时生效。
...
#### S1 算力调度与基准测试（产业验证）
- **空间落点**：众智园创新极 `[data:geometry/buildings.geojson#BLD-001]` · **服务对象**：AI研究员、企业团队
...
- **空间落点**：生活极 `[data:geometry/green_space.geojson#GR-001]` · **服务对象**：居民、老人、儿童、残障人士
- **数据流**：无识别环境感知→物理导引（无摄像头追踪）· **模型边界**：物理优先，不采集生物特征
- **运营主体**：社区+物业（概念）· **KPI**：无障碍路径连通率、老人/残障人士可达节点数 · **回退模式**：系统失效→物理铺装与标识 · **生命周期成本**：铺装/标识（中）

#### S5 AI+社区健康驿站（公共服务）
- **空间落点**：生活极 · **服务对象**：居民、老人 · **数据流**：个人健康数据本地化处理（不外传）→AI辅助预筛→医生复核
...
- **空间落点**：生活极 · **服务对象**：学生、家长 · **数据流**：儿童数据严格保护（最小化、不外传）→AI辅助学习→教师/家长复核
- **模型边界**：儿童数据最小化、不用于商业推荐、可删除 · **KPI**：儿童数据合规率、学习辅助满意度 · **回退模式**：AI失效→教师直接教学 · **生命周期成本**：设备（中）、内容（中）

#### S7 智能原生消费体验（产业）
- **空间落点**：大钟寺产业极 · **服务对象**：消费者、商户 · **数据流**：消费记录合规采集（需同意、可删除）→个性化推荐→可申诉
...
| 场景 | 空间落点(GeoJSON) | 运营主体 | NG-6 立约 | NG-6 计时 | NG-6 接管 | NG-6 退场 |
|---|---|---|---|---|---|---|
...
| S4 全龄慢行导引 | 生活极绿带 `[data:geometry/green_space.geojson]` | 社区+物业 | 无识别环境申明 | — | 物理铺装兜底 | — |
| S5 社区健康驿站 | 生活极 | 社区卫生机构 | 数据本地化申明 | 预筛响应时限 | 医生复核 | 异常→转医生 |
| S6 教育便民节点 | 生活极 | 学校+社区 | 儿童数据最小化 | — | 教师/家长复核 | — |
| S7 智能原生消费 | 大钟寺产业极 | 商户+平台 | 消费数据合规 | 申诉响应时限 | 人工服务 | 异常→人工 |
...
该矩阵确保每个场景都有明确的空间锚点、责任主体和 NG-6 契约覆盖，使"人字新线"从空间概念落为可操作的服务体系 `[depth:phasing_implementation]`。
...
前述 14 张 AI 场景卡解决的是"AI 服务什么"；本节回答一个更根本的问题：**AI 如何改变空间的物理形态本身？** 如果 AI 只是贴在传统空间之上的服务层，那城市并未因 AI 而不同。本方案提出四种机制，让 AI 从"服务空间"下沉到**"生成空间"** `[depth:overall_spatial_structure]` `[depth:height_massing_character]` `[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]`。

### 机制一：算力入地——地下空间由 AI 需求驱动

传统地下层以停车为主要功能。在"京张人字新线"方案中，INNO-A2（共享算力中心）和 INF-E2（算力管道与边缘节点）的地下空间由**算力基础设施需求驱动**：地下二层为区域级液冷算力机房（需大层高 6-8m、高承重、专用冷却管道路由）；地下一层为边缘推理柜走廊与人行检修通道；地面层仅留设备搬运口与通风井，把宝贵的地面留给公共空间。这一剖面设计使得**算力需求直接决定了建筑的地下形态**——层高、承重、冷却、管线路由都围绕算力基础设施组织，而非事后塞入 `[depth:municipal_new_infrastructure]`。

### 机制二：动态空间分配——同一空间按时段切换功能

AI 调度使得同一物理空间可以在不同时段承载不同功能。LIFE-D3（体验型零售带）在日间（7:00-20:00）为主步行商业街；在夜间（22:00-6:00）切换为 S8 机器人低速配送专用走廊（物理阻尼桩自动升起，分隔出 2m 宽机器人道）；在周末切换为节事集市广场（模块化摊位从地面储藏间升起）。这一机制要求**空间设计本身具备"可切换性"**——地面铺装需嵌入传感器与升降阻尼桩、建筑首层需预留模块化储藏空间、照明系统需支持多场景预设。空间的"功能"不再是固定的，而是**AI 调度下的时间切片** `[depth:land_use_layout]`。

### 机制三：机器人兼容街道——街道断面为人机共生而设计

S8/S9 场景要求街道断面**从一开始就为人类与机器人/低速载具共生而设计**，而非事后画线分隔。本方案的 INF-E1（主轴道路）标准断面为：中央 7m 公交/慢行混合车道 + 两侧各 1.5m 机器人专用低速道（物理分隔、充电接口嵌入路灯杆）+ 外侧 3m 人行道。机器人道与人行道之间设 0.3m 高物理路缘石 + 触觉警示带，确保低速载具不侵入人行空间。路口设 AI 感知桩（非摄像头，使用激光雷达+毫米波雷达），为机器人提供交通信号但不采集个人生物特征。这一设计使得**街道的物理形态由人机共生的安全需求直接塑造** `[depth:traffic_rail_slow_parking]`。

### 机制四：屏幕退场——以物理/环境界面替代数字屏幕

传统"智慧城市"往往在空间表面铺设大量屏幕。本方案反其道而行——**屏幕退场**，以物理/环境界面替代数字屏幕。GRN-B1（遗址公园主轴带）的导览不依赖屏幕，而是使用：热感铺装（温度变化标识路径）、声景节点（定向音响讲述历史故事）、触觉地图（凹凸浮雕地图替代电子地图）、耐候钢解说牌（低技术、高耐久、零能耗）。AI 退到后台——它决定"何时触发哪个声景节点""何时调节热感铺装温度"，但**不在城市表面留下屏幕**。这一原则使得**空间的物质性因 AI 而增强（更智能），而非削弱（更虚拟）** `[depth:blue_green_public_space]`。

四种机制共同把 AI 从"附加服务层"下沉到"空间生成层"——算力决定地下形态、调度决定时段功能、共生决定街道断面、退场决定物质界面。这正是"京张人字新线"概念的空间含义：**人字新线不是在城市表面贴 AI 标签，而是让 AI 从内部重新塑造城市的物理结构**。
