...
**状态映射**：计算层输出五级状态（<40 正常、40–60 关注、60–75 三级大客流、75–90 二级大客流、≥90 一级大客流）；公共展示界面归并为四级色彩语言：正常与关注→冷蓝，三级→青绿，二级→暖橙，一级→深红，并以形状符号 ●▲◆■ 作非颜色辅助编码，兼顾色觉差异用户。
...
| αt / αs | 时段修正 / 站型修正系数 | 公开标准条款与现场可测属性标定 |
...
**一期最小闭环的实施与运营框架（概念建议）**：责任分工上，组织方与属地街道负责场地协调（主责），技术提供方负责算法迭代与设备运维（执行），车站与公园运营主体负责现场动作与人工复核（会签），社区与企业用户作为知情参与方（知会）；服务水平目标为状态刷新周期不超过15分钟、设备故障24小时内响应；成效以"分级动作按剧本执行率"与"高峰预警提前量"两项指标年度跟踪；年度复盘决定扩缩，任何一方可退出，退出后空间恢复常态管理、不留废弃设施。以上均为概念建议，不构成政府承诺或采购安排 [depth:existing_conditions_diagnosis]。
