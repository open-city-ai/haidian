...
**铁路语汇状态机**：道岔=准入（通过/暂缓/驳回）；信号=运行状态（绿正常、黄整改、红牌暂停并切人工接管；红不得直转绿，须复核签认后转黄观察一期）；编组场=沙盒；折返=恢复；K标=版本里程碑（实体K标记文化，版本K标记数据重算）。全部场景走"概念→沙盒→试点→运行→退役"生命周期，退役为终态，状态迁移须人工签认，模型档案与失败记录公开留存。场景涉及的法定义务方向（个人信息最小化、数据分类分级、生成内容标识、智能网联测试规定、公共影像管理）逐卡登记为"法律接口"字段[source:CN-LEGAL-INTERFACE-REFS]——登记是运营透明层，不替代法定程序，本方案不作合规认定。

**场景的智能体接口**：状态机不止于描述，而被形式化为可执行协议——每张场景卡为运营智能体规定四件事：可读取什么数据、可自主决定什么、必须移交人类什么、如何被叫停；并配逐场景数据保护契约（最小字段/目的/期限/共享/删除五要素）。协议以机器可读文件随包交付（visual/assets/agent-protocol.json，含7状态8迁移的状态机形式化、红牌演练runbook与后续智能体的调用约定）——**城市对智能体开放的是接口，人对智能体保留的是中断权**：红牌是唯一无须事前签认、即时生效的迁移，其余每次状态变化都要人签字。
...
**首开预可研行动包**：四个不新增永久工程的行动先行——组件库快闪三处、K0/K8首批K标落位、开源之家pop-up首场活动、知春路智能过街试验；各按迷你实施卡预填六字段，并配角色级RACI（每项行动明确发起、决策、日常运营、维护、安全复核与公众代表六类角色，仅列角色不指认机构）。四行动分别是四个二三期项目的先导验证：组件快闪→组件库部署、K标落位→人字门与梯度长阶、开源之家pop-up→开源之家存量改造、智能过街试验→三廊缝合，验证不通过则对应项目降级或调整。预可研期内完整执行一次**红牌撤回演练**，runbook随包（agent-protocol.json）：触发→停止自动输出（目标时限≤30分钟，概念时限试点前冻结）→人工接管（等价路径顶上并记录响应时长）→复盘→黄牌观察签认（不得直转绿）→全流程时间线公开——目标是在预可研期内使"能撤回"成为有公开记录的演练事实（当前仅为runbook设计，无演练记录）。公众参与机制：意见与回应台账作为条件门验收证据公开留存；公开意见入口已在本仓库开放（issue #215），截至本版收到外部意见0条，不作为共创成果。
