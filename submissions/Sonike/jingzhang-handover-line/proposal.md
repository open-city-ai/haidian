...
总平面图同时标出**四处朝圣地标**——交接钟、公开交接桌、百年值班簿、零号接管亭——并在图内左侧列出各自的荣誉角色（逐字取自 [data:geometry/public_space.geojson]）。四处均为设计建议，启用门槛与回退状态随几何一并登记。

![京张交接线总体概念图：三座交接场、四处朝圣地标与各自的荣誉角色，临时边界](assets/figures/site-overview.png)

> **这张图怎么看。** 左侧地图里，红线是京张交接线主轴，蓝色横线是东西缝合支线，虚线框是临时边界；三个色块自北向南依次是研制、开源、城市三座交接场，红—青—黄对应 BUILD→VERIFY→SHARE→SERVE 的责任转换次序。四个菱形 01–04 是公共地标：交接钟、公开交接台、百年值班簿、零号接管亭。右栏四段是交接协议的四步，末句是全案的判据。
...
图内左侧另给出**用地构成堆叠柱**：七类用地由 `land_use.geojson` 的 `area_sqm_declared` 逐图斑相加，合计 11.413 km² 与场地面积一致，其中 1401 连续公园绿地占 19.8%——与 [metric:green_ratio] 的 19.84% 相互印证。**面积不只写在指标表里，也画在它对应的那张图上。**

![三层范围、用地分区与用地构成：七类用地的面积与占比逐条列出，合计与场地面积一致](assets/figures/land-use-structure.png)

> **这张图怎么看。** 左侧竖条是七类用地各自的面积与占比，逐图斑相加合计 11.413 km²，与场地面积一致；走廊图按用地代码着色，三个圆点是三座交接场，图上下两端标出中关村科技服务翼与小月河场景赋能翼。页脚「11 TOPOLOGICAL FEATURES / 0 INTENTIONAL GAPS」的意思是十一个图斑联合覆盖临时范围、没有有意留下的空隙。右栏 43.6 km² → 11.4 km² → 3 是三层范围逐级收敛的关系，不是三个并列数字。

## 统筹研究范围产业与未来城市研究

公告与面向智能体任务书已给定一带的三大定位、五大功能和“三区两翼”格局。下表把官方表述逐项对应到空间载体、运行机制与证据入口；“京张交接线”是这套对应的空间总名，不替换官方定位。
...
“海淀有丰富创新资源”这类判断无法核验。下表五条统计可追溯到发布机构原页，逐条标注统计尺度，用于收敛设计问题而非设定绩效目标 [source:HAIDIAN-2025-STATISTICAL-BULLETIN]。
...
五条数值的统计口径均为海淀全区，远大于 43.6 平方公里的统筹研究范围，因此登记为 `background_only` 与 `not_spatially_allocable=true`，不进入 metrics.json，不改变几何、面积、线位或分期；原文表述、采集方法与可用边界记录在 sources.json。走廊级的客流、OD、设施容量与投诉基线仍缺失，保持未知。

在此之下，本方案的三条工作主线是全球可信AI的公开验证走廊、AI原生城市生活的人工接管示范带、百年工程文化与开源文化的共同记忆场；五个动作是研发、验证、开源、服务与复盘。任务书提出建设“全球人工智能产业高地和朝圣地”，本方案把“高地”落实为可复用的测试协议、负面结果与转化接口，把“朝圣地”落实为任何人都能进入、质疑并看到责任人的公共制度，而不是楼宇高度、企业数量或一次性打卡流量。品牌Logo由两条平行铁轨、一个交接棒和一枚信号括号组成：煤黑代表可审计责任，信号红代表必须停下并由人判断的阈值，电气青代表开放接口，米白代表可阅读的公共记录。**主名称**为「京张交接线」，**英文名**为 JING-ZHANG HANDOVER LINE，二者在全部图纸、图件与两个离线页面中一致使用。**命名体系分三级且只用一套隐喻**：一带称交接线，三个节点统一为“研制交接场、开源交接场、城市交接场”，小型设施统一称“交接台、接管亭、值班簿”。**同一层级不出现第二种叫法**；图纸、图件与两个页面按此逐字校对。

![标识、色彩与品牌应用系统](assets/figures/logo-identity.png)

区域创新协同分“带内—城内—市域—区域”四层。带内由“三区两翼”组织研发、验证与服务的协同回路；城内联动公告提出的“两区一带”产业发展格局，把中关村科学城的技术供给对接本带的验证、开源和场景需求；市域层面只预留与未来科学城、怀柔科学城和北京经济技术开发区的概念接口，候选链条是“原始创新—工程验证—规模应用”，但不声称分工、合作或资源已经成立；区域层面沿京张方向为张家口等京津冀节点预留算力、能源与场景问题的拒收式接口。任务书点名的“北纬社区”缺可清权边界、运营主体与既有合作关系资料，本方案不设落点，只预留“社区问题—去标识反馈—限域复测—公开回执”接口，待主体确认后启用。海淀“1+X+1”产业体系由产业主管部门定义，本方案只在其“AI+”方向提供空间接口与验证工具。协同关系一律以接口、规则与可复核记录表达，不涉及行政区划调整、指标划转、投资安排或任何一方的承诺，可行性由相关主管部门核实。
...
六个全球案例只提取机制，末列写明不可直接移植的制度条件；比较范围受 A-CASES-001 限制，转译受 [depth:overall_spatial_structure] 约束。
...
| 伦敦 Knowledge Quarter [source:CASE-KNOWLEDGE-QUARTER] | 联盟式知识交换与公共可达 | 公开交接桌与全球交接周作为共同地面 | 机构自愿联盟与既有公共文化设施密度 |
...
产业环节与空间的对应由用地拓扑给出可复算比例：下表在 EPSG:4548 中按 [data:geometry/land_use.geojson#LU-001] 逐要素复算，九个命名分区联合覆盖临时范围、合计 100.00%。这是城市设计结构的用地面积占比，不是法定用地配比，不代表产值、就业或投资比例。

**三个数字是三个层级，不是同一件事的三种说法**：`land_use.geojson` 里是 **11 个图斑**（拓扑要素，`11 TOPOLOGICAL FEATURES / 0 INTENTIONAL GAPS` 指的就是它们联合覆盖且无有意留白）；这 11 个图斑按功能归并为下表的 **9 个命名分区**（`05` 因承载科技服务与智能原生消费两类功能而出现两次）；而部颁分类代码只有 **7 个**（05／0701／0702／0802／0803／0804／1401）。**面积占比按 11 个图斑逐要素复算，归并只影响命名与行数，不影响任何数字。**
...
创新资源要素以接口而非名单统筹：高校院所的接口是校园成果接力站与授权目录，孵化平台的接口是共享测试院与服务栈，独角兽与上市企业的接口是产业验证沙盒与城市问题发布。不列企业名称、不预设入驻主体，不代表任何一方已同意参与。

## AI 原生城市形态与规划方法

### 本方案提出的九件新东西，逐条对照常规做法

原创性不该由形容词来主张。下表把本方案提出的机制逐条列出，每条写明**它是什么**、**与常规做法差在哪**、以及**在包内哪个文件里可以找到它**——差别这一列才是原创性的证据，形容词不是。

| 机制 | 它是什么 | 与常规做法的差别 | 载体 |
| --- | --- | --- | --- |
| 交接协议四步闭环 | 研发→验证→开源→服务→**交回研发** | 常规智慧城市是单向交付；本方案第四步把投诉、维修、异议与停用原因逆向送回研发端，形成闭环 | [data:geometry/phasing.geojson] |
| 双联交接账 | 交出与接入是**两次独立判断**，交出方不能替接入方签字 | 常规验收是一方签收；本方案要求接收方独立复现最小证据后显式 ACCEPT 或 REFUSE，未决项不得在换班中消失 | [data:visual/assets/governance/shift-ledger.schema.json] |
| 无AI等价判据 | 任何AI服务，不用AI也能办成同一件事——办不成就不上线 | 常规指标是设备数、自动化率这类普通人无法核验的量；本判据每个人都能自己验一次 | 十二张场景卡的 `human_service_fallback` |
| 智能层开关 | 离线页面上可以**当场把智能层关掉**，十二张卡翻出无AI那一面 | 常规方案把无障碍写在正文里，读者需跨章节自行确认；做成开关后，缺哪一条一眼可见 | [data:visual/index.html] |
| 三座交接场 | 研制、开源、城市三座场，对应能力流动的三段 | 常规分区按功能命名（研发区/商业区）；本方案按**责任交接的阶段**命名，同一套隐喻贯穿全带 | [data:geometry/key_areas.geojson] |
| 八个共享栈 | 要素做成**接口＋规则＋可复核记录**，谁满足规则谁接入 | 常规产业方案列企业名单，主体不来则整套落空；接口式供给不依赖任何特定主体 | 正文《八个共享栈》一节 |
| 三道合并门 | 分期是**条件门槛**而不是时间表，未通过就回滚到上一个可用状态 | 常规分期承诺年份；本方案承诺的是**通过什么条件才允许合并**，不承诺日历 | [metric:phase_count] |
| 缺陷注入自检 | 对七条协议规则各做缺陷注入，**84 条合成缺陷全部被拦截、漏检 0（作者自测，非独立验证）** | 常规方案只声明规则；本方案把规则放到自己造的缺陷上跑一遍，**结果是「规则在纸面上闭合」，不是「现场拦得住」** | [data:visual/assets/governance/rule-check-report.json] |
| 触觉走廊图 | 不依赖颜色即可辨认的可制版矢量图，三座交接场用圆／方／三角区分 | 常规无障碍停在「设置无障碍设施」一句；本方案交付可直接制版的文件，并把尺寸留给规范与厂商核定 | [data:assets/tactile/tactile-corridor-map.svg] |

**九条都不是概念口号：每一条都能在包内找到对应文件，且都写明了失效条件。** 交接协议在缺少人工负责人时不得启用，双联交接账在未决项未闭合时保持智能层关闭，无AI等价判据在任一场景不成立时该服务不上线，三道合并门在条件未满足时回滚——**一个机制如果说不出它什么时候不成立，它就还不是机制。**
...
国土空间规划的创新思路由此收敛为三条可操作主张：其一，把"不知道"写进成果——未知项保留 status=unknown 并附口径、数据前置条件与赋值触发器，而不是用估算填满表格；其二，设计结论与法定控制始终分栏，设计建议标 design_proposal、临时边界标 official_boundary=false，二者不得混排；其三，把规划成果做成可复算的数据包而非静态图册，官方数据到位后由同一模型一次性重算，而不是人工修图。这与数字孪生的区别在于：本方案不追求实时镜像，只要求任何一个数字都能回到几何、公式和假设。

### 综合规划内涵与空间产业融合：本方案把「综合」和「融合」落在哪一层

**综合规划内涵，在本方案里不是把专项叠起来，而是让不同专业在同一个可复算的数据包上工作。** 传统「综合」的成果形态是一本把交通、市政、绿地、产业各章装订在一起的图册，各章之间靠人工对表；这套包里几何、指标、图件、图纸与三个矩阵**同源生成**，任何一章改动都会同时改变别章引用的那个数字，改不动的地方立刻暴露为矛盾。所以本方案主张：**综合的是"证据面"，不是"图册面"**——判断一份综合规划是否真的综合，看它的各专业结论能不能被同一次复算同时推翻。

**空间产业融合，落在接口而不是名录上。** 本方案不写企业名单、不承诺产值，产业与空间的关系由八个共享栈（土地、空间、产业、资金、人才、算力、数据、场景）在三区之间的**接口规格**表达 [depth:overall_spatial_structure]：每个栈写明可交换的最小成果、接入条件与拒收条件。这样做的理由是可证伪：**名录会过期，接口不会**——五年后企业换了一批，接口规格仍可被检验是否还成立。产业因此不是被"安置进"某个地块，而是通过一道可以拒收、可以退出的闸门接入空间；融合的程度由**有多少家能真的接进来并且能退出来**度量，而不是由入驻率度量。

**这两条与上面的国土空间规划创新共用同一个前提**：规划成果的可信度不来自结论的确定性，而来自结论可以被谁、用什么、在多久之内推翻。三条主张都可被专业团队在不联系设计方的前提下独立检验 [depth:existing_conditions_diagnosis]。以上属方法建议，不替代法定编制程序与审批。

## 总体设计范围城市更新与控规深度城市设计

总体设计范围北至北五环路，东至学院路与西土城路方向，南至西直门外大街，西至大钟寺东路与荷清路方向；京张遗址公园是贯穿其间唯一连续的公共载体。本方案把它当作活力带的骨架而不是背景绿地：交接线与遗址公园南北贯通、东西连通的步行骑行体系重合，八条东西支线缝合高校、园区、社区与轨道方向，三座交接场把这条线从景观通廊升级为承担研发、验证与城市服务交接的公共机制。遗址公园北端、南端以及上跨北五环的区段是三处必须专门处理慢行断点的位置，本方案在此只给出连接意图和界面原则，不给出桥隧或工程可行性结论。**景观节点不另起一套体系，就落在已登记的十六个点位上**：四处朝圣地标（交接钟、公开交接桌、百年值班簿、零号接管亭）承担停留与仪式，十二个场景节点承担日常使用 [data:geometry/public_space.geojson#LANDMARK-01]。**判据是「不消费也能停留」**——每个节点都须提供座椅、树荫与人工问询中至少两项，且不以购买或安装应用为进入条件；具体铺装、种植与设施选型须依公园实施边界和文保控制线另行确定。
...
**这套协议适合落在哪些片区、哪些节点、哪类空间，本方案给的是判据而不是清单。** 片区一级落在三处重点区：**责任在此处发生转换**的地方才设交接场——众智园（研发交给验证）、原点社区（验证交给开源）、大钟寺（开源交给城市服务）；换句话说，交接场不按用地规模或区位价值选址，按**交接动作**选址。节点一级落在十二个场景节点与八条东西缝合支线的交点上，判据是「此处有人会需要把控制权要回来」。空间类型一级只落在三类：**可封闭可计量的试验空间**（技术测试用）、**首层可进可绕行的公共界面**（人工窗口与无AI等价服务用）、以及**连续无高差的慢行面**（不依赖任何智能层的基础通行）。这三类之外的空间，本方案不主张改变——**判据比清单更可转化**：换一片场地，同一套判据仍然能用来选点，而清单不能。

### 众智园AI自主创新加速区：研制交接场 / Build→Verify

北段以“模型交付前必须留下可读记录”为核心，空间由公开验证街、共享测试院与临清河方向的低扰动绿色界面构成。公开验证街由 SCN-01 模型红队交接台、SCN-02 机器人路权沙盒、SCN-03 端侧算力能耗试验站三个节点串起，实测三者都落在本区粗略范围内 [data:geometry/public_space.geojson#SCN-01]；共享测试院是本区内以「验证工坊」为原型的更新单元 [data:geometry/buildings.geojson#BLDG-017]。建筑更新优先利用大开间与首层，布置红队交接台、路权沙盒、能耗试验站与标准协作室。物流与受控测试从园区内部进入，公众参观从交接线进入，二者以透明但不可穿越的安全界面分开。每次试验展示适用范围、数据来源、人工负责人、停机按钮与退出原因；缺少企业授权、道路条件或安全论证时不进行实地部署。

任务书把本片区定位为国家级AI集聚区。够到这一定位需要三件先具备的东西：可被第三方复现的验证记录、对外可核验的开放接口、不依赖单一主体的共享设施。三者在本片区各有载体——公开验证街、八个共享栈的接入口、共享测试院与开放研发院 [depth:key_area_detailed_design]。

潜力用地按现有证据分两档。落在本区粗略范围内的更新类型试验单元共六个，基底合计 63,766 ㎡ [data:geometry/buildings.geojson#BLDG-017]：四个按保留优先的适应性再利用处理，可先用空置时段与可移动布置验证需求；两个须逐栋核查，在结构、权属、消防与文保核查完成前不进入改造判断。**二十个更新单元是抽样的类型试验单元，不是完整的建筑清单。**

“花园型创新街区”在本区是可核算的比例：蓝绿基底与本区相交面积占该区 **25.5%** [data:geometry/green_space.geojson]，1401 公园绿地贯穿其间 [data:geometry/land_use.geojson#LU-001]。研发院落沿绿地一侧布置可进入的连续首层，测试与物流界面背向绿地，**绿地同时承担步行空间与安全隔离**，不另设围墙。

本片区承担标准制定与安全治理方向的空间需求，建筑、绿地与水系按同一套界面规则一体化设计。清河方向的滨水段作为“可解释的绿色空间”，把水质、径流、树荫与设备能耗以固定标识和纸本同时公示，使绿色空间本身成为 AI 服务的验证场。清河文化——水利工程、河道治理与沿线劳动记忆——以沿岸记录牌和值班簿展示，不改动河道形态。对外交通结合五环路沿线的区域一体化方向提出改善需求与优先级；断面、匝道、跨越方式与工程可行性由交通与市政专业依据官方资料判定 [depth:traffic_rail_slow_parking]。
...
**任务书点名的「人才特区」在本方案里是一组可核对的居住与服务条件，不是一项待申请的政策。** 本方案不承诺任何户籍、税收或补贴安排——那不属于城市设计能给出的结论；能给出的是空间侧的四条：**住得下**（0701 城镇住宅用地与人才生活混合社区提供近校步行范围内的居住供给 [data:geometry/land_use.geojson#LU-005]）、**办得成**（社区协作屋按「人工窗口先于智能界面」布置，十二个场景各有无AI等价路径）、**留得住**（可署名的贡献记录进入百年值班簿，青年人才的首次贡献到被复现的间隔是本方案的观测指标之一）、**退得出**（短期使用权与时段共享先于产权讨论，试错失败不产生长期负担）。本片区落在粗略范围内的更新单元为两个，基底合计 19,008 ㎡，一个按保留优先处理、一个待逐栋核查 [data:geometry/buildings.geojson#BLDG-001]。**这四条都不依赖任何一方先给出政策承诺，因此今天就能核对。**

### 1:1 原型：公开交接桌——把整套协议缩到一张桌子上

前面三节写的是片区尺度的策略。但一条以「交接」为名的线路，应该有一件东西小到可以先做出来、错了可以当天撤走、做对了别处可以照着做。**公开交接桌**就是这件东西：整套双联交接协议的最小物理载体，一张桌子加四件必须同时在场的东西。它在几何里有确定位置——`public_space.geojson` 的 `LANDMARK-02`，与交接钟、百年值班簿、零号接管亭同为四处朝圣地标之一，带各自的荣誉角色、启用门槛与回退状态 [data:geometry/public_space.geojson#LANDMARK-02]。

**为什么先做它，而不是先做交接场。** 交接场是一片区域，牵涉权属、消防、交通与持续预算，任何一项不清就动不了；而一张桌子可以放在既有的公共空间里，用可移动家具和短期使用权先跑起来。**协议的可靠性应该在最小单元上先被证明一次，再谈放大**——这与本方案「先证明拒收路径可靠，再谈放行」是同一条逻辑。

| 构成段 | 是什么 | 必须满足才能开桌 | 撤除后留下什么 |
| --- | --- | --- | --- |
| **① 交出位** | 交出方一侧的台面与版本卡插槽：模型/服务的版本、适用边界、已知故障、停用办法各一张 | 四张卡缺任一张即不开桌 | 卡片归档进值班簿，台面恢复公共座椅 |
| **② 接入位** | 接入方一侧的台面与判定牌：**接收 / 拒收 / 暂缓**三选一，必须署名 | 接入方与交出方不得为同一角色 | 判定记录归档，判定牌收回 |
| **③ 未决栏** | 一块公开可见的板：上一班没闭合的问题逐条挂在这里，不允许在换班时摘下 | 存在阻塞性未决项时，智能层保持关闭 | 未决项转入公开值班簿，不随桌子消失 |
| **④ 人工窗口** | 一张不依赖任何设备的服务位：纸本表单、人工问询、无AI等价服务 | **必须先于智能层存在**，且智能层关闭时仍可独立服务 | 人工窗口保留，这一段不撤 |

**四件东西的顺序不能调换**，这正是本方案的核心主张在一张桌子上的样子：人工窗口先立，未决栏公开，然后才谈交出与接入。**如果一张桌子只能保留一段，保留人工窗口。**

**当前状态：`not_authorized_not_run`。** 没有选址、没有主体、没有建造、没有运行过。可以现在就说清楚的只有三件事：它由什么构成、开桌前必须满足什么、撤除时留下什么。**位置、尺寸、材料、无障碍净宽与桌面高度须依现场测绘与无障碍规范确定，本方案不预设数字**——本包对 12 个场景的处理一贯如此，原型不因为「具体」而降低这条标准。

**开桌前的检查清单**（与 P03 行动包、双联交接账 12 条实例共用同一套口径，不另立标准）：

1. 场地权利与责任主体获授权 —— 否则不落点
2. 交出方与接入方分别指派，**且不得为同一人** —— 否则协议不成立
3. 人工窗口先行可用，并经一次无AI等价服务演练 —— 否则不开智能层
4. 回滚演练被观察到通过：从「智能层运行」回到「纸本 + 人工」全程不中断服务 —— 否则不进入试用
5. 数据最小化、许可、安全、无障碍与公共利益评估通过 —— 否则不面向公众

**五条缺一不开。** 这不是保守，是这套协议唯一的成立条件：一张不能被拒收、不能被撤走、没有未决栏的桌子，只是一件家具。

**四段的顺序已经做成了几何事实。** 三维关系模型 [data:visual/assets/public-handover-table-3d.json] 把四段沿同一轴线相接：交出位有四个版本卡插槽，接入位只有一个判定牌位，未决栏是唯一高出台面的竖板，**人工窗口在最外端、也是唯一从地面起建的一段**——「如果只能保留一段，保留人工窗口」这句话在模型里是可以看见的。**它是关系模型不是施工模型**：全部坐标为无量纲相对单位，只表达顺序、相邻与共同在场，**任何按它推算的实际尺寸都是误用**——尺寸仍须依现场测绘与无障碍规范确定，与上一段的口径完全一致。文件内附重建参数与 sha256，解码后可自校验。

**它可以怎样被复制。** 桌子的价值不在造型，在那四件东西的**共同在场**与**不可调换的顺序**。任何机构——高校实验室、社区服务站、企业开放日——都可以用自己的家具、自己的表单搭一张，只要四段齐全、顺序不变、判定署名、未决项不摘。**这是本方案最容易被带走的一件东西，也是最应该被带走的一件。**
...
本区临时范围内可核验的落位有四类：两个更新类型单元 [data:geometry/buildings.geojson#BLDG-001]、两片用地分区 [data:geometry/land_use.geojson#LU-002]、场景节点「全球交接周线路」SCN-12 与地标「百年值班簿」[data:geometry/public_space.geojson#LANDMARK-03]。粗略范围面积 720,454.219 ㎡，与公告约 72.0 公顷相差 0.06% [metric:key_area_dazhongsi_sqm]。该范围由公告名称、自北向南次序与约面积推得，四至不是官方边界，矩形边不得读作地块或道路红线 [data:geometry/key_areas.geojson#PROV-KEY-003]。
...
**三处重点区以同一比例尺的放大平面表达**：区内的用地分区、连续绿廊、二十个更新类型单元的建筑占位、主轴与东西缝合支线、以及落在区内的场景节点编号，全部按提交几何绘出——**「详细设计」这四个字由图面自己证明，不只写在深度矩阵里**。范围仍是临时的，三张卡各自标注 `PROVISIONAL KEY AREA`。

![三处重点区域放大平面：用地分区、绿廊、更新类型单元、主轴与缝合支线、场景节点编号，同一比例尺，临时范围](assets/figures/key-areas.png)

> **这张图怎么看。** 三张卡共用同一比例尺，因此卡与卡之间的大小差异是真实面积差异。主图画的是各重点区的真实轮廓，以及区内的用地分区、连续绿廊、更新类型单元与落在区内的场景节点编号。更新单元分两档：米色实填的是保留优先、适应性再利用，只描边不填色的是介入前先查证——图面上「还没定」不做成已定的样子。每张卡右侧细长条是沿线定位，深色块表示本区在整条带中的位置。

## AI 创新生态、人才画像与 AI+ 场景

场景设计不把AI当成自动化装饰，而把它视为一项随时需要交接的公共服务。八类用户画像各自对应明确的空间载体、场景与评价口径；任何画像都不用于商业追踪，也不以画像分配资源或排序个人。

十二个场景按**可体验、可展示、可推广**三条不同的标准分别验收，而不是笼统地称作"示范"。**可体验**指现场不预约、不装 App、不识中文也能完整走完一遍，判据是本方案那条底线：不用 AI 也能办成同一件事。**可展示**指它在失败的时候同样能被展示——公开值班簿与年度失败/修复摘要本身就是展示物的一部分。**可推广**指别处能用自己的家具、表单和人员照着搭一套，而不必引进同一批设备或同一家供应商，最小载体「公开交接桌」就是按这条标准设计的。三条里最难的是第二条：**只展示成功的场景无法被推广，因为没有人知道它在什么条件下会坏。**

人群覆盖逐类列出，不合并成「公众」一词：居民、青年人才、企业与初创团队、高校师生、产业维护者、国际访客与游客，以及在智能化中最容易被落下的弱势群体——老年人、残障与临时行动不便者、听障与言语障碍者、视障与低视力者、无智能手机或不使用 App 者、儿童与青少年。每一类写明空间载体与关联场景；评价口径一律聚合，不识别个人。

| 用户画像 | 核心需求 | 空间载体 | 关联场景 | 评价口径（聚合、不识别个人） |
| --- | --- | --- | --- | --- |
| 开发者 | 可复现的测试与贡献记录 | 公开验证街、公开交接桌 | SCN-01、SCN-07 | 贡献条目数、复现成功率 |
...
| 无智能手机或不使用 App 者 | 通行、问路与办事不以持有设备或安装 App 为条件 | 固定导视、人工窗口、纸本目录、零号接管亭 | SCN-06、SCN-08、SCN-12 | 非 App 通行可用率、纸本目录覆盖率 |
| 听障与言语障碍者 | 不依赖语音交互即可完成求助与办事 | 文字受理的人工窗口、纸本表单、固定导视 | SCN-06、SCN-08 | 文字受理窗口覆盖率、书面回执时长 |
| 视障与低视力者 | 路径与场景信息可触可听，不依赖屏幕 | 触觉走廊图、触觉导视牌、三分钟音频导览、人工问路 | SCN-05 | 触觉件到位率、断点修复时长 |
| 儿童与青少年 | 公共空间可安全通行，且不被识别、画像或计入任何模型 | 无障碍连续路径、夜班安全灯、社区照护排班桌 | SCN-05、SCN-09、SCN-10 | 未成年人相关采集条目数（目标恒为 0）、夜间照明连续率 |
...
### 八个共享栈：要素保障不是名单，是可被占用也可被收回的接口

产业政策最常见的失败方式，是把生态写成一份入驻名单——名单上的主体一旦不来，整套空间安排就落空。本方案不列企业名称、不预设入驻主体，改为把要素做成**八个共享栈**：每一栈都是「一段可逆空间 + 一套接入规则 + 一份可复核记录」，谁满足规则谁接入，不满足就退出，空间回到公共使用。八栈的空间载体不是新造的概念，就是二十个更新类型试验单元 [data:geometry/buildings.geojson#BLDG-001] 的四种类型与用地分区 [data:geometry/land_use.geojson#LU-001]。

**产业支撑因此由四件事共同构成，缺一件另外三件都不成立**：AI 创新生态（八个共享栈的接口与回路）、要素保障（土地、空间、资金、人才、算力、数据六栈的接入规则）、**技术测试**（研制交接场的封闭试验带、机器人路权沙盒与端侧算力能耗试验站——**测试场地必须先于产业招引存在**，否则新技术在这条带上只能被展示而无法被验证）、以及场景开放（十二个场景节点按问题票公开征集接入方，不定向指派）。四件事的共同点是都以**可拒收的接口**表达：接不上就退出，空间回到公共使用，不产生沉没的专用设施。

| 共享栈 | 供给什么 | 空间载体（更新单元类型 / 用地） | 接入规则 | **明确不承诺** | 可复核记录 |
| --- | --- | --- | --- | --- | --- |
| 土地栈 | 可逆的空间使用权，先时段、后产权 | 全部四类单元；LU-008 全栈自主研发与试验用地 | 先申请时段与可撤除布置，验证需求成立后再谈改造 | 不承诺产权、租金优惠或长期排他使用 | 时段占用与撤除记录 |
| 空间栈 | 开放研发院 5、验证工坊 5、共享服务栈 5、社区协作屋 5 | BLDG-004/008/012/016/020 等四类各 5 个 | 按单元类型匹配用途，首层须保持连续可进入 | 不承诺建筑面积指标或新建规模 | 单元用途与开放时段台账 |
| 产业栈 | 模型、芯片、机器人与垂直应用之间的接口规格 | 验证工坊；LU-009/LU-010 模型验证与产业协作用地 | 接口先公开规格与失败边界，再开放对接 | 不承诺技术路线选择或产业链配套 | 接口版本与变更记录 |
| 资金栈 | 透明申请入口与服务导航 | 共享服务栈；LU-007 科技服务与成果转化用地 | 只做流程解释与材料指引，申请仍走既有渠道 | **不承诺补贴、投资、税收优惠或评审加分** | 咨询量与流程指向的聚合统计 |
| 人才栈 | 高校、开发者、维护者与居民之间的岗位与协作接口 | 社区协作屋；LU-006 教育协作与开源学习用地 | 岗位规格先于人选，八个角色的资质与缺岗禁止条件公开 | 不承诺落户、编制、职称或薪酬安排 | 岗位规格版本与在岗状态 |
| 算力栈 | 端侧计量与按需接入 | 验证工坊内的端侧能耗试验站 SCN-03 | 先独立计量、后接入；能耗与温升阈值触发即停 | 不承诺算力配额、价格或优先级 | 独立电表读数与停机记录 |
| 数据栈 | 授权、最小化、撤回与审计四件套 | 数据授权演练场 SCN-04；开源交接场 | 无授权不采集，撤回即删除，删除须可验证 | 不承诺数据开放范围，也不代行数据权属判定 | 授权单、撤回回执与删除验证 |
| 场景栈 | 城市问题、测试规则、正负结果与退出记录 | 全部十二个场景节点 [data:geometry/public_space.geojson#SCN-01] | 问题先公开、规则先写清，结果正负同时发布 | 不承诺场景开放数量或验证通过率 | 十二条双联交接账与规则检查报告 |

八栈之间不是并列的服务菜单，而是**一条有次序的接入链**：土地栈与空间栈决定「进不进得来」，产业栈与算力栈、数据栈决定「能不能做」，人才栈与资金栈决定「做不做得下去」，场景栈决定「做完了算不算数」。次序颠倒就会出现常见的失败形态——先谈补贴与产权、再补规则与记录，最后无人能说清一项服务为什么上线、由谁停用。

### 不同阶段的团队在这条带上分别拿到什么

生态的可用性取决于最小单位能否独立成立。下表按团队规模而非企业名录组织，每一行都指向已有的空间载体与场景节点；门槛与退出条件与十二个场景共用同一套口径，不因主体规模而放宽或收紧。

| 团队类型 | 核心诉求 | 主要落点 | 常用共享栈 | 进入门槛 | 退出与回收 |
| --- | --- | --- | --- | --- | --- |
| 个人开发者与学生 | 一张能复现的桌子、一份能引用的记录 | 校园成果接力站 SCN-07、公开交接桌 | 场景栈、数据栈 | 提交可复现说明与许可标注 | 记录留存，工位随时释放 |
| 初创团队（试验期） | 低成本试验、合规入口、失败不致命 | 验证工坊、共享服务栈 | 空间栈、算力栈、资金栈 | 时段申请 + 最小数据声明 + 停用办法 | 时段到期即撤除，空间当日恢复 |
| 成长期企业（产品化） | 真实场景验证与责任边界清晰 | 模型红队交接台 SCN-01、机器人路权沙盒 SCN-02 | 产业栈、场景栈 | 版本、风险条目与人工负责人齐备 | 未闭环风险即退出，沙盒围界拆除 |
| 成熟企业与院所 | 标准共建、成果开源与长期协作 | 开放研发院、开源交接场 | 产业栈、人才栈 | 接口规格公开、失败结果同样公开 | 撤回须同步撤回其接口依赖方 |
| 运营与维护方 | 故障、版本与责任可见 | 维修可见驿 SCN-08、百年值班簿 | 人才栈、场景栈 | 值班簿签收与工单闭环 | 交班未闭环项不得消失 |

**这张表刻意不写「引进多少家、孵化多少个」。** 那类数字需要产业主管部门的目标与真实招商安排支撑，本方案既无授权也无依据；能由本方案负责的，是让每一类团队都能指着某一个具体单元、某一个具体场景说出「我从哪进、按什么规则做、做完留下什么、什么情况下必须走」。海淀「1+X+1」产业体系的内涵由产业主管部门定义 [source:HAIDIAN-2025-STATISTICAL-BULLETIN]，本方案只在其「AI+」融合方向上提供空间接口与验证工具，不代表任何一方已同意参与。

### 产业环节与空间供给：五个环节，四类单元，一条回路

创新不是从研究直接到应用的一条直线，中间的验证与开源两段最容易缺空间——它们既不属于实验室，也不属于市场，因此在常规供给里没有位置。本方案把这两段单独给出空间。

| 环节 | 缺的是什么 | 本带的空间供给 | 对应用地 | 可逆性 |
| --- | --- | --- | --- | --- |
| 研究 | 不缺（高校院所已有） | 不重复建设，只做接口 | LU-008 | — |
| **验证** | 能被独立方看见的测试场地 | 验证工坊 ×5 + 三个产业验证场景 | LU-009 / LU-010 | 围界与设备可撤除 |
| **开源** | 成果交出时的公共桌面 | 开源交接场 + 校园成果接力站 | LU-006 / LU-007 | 展陈可移动 |
| 应用 | 真实但可退出的城市场景 | 十二个场景节点 | LU-003 / LU-005 | 退出后恢复公共使用 |
| 反馈 | 问题回到研发端的通道 | 百年值班簿 + 双联交接账 | 全带 | 记录不可撤销 |

四类更新单元与五个环节的关系因此是明确的：**开放研发院**承接研究到验证的交出，**验证工坊**承接验证本身，**共享服务栈**承接开源与要素接入，**社区协作屋**承接应用与反馈。二十个单元的联合基底为 221,014.099 平方米 [metric:building_footprint_area_sqm]，**只用于比较空间供给类型，不表示现状建筑总量、拆除对象或获批新建规模**。
...
**这条带把「连续无高差」写成开放前提，依据是它继承了一条铁路的纵坡——这是线形事实，不是坡度实测。** 主轴 [data:geometry/roads.geojson#ROAD-001] 沿废弃铁路线形布置，而铁路线形本身就是为低而连续的纵坡而设计的——1909 年詹天佑在关沟遇到爬不上去的坡，用「人」字形折返换高度，正是同一个约束的另一面。普通城市街道随地形起伏，这条带不是：**它的可达性不是设计出来的优待，是线形遗产给的**。**所以这里给出的是一个有出处的预期，核验它的是纵断面测量，不是这段话**；未测出之前，主轴按「目标状态：连续无高差」处理，与已开放同义的写法在本包任何地方都不成立。

**因此坡度检查的位置也是设计出来的：八处交汇点，而不是全线均匀布点。** 八条东西缝合支线 [data:geometry/roads.geojson#ROAD-002] 要从铁路标高接回城市街道标高，交汇点数等于支线条数，共 **8 处** [metric:stitch_grade_break_crossing_count]。因此本方案把坡度检查放在这 8 处而不是全线：**任一处不能做到无高差连续通行，该处支线即不开放**，回到固定导视与人工服务；这与十二个场景「缺一不开」的口径一致，判据在同一层。**主轴最大纵坡保持未赋值** [metric:spine_longitudinal_gradient_max_pct]——需现状纵断面测量，几何图层无高程字段，本方案不以「铁路通常平缓」推算出一个数字来充数。
...
**慢行与蓝绿一图之内同时给出走廊定位与交接断面**：左侧是走廊真实轮廓与十二个接管点的沿线位置，右侧是断面的六段次序与相邻规则——**规则本身就是设计内容，所以它必须和它约束的那条走廊出现在同一张图上**，而不是分置两图各自成立。

![慢行主轴与蓝绿基底：走廊真实轮廓、十二个接管点，以及首层交接台—无障碍步道—零号接管亭—保留铁轨—机器专用道—绿化休憩带的断面次序与相邻规则](assets/figures/mobility-bluegreen.png)

> **这张图怎么看。** 左图红线是连续步行骑行主轴，八条蓝线是东西缝合支线，浅绿是蓝绿舒适基底；青色圆点 01–12 是十二个人工接管点，编号与右侧清单逐条对应。右上 A／B／C 三条是接管的三个前提。右下 SECTION 的六段只表达次序与相邻规则，不表达任何尺寸；底部黑条是那条相邻规则本身——人工窗口、步道、停用入口三者必须连续可达，机器专用道不得插入其间。
...
公共界面由要素次序决定：人工窗口、连续无障碍步道与物理停用入口三者连续可达，机器专用道不插入其间；保留铁轨齐平嵌入铺装。断面只固定次序与相邻规则，不标注尺寸——宽度、标高与工程做法取决于道路红线和现状测绘，公开资料包尚未提供，道路面积与道路率保持未赋值 [metric:road_area_sqm] [metric:road_ratio]。

![交接断面：首层交接台—无障碍步道—零号接管亭—保留铁轨—机器专用道—绿化休憩带的次序与相邻规则；不表达尺寸](assets/figures/handover-scene.jpg)

公共空间由一套可复制、可撤除、可维护的组件库承担。十个组件共用煤黑—信号红—电气青—米白四色语汇与同一套标识规则，任一处可在不改动周边的前提下增设或撤除；组件不含摄像与人脸能力，智能层作为可拔插附件。

| 组件 | 功能 | 材料与尺度原则 | 可撤除性 | 主要部署位置 |
| --- | --- | --- | --- | --- |
| 公开交接桌 | 代码评审、市民质询、成果发布的长桌 | 耐候木与钢，可分段拼接 | 整体可拆运 | 原点社区交接场 |
...
**触觉导视牌一行已有可制版的实物文件**：触觉走廊图 [data:assets/tactile/tactile-corridor-map.svg]，中英各一份，可用于热敏膨胀纸打印或激光雕刻。走廊轮廓、慢行主轴与八条支线取自真实几何；三座交接场用圆、方、三角三种可触辨形状区分，十二个接管点用实心圆点，四处地标用菱形，**全图不依赖颜色即可辨认**。凸起高度、点径与间距按 GB/T 15720 与无障碍设计规范由制版厂商核定。

四个“朝圣地标” [metric:civic_landmark_count] 是可使用、可维护、可撤回的公共设施：北部“交接钟”显示模型版本、适用范围和下一次人工复核时间 [data:geometry/public_space.geojson#LANDMARK-01]；中央“公开交接桌”是一张可举行代码评审、市民质询与成果发布的长桌 [data:geometry/public_space.geojson#LANDMARK-02]；南部“百年值班簿”把铁路维护记录、开发者贡献和城市服务复盘并置 [data:geometry/public_space.geojson#LANDMARK-03]；沿线“零号接管亭”提供实体停用按钮、人工求助、纸本地图和无障碍服务 [data:geometry/public_space.geojson#LANDMARK-04]。荣誉系统记录修复漏洞、维护公共代码、提出有效异议和改善无障碍的贡献，不按资本或流量排名。

城市风貌采用“铁路运维账簿”语汇：煤黑结构、信号红阈值、电气青接口、米白记录面；导视以站号、班次、版本、责任人和更新时间组织。文化叙事连接三种劳动伦理：京张工程建设的自主与维护，中关村创新文化的试验与分享，AI 时代对数据、模型和公共后果负责的日常值守；不把蒸汽机符号贴到科技园。国际传播语为 “Intelligence moves. Responsibility stays visible.”，中文对应“智能流动，责任可见”。

文化与艺术资源以“连接而不占用”的方式进入这套语汇：清华园火车站方向作为交接线北段的历史起点，其展示以站台尺度的记录装置、纸本档案和口述史接口衔接，不主张对文物本体作任何改动，具体保护措施与开放方式以文保主管部门意见为准 [depth:risk_missing_data]；北京电影学院所在的西土城路方向作为艺术资源接口，承担影像记录、公共放映与叙事共创的合作可能，属于协作意向而非已确定安排。清河与小月河两条水系分别锚固北段与中段的公共生活：滨水段优先树荫、可坐可停与连续慢行，水岸设施可撤除、可维护。对具备更新潜力的区域，风貌管控采用“可校验的引导”而非固定数值：建筑高度与强度待官方控规确定；体量以院落、穿行门廊和界面分段控制；屋顶形态按“设备整合优先、第五立面可读、避免大面积反光与眩光”引导，并为分布式能源与雨水设施预留位置。上述引导属城市设计建议，最终以法定审批为准 [depth:height_massing_character]。《城市设计管理办法》把公共空间、城市风貌、建筑高度体量与界面控制列为城市设计可表达的内容，把项目规划许可与场地量化控制留给法定程序 [source:MOHURD-URBAN-DESIGN-MEASURES]；成果据此分两类：城市设计能回答的写成可核对的形态原则，须由法定程序回答的留空并写明触发条件。

## 更新项目清单、实施政策与分期计划

### 可实施性四件事：阶段路径、试点区域、参与主体与指标，逐项对齐

可实施性最容易被谈成一句"分三期推进"。本节拆成四件可分别核验的事，每件指向包内可打开的文件。

| 要素 | 本方案给出什么 | 在哪里可以打开核验 | 推进条件 |
| --- | --- | --- | --- |
| **阶段路径** | 三道**条件门**而非时间表：一闸「交回基线」（官方资料补齐、现状盘点、公共导视、低风险人工主导试点）；二闸「开源交接场」（须继承一闸的正负结果，权属、消防、社区与无障碍评估通过）；三闸「全带运营」（须再过交通、市政、文保、隐私、安全与持续预算审查） | `geometry/phasing.geojson` 逐期写入进入闸门、验收闸门与回滚状态，**不写固定年份** | 前一闸的正负结果未继承即不进入下一闸 |
| **试点区域** | 三座交接场（众智园研制、原点社区开源、大钟寺城市）＋两段自选垂直试验段（学院路教育信软段、西直门外生活医疗段）＋十二个场景节点 | `geometry/key_areas.geojson`、`geometry/public_space.geojson#SCN-01…12` | 场地权利与责任主体未获授权即不落点 |
| **参与主体** | 八个**岗位规格**（不是任命）：资质、权限、缺岗禁止条件与升级路径；P01—P06 每个行动包写明 A / R / C-I 三类角色 | `visual/assets/governance/role-spec.json`，`assignment_status` 一律 `unassigned` 且在 schema 中被约束为常量 | 角色归属由主管部门确定；双联两侧未指派即不放行 |
| **指标** | **49 项指标，37 项已赋值可复算**；12 项未赋值的每一项都有完整测量协议 | `metrics.json`（含 `formula` 与 `source_files`）、`visual/assets/governance/measurement-protocol.json` | 发布门槛任一不成立即保持未赋值 |

**指标这一项的口径。** 本包 49 项指标中 **37 项已赋值**——场地面积、绿地率、公共空间占比、主轴长度、场景节点数、用地要素数等，全部在 EPSG:4548 中按 `formula` 与 `source_files` 可独立复算，任何人拿同一份几何都能得到同一个数。其余为法定控制与产业绩效两类，其目标值须由主管部门依官方控规、导则与运营基线确定，本方案不代为设定，但已为每一项写明可直接执行的测量协议（定义、采集角色、采样单元与范围、频率或触发、分母、缺失值处理、质量检查、争议复核、隐私与留存、发布门槛），并把三条边界写进 schema 而非散文：目标值类型约束为 `null`、缺失值禁止插值与区级平均替代、发布门槛任一不成立即不发布 [data:visual/assets/governance/measurement-protocol.json]。**发布一套测量协议不等于发布一个目标——方法做完，决定留给有权做决定的人。**

更新项目不以一次性大拆大建启动，而以可逆、公开、能形成反馈的最小单元启动。**十二项概念项目此前只写成一行顿号串，紧跟一句"每项都需明确业主、责任人、许可、预算、维护、退出"——把字段列成要求，却一格没填。** 下表把能填的都填上：落点回到几何、承载场景回到 SCN 编号、责任角色回到 `role-spec.json` 的八类岗位、放行条件回到三道闸门。**两件事全表相同，因此只说一次**：八类角色的 `assignment_status` **一律是 `unassigned`**——本方案给的是岗位规格而不是人事安排，任何一格写上具体单位或姓名都是虚构；**一次性与持续预算一律未赋值**，理由不是估不出来，而是预算取决于权属、许可与规模三者确定之后的实际条件，先给一个"量级"会在后续被引用成本方案的承诺（这与法定容积率、高度保持 `null` 是同一条口径）。

| 项目 | 落点（回到几何） | 承载场景 | 责任角色类别（均未指派） | 放行条件 | 规模口径 | 退出处置 |
| --- | --- | --- | --- | --- | --- | --- |
| 01 交接线树荫与基础导视 | 沿主轴全线 [data:geometry/roads.geojson#ROAD-001]，9,499.778 m [metric:handover_spine_length_m] | 不承载单一场景，是其余十一项的前置条件 | R-PUBLIC-SPACE 公共空间管理 | 第一闸「交回基线」 | 线性工程，株距、树种与导视版面依市政规范与现场测绘 | 导视为固定牌面，无智能层可撤 |
| 02 北部模型红队交接台 | 众智园 [data:geometry/key_areas.geojson]，研发院首层 | SCN-01 模型红队交接台 | R-TEST-SAFETY 受控测试安全 | 第二闸「开源交接场」 | 首层内点状设施，尺寸依现场测绘 | 恢复为共享评审室 |
| 03 机器人路权封闭沙盒 | 众智园，封闭测试院 | SCN-02 机器人路权沙盒 | R-TEST-SAFETY 受控测试安全 | 第二闸；另需安全员到岗与保险生效 | 封闭院落，围界范围依安全评估 | 围界拆除，回到内部院落 |
| 04 端侧能耗试验站 | 众智园，设备平台 | SCN-03 端侧能耗试验站 | R-TEST-SAFETY 受控测试安全 | 第二闸；另需独立计量与热管理核定 | 设备平台上的计量箱，容量由能源与市政主管部门核定 | 计量箱撤除，平台转设备位 |
| 05 中央公开交接桌 | 原点社区，公共交接面 | SCN-04 数据授权演练场 | R-PUBLIC-SPACE 公共空间管理 | 第二闸；另需七项 go/no-go 档案全部成立 | 点状构筑物，**位置、尺寸、材料、无障碍净宽与桌面高度须依现场测绘与规范确定，本方案不预设数字** | 可逆构件拆除，恢复通行 |
| 06 校园成果接力站 | 原点社区，校园边界 + 开放发布厅 | SCN-07 校园成果接力站 | R-SERVICE-OPS 公共服务运营 | 第二闸；另需高校成果授权审查通过 | 既有空间改造，不新增建筑面积 | 恢复为发布与展陈空间 |
| 07 无障碍路径副驾 | 全带连续步行面 + 触觉导视 | SCN-05 无障碍路径副驾 | R-SERVICE-OPS 公共服务运营 | 第一闸；另需八处支线交汇点逐处通过坡度检查 [metric:spine_longitudinal_gradient_max_pct] | 沿线系统，非点状 | 触觉导视保留，智能层下线 |
| 08 维修可见驿 | 沿线小型服务点 | SCN-08 维修可见驿 | R-PUBLIC-SPACE 公共空间管理 | 第二闸 | 沿线点状，数量与间距依既有工单体系 | 纸面报修点保留 |
| 09 南部城市交接厅 | 大钟寺 [data:geometry/key_areas.geojson]，社区服务设施首层 | SCN-06 公共服务翻译台、SCN-09 社区照护排班桌（一个场所承载两个场景） | R-SERVICE-OPS 公共服务运营；R-COMMUNITY-CARE 社区服务与照护 | 第三闸「全带运营」 | 既有首层改造，不新增建筑面积 | 人工窗口继续，屏幕移除 |
| 10 百年值班簿 | 全带，与 11 共用值班簿柱 | 贯穿十二个场景的公开记录载体 | R-INDEPENDENT-REVIEW 独立复核；R-DISSENT 异议代表 | 第一闸 | 纸本＋柱体，纸本为主、无智能层依赖 | 纸本继续，无可撤部分 |
| 11 口述史问答亭 | 大钟寺，遗址公园节点 | SCN-11 京张口述史问答亭 | R-CULTURE-VENUE 文化与场地责任 | 第二闸；另需馆藏清权完成 | 点状亭体，尺寸依现场测绘与文保审查 | 亭体保留为纸本目录点 |
| 12 全带开放交接周 | 全带公共交接面 | SCN-12 全球交接周线路 | R-EVENT-OPS 年度活动运营 | 第三闸 | 年度临时布置，事后全部撤除 | 全部恢复日常通行 |

**这张表能填满的原因，是它只填了本包已经有实体的东西**：落点回到 `geometry/*.geojson`，场景回到十二个 SCN 编号，角色回到 `role-spec.json` 的八类岗位规格，放行条件回到 `phasing.geojson` 的三道闸门。**填不了的两列（具名主体与预算）没有用量级或"参考值"补上**——未完成前不得把概念项目写成已落地安排，这一条不因为表格看起来更完整而放松。
...
### 首个原型的单页 go/no-go 档案：七项缺一不可，未知处写 null 并给启用条件

**选「公开交接桌」作为第一个原型，因为它是全案里风险最低的一件**：一张桌、两块台面、四张卡，无供电、无联网、无固定设施，撤除后地面不留痕迹。下表是它的 go/no-go 档案，**七项字段一项不省**；本包能确定的填实值，不能确定的写 `null` 并写清启用条件——**不以「机器校验通过」冒充「可以开工」**。

| 字段 | 当前值 | 启用条件（满足后才由 no-go 转 go） |
| --- | --- | --- |
| 场地授权 | `null` | 取得场地权属方书面同意与临时占用许可；范围以一张桌的占地为限 |
| A / R / C-I 角色 | **A：`R-PUBLIC-SPACE`**；R：交通、景观、无障碍、照明、维护；C-I：沿线使用者与 `R-DISSENT` [data:visual/assets/governance/role-spec.json] | 岗位由具名人员实际指派并公布，`assignment_status` 由 `unassigned` 转为已指派 |
| 许可与保险 | `null` | 公众责任险生效，且活动许可覆盖「桌面无电、无网、无采集」的实际形态 |
| 一次性与持续预算 | `null` | 运营主体确定后给出一次性布设与年度维护两项数字；无数字前不排期 |
| 维护与数据删除责任 | **有**：桌面只产生纸质卡片，卡片归档进公开值班簿；**不产生任何电子个人数据，因此无需删除窗口**——这是选它作首个原型的另一个理由 | — （已成立） |
| 人工兜底 | **有**：桌面本身即无 AI 形态；智能层不参与交出与接入判断，触觉走廊图与人工问路独立可用 | — （已成立） |
| 回滚演练结果 | `null`｜**现为 0/12，未执行** | 在获授权、交出与接入由两个不同角色分离担任的条件下完成一次可观察演练，记录正负结果、服务中断时长与复核人；未完成前智能层保持 OFF |

**七项里三项已成立、四项为 `null`，因此本原型当前判定为 `no-go`。** 这不是保守表述，是这份档案的用法：任一项为 `null` 即不得开工，而每个 `null` 都写明了由谁在什么条件下才能填上。
...
品牌与 IP 是一套可延展、可授权、可撤回的**视觉系统**，按判据的三个问法各有落点：**辨识度**由「两条铁轨＋交接棒＋信号括号」的母题与四色语义承担，四色各自绑定一种判断而不是装饰色；**延展性**由同一套标识规则贯穿导视、组件库、出版物与四个季节性 IP 承担，新增载体照规则套用即可，不需要重新设计；**国际传播力**由中英双语同级呈现、可直接使用的国际传播短文与全球交接周承担。整套系统的图面定义见 [data:assets/figures/logo-identity.png]。基础层是名称与标识：中文“京张交接线”、英文 JING-ZHANG HANDOVER LINE，Logo 由两条平行铁轨、一个交接棒与一枚信号括号组成，煤黑代表可审计责任、信号红代表必须停下并由人判断的阈值、电气青代表开放接口、米白代表可阅读的公共记录。应用层是导视、组件库与出版物共用同一套标识规则：站号、班次、版本、责任人、更新时间五项信息必须同时出现，缺一项即视为标识不完整。活动层是四个季节性IP——开源交班、夜班维护者之夜、全球交接周、年度复盘班，各自有固定视觉母题但共用同一标识系统。授权层规定：标识可供非商业的公共传播、学术与社区活动使用，商业使用须另行授权；文化标识系统与一带整体Logo系统分列，不互相替代；任何使用都不得暗示政府背书或实施承诺。

**一处需要说明的例外**：`report/proposal.html` 与 `report/proposal.en.html` 的配色（藏蓝 `#245b8f` 与冷灰蓝底）**不属于本视觉系统**——这两个页面由仓库的公共渲染器 `scripts/render_proposal_html.py` 生成，样式硬编码在该脚本内、全场参赛者一致（实测其他投稿包的同名文件亦含 `#245b8f`）。投稿 PR 不得改动 `scripts/`，因此本包无法把它们纳入四色系统。**本包自有的视觉系统覆盖范围是**：26 张图件、四套图纸、`visual/index[.en].html` 两个离线页面与画廊封面。

**这套方案主张无障碍，所以先量了自己的图。** 四色此前既当填充色又当文字色，量过之后发现后一种用法不达标：值黄写在卡面上只有 **1.50:1**，绿廊色 1.73、电气青 2.53、信号红 3.37，全部低于 WCAG 2.1 SC 1.4.3 的正文阈值 4.5 [standard:WCAG-CONTRAST] [source:W3C-WCAG-21]。**最严重的一处正是最该看清的一处**——三处重点区卡片的「复算面积」大数字用卡片主色写在卡面上，大钟寺那张是黄色，一个关键指标几乎读不出来。2026-08-17 的改法是**把填充与文字拆成两组值**：亮版继续用于色条、色块与图例（那里承担的是区分而不是阅读），文字改用同色相只降明度的深版，因此「四色各绑一种判断」这层语义一字未动。改完后 26 张图件的七种文字色在两种纸面底上**最低 4.60:1**，逐色测量值、公式与被替换的旧值见 [data:visual/assets/governance/figure-contrast-report.json]，一行命令即可复算。**判定用的是更严的一档**：WCAG 对大号文字允许放宽到 3.0，本包一律按正文阈值 4.5 判，不为大字号开例外。**WCAG 是自愿采纳的技术标准、不是中国法定强制要求**，因此按前述「法定下限与自设标准分列」的同一口径登记为自设标准 [standard:WCAG-CONTRAST]。报告的适用范围与三项未决（填充色块之间的相互区分度、公共渲染器生成的两个页面、真实用户可读性）连同复核触发器一并登记在 `not_claimed` 字段与未决项 A-CONTRAST-001 里，其中一条已排除的做法也写明了：不靠加深填充色块提高区分度——那会破坏它与文字深版的层级关系；可行方向是形状／纹理冗余编码，触觉走廊图的圆／方／三角已是同一思路。

实施主体按“谁受益、谁维护、谁能停用”确定。八个角色写成可交接的岗位规格 [data:visual/assets/governance/role-spec.json]，每个角色写明最低资质、能决定什么、不能决定什么、值守方式、无人值守时的兜底、启动权与停用权、需双人复核的事项及其独立第二复核角色、分歧升级路径、交班时必须移交的证据，以及缺岗时什么不得开放。

| 角色 | 停用权 | 需双人复核的事项 | 第二复核角色 | 缺岗时什么不许开（逐字取自 `prohibition_when_unstaffed`） |
| --- | --- | --- | --- | --- |
| R-PUBLIC-SPACE 公共空间管理 | 可单方停用 | 无障碍路径整改验收、严重安全断点解除隔离 | `R-INDEPENDENT-REVIEW` | 该段不得开放智能层服务；仅保留固定导视、常亮照明与人工求助电话。 |
| R-TEST-SAFETY 受控测试安全 | 可单方停测 | 急停演练结果确认、严重风险后恢复测试 | `R-INDEPENDENT-REVIEW` | 任何受控测试不得启动；机器专用道回到封闭或普通设备位。 |
| R-SERVICE-OPS 公共服务运营 | 可单方切回人工 | 发布对象的许可与撤回入口核验、涉及权利争议的下架决定 | `R-INDEPENDENT-REVIEW` | 不得只提供智能界面；无人工窗口时该项服务不得上线。 |
| R-COMMUNITY-CARE 社区服务照护 | 可单方切回人工派单 | 涉及居民权益的服务变更、自动派单冲突后的恢复 | `R-DISSENT` | 不得以自动派单单独运行；无人工兜底时相关场景不得开放。 |
| R-CULTURE-VENUE 文化与场地 | 可单方隐藏争议条目 | 荣誉条目的证据是否足以记录、权属争议条目的恢复展示 | `R-INDEPENDENT-REVIEW` | 不得展示来源或授权不清的对象；无人值守时智能展项停用。 |
| R-EVENT-OPS 年度活动运营 | 可单方缩小或取消 | 无障碍与应急检查覆盖确认、撤场完成确认 | `R-PUBLIC-SPACE` | 不得举办；已布设的临时设施须在无运营角色前撤除。 |
| R-INDEPENDENT-REVIEW 独立复核 | 可单方否决恢复 | 自身复核意见的归档 | `R-DISSENT` | 任何需双人复核的合并、发布或恢复均不得进行。 |
| R-DISSENT 异议与被忽略人群代表 | 可单方触发停用 | 单方停用后的公开说明 | `R-INDEPENDENT-REVIEW` | 不得新增面向公众的智能场景，也不得扩大既有场景范围。 |
...
**规格能不能被检验，不看「谁负责」，看「没人负责时什么必须停」。** 一份只写「某角色负责某事」的规格，在角色被指派之前无法验证——没有人可以对照它检查任何东西。而「缺岗时什么不许开」是一条**今天就能核对**的规则：它不依赖任何任命，只依赖现场此刻有没有人。上表最后一列即为这一条，逐字取自 [data:visual/assets/governance/role-spec.json] 的 `prohibition_when_unstaffed`；停用权、需双人复核的事项与第二复核角色一并列在同一行——**同一字段不再有转述版与逐字版并存**。

**八条读下来是同一句话的八个具体形态：智能层是可以停的那一层，人工兜底是不能停的那一层。** 缺岗时被关掉的一律是智能服务、临时设施与展项，被保留的一律是固定导视、常亮照明、人工窗口与纸面/电话通道。**没有任何一条允许「无人时由系统自行继续」**——这正是本方案与「无人值守智能化」的分界线。第二复核角色一列还给出一条可校验的结构约束：**没有任何角色的第二复核指向自己**，`R-DISSENT` 与 `R-INDEPENDENT-REVIEW` 互为复核，不构成自证闭环。

年度运营形成四季班表：春季“开源交班”发布城市问题与公共代码；夏季“夜班维护者之夜”展示维修和可靠性；秋季“全球交接周”组织模型、机器人与公共服务的可解释演练；冬季“年度复盘班”公布停用、投诉、修复和下一年退出清单。开发者社区按“问题票—公开遴选—最小复现—受控验证—维护者接班—可复用成果”运行，每一步都留下责任角色和许可证；居民通过问题发布、场景陪审和无AI服务测试持续参与。人才与企业转化按“参观—问题配对—受控试验—正负结果公开—专业转介—本地合作或明确退出”推进，不承诺招商金额、补贴或政府采购。国际传播除口号外固定输出双语协议卡、90秒无障碍路线说明与年度失败/修复摘要，使“全球人工智能产业高地和朝圣地”目标可被外部复核而不只可被拍摄。

### 长期运营价值：品牌资产、活动机制与合作通道三件事分开写

任务书把长期运营价值拆成**品牌资产、活动机制、合作通道**三项。三者最常见的混淆，是把品牌当成 Logo、把活动当成一年一次的开幕式、把合作当成签约仪式——那样的话，方案结束之日就是资产归零之时。本方案按「离开设计团队之后还能不能继续运行」来组织这三项。

| 要素 | 沉淀为什么 | 谁在没有我们的情况下也能继续用 | 失效条件 |
| --- | --- | --- | --- |
| **品牌资产** | 一套可延展、可授权、**可撤回**的标识系统：名称与四色语义、五项必现信息（站号/班次/版本/责任人/更新时间）、组件库与出版物规则 | 任何运营方按同一规则做导视与出版物；缺任一项即视为标识不完整，不需要我们判定 | 出现暗示政府背书的用法，或商业使用未另行授权 |
| **活动机制** | 四季班表：春「开源交班」、夏「夜班维护者之夜」、秋「**全球交接周**」、冬「年度复盘班」，各有固定视觉母题但共用同一标识系统 | 主办方按 P06 行动包的开放门槛与撤场检查自行组织；正负结果同时公开是硬要求 | 主体、许可、持续预算、安全、值守或撤场任一不清即缩小或取消 |
| **合作通道** | 五条常设接口（北纬社区、未来科学城、怀柔科学城、北京经济技术开发区、京津冀节点）＋开发者问题票＋高校成果接力站；每条都写明**可交换的最小成果与拒收条件** | 合作方按接口规格自行接入，不必先与任何一方达成框架协议 | 主体或许可未确认不落点；知识产权、测试责任或适用范围不清即不接收 |

**三项共用一条底线：都以「可撤回」为前提。** 品牌可撤回授权，活动可缩小或取消，合作通道可拒收也可退出。一个不能被退出的合作不是通道，是绑定；一份不能被撤回的授权不是资产，是包袱。

### 国际传播力：把「全球关注」建立在可复核的记录上，而不是口号上

提升一带的**全球关注**度与辨识度，本方案不靠形容词，靠三样别处不容易复制的东西：

1. **公开的失败记录。** 年度失败/修复摘要与停用原因逐条公布——多数城市 AI 项目只发布成功案例，一条愿意公布自己停过什么、为什么停的线路本身就是辨识度。
2. **可被任何人复核的协议。** 双语协议卡、双联交接账 schema 与十二条场景实例随包提供，境外同行不必来现场也能复用规则、指出问题。
3. **一个不需要门票和 App 的现场。** 访客沿无障碍路线即可查看成功、失败与修复；**选择不用 AI 也能获得等价基础服务**——这一点对国际访客与本地居民同样成立。

固定输出为双语协议卡、90 秒无障碍路线说明与年度失败/修复摘要；传播不得暗示政府背书、采购承诺或法定实施。

### 可转化性：专业团队、运营团队与传播团队各自能从哪一件东西开始接着做

一份城市设计方案的可转化性，不取决于它写得多完整，而取决于**接手的人第一天能不能不联系设计方就开始工作**。本方案对三类接手团队各留了一个明确的起点，起点都是包内可直接打开的文件，不是需要再解释一遍的理念。

**这句话要和一件事一起读，否则它是空的**：本包的确定性渲染脚本**未随包提交**（投稿目录不接受 `.py`，见构建溯源一节）。因此「第一天能开始」的准确边界是——**不需要脚本就能做的**：读九个图层、按 `metrics.json` 里逐条给出的 `formula` 与 EPSG:4548 用 shapely＋pyproj 复算全部面积与比值（本包已实测 20 项、不一致 0 条）、按 `role-spec.json` 指派岗位、按 P01–P06 与 go/no-go 档案启动一个原型；**需要脚本才能做的**：逐字节复现同一批图件与图纸。后者可通过 Issue 索取补丁获得。**把「可转化」写成「什么都不缺」是不诚实的，这里给的是它实际能到哪一步。**

- **专业团队继续深化，从几何与未知项开始。** 九类 GeoJSON 在 EPSG:4548 下可独立复算，任何人拿同一份几何得同一个数；十二项未赋值指标各自带完整测量协议（定义、采集角色、采样单元、频率、分母、缺失值处理、质量检查、争议复核、隐私与留存、发布门槛）[data:visual/assets/governance/measurement-protocol.json]。**专业团队要做的是把官方边界与审定控制换进来重算一次**：复算触发器、受影响文件与责任角色逐条登记在 assumptions.json。
- **运营团队继续深化，从八个岗位规格与十二条交接账开始。** `role-spec.json` 给的是资质、权限、缺岗禁止条件与升级路径，`assignment_status` 一律 `unassigned` 并被 schema 约束为常量——**规格不是任命，接手方按自己的组织架构去填，不必推翻规格本身**。十二条双联交接账实例连同 96 条规则检查是可运行的验收模板，运营团队可以先在一张桌子上跑一遍，再决定要不要放大。
- **传播团队继续深化，从双语协议卡与失败记录开始。** 中英正文、双语网页、三分钟音频导览与可直接取用的国际传播短文构成一套不需要重写的素材；**唯一的硬约束是不得暗示政府背书、采购承诺或法定实施**，这条约束同样写在包里，接手方不必自行判断边界在哪。

三条起点共用同一个设计选择：**本方案交出的是规格与协议，不是成品与结论**。规格可以被修改、拒收和替换，成品只能被推翻重做——前者可转化，后者不可转化。

### 可纠错荣誉记录与公共知识沉淀

“百年值班簿”是一套可纠错的贡献凭证，不按流量排名。任何居民、维护者、开发者或公共机构都可用问题票提名；条目必须同时提交贡献对象、可公开证据、许可、影响边界和是否愿意署名。独立的社区—专业双人复核只判断证据是否足以记录，不评价身份或声量；被提名者可选择实名、化名或匿名。条目发布后保留公开异议、补证、纠错、合并、撤回和申诉入口，争议未解决先隐藏荣誉标识但保存审计链；个人数据按同意期限删除，公共方法、正负结果和版本记录长期归档。每年复盘班公布新增、纠错、撤回与未解决条目，不以数量排名。这样同时回应公共知识沉淀、贡献可记忆与人的尊严：记住谁承担过责任，也允许任何人改正记录或离开系统。

### 三分钟音频导览：不识字、不看屏幕也能听完这份方案

任务书把表达完整度定义为「以人类可直观理解的文本、图像、示意图、表格、场景卡、**视频/声音**、三维或**交互网页**形成完整成果」。本包的多模态表达含六类，文本、图像、静态网页之外还有**声音**、**视频**与**三维**——中英各一段三分钟音频导览 [data:assets/media/audio-guide.m4a] [data:assets/media/audio-guide-en.m4a]，讲清判据、三座交接场、最小载体「公开交接桌」与效力边界；再把网页从静态读物变成**可操作的交互网页**（见下一小节）。**视频这一类由三分钟导览视频补上** [data:assets/media/guided-tour.mp4]：音轨与音频导览完全相同、字幕逐句一致、画面全部取自包内已有图件，**因此它不引入任何新主张**；九个切换点按音轨在讲什么设置而不是均分——讲三座交接场时切 F/03，讲公开交接桌时切 F/10，讲十二个场景的拒收状态时切 F/05 的 96 格矩阵，分镜表逐段写在文字稿里 [data:assets/media/guided-tour.md]。**视频面向不读长文的人：三分钟看完判据、三座交接场与效力边界。**
...
| 图像 | 26 张图件（24 张 PNG + 2 张 JPEG）在 `assets/figures/`，中英各一套；画廊封面 `assets/media/cover.png` 另计 | 每张图的结论同时以文字写在正文对应表格里 |
...
### 把判据做成一个开关：智能层可以在页面上被关掉

本方案的判据是一句话：**任何一项 AI 服务，不用 AI 也能办成同一件事——办不成，就不上线。** 一句话可以被写进正文，也可以被读者自己验一次。离线评审台 [data:visual/index.html] 顶部现在有一个「智能层：在线 / 停用」开关：拨到停用，十二张场景卡同时把智能层那一段收起、露出无 AI 那一面，每张显示的是**同一件事在没有 AI 时怎么办成**——人工评审与纸面风险登记、触觉地图与人工问路、值班簿与人工报修、固定照明与人工巡查……逐条对应到该场景 [data:visual/assets/governance/shift-ledger-suite.json] 里 `human_service_floor` 已经登记的渠道。

**这个开关不产生新主张，只是把包内已有的登记摆到可操作的位置。** 卡面上的每一句都取自双联交接账：无 AI 渠道取 `human_service_floor.channels`，是否依赖个人设备取 `device_free_access`，演练状态取 `rollback_rehearsal.execution_state`。因此它给出的是**一对数字，而不是一个结论**：十二个场景全部写明了无 AI 等价服务且不依赖个人设备（12/12），而实地演练过的是**零个**（0/12，全部为 `specified_not_run`）。这两个数字并排显示在页面第一屏，谁也不遮谁——把「已规定」误读成「已验证」，正是这套交接协议最想防的事。

**为什么是开关而不是一段文字。** 无 AI 等价服务如果只写在正文里，读者要在十二个章节之间来回翻才能确认它真的逐条存在；做成开关，缺哪一条一眼就能看见——**这条判据因此从一句主张变成一次可以当场失败的检查**。页面为纯离线内联脚本，不联网、不存储、不提交表单，关闭脚本时十二张卡仍按静态形态完整可读。
...
七类用地面积与指标键逐行对应，面积单位为㎡；代码列的类名取自《国土空间调查、规划、用途管制用地用海分类指南》的一级类与二级类代码表 [source:MNR-LAND-USE-CLASSIFICATION-GUIDE]：

| 用地代码与类别 | 面积 | 指标键 |
| --- | --- | --- |
| 05 商业服务业用地 ⚠ | 2,133,693.511 | [metric:land_use_05_sqm] |
| 0701 城镇住宅用地 | 1,459,314.115 | [metric:land_use_0701_sqm] |
| 0702 城镇社区服务设施用地 | 1,332,004.101 | [metric:land_use_0702_sqm] |
| 0802 科研用地 | 1,899,736.874 | [metric:land_use_0802_sqm] |
| 0803 文化用地 | 1,038,498.555 | [metric:land_use_0803_sqm] |
| 0804 教育用地 | 1,284,882.427 | [metric:land_use_0804_sqm] |
| 1401 公园绿地 | 2,264,695.802 | [metric:land_use_1401_sqm] |

**⚠ 标记的那一行沿用组织方枚举，与部颁指南冲突。** 本包七个 `land_use_code` 全部取自 `brief/site-package/enums/land_use_codes.json`（确定性校验以该文件为硬性白名单）。该枚举 16 条中 15 条与《国土空间调查、规划、用途管制用地用海分类指南》一致，唯 `05` 一条对不上——指南中 `05` 是湿地，商业服务业用地是 `09` [source:MNR-LAND-USE-CLASSIFICATION-GUIDE]。本包保留 `05` 以维持全场用地统计的可比性，冲突已提 Issue （open-city-ai/haidian#2977）；核对过程与两条独立验证路径逐条记在 `sources.json` 的 `observations` 与 `transformations` 中，可由第三方按同一份公开代码表复核。**复算触发器**：枚举修正后两个要素改 `09`、指标键相应更名，F/02 与 F/03 图例、A0 B/01 与 A3 JZ/01 色标一并同步，**几何、面积与占比不会因此变动任何一位数** [data:geometry/land_use.geojson]。

**`05`（正名 `09`）停在一级类是有意的，不是漏填。** 指南在该类下设四个二级类，而本包这两块是消费、商务与科技服务的混合业态，业态比例要由控规阶段依实际招商与产业方案确定 [source:MOHURD-CONTROL-DETAILED-PLANNING]。在没有业态比例依据时挑一个二级类填上，会让这张表看起来比实际更确定；停在一级类并写明细分条件，是把「还没定」如实留在表上。业态构成给出后，这两个要素按实际比例细分，面积按细分后重新登记。
...
公告要求研提“AI创新指数、人才密度、产值规模”等规划指标体系。本方案给出口径而不给出数值——但这不是“找不到数据”。区级公开统计确实存在且已逐条登记 [source:HAIDIAN-2025-STATISTICAL-BULLETIN]：备案上线大模型 123 款、全国重点实验室 92 家、每万人口高价值发明专利 599 件、技术合同成交额 4053.1 亿元。问题在于它们的统计单元是海淀全区，无法分配到 43.6 平方公里的走廊；把区级数字直接写成本带指标，得到的是不可复核的伪精度，反而削弱指标体系的用途。因此这些数值全部停留在 `background_only`，只用来定义口径与判断瓶颈。下表把这三项与另外八项未赋值指标合并，逐项给出八列可执行内容——采样单元与范围、频率或触发、分母、聚合与统计方法、逐项质量检查、隐私与留存、发布门槛；官方数据到位后由同一模型一次性赋值并进入 metrics.json，与几何类指标共用同一套复算规则。

下表把这十一项**逐项摆开**，不是列出字段名而是填满字段值。**三条规则十一项完全相同，因此提到表外说一次，不在表内重复十一遍**：采集与复核角色一律是 `R-INDEPENDENT-REVIEW`（独立复核角色，不是提出该指标的人）；缺失值处理一律是「输入缺失时保持 unassigned，不插值、不以区级或全市平均值替代、不以历史值顺延」；异议复核一律是「任何居民、维护者、开发者或独立复核角色可在公开值班簿提出质疑，由独立复核角色复算并公开复算过程，原值与更正值并列保留」——**更正不覆盖原值，两个数并排留着，因为读者需要看见它改过**。表内只放逐项不同的东西。

| 指标 | 采样单元／范围 | 频率或触发 | 分母 | 聚合与统计方法（含禁止性规则） | 逐项质量检查 | 隐私与留存 | 发布门槛（须全部成立） |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 容积率（法定） [metric:floor_area_ratio] | 一个控规单元／总体设计范围内每一控规单元 | 控规批复或调整后一次性赋值 | 该单元的法定用地面积 | 不适用统计估计——法定审定值，逐控规单元登记原值。**多单元并存时不加权平均成一个数**：全带只有一个容积率数字必然掩盖单元差异，故按单元并列呈现。 | 与批复文件逐单元核对；单元面积之和与官方边界面积一致；计容规则与批复口径一致 | 仅使用公开批复文件中的数值，不涉及个人数据；发布时一并标注批复文号。 | 控规批复已公开；官方用地边界已发布；计容规则口径已明确 |
| 最高建筑高度（法定） [metric:maximum_building_height_m] | 一个高度分区／总体设计范围内每一高度分区 | 高度分区批复或调整后一次性赋值 | 无分母 | 不适用统计估计——按高度分区并列登记。**不取各分区最大值代表全带**，那会把一处高点读成普遍允许高度。 | 与批复的高度分区图逐区核对；与视线廊道及文保建设控制要求交叉检查 | 不涉及个人数据；仅发布公开批复或测绘文件中的数值并标注其出处。 | 高度分区批复已公开；视线廊道与文保建设控制要求已明确 |
| 现状总建筑面积 [metric:total_floor_area_sqm] | 一栋现状建筑／总体设计范围内全部现状建筑 | 普查完成后一次性赋值，其后随许可变更更新 | 无分母 | 逐栋普查值直接加总。**禁止用类型单元均值外推未普查建筑**——本包二十个类型单元是设计示范，不是抽样框，用它们外推会把设计假设洗成现状数据。 | 逐栋核对权属与许可面积；抽样 5% 现场复核；与建筑基底面积和层数交叉验算 | 按栋汇总发布，不发布产权人姓名或联系方式。 | 逐栋普查已完成；权属与许可数据可用；抽样复核偏差在可接受范围内 |
| 建筑密度 [metric:building_density] | 一个控规单元／总体设计范围内每一控规单元 | 普查完成后一次性赋值 | 该单元的法定用地面积 | 先逐单元计算再按单元并列。**需要全带值时必须以官方用地面积为分母整体重算，不得取各单元密度的算术平均**——单元面积不等，算术平均会给小单元过高权重。 | 基底面积由普查数据复算而非由概念几何推得；单元内建筑数量与普查清单一致 | 按控规单元汇总发布，不发布产权人姓名、联系方式或具体门牌。 | 逐栋普查已完成；官方用地边界已发布 |
| 道路面积 [metric:road_area_sqm] | 以交叉口为界的道路段／总体设计范围内全部道路段 | 道路红线发布或调整后一次性赋值 | 无分母 | 逐路段加总，**交叉口面积只计入一次**（按相邻路段的红线中线划分归属），避免两侧路段重复计入同一交叉口。 | 以红线面积复算而非以中心线长度换算；相邻段接缝无重叠或缝隙 | 不涉及个人数据；仅发布公开批复或测绘文件中的数值并标注其出处。 | 官方道路红线已发布；断面宽度已明确 |
| 道路率 [metric:road_ratio] | 总体设计范围整体／总体设计范围整体 | 道路面积与官方场地面积同时可用后赋值 | 官方场地面积 | 单一比值。**分子与分母必须来自同一发布批次**——道路红线与场地边界若跨批次相除，分母变了而分子没变，比值的变化不代表道路建设的变化。 | 分子分母同源同期；与 road_area_sqm 一致 | 不涉及个人数据；仅发布公开批复或测绘文件中的数值并标注其出处。 | road_area_sqm 已赋值；官方场地面积已发布 |
| 更新后建筑总规模 [metric:renewed_building_scale_sqm] | 一个更新单元／总体设计范围内每一更新单元 | 各单元规划条件核发后累加更新 | 无分母 | 逐更新单元累加，随规划条件核发滚动更新；**撤销或调整的单元同步扣减，不保留已撤销单元的贡献**——只加不减会让累计值单调上升，失去指示意义。 | 与规划条件逐单元核对；与 total_floor_area_sqm 的增减关系可追溯 | 按控规单元汇总发布，不发布产权人姓名、联系方式或具体门牌。 | 各单元规划条件已核发；现状建筑面积已赋值 |
| AI 创新指数 [metric:ai_innovation_index] | 一带范围内单个场景的一个运行周期／十二个场景全部运行周期 | 每季度一次，且任一场景停用时即时补记 | 当期已授权运行的场景数 | 四项子项各自先在**场景层**计算，再以当期已授权运行的场景数为分母取算术平均。**停用场景计入分母、不计入分子**——否则关掉表现差的场景就能提高指数，这条是为了让指数不能靠减少暴露面来改善。不做跨季度平滑，逐季原值全部保留。 | 四项子项分别可独立复算；台账条目均可回到原始记录；停用与失败记录不得省略 | 仅发布聚合值；不采集也不发布人脸、身份、住址、个人轨迹或订单明细。单元内样本不足 5 时并入相邻单元后发布。 | 公开贡献台账已建立并可查；场景运行记录含正负结果；四项子项权重经公开征求意见 |
| 人才密度 [metric:talent_density] | 一个统计小区／总体设计范围覆盖的全部统计小区 | 官方统计年度发布后一次性赋值 | 该统计小区的就业用地面积 | 逐统计小区计算后**以就业用地面积为权重加权**，不取小区算术平均——小区面积差异大，未加权会让小面积小区主导结果。 | 就业与学籍口径与发布机构一致；用地面积与官方权属数据一致；不以企业自报数据替代 | 仅发布聚合值；不采集也不发布人脸、身份、住址、个人轨迹或订单明细。单元内样本不足 5 时并入相邻单元后发布。 | 官方就业与学籍统计已发布；用地权属数据可用；统计小区与设计范围的空间对应关系已确认 |
| 产值规模 [metric:output_scale] | 一个统计年度／统计部门界定的产业范围与地域范围 | 官方统计年度发布后一次性赋值 | 无分母 | 直接采用统计部门发布值。**方案不重算、不换算口径、不按面积或人口比例把全区产值分摊到本带**——那样得到的是不可复核的伪精度。 | 产业范围与统计部门口径一致；地域范围与设计范围的差异明确标注；不以区级数值直接充当走廊数值 | 仅发布统计部门已公开的汇总值并标注口径与年度，不涉及个人数据。 | 统计部门口径已明确；地域范围与设计范围的对应关系已说明 |
| AI 企业聚集目标 [metric:ai_enterprise_target_count] | 一家企业或研发机构／总体设计范围 | 主管部门设定目标后记录，方案不代为设定 | 无分母 | 不适用——目标值由产业主管部门设定。**本方案既不代为设定，也不给出建议区间**：给出一个'参考数'会在后续被引用成本方案的承诺。 | 目标值出处为主管部门公开文件；统计口径与企业名录一致 | 仅记录主管部门公开文件中的目标值并标注出处，不涉及个人数据。 | 主管部门已公开目标值；企业名录口径已明确 |
| 主轴最大纵坡 [metric:spine_longitudinal_gradient_max_pct] | 主轴上以坡度转折点为界的一个纵断面区间／ROAD-001 全线，另在八处东西缝合支线交汇点加密 | 现状纵断面测量完成后一次性赋值；此后主轴或任一支线交汇点竖向改造后该区间重测 | 该区间的水平投影长度 | **取全线最大值，不取平均值也不取分位数**——纵坡的约束是逐点成立的，一处超限就阻断连续通行，用均值或 P90 会把一个断点稀释成合格。八处支线交汇点的区间单独列出并单独判定，不并入全线最大值取代。 | 高程数据的垂直基准与水平坐标系一并记录；坡度转折点划分与实测断面一致，不按等距人为切分；八处支线交汇点区间逐处单列，不并入全线统计 | 不涉及个人数据；仅发布纵断面测量得到的坡度值与其垂直基准，并标注测量单位与日期。 | 现状纵断面测量已完成且垂直基准已记录；无障碍规范的坡度与休息平台限值口径已明确；八处支线交汇点的竖向衔接方案已可判定 |

**这张表最值得看的一列是「聚合与统计方法」，因为它多数格子写的是禁止什么。** 不外推（不用二十个类型单元的均值去补未普查的建筑——那些单元是设计示范不是抽样框，外推会把设计假设洗成现状数据）、不取算术平均（单元面积不等，密度取算术平均会给小单元过高权重）、不跨批次相除（道路红线与场地边界若来自不同批次，分母变了分子没变，比值的变化不代表任何道路被修）、不分摊（不按面积或人口把全区产值摊到本带）、交叉口只计一次、撤销的更新单元同步扣减。**这些正是把数字做好看最方便的六个入口，因此写成方法的一部分，而不是注意事项。** 其中一条值得单独说：AI 创新指数的**停用场景计入分母、不计入分子**——否则关掉表现差的场景就能让指数上升，这条规则的作用是让指数不能靠减少暴露面来改善。

**十一项里有三项明确写「不适用统计估计」**——容积率、最高建筑高度与 AI 企业聚集目标。它们是主管部门审定值或由主管部门设定的目标，不是估计量；为它们编一套统计方法，等于把法定决定伪装成本方案的测算。**该空的地方写清为什么空，比填一个方法更接近可实施。** 表内八列只放逐项不同的内容，完整的十七字段（含定义、数据源类型与中英双语镜像）以机器可读形式留在 [data:visual/assets/governance/measurement-protocol.json]。
...
compliance_matrix.json逐条覆盖公告1.3、1.4、1.5和agent.1—agent.6；standard_matrix.json**覆盖组织方 `brief/site-package/standards/standards.json` 的全部九项标准**，含三项合规类标准（生成式AI暂行办法、无障碍环境建设法、国办发〔2020〕45号），九条的章节、图纸、几何、指标、来源与假设引用逐项可解析，断链为 0；design_depth_matrix.json覆盖十五项专业深度、状态一律 `complete`——**这不是自评满分，而是正式阶段的硬性要求**：`validate_submission.py:2010` 规定 formal 阶段每一项必须为 `complete`，否则确定性校验直接失败（schema 允许的 `data_gap` 只适用于更早阶段）。**因此「complete」在这里的含义是「该深度项已被回应」，不等于「其中的法定量已确定」**：开发强度与建筑高度的定量控制当前仍未赋值，缺口按口径记在 metrics.json 的 unknown、A-CONTROLS-001 与本节，不以估算填充。确定性、空间、视觉和专业检查共同验证机器可读包，人工评审仍拥有最终判断。建筑强度与高度保持unknown，是合规结果而非遗漏。整个矩阵由 [standard:PROJECT-OFFICIAL-ANNOUNCEMENT] 和 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] 统领。

**规则检查以矩阵示于图面：每一格是一次检查。** 上排十二格是十二条交接账的基线，下面七行是七条协议规则各自的缺陷注入，横向十二个场景——12 + 84 = **96 格**。**七行各自写着它拦的是什么**——R1 双控分离、R2 人工优先、R3 未决即停、R4 回滚先行、R5 合成隔离、R6 拒收具因、R7 未指派不签，读图的人不必回正文查编号。图上带色键：青格是基线通过，**炭黑格是「注入了缺陷并被挡下了」，信号红只留给漏检——因此图上没有红色，本身就是「漏检 0」这个结论**。84 条注入全部被拦截，漏检 0，schema 错误 0。把它画出来而不只写成数字，是因为「规则真的会拦住东西」这件事只有逐格摆开才可核对 [data:visual/assets/governance/rule-check-report.json]。

![可复算指标、测量协议、规则检查矩阵（12 基线 + 84 注入 = 96 格，注入全部被拦截）与五步证据链](assets/figures/metrics-evidence.png)

> **这张图怎么看。** 上排六张卡是可按公式复算的已知值，未赋值的指标不在图上出现。RULE CHECK 是 96 格检查：第一行十二格是基线，用电气青；下面七条规则 × 十二个场景共 84 格是逐条注入的缺陷，用炭黑表示已被拦截；信号红只代表漏检——因此**图上没有红色，就是漏检为 0** 这个结论本身。下方五步是证据链，黑条三段分别是基线、缺失值处理与发布门槛的口径。

### 十四道包自检闸门：不检验设计对不对，只检验这份包自己有没有对齐

上面那 96 格检验的是**交接协议规则**闭不闭合。但一份投稿还有另一层可能失败的地方，与设计好坏无关：矩阵指向了不存在的章节、字段没用组织方的词汇、中英章节数对不上、图件已经改不回原样、图纸印着两个版本号、送审图像多了一张——**这些都不是观点分歧，是可以被机器判定对错的事实**。本包为此固定了十四道闸门 [data:visual/assets/governance/package-integrity-gates.json]。

| 闸门 | 判据 | 当前结果 |
| --- | --- | --- |
| G01 矩阵引用完整性 | 三个矩阵的每条引用须解析到实体 | 1025/1025，断链 0 |
| G02 分类词汇覆盖 | 几何分类字段须取自场地包枚举，且**凡提供了枚举就必须使用** | 三套枚举均已使用，取值全部合法 |
| G03 中央登记表挂接 | 用到的登记表来源须逐字挂接 | 9/9 |
| G04 官方标准覆盖 | standards.json 每条须在标准矩阵登记 | 9/9 |
| G05 中英结构对称 | 两侧 h2、h3 计数须相等 | 15/15、29/29 |
| G06 图件字节可复现 | 生成器重跑须与包内 sha256 相同 | 重跑验证的三个文件字节相同 |
| G07 页数—印记自洽 | 页数须等于版本印记命中数 | 6/6、6/6、13/13、13/13 |
| G08 中文字体嵌入 | 中文字体须 emb=yes 且 sub=yes | 四套 PDF 全部嵌入 |
| G09 几何非回归 | 只改属性时几何须逐条不变 | 29 个要素几何全等 |
| G10 证据标记密度 | 单个论断不超过三个标记 | 0 处告警 |
| G11 用地代码对部颁表 | 代码须同时满足枚举与分类指南 | 7 个中 6 个一致，第 7 个冲突已标注 |
| G12 HTML 离线自足 | 不得有任何外部 http(s) 资源 | 0 处 |
| G13 送审视觉预算 | 须精确等于 18 张 | 18/18 |
| G14 首屏图不被裁切 | 首屏内图件的标题与免责声明须完整 | 已修复，中英均完整 |

**十四道里有十二道在真实使用中抓到过缺陷**——每道闸门的 `caught_real_defects` 字段逐条记着抓到的是什么。其中 `G01` 记录的多数条目来自本包自己写矩阵引用时产生的错引，`G11` 记录的则是组织方场地包的枚举缺陷；`G02` 的判据本身也被修订过——只查非法值的闸门，对「整个字段都没填」是盲的。**本轮又抓到两处，都出在作者自己手上**：G01 在 1025 条引用里恰好只断掉本轮新写的那一条——为 WCAG 条目凭印象编了个不存在的章节名；G05 与本表的 G01 行则第三次被发现自陈落后于实际（154/154→1025/1025、27/27→29/29），原因每次相同：补了内容却没重跑自陈数字。**这类「讲一致性的文档自己不一致」是本包最常犯的一种错**，因此本轮起把两道闸门的重测并入提交前的固定动作，不再依赖记得去做。

**边界要说清楚，否则闸门自己就成了一种夸大**：它检验的是包内一致，**不证明设计正确、不证明现场可行、更不构成任何审定结论**；判据可被任何人拿本包与公开场地包复算，但执行脚本不随包分发（投稿目录禁止 `.py`）；它不是持续集成，**可靠性取决于是否真的跑了，而不取决于它被写下来**。

## 风险、版权与合规说明

最大风险是把临时数据与概念方案误读为官方决定。风险与缺资料深度由 [depth:risk_missing_data] 控制；[data:geometry/constraints.geojson#CONSTRAINTS-DATA-GAP] 为空表示没有公开约束几何可用，不表示不存在约束。
...
隐私边界为数据最小化、自愿参与、目的限定、可撤回、限时保存与人工申诉；不使用秘密地图、非公开表格、未经授权个人数据、强制人脸识别或无法人工复核的权益决定。公平性底线是无智能手机、不会中文、残障、老人与选择不用 AI 的人仍获得等价基础服务。**法定下限只覆盖医疗健康、社会保障、金融业务、生活缴费等场所的人工办理** [source:BARRIER-FREE-ENVIRONMENT-LAW] 第三十九条；[source:ELDERLY-SMART-TECH-PLAN] 属政策要求。**把等价服务推广到全部十二个场景，以及“可停用、可申诉”，是本方案自设标准。** 技术失败时先保证照明、通行、求助与公共服务，再处理模型恢复；停止与投诉入口对应 [source:GENERATIVE-AI-INTERIM-MEASURES] 第十四、十五条。

**不当内容与侵权内容按两条独立的口径分别排除，各自可核。** 不当内容一侧：全包不含个人身份信息、人脸或生物特征数据、非公开空间数据与涉密信息；十二个场景无一以人脸识别或强制安装 App 为前提，每个场景都写明人工兜底与退出条件；未成年人相关采集条目的目标恒为 0；不出现攻击性、歧视性表述，也不把任何概念建议写成已确定的政府决策。侵权内容一侧：图形、Logo 方向、地图符号、排版、文字、几何与 PDF 均为本次原创或基于仓库清权数据程序化生成；外部案例只作文字转述，**未复制图片、地图、商标或受保护版式**；字体分五种情形逐行给出权利依据与独立核验命令（见下表）；包内不含任何字体文件，可用 `find <包> -name '*.tt[cf]' -o -name '*.otf'` 当场核验，结果为 0。版权声明另见 report/copyright_statement.md。字体分五种情形，权利依据与独立核验命令逐行见下表。方案是开放共创的概念建议，不替代规划、建筑、交通、景观、市政、消防、文保、法律、隐私与工程专业服务，不代表政府批准、投资、采购、招商或实施承诺。
...
**两类字体的义务不同，因此权利依据分开给。** OFL-1.1 明确允许嵌入与再分发；macOS 系统字体依据的是 Apple 软件许可协议第 2 节 E 条，该条把「使用字体 display and print content」与「嵌入字体」写成两种不同的行为 [source:FONT-APPLE-SLA-CLAUSE-E]，**本包只做前一件**。两项事实可独立核验：包内字体文件数为 **0**，四套 PDF 中 `emb=yes` 的中文字体**只有 NotoSerifSC**。条款用语与像素产物之间的对应关系属本包的理解，其边界逐条记在 `sources.json` 的 `limitations` 中。
...
| **全部栅格产物的文字（24 张 PNG 与 2 张 JPEG，中英各一套）** | OFL-1.1 的 Noto Sans CJK SC（Medium／Light）与 Noto Sans（Regular／Bold／Italic／BoldItalic），经 Pillow 栅格化 [source:FONT-NOTO-RASTER] | **SIL Open Font License 1.1 明文允许使用与再分发**，因此栅格产物的权利依据**不依赖对任何条款的解释** | `find assets -name '*.tt[cf]' -o -name '*.otf'` → 无结果；字体文件在仓库外的工具链目录，只在渲染阶段读取字形 | 2026-08-17 分两步由 macOS 系统字体迁移完成：先迁五张送审图件，再重建其余七张图件与断面图的生成器后一并迁移。中文取 Medium 档而非 Regular，以补回 Noto CJK 相对 STHeiti 偏细的字面差 |
...
| 几何与指标 | 九类 GeoJSON 与 metrics.json | 由本包程序化生成，公式与源文件逐项登记 | 按 metrics.json 的 `formula` 与 `source_files` 在 EPSG:4548 复算 | 生成脚本不在包内，故**只主张指标可独立复算，不主张逐字节可复现** |
...
**栅格字体的迁移过程如实留档。** v1.15 之前，全部栅格产物由 macOS 系统字体 STHeiti Medium.ttc [source:FONT-STHEITI-RASTER] 与 Helvetica.ttc [source:FONT-HELVETICA-RASTER] 栅格化；当时能给出的只有「未复制、未嵌入、未再分发字体文件」这一侧，而「生成并分发像素产物」与 Apple 条款的对应关系属本包理解，不是确定性证明。2026-08-17 分两步迁移完毕：先迁五张送审图件，再重建其余七张图件与断面图的生成器后一并迁移，**至此包内 24 张 PNG 与 2 张 JPEG 全部改由 OFL-1.1 字体生成**。上述两条来源已降级为历史登记，只说明旧版本产物由何种字体生成，不再作为当前任何产物的权利依据。
...
**图纸中文可读、可打印，也可检索。** v1.11 换嵌字体时留下的两处缺陷已于 2026-08-17 修复并复验：A3 中文本第 6 页 7 处 `·`(U+00B7) 缺字方框已消除（该字在嵌入子集中无字形，按 UCS-2 码元边界改为顿号 U+3001，全文恰好 7 处）；四套图纸的中文 `/ToUnicode` 原写的是 code→字形索引而非 code→Unicode（如 `<0020>→<003f>`、`<0041>→<0060>`），已整体换成恒等映射——内容流的 CJK 走十六进制串、编码为 `UniGB-UCS2-H`，串内即 UCS-2 码位，恒等即正确。复验：poppler、PyMuPDF、pypdf **三引擎抽取的中文一致且与图面相符**（A3 第 6 页「八个共享栈，把能力交到下一班」、A0 第 1 页「京张交接线」，修复前分别抽成「頙釮頠銸䐛」「銺㬪銯㾲」）；四套 `qpdf --check` 全 PASS，页数与版本印记 13/13、13/13、6/6、6/6 相等。逐项证据见 `assumptions.json` 的 `A-FONT-001`（状态已转为 resolved）。
...
**终止与版本（Termination and versioning）。** 违反上述限制时本许可对违反方自动终止。本许可为版本 1.0；后续版本只在本文件与 `report/copyright_statement.md` 中同步发布，既有分发件继续适用其随附的版本。**这个标识是组织方的，不是本包自造的，而它的条款确实没人定义过——两件事都可当场核验。** `COMMUNITY-DISPLAY-ONLY` 是 `schema/proposal.schema.json` 的 `license` 枚举三个取值之一（`git show upstream/main:schema/proposal.schema.json | grep -A3 '"license"'`）；而在主线上除 schema 与 `templates/proposal.md` 之外，**它一次也没被定义过**（`git ls-tree -r --name-only upstream/main | grep -v submissions/ | xargs -I{} git show upstream/main:{} 2>/dev/null | grep -c COMMUNITY-DISPLAY-ONLY` → 只命中那两个文件）。上面这份条款因此是**本包对一个组织方留空的标识给出的读法**，作用是让第三方拿到一条具体边界而不是一个没有内容的标签；组织方若发布正式条款，以其为准，本文本作废。

**为什么不直接用标准许可证：枚举里没有非商业选项。** 三个取值中另两个是 `CC-BY-4.0` 与 `CC-BY-SA-4.0`，**两者都允许商业使用**；而本方案必须守住的是非商业与限制第 (2)(3) 项——通用许可证不含「不得表述为政府决定」这类约束。所以这不是偏好自造条款，是枚举里不存在能同时满足这两条的标准许可。

**但读者不必依赖本包的条款文本才能行动，可以退回一个已知许可判断边界：本许可授予的每一项使用，CC BY-NC 4.0 都同样允许——即本许可不比 CC BY-NC 4.0 更宽。** 两份文本可逐条对读验证：双方都要求署名、都排除商业使用，而本许可另把可作的演绎收窄为「标注过的节选与译本」，并加了三条 CC 不含的限制。**因此按 CC BY-NC 4.0 行事的第三方不会越出本许可**（反之不成立）。这条上界的用处是：即便有人认为一个未经组织方定义的标识不可依据，仍有一份成熟许可可作判断依据，不必等待法律意见。
...
判断某一成果是否仍然有效，仍应以 manifest 的 sha256 与 changelog 的逐版记录为准；一旦官方边界发布触发全量重算，全部成果将再次在同一次构建中统一。本包的确定性渲染脚本（图件样式底座、图纸版式底座、几何管线、图件与图纸页生成、图件重印、图纸总装）**未随包提交**：仓库的 `validate_submission.py` 只允许投稿目录内出现数据与素材类文件，`.py` 不在允许扩展名内。因此本包只主张**指标可独立复算**，不主张产物逐字节可复现；脚本可按任务书 `continuous_participation.issue_collaboration_zh` 的约定另开 Issue 附补丁提供。
