...
总平面图同时标出**四处朝圣地标**——交接钟、公开交接台、百年值班簿、零号接管亭——并在图内左侧列出各自的荣誉角色（逐字取自 [data:geometry/public_space.geojson]）。四处均为设计建议，启用门槛与回退状态随几何一并登记。
...
前面三节写的是片区尺度的策略。但一条以「交接」为名的线路，应该有一件东西小到可以先做出来、错了可以当天撤走、做对了别处可以照着做。**公开交接桌**就是这件东西：整套双联交接协议的最小物理载体，一张桌子加四件必须同时在场的东西。
...
| --- | --- | --- | --- | --- | --- |
| 个人开发者与学生 | 一张能复现的桌子、一份能引用的记录 | 校园成果接力站 SCN-07、公开交接台 | 场景栈、数据栈 | 提交可复现说明与许可标注 | 记录留存，工位随时释放 |
...
可实施性最容易被谈成一句"分三期推进"。本节把它拆成四件可以分别核验的事，每件都指向包内可打开的文件，而不是正文自述。
...
- **专业团队继续深化，从几何与未知项开始。** 九类 GeoJSON 在 EPSG:4548 下可独立复算，任何人拿同一份几何得同一个数；十一项未赋值指标各自带完整测量协议（定义、采集角色、采样单元、频率、分母、缺失值处理、质量检查、争议复核、隐私与留存、发布门槛）[data:visual/assets/governance/measurement-protocol.json]。**专业团队要做的不是重画，而是把官方边界与审定控制换进来重算一次**——复算触发器、受影响文件与责任角色已逐条登记在 assumptions.json，不必反向推断哪些图会跟着变。
...
任务书把表达完整度定义为「以人类可直观理解的文本、图像、示意图、表格、场景卡、**视频/声音**、三维或**交互网页**形成完整成果」。本包此前只有文本、图像与静态网页三类；先补上**声音**这一类——中英各一段三分钟音频导览 [data:assets/media/audio-guide.m4a] [data:assets/media/audio-guide-en.m4a]，讲清判据、三座交接场、最小载体「公开交接桌」与效力边界；再把网页从静态读物变成**可操作的交互网页**（见下一小节）。
