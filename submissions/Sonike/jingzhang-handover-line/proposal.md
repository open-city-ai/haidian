...
自适应、可进化的城市发展模式在此有一个具体定义：**把城市更新当作有版本、可回滚的提交，而不是一次性建设**。每一个场景与更新单元都带版本号、适用范围、责任人、下次复核时间和退出条件；三期不是时间表，而是三道合并门槛——资料、权属与专业评估通过才允许合并，未通过就回滚到上一个可用状态。可进化的前提是可回滚，这正是组件库要求全部可撤除、建筑四步判定里"可逆插入 I"必须排在"待核 D"之前的原因。一座不能撤销自己决定的城市，不可能自适应。
