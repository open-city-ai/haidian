...
产业环节与空间的对应由用地拓扑给出可复算比例：下表在 EPSG:4548 中按 [data:geometry/land_use.geojson#LU-001] 逐要素复算，九个分区联合覆盖临时范围、合计 100.00%。这是城市设计结构的用地面积占比，不是法定用地配比，不代表产值、就业或投资比例。
...
版权声明见 report/copyright_statement.md。图形、Logo 方向、地图符号、排版、文字、几何与 PDF 均为本次原创或基于仓库清权数据程序化生成；外部案例只作文字转述，未复制图片、地图、商标或受保护版式。字体分五种情形，权利依据与独立核验命令逐行见下表。方案是开放共创的概念建议，不替代规划、建筑、交通、景观、市政、消防、文保、法律、隐私与工程专业服务，不代表政府批准、投资、采购、招商或实施承诺。
