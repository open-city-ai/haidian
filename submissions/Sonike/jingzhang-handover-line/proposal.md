...
| 缺陷注入自检 | 对七条协议规则各做缺陷注入，**84 条注入全部被拦截、漏检 0** | 常规方案只声明规则；本方案证明规则**拦得住**它该拦的东西 | [data:visual/assets/governance/rule-check-report.json] |
...
| **指标** | **47 项指标，36 项已赋值可复算**；11 项未赋值的每一项都有完整测量协议 | `metrics.json`（含 `formula` 与 `source_file`）、`visual/assets/governance/measurement-protocol.json` | 发布门槛任一不成立即保持未赋值 |

**指标这一项的口径。** 本包 47 项指标中 **36 项已赋值**——场地面积、绿地率、公共空间占比、主轴长度、场景节点数、用地要素数等，全部在 EPSG:4548 中按 `formula` 与 `source_file` 可独立复算，任何人拿同一份几何都能得到同一个数。其余为法定控制与产业绩效两类，其目标值须由主管部门依官方控规、导则与运营基线确定，本方案不代为设定，但已为每一项写明可直接执行的测量协议（定义、采集角色、采样单元与范围、频率或触发、分母、缺失值处理、质量检查、争议复核、隐私与留存、发布门槛），并把三条边界写进 schema 而非散文：目标值类型约束为 `null`、缺失值禁止插值与区级平均替代、发布门槛任一不成立即不发布 [data:visual/assets/governance/measurement-protocol.json]。**发布一套测量协议不等于发布一个目标——方法做完，决定留给有权做决定的人。**

更新项目不以一次性大拆大建启动，而以可逆、公开、能形成反馈的最小单元启动。十二项概念项目为：01交接线树荫与基础导视；02北部模型红队交接台；03机器人路权封闭沙盒；04端侧能耗试验站；05中央公开交接桌；06校园成果接力站；07无障碍路径副驾；08维修可见驿；09南部城市交接厅；10百年值班簿；11口述史问答亭；12全带开放交接周。每项都需明确业主或运营方、公共责任人、许可、预算、维护、退出和数据删除，未完成前不得把概念项目写成已落地安排。
...
- **专业团队继续深化，从几何与未知项开始。** 九类 GeoJSON 在 EPSG:4548 下可独立复算，任何人拿同一份几何得同一个数；十一项未赋值指标各自带完整测量协议（定义、采集角色、采样单元、频率、分母、缺失值处理、质量检查、争议复核、隐私与留存、发布门槛）[data:visual/assets/governance/measurement-protocol.json]。**专业团队要做的是把官方边界与审定控制换进来重算一次**：复算触发器、受影响文件与责任角色逐条登记在 assumptions.json。
...
只给“口径定义”与“数据前置条件”，读者无从判断数值将来由谁、按什么单元、多久采一次。本包把十一项未赋值指标（含容积率、建筑高度、总建筑面积、建筑密度、道路面积与道路率）升级为一份机器可读的测量协议 [data:visual/assets/governance/measurement-protocol.json]：每项写明定义、数据源类型、采集角色、采样单元与范围、频率或触发、分母、缺失值处理、质量检查、争议复核、隐私与留存，以及发布门槛。
...
| 闸门 | 判据 | 当前结果 |
| --- | --- | --- |
| G01 矩阵引用完整性 | 三个矩阵的每条引用须解析到实体 | 154/154，断链 0 |
...
| G05 中英结构对称 | 两侧 h2、h3 计数须相等 | 15/15、27/27 |
...
**十四道里有十二道在真实使用中抓到过缺陷**——每道闸门的 `caught_real_defects` 字段逐条记着抓到的是什么。其中 `G01` 记录的多数条目来自本包自己写矩阵引用时产生的错引，`G11` 记录的则是组织方场地包的枚举缺陷；`G02` 的判据本身也被修订过——只查非法值的闸门，对「整个字段都没填」是盲的。
