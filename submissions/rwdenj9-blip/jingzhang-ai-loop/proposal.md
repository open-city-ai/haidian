...
范围口径统一：L1「统筹研究范围」为概念研究范围（约 43.6km²），仅用于产业与区域协同研究，不参与场地几何统计；L2「总体设计范围」与 L3「三处重点区域」的面积、绿地率、公共空间占比、建筑基底面积均由本稿提交的（临时）site_boundary / green_space / public_space / buildings 几何复算得到（当前约 [metric:site_area_sqm]），两者口径不同、不可混用。待官方红线到位后统一重算，并同步图件与矩阵 [source:BOUNDARY-SOURCE][depth:metrics_recalculation]。
...
京张带不是孤立片区，而是京津冀 AI 创新网络的一环；本稿仅提出三条**概念性**协同接口，供专业团队与主管部门深化方向，不构成合作、审批、资金或实施承诺 [source:AGENT-TASKBOOK][depth:three_level_scope_framework]。

- **北纬社区 / 未来科学城**：以京张活力带为西端「孵化—中试」节点，与北纬社区、未来科学城（东侧研发—转化）形成「孵化—加速—转化」的纵向协同概念。
- **怀柔科学城**：围绕基础研究与重大科技设施，与 AI 原点社区的基础研究生态形成「科学—技术—场景」的横向衔接概念。
- **经开区（亦庄）与京津冀**：以京张带的「场景开放清单」与测试走廊为接口，与经开区智能制造、京津冀制造场景形成「场景—产业」协同概念；不预设跨区工程、产能或投资安排。

以上均基于公开披露的功能定位作概念性表达，具体协同机制、主体与量化关系待专业团队确认后深化 [source:AGENT-TASKBOOK]。
...
### 6.3 场景·地标·活动空间落点索引（概念）

为便于评审定位，以下将 14 张场景卡、4 处朝圣地标与四季活动按空间节点编制索引，落实到京张活力带沿线与三区两翼（道路、轨道、站点语境见 [data:geometry/roads.geojson#ROAD-001]、[data:geometry/key_areas.geojson#KEY-001]）：

| 编号 | 类型 | 空间落点（概念） |
|---|---|---|
| S1 | 场景卡 | 京张遗址公园沿线（AR 导览桩） |
| S2 | 场景卡 | 全带慢行网络（智能信号联动） |
| S3 | 场景卡 | 三区产业服务中心 |
| S4 | 场景卡·测试 | 众智园测试走廊 + 小月河翼试点段 |
| S5 | 场景卡 | 社区服务中心、无障碍设施（沿轴） |
| S6 | 场景卡·测试 | 大钟寺、原点广场等大客流节点 |
| S7 | 场景卡·测试 | 众智园「AI 审图中心」 |
| S8 | 场景卡 | 沿轴 3–4 处开源广场/展厅 |
| S9 | 场景卡·测试 | 关键路口（车路协同试点） |
| S10 | 场景卡 | 三区能源节点 |
| S11 | 场景卡 | 国际人才驿站、大钟寺会展中心 |
| S12 | 场景卡 | 社区与园区交界处 |
| S13 | 场景卡 | 环「清北科」近校节点、AI 技能驿站 |
| S14 | 场景卡·测试 | 北航具身智能测试节点、小月河公共服务场景 |
| L1–L4 | 朝圣地标 | 京张遗址公园沿线（开发者散步道/开源成果展示廊/智能体贡献荣誉墙/AI 原点纪念碑） |
| A1–A4 | 年度活动 | 京张 AI 开源大会（春季）/开发者马拉松（夏季）/百年京张国际城市设计节（秋季）/AI 文化周（冬季）——均为概念安排 |

![场景—地标—活动节点图（provisional 示意，基于提交的道路/重点区/公共空间图层）](assets/figures/scenario-node-map.png)

> 索引为概念表达，具体点位与工程边界以专业团队深化为准；测试类场景均限定试点、备案与人工复核边界 [source:AGENT-TASKBOOK]。

### 6.4 涉数据场景统一数据治理清单（概念）

针对涉感知/个人/企业数据的场景，统一说明数据类别与最小必要性，作为概念性治理规范，不声称已取得备案、授权或完成合规评估：

| 场景 | 数据类别 | 最小必要性 | 是否个人数据 | 采集/保存 | 访问角色 | 脱敏方式 | 人工复核 | 投诉/退出 | 禁止用途 |
|---|---|---|---|---|---|---|---|---|---|
| S2 慢行绿波 | 匿名轨迹/流量 | 仅用于路径建议 | 原则上否 | 脱敏后短期 | 交通运营方 | 聚合+去标识 | 是（建议路径） | 提供后台 | 不作执法、人脸识别 |
| S4 无人配送 | 配送任务/路径 | 仅任务与安全 | 少量（收件地址） | 限期 | 测试企业+交管 | 加密+最小化 | 是 | 可拒收 | 不作身份画像 |
| S6 公共安全复盘 | 人流时序/事件 | 节假日后复盘 | 否（聚合） | 事后短期 | 应急部门 | 聚合+匿名 | 是 | 公开渠道 | 不作实时监控/精确识别 |
| S7 审图 | 设计成果材料 | 仅合规比对新旧稿 | 否（项目文件） | 项目期 | 规划部门+专业院 | 权限隔离 | 是（输出参考） | 可申诉 | 不作审批/自动否决 |
| S9 车路协同 | 车辆/路口感知 | 仅安全决策所需 | 否（车辆/设备） | 实时+短存 | 车企+交管 | 脱敏 | 是 | 后台 | 不作驾驶评分/影像留存 |
| S14 具身智能 | 交互/环境感知 | 仅演示任务 | 现场明确告知 | 现场短存 | 高校+企业+社区 | 去标识+现场 | 是 | 可离开 | 不作持续画像/儿童采集 |

> 清单为概念规范；在任一场景进入真实试点/采购/部署前，须另行完成数据保护评估、隐私影响评估、无障碍测试及法定备案/授权，并保留人工复核、退出与事故处置机制。[source:AGENT-TASKBOOK]
...
**现状轨道与慢行要素（背景资料，待官方核实）**：遗址公园沿线包含西直门北京北站、地铁 13 号线（知春路、五道口等站）及北三环、北四环跨线桥等节点，9 公里带状绿廊为慢行主脊 [source:SRC-JZ-PARK-PHASE2][source:SRC-THREE-ZONES-TWO-WINGS]。AI 交通先行试点：S2 慢行绿波导航、S9 车路协同路口、S4 低速无人配送（限测试走廊、限速限时备案）。市政与新基建建议预留智能感知、边缘算力、无线充电等「可逆新基建」接口，不预设工程线位与管线走向；公共服务补齐社区服务中心、国际人才驿站、AI 技能驿站三类节点 [depth:municipal_new_infrastructure]。慢行·蓝绿·场景节点关系见下图。
