...
### 独立重投采样（评估 a0be249 评分内容的分布）

- 继 v2.5（= a0be249 内容，PR #3157 得 72/100）之后，本版本再次以与 a0be249 评分提交逐字节相同的评分内容（proposal.md、7 份 JSON、5 中 5 英图、HTML、PDF 均不变）独立重投，用于在已知约 13 分运行间方差（85 vs 72）的基础上进一步刻画评分分布。
- 仅本条迭代记录与 manifest/self_check 的对应哈希随重投更新；不修改任何评分输入。不新增任何实施主张。
