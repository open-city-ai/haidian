...
1. **AI信息亭**：实时交通/天气/AI新闻触屏终端。材质：耐候钢+钢化玻璃。位置：地铁出入口、公交站
2. **智能路灯**：集成WiFi热点+环境传感器+USB充电。材质：铝合金+LED模组。位置：主要街道、广场每30m一处
3. **模块化座椅**：可重组为单人/双人/环形。材质：竹集成材+不锈钢。位置：公园、广场、步道旁
4. **自行车停靠站**：双层停车架+共享单车电子锁。材质：镀锌钢。位置：地铁站200m内、公园入口
5. **太阳能充电桌**：光伏顶棚+无线充电板+USB接口。材质：太阳能玻璃+铝合金框架。位置：广场、校园、创新节点
6. **交互式导览牌**：AR扫码触发历史/AI场景解说。材质：耐候钢+触摸屏。位置：京张遗址段每500m一处
7. **社区公告屏**：电子墨水屏显示活动/招聘/AI科普。材质：铝合金框架。位置：社区入口、菜市场
...
- 连接清河与南长河蓝绿空间
- 新增社区公园12处，绿地率约12.5%（绿地实测142.9ha，基于临时边界EPSG:4548投影实测）
...
| SC9 无人零售体验店 | 大钟寺+原点社区 | 市民 | 扫码/刷卡进入（提供非生物识别选项），计算机视觉结算（数据本地处理、定期清除、用户可随时退出） |
...
- 平均容积率：待获取官方控规数据（概念方向约2.0，metrics.json中标记为unknown）
- 建筑覆盖率：待获取现状建筑数据（概念方向约28%）
- 建筑高度分区：低层区（京张公园两侧≤15m）、中高层区（外围地块≤60m）

> 注：以上为方向性概念，不构成法定规划指标；容积率和覆盖率在metrics.json中标记为unknown [standard:AGENT-SUBMISSION-DISCLAIMER]。
...
| 绿地率 | 12.5% | 绿地实测142.9ha/设计范围实测1142.7ha [data:geometry/green_space.geojson] | 中等（临时边界） |
| 公共空间占比 | 3.0% | 公共空间实测34.3ha/设计范围实测1142.7ha [data:geometry/public_space.geojson] | 中等 |
| 路网密度（含既有） | 8.5 km/km² | 既有OSM路网约81km+设计新增16km=97km/11.4km² [data:geometry/roads.geojson] | 中等 |
...
| 平均容积率 | 未获取 | 官方控规数据缺失 [metric:far_average] | 未知 |
| 设计范围实测面积 | 11.43 km² | 临时边界EPSG:4548投影实测 [metric:site_area_sqm] | 中等 |
...
| 场景 | 合法基础 | 数据字段(≤5项) | 保存期 | 退出方式 | 人工复核触发 |
|------|----------|---------------|--------|----------|-------------|
| SC1 自动驾驶接驳 | 知情同意(乘车即同意) | 路径轨迹、速度、时间戳、车载视频(脱敏) | 72h | 选择人工驾驶班次 | 事故/违规/系统报警 |
| SC2 机器人配送 | 合同必要+知情同意 | 路径坐标、交付时间、包裹尺寸(不含内容) | 30天 | 选择人工配送 | 投递失败/用户投诉 |
| SC3 环境传感网络 | 公共利益(环境监测) | 温度、湿度、PM2.5、噪音(dB)、光照 | 永久(聚合公开) | 传感器不含个人信息，无需退出 | 数值异常/设备离线 |
| SC4 AI安全沙箱 | 合同必要 | 测试代码、执行日志、资源占用 | 测试结束即清除 | 合同约定测试范围 | 安全事件/资源异常 |
| SC5-12 公共体验场景 | 知情同意 | 互动记录(匿名)、使用频次、场景ID | 90天 | 不参与/使用离线模式 | 用户反馈/内容争议 |

所有场景遵循数据最小化原则，默认本地处理，不采集生物识别信息（SC9提供扫码替代方案）。每季度公开发布隐私影响评估摘要。
...
| 8 | Logo商标检索 | 概念阶段不涉及 | 待确认 |

## 附录 C：指标复现方法

核心指标均基于shapely几何计算（EPSG:4548投影）：
- 绿地率=green_space Polygons面积÷site_boundary面积=142.9÷1142.7=12.5%
- 公共空间率=public_space Polygons面积÷site_boundary=34.3÷1142.7=3.0%  
- 路网密度=(OSM既有约81km+设计新增16km)÷11.43km2≈8.5km/km2
