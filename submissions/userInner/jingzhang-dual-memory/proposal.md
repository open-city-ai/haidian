...
> **v9.0 核心判断：** 好的城市智能不仅要证明“完整时能运行”，还要证明“缺项和故障时会拒绝”。本版把一脊、三站、两翼、九条东西缝合与三处样段重画为城市空间系统，并新增“双记故障注入协议”：12 个完整合同只获得现场复核资格，96 个单字段缺失全部被阻断，12 类运行故障全部转入普通服务与 `HOLD`。120/120 是合成负面测试结果，现场性能仍为 0/2 未运行。

| 评审维度 | v9.0 的实质深化 | 核验入口 |
| --- | --- | --- |
| 任务契合与原创性 | “证明里程”把公共记忆脊、日常路线、记忆边界护照和实施闸门组织成一条可追读的方法 | `proof-mile-spatial-atlas.json` |
| AI 与规划创新 | 八字段放行合同与十二类故障注入，使 AI 在缺少边界、责任、普通服务、最小数据、知情选择、人工停止、验收证据或复原机制时自动阻断 | `double-memory-fault-injection.json`、离线审计脚本 |
| 实施可行性 | P01 平面、S01 剖面、D01 构造、O01 运营、T01 测试组成可复核交付包；尺寸、投资与退场规则由脚本复算 | `proof-block-technical-spec.json`、技术审计脚本 |
| 公共利益 | 4.0 米无 AI 路线持续开放，最小无障碍净宽 1.8 米；普通服务不依赖账号，也不穿越试验区 | P01、S01、T01 |
| 风险与表达 | 120/120 合成负面测试与 0/2 现场性能分栏显示；总图、三处小总平、九条缝合路径和四带剖面构成同一空间叙事 | 第 1 张展板、五张核心图 |
...
![60 × 14 米可判废技术包：尺寸化平面、构造剖面、验证票据与放行门](assets/boards/board-01.jpg)

![三处双记样段：从总体愿景收束到三个可撤回、可核验、可复原的最小实施单元](assets/boards/board-09.jpg)

![证据落位与现场任务：公开记录、可复算指标、待调查路线与禁止推定边界](assets/boards/board-10.jpg)
...
沿线 HD00-1601、1602、1603、1604、1605 街区控规草案曾于 2024 年 12 月 19 日至 2025 年 1 月 19 日公示；海淀区 2026 年一季度进展仅说明该控规已通过市级部门联合审查并继续推进报批实施。本版据此确认“程序正在推进”，不把它写成已批准控规，也不从公告反推官方边界、用地或指标。[source:JZ-CONTROL-PLAN-NOTICE] [source:JZ-CONTROL-PLAN-PROGRESS]

清河段已核到一份官方公共工程合同：京张高铁东侧、小营西路至西二旗北路，面积 134,619.3 平方米，合同价 28,986,162.09 元，建设单位为清河街道办事处，计划竣工日为 2026 年 5 月 29 日。该记录可证明合同范围和合同主体，不证明实际竣工，也不证明建设单位拥有土地权利；折算 215.32 元/平方米只用于记录量级，不与本方案样段概念成本直接比较。[source:QINGHE-CONTRACT] [metric:verified_qinghe_public_project_area_sqm]

大钟寺重点区附近已核到 HD00-1603-01、HD00-1603-03A 两宗地成交记录：土地面积合计 39,522.11 平方米，地上建筑规模上限 96,811.51 平方米，折算容积率上限 2.4496，成交价 28 亿元，竞得人为北京云跃长实科技有限公司。这个权利记录只适用于两宗地，不能替代约 72 公顷重点区的完整权属图，也不能据此判断相邻道路、站点或建筑的权属。[source:DAZHONGSI-LAND-TRANSACTION] [metric:verified_dazhongsi_parcel_far_cap]
...
v9.0 将抽象梯度压成一个可异地复用、现场再校准的 60 米 × 14 米原型包络：4.0 米连续无 AI 步行带、2.5 米匿名辅助带、5.0 米受控试验庭与 2.5 米种植安全缓冲。尺寸不是地块控制，而是进入专业测量前的最低功能检验；无障碍净宽任何时点不得低于 1.8 米。传感器必须外露，端侧柜可断电，试验庭有物理急停和有人值守控制台，公众绕行不经过试验区。完整构造、责任、数据和停止条件见 `visual/assets/memory-boundary-passports.json`。[depth:traffic_rail_slow_parking]

### v9 放行编译器与故障注入

最小样段不是一张效果图，而是五张必须相互对账的交付图：P01 尺寸化平面锁定 14.0 米总宽和四带分配；S01 剖面锁定普通路线连续开放；D01 构造锁定门槛不高于 6 毫米、横坡不高于 2%、净空不低于 2.2 米、轮椅回转不小于 1.5 米和盲道警示不小于 0.6 米；O01 运营锁定人工接管目标不超过 10 秒、本地隔离不超过 120 秒和数字构件 48 小时内可拆；T01 测试登记每一条放行与判废条件。

技术审计已复算四带宽度合计 14.0 米、普通路线 4.0 米、任一点急停设计覆盖不超过 15 米、三样段投资 350—650 万元与项目总投资 750—1350 万元之间的对账关系，共 10/10 项桌面检查通过。合格现场观察和物理急停计时均未运行，现场结果保持 0/2；这两项未通过前，`release_decision` 固定为 `HOLD`。结构化规格与复算入口见 `visual/assets/proof-block-technical-spec.json` 和 `visual/assets/run-proof-block-technical-audit.js`。[metric:technical_desktop_check_count] [metric:field_performance_test_pass_count]

在此基础上，v9.0 为十二个场景建立同一份八字段放行合同：边界用途、责任运营者、无需 AI 的普通等价服务、最小数据与留存、可见告知与选择、人工接管与物理停止、验收证据、回退与复原。每个场景运行一个完整合同和八个单字段删除病例，形成 12 个基准病例与 96 个缺项病例；另以传感越界、删除超时、账号锁定、无障碍路线受阻、急停不可达、运营者缺席、来源过期、临时边界漂移、投诉通道中断、模型或网络故障、人流冲突、复原超期构成 12 张跨场景故障卡，共 120 项确定性测试。[metric:synthetic_fault_test_count]

审计结果为 120/120 `PASS`：这里的“通过”只表示完整合同没有被误拒、每个缺项都被正确阻断、每个故障都触发普通服务或局部隔离并保持 `HOLD`。它不是安全认证、现场绩效、公众同意或实施许可。数据与复跑入口见 `visual/assets/double-memory-fault-injection.json` 和 `visual/assets/run-double-memory-fault-injection.js`。[metric:omission_case_block_count] [metric:fault_safe_fallback_count]
...
v9.0 不把三个重点区继续放大成未经核实的“大项目”，而是每处先完成一个最小样段：众智园“停机花园”以 60 米 × 14 米同意剖面为包络，保留 4 米无 AI 通行和围合低速测试；AI 原点“来源门诊”以可逆首层和前场承载纸面登记、人工申诉与可撤回来源记录；大钟寺“免账号会客厅”以无障碍到达、多语人工和端侧辅助建立站城公共界面。
...
本版把下一步踏勘转换为可执行协议：FS-01 沿清河合同段至少记录 12 个点，覆盖工作日早高峰、午间和晚高峰；FS-02 连接五道口与清华东路西口，至少 15 个点，覆盖工作日下午和周末白天；FS-03 覆盖大钟寺站四象限及两宗已知地块公共边界，至少 16 个点，覆盖工作日早高峰、晚高峰和周末白天。每条观察必须同时具备时间戳、定位与精度、拍摄方向、公共空间位置、结构化发现、隐私清理后的照片哈希和复核人，才可进入正式证据账本。

截至本版，符合上述条件的现场观察为 0 条；三条路线状态均为 `planned`，任何通行表现、使用者反馈、实际竣工状态和共同设计结果均不得声称已验证。桌面证据审计的 8 项检查已全部通过，它只证明来源、计算和措辞边界自洽，不证明现场性能。[metric:completed_field_observation_count] [depth:key_area_urban_design]
...
包内提供三项无外部依赖的结构审计：记忆护照审计、空间图谱审计与双记故障注入审计。当前回放均为 `PASS`：前两项核对责任、数据、普通等价、四级证明里程和法定控制缺口；第三项复跑 120 个合成病例，确认 96 个缺项全部阻断、12 个故障全部安全回退。所有 `PASS` 都只证明材料逻辑和拒绝机制可复算，不证明空间已建成、系统安全、公众接受或取得许可。

## 风险、版权与合规说明

本方案将公开资料边界、个人隐私、版权授权、人工复核、实施风险与合规责任作为同一条放行链；任何资料来源、使用权限或责任主体不能核验时，相关空间判断和 AI 场景保持 `HOLD`。
