...
| `assets/media/film.mp4`, `film.en.mp4` (75 s each) | Original motion graphics. Every frame is drawn programmatically by Matplotlib from **the same geometry layers and palette as the static figures**, then encoded frame by frame. No stock footage, no filmed material, no third-party clip, no motion template, and no generative video model. | Derived only from package geometry and package text. | Same display-only license. |
| `assets/media/film.vtt`, `film-en.vtt`, `film-transcript.md`, `film-transcript-en.md` | Original captions and transcripts, authored to match the on-screen text exactly. | Written for this submission. Provided so the content survives when video cannot be played. | Same display-only license. |
...
**Both films are silent - they contain a video stream only and no audio track.** No music, no voice-over, no sound effect, and no sampled recording is present, so no musical composition, sound-recording, performer, or voice right is implicated anywhere in this package. Meaning is carried by on-screen text plus the supplied WebVTT captions and transcripts. This is stated explicitly so that no reviewer needs to assume audio clearance that was never required.
...
- The two MP4 files carry H.264 video in an MP4 container with `yuv420p` pixel format, chosen for broad offline playback. No codec implementation is redistributed here; only the encoded output is submitted.
