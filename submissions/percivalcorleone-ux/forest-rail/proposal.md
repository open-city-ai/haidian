...
summary: "林轨是沿京张遗址走廊的公园层运行系统：三站共用锈轨指路、树冠安静、双轨分区与林轨终端；公共工位、低照度灯带、无品牌日用与概念智驾均可审计、可退出，安全第一。"
...
**总判断**：林轨是沿京张遗址走廊的**公园层运行系统**——锈轨指路、树冠安静、林轨终端调控公共座位/灯带/日用；**众智园、原点社区、大钟寺三站分工不同，但共用同一套安全分区、人工兜底与可退出规则** [source:AGENT-TASKBOOK]。

回答叫「林轨」：

- **林**：三站都按公园层做——疏林、树冠、软铺；望出去是树，不是幕墙秀场。公园让人安静下来，优先人与自然。
- **轨（双轨分区）**：京张遗产轨是城市的线索。装饰引导轨给方向；AI 运行轨上的智驾短驳车可载人 + 自动配置货物，**安全性第一**；与步行、装饰轨分区隔离。
- **林轨终端**：智能调控**公共开放座位/共用工位**、**地面低照度灯带**、**无品牌日用点供给**与节点开闭。不做人脸、不推品牌广告。
- **三站节点**：`FR-NODE-ZZY` / `FR-NODE-YY` / `FR-NODE-DZS` 分别挂在三处重点区，可随正式 polygon 重定位 [data:geometry/public_space.geojson#FR-NODE-DZS] [metric:forest_rail_node_count]。

> 一句话：铁轨是城市的线索，公园让人安静下来；三站共用一套规则——锈轨指路，智驾车先保安全，座位、灯带与日用可智能调节且可退出。

本案传承京张铁路敢为人先的精神；但不空喊赋能、不做网红打卡。

**本方案不做的事**：全区楼宇总控、网红打卡装置、灯光秀、把绿化条当成公园、把共用工位做成私人舱、把装饰轨做成攀爬设施、用监控换「智能」、用品牌门店矩阵淹没公园。

动线只加一条轨导走廊（引导轨为主，运行轨仅在隔离段概念保留）。走廊以外，路网怎么走还怎么走。**AI 失败时，公园主体（软铺、树冠、最低座位、人工服务）仍可独立成立。**

## 一页执行摘要

| 评审问题 | 林轨的回答 | 可核验成果 |
| --- | --- | --- |
| 核心命题 | 公园层运行系统：安静日常 + 可退出的环境/日用/短驳智能 | 四系统表 + Gate + 桌面演练证据 |
| 空间响应 | 一脊三站；装饰轨/运行轨分区；横向缝合公园—站/校/园/河 | 9 类 GeoJSON、三节点、五证据图 |
| 实施起点 | 近：软铺树冠人工服务；中：终端+日用试运行；长：隔离运行轨仅 Gate 通过后 | FR-01–09 + 前置清单 |
| 公共价值 | 无 App 也能坐下/避雨/买水/问路；最低席位与低需求供给不可撤光 | 无障碍表 + 最低供给规则 [metric:min_open_seats_count] |
| 证据状态 | 几何/指标可复算；行政统计只校准问题；概念试点非已部署 | metrics / assumptions / self_check |
| 决策边界 | 全部为概念建议，非法定规划、政府安排或投资承诺 | 风险专节与停止条件 |
...
| 层级 | 面积 | 林轨在这层做什么 |
|------|------|------------------|
| 统筹研究（43.6 km²） | AI 产业生态与城市形态判断 | 京张走廊为主轴；三站+两翼协同；AI 生态与八要素机制 |
| 总体设计（11.4 km²） | 遗址公园周边城市更新框架 | 轨导脊柱；公园慢行/软铺/树冠；四系统责任语法 |
| 重点区域（368.4 ha） | 三处详细设计 | 众智园/原点/大钟寺**同等深度**落组件与测试卡 [metric:key_area_count] |
...
### 城市公园与线性公园：全球 / 国内案例（背景·可迁移边界）

下列案例为公开建成或已运营的公共项目，**仅作背景机制启发**，不替代任务书要求的 AI 生态交付；不照搬网红装置或攀爬式遗产消费。

| 案例 | 可迁移机制 | 对本案边界（学什么 / 不学什么） | 不能证明 |
|------|------------|--------------------------------|----------|
| 纽约 Central Park | 多入口 + 分层慢行 | **学**：公园作公共权利底盘；**不学**：巨型封闭式管理 | 海淀客流或面积目标 |
| 纽约 High Line | 废弃铁路→线性步行 | **学**：铁路再利用叙事；**不学**：高架游览消费、抬升轨当攀爬 | 本案轨高程或造价 |
| 费城 Rail Park | 分段滚动开放 | **学**：一期一段轻量试点；**不学**：未完成段包装成已建成 | 分期资金已到位 |
| 伦敦 King's Cross | 站点—广场—慢行缝合 | **学**：出站即入公共空间；**不学**：商业综合体主导 | 站口客流 OD |
| 马德里 Río | 让位释放滨水绿带 | **学**：缝合被切开街区；**不学**：大规模隧道造价 | 本地开挖可行性 |
| 首尔清溪川 | 线性公共廊道 | **学**：日常线性使用；**不学**：泵水假生态叙事 | 本地水文工程 |
| 上海徐汇跑道公园 | 遗存→日常线性空间 | **学**：叙事可本土化；**不学**：宽幅硬质打卡 | 路面材料已批准 |
| 北京奥林匹克森林公园 | 疏林草坪全龄 | **学**：城市森林气质；**不学**：另辟巨型新公园 | 国家级尺度适用 |
| 京张铁路遗址公园（已建） | 铁轨留城、日常已在走 | **学**：增量连接三站日常；**不学**：抢功重做平行绿廊 | 接口改造已获批 |

本案立场：把遗产轨从「可看」推进到「可引导、可共事、可概念短驳」；突破落在安全分区与日常公共性。

城市公园案例背景来源登记于 `sources.json`（非法定依据）。北美见 [source:CASE-CENTRAL-PARK]、[source:CASE-HIGH-LINE]、[source:CASE-RAIL-PARK]。欧洲与东亚见 [source:CASE-MADRID-RIO]、[source:CASE-CHEONGGYECHEON]。国内与在地见 [source:CASE-XUHUI-RUNWAY]、[source:CASE-OLYMPIC-FOREST]、[source:CASE-JZ-HERITAGE-PARK]。
...
| 项 | 内容 |
|----|------|
| 对象 | 林下共用工位、弯道座位、雨棚热桌 [metric:desk_zone_count] |
| AI | 噪声/雨情/匿名密度 → 开闭与电源；安静/讨论分区 |
| 硬约束 | 最低开放席位 [metric:min_open_seats_count]；不记录身份；不强制扫码；轮椅席不被关光 |
| Gate | 环境超阈值可关，人工窗口常开；申诉路径；G2 未指派则 NOT AUTHORIZED |
| 证据 | `visual/assets/run_desk_tabletop.js` / `desk-tabletop-evidence.json` |

#### 2）安全优先智驾短驳（Safety-First AV Rail）

| 项 | 内容 |
|----|------|
| 分区 | 装饰轨 ≠ 运行轨；运行轨与步行物理隔离 |
| AI | 班次、避让、日用补货路径；视听专用接驳为扩展概念 |
| 安全 | 急停、越界即停、让行步行、失败即停运 |
| Gate | 无安全评估/责任主体/急停演练 → **NOT AUTHORIZED** [data:visual/assets/av-safety-gates.json] |
| 指标 | 任何精确公里数未入 metrics 者不得正文主张；一律 concept |
...
**公园层角色**：站城入园 + 城市森林日常——出站即入公园，公园里能共用一张桌子。

**林轨终端**：全套座位/灯带/日用试运行；断网退回纸面规则。

**智驾/物流（概念）**：日用补货 + 视听专用接驳扩展概念；与步行分区；非商用结论。
...
| 林下办公 | 玻璃隔断多工位，谁来谁坐 | 私人舱、LED 外立面、打卡舱 | 「高协同」已证实 |
| 锈 | 自然氧化时间标记 | 仿锈面板、人工做旧喷漆 | 必为原生锈 |

**不许做什么**：品牌门店矩阵；人脸监控/广告推送；灯光秀；关光全部座位；自称智驾已商用。

**大钟寺站一体化**：A/B/C/D 四出口各引轨径进四象限。第一眼是公园与锈轨，不是广告屏。

**商业服务**：无品牌日用试运行 → 契合度 → 开放准入摊位 / 社区快周转店；低需求保留最低供给。
