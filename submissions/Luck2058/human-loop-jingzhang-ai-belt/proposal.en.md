---
title: "Curved Line · Loop —— Urban Design of the Centennial Jing-Zhang AI Innovation Belt"
...
> This proposal package is generated by WorkBuddy Urban Design Intelligence based on publicly available information and the organization's Provisional Boundary; all spatial conclusions are design recommendations to be recalculated upon the release of the Official Boundary, and do not substitute for statutory planning or government-approved documents.

# Curved Line · Loop —— Urban Design of the Centennial Jing-Zhang AI Innovation Belt
...
The readable explanation of boundaries and key areas corresponds to [data:geometry/site_boundary.geojson#SITE-001], [data:geometry/key_areas.geojson#PROV-KEY-001] and [metric:site_area_sqm], [metric:key_area_count]. This means that readers can refer back to the GeoJSON to view the boundary sources, check the recalculated area metrics, and review the sources of information, rather than relying solely on textual descriptions. The land use, buildings, roads, green spaces, Public Spaces, phased development, and key area maps in this proposal are design suggestion geometries generated by agent based on publicly available data [source:AGENT-DESIGN-GEOMETRY], with their sources, accuracy, and confidence levels annotated in the `geometry/*.geojson` file under the `source_type`, `confidence`, and `geometry_role` fields, and in the `assumptions.json` file.
...
| International Talent | Short-term Study, Cross-border Collaboration, International Roadshows, Cultural Exchange | Dazhongsi International Roadshow Living Room, International Living Room, Bilingual Signage and Barrier-free Services | Requires compliance with foreign affairs regulations; does not collect nationality-sensitive labels, only performs aggregated statistics |
...
| 11 AI Sanctuary Landmark · Contribution Wall | Three Key Areas | Visible Honor and Attribution System for Community Contributors and Open Source Collaboration |
| 12 End-side AI Public Safety Node | Public Space and Blue-Green Corridors | Low-Intrusion Sensing, Activity Safety Assessment with Human Review Node |
...
| JZ-07 | Qinghe Low-Carbon Innovation Corridor | Blue-Green/Low-Carbon | River Blue Line, Ecological and Flood Control Conditions | [data:geometry/green_space.geojson#GREEN-001] |
| JZ-08 | Open Source Exhibition Hall and Near-School Incubation Street | Urban Renewal/Innovation | campus boundaries, ownership, first-floor uses | [data:geometry/buildings.geojson#BLDG-001] |
| JZ-09 | International Roadshow Living Room and Data Element Living Room | Operations/International Engagement | Integrated Station and City, Municipal Utilities, Activity Safety | [data:geometry/public_space.geojson#PUBLIC-001] |
| JZ-10 | AI Slow-Travel Navigation and Accessibility Perception | New Infrastructure/Slow Travel | right-of-way, bridge underbelly space, review of traffic organization | [data:geometry/roads.geojson#ROAD-001] |
| JZ-11 | AI Pilgrimage Landmark and Contribution Wall | Operations/Culture | Public Space Permits, Activity Safety, Copyright Clearance | [data:geometry/phasing.geojson#PHASE-001] |
| JZ-12 | AI Life Service Sample Street | Urban Renewal/Life Services | First Floor Business Types, Ownership, Operating Entity | [data:geometry/land_use.geojson#LU-001] |
...
The depth of metrics recalculation is managed by [depth:metrics_recalculation]. All known metrics are recalculated from the generated geometry projected into `EPSG:4548` (CGCS2000 3° Gauss-Kruger with central meridian at 117E) [source:AGENT-DESIGN-GEOMETRY]; declared area equals recomputed area (topologically strict, bit-identical), thus the numerical values in the table can be independently verified by `spatial_review.py` and `self_check_submission.py`. This scheme explicitly references all 26 known metrics and specifies their source layers:

| Indicator Name | Value | Unit | Recalculated Source |
| --- | --- | --- | --- |
| [metric:site_area_sqm] | 11,412,825.386 | ㎡ | [data:geometry/site_boundary.geojson#SITE-001] |
| [metric:coordinated_research_area_sqm] | 43,609,232.558 | ㎡ | Coordinated Research Area Calculation |
| [metric:key_area_total_sqm] | 3,692,893.005 | ㎡ | [data:geometry/key_areas.geojson#PROV-KEY-001] |
| [metric:key_area_count] | 3 | units | [data:geometry/key_areas.geojson#PROV-KEY-001] |
| [metric:land_use_parcel_count] | 143 | | [data:geometry/land_use.geojson#LU-001] |
...
| [metric:building_count] | 336 | units | [data:geometry/buildings.geojson#BLDG-001] |
...
| [metric:renewal_project_count] | 12 | | [depth:renewal_project_list] |
| [metric:ai_scenario_card_count] | 12 | pieces | [depth:three_key_area_detailed_design] |
| [metric:persona_count] | 6 | Type | [depth:blue_green_public_space] |

There are 5 unknown indicators (`floor_area_ratio`, `official_site_area_sqm`, `building_height_limit_m`, `existing_building_footprint_area_sqm`, `resident_population_capacity`) that are marked `status=unknown` in `metrics.json` due to the lack of official control plans, current survey data, or population baseline. These are listed as pending prerequisites and are not included in the formal conclusions.
