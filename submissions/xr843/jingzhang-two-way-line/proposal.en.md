...
See chapter 8. At regulatory-plan depth the ruling is explicit: all transport and municipal content is a systemic concept; alignments, pipelines, and capacities require professional calculation [standard:MOHURD-CONTROL-DETAILED-PLANNING].

### 4.4 The relic-park vitality belt

The core move is "**eight crossings, one through-line**": today's park breaks concentrate where east-west streets sever it, so crossings are set at Xueyuan South Rd, the North Third Ring, Zhichun Rd, the North Fourth Ring, Chengfu Rd, Qinghua East Rd, Yuequan Rd, and Qinghe Bay [metric:crossing_node_count], each combining a through-route, an activity ground inside the park band, and an AI scenario port [data:geometry/public_space.geojson#PB-X01]. The south end meets the Two-Way Gate and the Beijing North arrival sequence; the north end meets the Qinghe waterfront; the two ring-road overpasses become landmark nodes (bridge feasibility to be professionally verified; concept only). Parking, sports, and social facilities line the spine; testing and demonstration ride the scenario ports under Timetable management [depth:blue_green_public_space].

### 4.5 Urban character of the AI era

The keynote is the Jing-Zhang tricolor — **brick red of station houses, green of green power, deep blue of the tunnel**: heritage frontages (the Qinghuayuan Station view corridor, the Juesheng Temple surrounds) keep low brick-and-timber grain and heritage setbacks [source:QHY-STATION-HERITAGE]; buildings step back from the spine like platform sections, rising toward Xueyuan and Xitucheng Roads; concept height zones run 12–24 m at heritage frontages, 24–45 m along the spine, 45–80 m on the eastern arterials — all guidance pending regulatory confirmation [depth:height_massing_character]. Roofs are encouraged toward photovoltaic fifth facades; night lighting is scripted by the Timetable so the city's rhythm of light itself displays the state of AI operations [standard:MOHURD-URBAN-DESIGN-MEASURES].
...
**Position**: a garden-type AI innovation district, the "locomotive depot" of full-stack training and standards governance, the corridor's northern interface [metric:key_area_zhongzhiyuan_sqm] [data:geometry/key_areas.geojson#PROV-KEY-001]. **Structure**: Qinghe Bay plaza as the core, research clusters along the northern spine, white-land reserve near the Fifth Ring; buildings, green space, and water designed as one with Qinghe culture as the ground [data:geometry/green_space.geojson#GS-002]. **Building renewal**: concept clusters of 8–12-storey research and laboratory bars, display uses fronting the plaza. **Mobility**: interfaces reserved for the ring-road integration program (concept); internal circulation via Yuequan and Shuangqing Roads. **Public space**: the Qinghe waterfront, Qinghe Bay crossing, and reserve buffer. **AI scenarios**: the low-speed delivery corridor test (Run T1), waterfront environmental sensing, standards and safety-governance hall. **Risk**: the timing of ring-road integration and national platforms is external; related content is an interface reservation [depth:municipal_new_infrastructure].

### 5.2 Origin Station · Beijing AI Origin Community (about 104.3 ha)

**Position**: a near-campus innovation district, the "switch-back station" where research reverses into product and campus reverses into city [metric:key_area_origin_sqm] [data:geometry/key_areas.geojson#PROV-KEY-002]. **Structure**: Wudaokou plaza as the core, an incubation-conversion belt along the west spine, education and talent housing north, commercial frontage east. **Building renewal**: low-disturbance organic renewal keeping courtyard grain, inserting incubators and a release hall; building-by-building retain-renovate-demolish awaits survey — the concept strategy is "retain first, renovate to activate, demolish minimally" [depth:retain_renovate_demolish]. **Mobility**: station-city integration around Wudaokou and Qinghua East Rd West; Chengfu and Qinghua East crossings stitch campus and district. **Public space**: Wudaokou plaza, Chengfu crossing, the release-hall forecourt. **AI scenarios**: the traffic-and-walkability dispatch test (Run T2), campus first releases, multilingual academic guidance. **Risk**: the openness of campus walls decides the stitch depth; campus-city negotiation must lead.

### 5.3 Bell Station · Dazhongsi AI Industry Cluster (about 72.0 ha)

**Position**: an urban-type innovation district, the "down-line terminus" of agents, devices, and content consumption — the interface of market and data assets [metric:key_area_dazhongsi_sqm] [data:geometry/key_areas.geojson#PROV-KEY-003]. **Structure**: the Green-Power Bell plaza as core, native-intelligent consumption streets along Dazhongsi East Rd, the Juesheng Temple cultural frontage opening a westward view corridor. **Building renewal**: existing commercial boxes converted to device flagships and content scenes; taller business clusters gathered on the Third-Ring frontage (10–16-storey concept volumes). **Mobility**: four-quadrant pedestrian connection at Dazhongsi metro, restructured bicycle parking and static traffic; a Third-Ring deck crossing as a long-term concept. **Public space**: the Bell plaza and the temple-front cultural green used compositely. **AI scenarios**: the enterprise-copilot compliance sandbox (Run T3), the annual archive-casting ceremony, data-asset circulation display (mechanisms described as exploratory, not settled policy). **Risk**: everything near the heritage compound is premised on heritage-authority approval; only view-corridor and setback guidance is offered [source:QHY-STATION-HERITAGE].
...
Every spatial metric is recomputed from the packaged GeoJSON in EPSG:4548, matches metrics.json, and passes spatial review [depth:metrics_recalculation]. Core indicators and their design meaning:

| Indicator | Value | Design meaning |
| --- | --- | --- |
| Site area | 11,400,064 m² [metric:site_area_sqm] | +0.0006% against the announced 11.4 km² |
| Land-use coverage | 11,399,407 m² [metric:land_use_area_sqm] | Full partition, gap about 658 m² |
| Key areas total | 3,683,997 m² [metric:key_area_total_sqm] | Calibrated within ±0.01% of announced |
...
| Spine length | 7,829 m [metric:greenway_length_m] | The through slow route |
...
| Personas / landmarks / cases | 6 / 4 / 7 [metric:user_persona_count] [metric:ai_pilgrimage_landmark_count] [metric:global_case_study_count] | Above taskbook minimums |
...
**Source legality**: only public or cleared materials are used; OpenStreetMap data follows ODbL with "© OpenStreetMap contributors" attribution on drawings and pages [source:OSM-CONTEXT]; no unauthorized fonts, images, trademarks, likenesses, or corporate marks; case references are qualitative public knowledge [depth:risk_missing_data]. **AI-generation responsibility**: the entire package is generated by an AI agent; the human account owner only authorized participation; generation methods, model, and pipeline are disclosed in agent.json and the copyright statement [source:PROCESSED-FACT-PACK]. **Claim boundaries**: no "approved" or "guaranteed" statements anywhere; FAR, heights, retain-renovate-demolish, alignments, and municipal capacities are concept references or pending items; data-element and digital-asset mechanisms are exploratory. **Privacy**: all scenario cards enforce "no surveillance control, stoppable, human path available". **Main risks** (eight-dimension scores in risk.json): the dual provisional-boundary frame (an Issue is being filed for community review), missing regulatory controls, cross-regional mechanism uncertainty, the un-calculated energy system, heritage-approval precedence, and resident-negotiation costs. **Pending data**: official redlines and key-area polygons, regulatory conditions, building ownership and condition surveys, road redlines, municipal capacities, and the park's full-line design records [depth:risk_missing_data]. Licensing details are in `report/copyright_statement.md`.
