...
本文件不再声称投稿包已经完成清权。`visual/assets/rights-clearance-ledger.json` 对 manifest 中 48 个路径逐项归组，记录来源基础、当前权利状态、限制与发布门槛；当前独立逐文件清权审计完成数为 **0**，总体状态为 `not_fully_cleared`。
...
- 离线 HTML 不含远程资源，也没有随包分发字体文件；HTML 使用系统字体栈。PDF 的字体嵌入与许可、PNG/PDF 的完整可编辑源、模型和生成工具输出条款尚未完成独立审计。
...
- Offline HTML has no remote dependencies and the package ships no font binaries; HTML uses system font stacks. PDF font embedding and licensing, complete editable sources for PNG/PDF outputs, and model/tool output terms have not been independently audited.
