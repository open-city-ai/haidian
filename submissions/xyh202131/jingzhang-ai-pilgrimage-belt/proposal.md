...
> **四态结论：** 普通任务先成立；可选验证只在全部前置条件满足后出现；故障仅停止验证叠层，连续日常轨、完整非 AI 路径、人工交接、撤回与申诉继续；恢复先还普通使用，但不等于授权、重启或 G1。48 秒只是普通—验证—故障—恢复的展示节奏（每态 12 秒），不是现场故障恢复时长。本轮新增 1 段概念视频（`assets/media/4-state-motion.mp4`）+ 1 段旁白音频（`4-state-narration.mp3`）+ WebVTT 字幕（`4-state-narration.vtt`）+ 文字稿（`4-state-narration-script.md`），全部在 `visual/index.html#four-state-media` 入口展示，不自动播放，权利台账登记为 ASSET-150..153。

![三框场地读取：公开背景定位、临时设计容器与双轨设计关系分开表达，绝不套准；G0 概念、临时几何、不按比例、不证明官方红线或站点锚定](assets/figures/site-overview.png)

> **编辑层级：13 个必答章 + 8 个扩展证据单元。** 必答章由“设计依据与资料清单”至“风险、版权与合规说明”的第 1—12 章，以及文末“参考资料”第 13 章组成。本“阅读入口”为扩展证据 E0；第 12 章后、第 13 章前的七个 H2 单元为 E1—E7。扩展证据用于展开核验、专业交接和现场替换规则，不替代必答章，也不改变任何 G0、临时边界、未知项、非 AI 路径、权利阻断或专业否决。
...
以下每问只指定一个前台主答案；proposal、A3 与 A0 是等价复核面，不形成第二套结论。冷读记录不是公众反馈、专家意见、评审结果或批准。本轮无新媒体。

| 编号与问题 | 唯一主答案 | 前台唯一入口 | 印刷复核位置 |
|---|---|---|---|
| Q1 · 方案唯一核心概念是什么？ | “双轨京张”：连续日常轨先成立，间歇验证轨只在旁侧出现；三处原型不可互换；全案仍为 G0。 | `visual/index.html#jury-entry` | A3 P1；A0 P1 |
| Q2 · 不注册、不扫码、不使用 AI 能否完成普通任务？ | 能。实体导向、无屏信息、连续非 AI／无障碍意图路径、纸面或口头说明、人工交接和清晰退出先独立成立。 | `visual/index.html#ordinary-life` | A3 P8；A0 P5 |
...
![三座换轨场空间图谱：绿色普通／非 AI 连续路径，蓝色旁侧验证，红色停止对象，H 人工交接与灰色恢复回归；三处分别呈现平行旁路、一街两院和四向通勤。](assets/figures/spatial-atlas.png)

**空间图谱读法。** 先读每处普通任务的绿色连续路径，再读可停的蓝色旁侧验证、红色停止对象、H 人工交接和灰色恢复对象。众智园在平行旁路和验证庭之间切换；原点社区只逐点撤回四节点而不关闭居民街；大钟寺把来源、屏幕或发布服务停在通勤十字之外。故障停止的只是验证／服务叠层，普通生活与完整非 AI 路径继续；恢复只回到被命名的普通空间关系，不构成授权、批准、G1 或重启。该图是 G0、临时关系、非比例的编辑性概念图，不证明真实地点、已建成状态、无障碍结果、人员、工期、恢复时长或公众意见 [data:visual/assets/spatial-atlas-contract.json]。
...
**R46（2026-08-18）三处重点区概念叙事脚本登记。** 为第 6 章的三处临时重点区（PROV-KEY-001 众智园 AI 自主创新加速区、PROV-KEY-002 北京 AI 原点社区、PROV-KEY-003 大钟寺 AI 产业聚集区）补 3 段普通话旁白文字稿（`assets/media/key-area-1-narration-script.md` / `-2` / `-3`）和 3 段 WebVTT 字幕（`-1-narration.vtt` / `-2` / `-3`），与 R45 的"普通—验证—故障—恢复"四态脚本并列形成操作层 + 空间层双层多媒体骨架；对应的 mp3（TTS）与 mp4（AIGC 概念视频）二进制**不在本轮生成**，显式延后到 R47，访问者当前看不到任何新增的视频或音频入口，visual/index.html 也不嵌入新的 `<video>` 或 `<audio>` 元素。本轮不新增场景、项目、治理合同、成熟度或页面；geometry、metrics、sources、assumptions 与 R45 完全一致；权利台账新增 ASSET-154..159 共 6 条，全部 `not_fully_cleared`，新增 P2 unresolved `RIGHTS-OPEN-R46-MEDIA-DEFERRED` 标记 mp3/mp4 二进制延后 [source:SOURCE-JZ-KEY-AREA-NARRATION-G0-V1] [source:SOURCE-JZ-KEY-AREA-NARRATION-PROPOSAL-R46] [data:visual/assets/rights-clearance-ledger.json#asset_records.ASSET-154..159] 。

**R46 (2026-08-18) key-area concept narration script registration.** For the three provisional key areas in Section 6 (PROV-KEY-001 Zhongzhiyuan AI Innovation Acceleration Area, PROV-KEY-002 Beijing AI Origin Community, PROV-KEY-003 Dazhongsi AI Industry Cluster), R46 adds three Mandarin narration scripts (`assets/media/key-area-1-narration-script.md` / `-2` / `-3`) and three WebVTT subtitle tracks (`-1-narration.vtt` / `-2` / `-3`), joining the R45 "ordinary–verify–fail–recover" four-state script as a dual-layer (operational + spatial) multimedia skeleton. The corresponding mp3 (TTS) and mp4 (AIGC concept video) binaries are not generated in this round and are explicitly deferred to R47; visitors currently see no new video or audio entry, and visual/index.html does not add new `<video>` or `<audio>` elements. R46 introduces no new scene, project, governance contract, maturity or page; geometry, metrics, sources and assumptions are identical to R45. The rights ledger adds ASSET-154..159 (six records, all `not_fully_cleared`) and a new P2 unresolved `RIGHTS-OPEN-R46-MEDIA-DEFERRED` noting the deferred mp3/mp4 binaries [source:SOURCE-JZ-KEY-AREA-NARRATION-G0-V1] [source:SOURCE-JZ-KEY-AREA-NARRATION-PROPOSAL-R46] [data:visual/assets/rights-clearance-ledger.json#asset_records.ASSET-154..159] .


**R47（2026-08-18）三处重点区概念叙事多媒体完整版。** 在 R46 文本骨架基础上补 3 段概念视频（`assets/media/key-area-1-concept.mp4` / `-2` / `-3`，10 秒 16:9 720P，muted earth tones 抽象几何，无真人/无真建筑/无真地点，持续水印）和 3 段普通话旁白（`assets/media/key-area-1-narration.mp3` / `-2` / `-3`，本地 espeak-ng 1.51 cmn 男声 speed 158 生成的约 55-59 秒音频），并把 3 个 WebVTT 时码按实测 mp3 长度（58.8s / 55.7s / 57.2s）重新对齐；visual/index.html 现已在 R45 四态视频/音频旁边嵌入 3 套 `<video controls preload="none" playsinline>` + `<audio controls preload="none">` + `<track kind="captions">` + 文字稿链接，**全部不自动播放**。本轮关闭 R46 留下的 `RIGHTS-OPEN-R46-MEDIA-DEFERRED`（标 `resolved_in_r47`），新增 P2 unresolved `RIGHTS-OPEN-R47-VIDEO-PRODUCER-VARIANCE`（同一 prompt 在视频平台上由 MiniMax / Qwen 两条路径返回不同 ContentProducer，已如实登记到 ASSET-160/162/164）。原本计划的"平台 TTS API（voice male-qn-qingse, speed 0.9）"在本环境无商业 TTS 时被本地 espeak-ng 替代，**这是现实约束降级（机械音色，非神经网络 TTS）**，所有产物仍 `not_fully_cleared`、`audit_records` 仍为 0、`public_or_professional_reuse_blocked` 仍 true；geometry / metrics / sources / assumptions 与 R46 完全一致；本轮不新增场景、项目、治理合同、成熟度或页面 [source:SOURCE-JZ-KEY-AREA-NARRATION-REGISTER-R46] [data:visual/assets/rights-clearance-ledger.json#asset_records.ASSET-160..165] [data:assets/media/key-area-1-narration-script.md] 。

**R47 (2026-08-18) key-area concept narration multimedia completion.** Building on the R46 text skeleton, R47 adds three concept videos (`assets/media/key-area-1-concept.mp4` / `-2` / `-3`, 10-second 16:9 720P, muted earth tones, abstract geometric, no real people/architecture/place, persistent watermark) and three Mandarin narrations (`assets/media/key-area-1-narration.mp3` / `-2` / `-3`, about 55-59 seconds each, generated by local espeak-ng 1.51 cmn male voice speed 158), then re-aligns the three WebVTT timecodes to the actual mp3 lengths (58.8s / 55.7s / 57.2s). visual/index.html now embeds three sets of `<video controls preload="none" playsinline>` + `<audio controls preload="none">` + `<track kind="captions">` + script.md links next to the R45 four-state video/audio, all without autoplay. R47 closes the R46-deferred `RIGHTS-OPEN-R46-MEDIA-DEFERRED` (marked `resolved_in_r47`) and adds a new P2 unresolved `RIGHTS-OPEN-R47-VIDEO-PRODUCER-VARIANCE` recording that the same prompt was routed by two platforms (MiniMax and Qwen) and returned different ContentProducers, faithfully logged in ASSET-160/162/164. The originally planned "platform TTS API (voice male-qn-qingse, speed 0.9)" was substituted by local espeak-ng because no commercial TTS API was reachable from the local OpenClaw environment; this is a reality-bound downgrade (mechanical voice, not neural) and is recorded explicitly. All R47 media remain `not_fully_cleared`; `audit_records` stays 0; `public_or_professional_reuse_blocked` stays true; geometry, metrics, sources and assumptions are identical to R46. R47 introduces no new scene, project, governance contract, maturity or page [source:SOURCE-JZ-KEY-AREA-NARRATION-REGISTER-R46] [data:visual/assets/rights-clearance-ledger.json#asset_records.ASSET-160..165] [data:assets/media/key-area-1-narration-script.md] .


**R49（2026-08-18）双语多媒体质量升级。** 把 R47 用本地 espeak-ng 共振峰合成（机械音色，22kHz，约 47% 静音）的 3 段中文 mp3 升级为 edge-tts zh-CN-YunyangNeural 真人级神经网络 TTS（24kHz，平滑，~5% 静音）；补 3 段英文 mp3（edge-tts en-US-GuyNeural，43.4s / 41.3s / 40.9s，24kHz）实现完整双语覆盖；3 段中文 WebVTT 时码重对齐到 edge-tts 实测长度（37.9s / 35.2s / 34.5s）；3 段英文 WebVTT 时码锁英文 mp3 长度（43.4s / 41.3s / 40.9s）。visual/index.html 现已在 R45 四态视频/音频 + R47 三处重点区视频/音频之外，加 3 段英文 mp3 + 3 段英文 WebVTT 链接（双 `<a href>` 在 figcaption 下），让英文访问者直接听 + 读双语叙事。本轮关键诚实披露——上游 Microsoft Azure neural TTS 服务**不在 mp3 元数据块内嵌入 ContentProducer 或 ProduceID**（与 R47 mp4 TC260PG 元数据形成对照），rights ledger 新增 P2 unresolved `RIGHTS-OPEN-R49-TTS-NO-METADATA`，本轮不声明任何平台标识符；权利状态全部 `not_fully_cleared`、`audit_records` 仍为 0、`public_or_professional_reuse_blocked` 仍 true；geometry / metrics / sources / assumptions 与 R47 完全一致；本轮不新增场景、项目、治理合同、成熟度或页面 [source:SOURCE-JZ-KEY-AREA-NARRATION-REGISTER-R46] [data:visual/assets/rights-clearance-ledger.json#asset_records.ASSET-166..171] [data:assets/media/key-area-1-narration.en.mp3] 。

**R49 (2026-08-18) bilingual multimedia quality upgrade.** R49 upgrades the 3 zh mp3 from R47's local espeak-ng formant synthesis (mechanical voice, 22kHz, ~47% silence) to edge-tts zh-CN-YunyangNeural human-grade neural TTS (24kHz, smooth, ~5% silence); adds 3 en mp3 via edge-tts en-US-GuyNeural (43.4s / 41.3s / 40.9s, 24kHz) for full bilingual coverage; re-aligns the 3 zh WebVTT timecodes to actual edge-tts lengths (37.9s / 35.2s / 34.5s); adds 3 en WebVTT timecode-locked to the en mp3 lengths. visual/index.html now provides 3 en mp3 + 3 en WebVTT links in addition to the R45 4-state video/audio and the R47 3-key-area video/audio, enabling English-speaking visitors to listen and read the bilingual narrative directly. critical honest disclosure: the upstream Microsoft Azure neural TTS service does NOT embed ContentProducer or ProduceID in the mp3 metadata block (contrast with the R47 mp4 TC260PG metadata); the rights ledger adds new P2 unresolved `RIGHTS-OPEN-R49-TTS-NO-METADATA` and this round claims no platform identifier beyond the bytes themselves. All R49 media remain `not_fully_cleared`; `audit_records` stays 0; `public_or_professional_reuse_blocked` stays true; geometry, metrics, sources and assumptions are identical to R47. R49 introduces no new scene, project, governance contract, maturity or page [source:SOURCE-JZ-KEY-AREA-NARRATION-REGISTER-R46] [data:visual/assets/rights-clearance-ledger.json#asset_records.ASSET-166..171] [data:assets/media/key-area-1-narration.en.mp3] .

**R50（2026-08-18）英文旁白完整版升级。** 把 R49 的英文旁白从简略版（每段 1-2 句，43.4s / 41.3s / 40.9s）扩展为完整版（每段 3-4 句，66.1s / 60.8s / 63.1s），忠实对应中文脚本的 4 段叙事结构（PROV-KEY-001 三段切分；PROV-KEY-002 三类公共节点；PROV-KEY-003 分时复用 + 错峰调度）；3 段英文 WebVTT 时码重对齐；manifest / rights ledger 6 条 en ASSET 全部刷新 sha256 + description。本轮不引入新场景、新项目、新重点区、新治理合同、新成熟度或新页面；geometry / metrics / sources / assumptions 与 R49 完全一致；R49 zh mp3、R47 mp4 二进制保持不变；权利状态继续 `not_fully_cleared`，`audit_records` 仍为 0，`public_or_professional_reuse_blocked` 仍 true [data:assets/media/key-area-1-narration.en.mp3] [data:visual/assets/rights-clearance-ledger.json#asset_records.ASSET-166..171]。

**R50 (2026-08-18) English narration full version upgrade.** R50 expands the English narration from the R49 simplified version (1-2 sentences per segment, 43.4s / 41.3s / 40.9s) to the complete version (3-4 sentences per segment, 66.1s / 60.8s / 63.1s), faithfully mapping the Chinese scripts' 4-segment narrative structure (PROV-KEY-001 three-segment isolation; PROV-KEY-002 three public-node types; PROV-KEY-003 time-shifted envelope + off-peak dispatch). The 3 English WebVTT timecodes are re-aligned; the 6 en manifest / rights ledger ASSET records have refreshed sha256 and description. R50 introduces no new scene, project, key area, governance contract, maturity or page; geometry, metrics, sources and assumptions are identical to R49; R49 zh mp3 and R47 mp4 binaries are unchanged; all assets remain `not_fully_cleared`; `audit_records` stays 0; `public_or_professional_reuse_blocked` stays true [data:assets/media/key-area-1-narration.en.mp3] [data:visual/assets/rights-clearance-ledger.json#asset_records.ASSET-166..171].
...
> **扩展证据 · E1—E7（E0 为文首阅读入口）。** 以下七个 H2 单元展开核验、专业交接、可逆构件、可访问追证和现场资料替换；它们是证据补充，不是新增必答章，也不另立方案或升级成熟度。
