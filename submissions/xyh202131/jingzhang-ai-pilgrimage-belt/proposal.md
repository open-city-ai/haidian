...
> **就绪状态双轴。** 四门本地机器闸门与 `self_check.json` 的 `can_enter_formal_review=true` 只表示投稿包结构、引用、图面和文件一致性已通过参与者自检；它不等于正式专业评分已解锁。当前 `manifest.validation_claim.known_blockers` 明确保留权利 P0、中央来源登记和发布复用阻断，因此专业评分与公共／专业复用仍为 **BLOCKED**。这些外部事项不能用渲染、PR 审查或合并替代。
...
下表把“测试接力”收束为可交接的建议合同。五个节点都只是待确认的角色，不是合作方、责任主体、预算来源或已批准场地；任何一行缺少书面接受、批准范围、现场普通任务基线、数据安全、停止恢复和独立复测记录时，均保持 G0 / NO-GO。接力只携带任务 ID、版本化协议、匿名汇总与失败记录，不携带原始敏感数据。

| 建议节点（未确认） | 建议角色 | 输入边界 | 复测输出 | 停止条件 | 当前状态 |
|---|---|---|---|---|---|
| 北纬社区 | 以居民日常服务和社区共创为参照，检查原点社区的非 AI 同任务路径 | 只接收已同意的任务描述、无个人识别信息的协议版本和汇总阈值 | 同任务完成／放弃／人工交接／撤回记录，以及社区纠正回执 | 非 AI 路径变长或中断、无屏入口缺失、撤回或人工交接失败 | `unconfirmed`；不构成居民意见或合作承诺 |
| 未来科学城 | 承接基础科学或前沿装备的离线／影子复测 | 只接收测试协议、设备类别和匿名失败分类；不传原始数据或未批准设备 | 版本化技术结果、失败原因和是否建议回到 G0 的决定包 | 来源过期、边界冲突、无安全责任接受或普通任务受影响 | `unconfirmed`；不构成伙伴或场地确认 |
| 怀柔科学城 | 承接科学设施相关的可重复性检查 | 只接收协议、合成或经授权的汇总结果和复测前提 | 独立复测摘要、偏差清单和停止／恢复建议 | 设施条件、审批范围、数据安全或专业责任任一项缺失 | `unconfirmed`；不构成设施接入 |
| 北京经开区 | 承接制造、机器人和具身系统的工程复测 | 只接收接口契约、匿名性能区间和故障分类；不转移生产或个人数据 | 工程复测包、故障隔离记录、人工接管与还场结论 | 安全边界不清、人工接管失败、故障影响普通通行或未完成独立复测 | `unconfirmed`；不构成制造试点 |
| 京津冀节点 | 检查跨区域复制时的协议、来源和普通服务同权 | 只接收任务／协议版本、匿名结果和失败记录；不主张跨域授权 | 可复制性差异表、地方化待补项和是否保留 NO-GO 的建议 | 规则、来源、权利、普通路径或责任边界无法对齐 | `unconfirmed`；不构成区域合作或落地承诺 |

矩阵输出只支持专业交接和后续资料替换，不增加场景、项目、治理合同或成熟度；正式来源仍以中央 `data/source_registry.json` 为准，未登记来源不得承担新的正式事实 [assumption:A-EXTERNAL-COLLAB-005] [data:visual/assets/implementation-handoff-matrix.json#regional_synergy_retest_matrix]。
