...
| A3/A0 PDFs | `drawings/a3-booklet*.pdf`, `drawings/a0-boards*.pdf` | Generated locally from submission-original text/geometry/figures | None beyond assets listed above | Submission-original compilation | Must be regenerated after v0.6 proposal/visual integration before the next formal PR |
...
### v0.6 additional visual-asset ledger

| Asset group | Paths | Author / generation method | Third-party material embedded | Rights / status |
| --- | --- | --- | --- | --- |
| Flagship pilot protocols | `assets/figures/flagship-pilot-protocols.svg`, `.en.svg` | Original bilingual SVG converting three existing proposal scenarios into explicit baseline / prerequisite / test / stop / exit protocols | None | Submission-original; concept protocols are not permits, contracts, safety certification or operating approval |
| Inclusion and burden matrix | `assets/figures/inclusion-burden-matrix.svg`, `.en.svg` | Original bilingual SVG expanding proposal personas into benefit, burden, exclusion-risk, non-AI-equivalent and help/appeal checks | None | Submission-original; persona categories are design-test groups, not demographic survey findings |
| Implementation resource and RACI gates | `assets/figures/implementation-resource-raci.svg`, `.en.svg` | Original bilingual SVG specifying concept quantity basis, A/R/C/I roles, maintenance cadence, prerequisite evidence gates and GO/REVISE/STOP receipts | None | Submission-original; currency cost, final FTE, named organizations, procurement, insurance and permits remain unverified/UNKNOWN |
| Reviewer evidence index | `assets/figures/reviewer-evidence-index.svg`, `.en.svg` | Original bilingual SVG mapping public rubric dimensions and agent.1–agent.6 tasks to existing package evidence and claim boundaries | None | Submission-original; traceability aid only, not a participant-generated score or maintainer decision |
| Review evidence dashboard | `assets/figures/review-evidence-dashboard.svg`, `.en.svg` | Original bilingual SVG condensing package metrics, C7, flagship pilots, inclusion, implementation gates and claim limits for above-fold visual review | None | Submission-original; no claim of official validation or final v0.6 PASS is embedded |

## Fonts, icons, code, and toolchain

The v0.5/v0.6 SVG assets use generic CSS font-family declarations such as `sans-serif`, `Arial` and common system CJK fallbacks and do not redistribute font files. No downloaded icon font, commercial typeface, third-party icon pack or proprietary design-system asset is embedded. Symbols in the C7/VI and component diagrams are original geometric primitives authored in SVG.
...
`visual/index.html` and `visual/index.en.html` are self-contained offline pages using only embedded CSS and local package images/SVG. They load no remote JavaScript, map tiles, fonts, iframes, forms, analytics or tracking resources. The v0.6 above-fold evidence dashboard is loaded only from the submission's own `assets/figures/` directory.
...
- prepare the v0.6 flagship-pilot, inclusion/burden, implementation/RACI, reviewer-index and above-fold dashboard diagrams.
