...
summary: "JINGZHANG MAINLINE merges the Jing-Zhang Railway—the first trunk railway independently designed and built by Chinese engineers—with the open-source mainline-branch-merge workflow: one 9 km mainline greenway links three Merge nodes (Zhongzhiyuan CI Yard training ground, Beijing AI Origin Community Kernel, Dazhongsi Release Market) and two wings (Zhongguancun Registry services wing, Xiaoyue River Staging sandbox wing), organizing the 11.4 km² overall design and 368.4 ha key areas through a fork→branch→commit→review→merge→release→maintain innovation loop. v0.4 adds a second verifiable track: the DOUBLE-TRACK validation principle—every AI scenario merged into the mainline must first prove a working no-AI baseline, pass an unplug test, and leave a public dividend that remains usable without models, accounts, or network after leaving. Core deliverables are 12 AI scenario cards (each with a four-part verifiable contract and an immediate-exit condition), dual project-plus-scenario admission gates, reproducible synthetic tabletop evidence, and an offline public-safety net, alongside 6 user personas, 4 pilgrimage landmarks, and a long-term open-source operation system. All spatial proposals are conceptual, based on the provisional boundary; the entire package must be recalculated when official boundaries are released."
