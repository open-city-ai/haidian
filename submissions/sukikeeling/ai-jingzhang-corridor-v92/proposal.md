...
### 折返点＝拔线点（AI关闭等价操作层）
折返点不只是"停车评估"，更是一个**拔线测试点**：每个场景在折返点必须声明五项可机器核对的字段——`ai_off_path`（AI关闭后的等价路径，不得指向仍依赖同一系统的线上办理）、`human_handoff`（人工接管角色）、`gate_id`（所处闸门）、`operating_mode`（正线/侧线/入段）、`responsible_role`（责任专业）；缺任一字段的点位不得计入服务覆盖 [data:geometry/constraints.geojson#SWB-01]。本方案已为 8 个折返点（覆盖缓坡/中坡/陡坡三级与三态）声明完整五字段，完备率与人工接管指定率均为 1.0，可逐点核对 [metric:ai_off_path_completeness] [metric:human_handoff_designation_rate]。把折返点做成拔线点，是把"人字三义"从哲学落到数据：到站必须由人确认，而"由人确认"的第一条证据，就是城市在AI关闭时仍可用。停止与恢复规则集（8 项风险维度的 mitigation 与 human_review，触发即停、恢复须交证据而非承诺）见 `risk.json` [depth:risk_missing_data]。

## 折返等价基准 SWB（可取走即用的公共产品）
前面把判据散在场景卡、闸门、指标与风险条目里，读者要拼出"怎样才算做到了"需在多处来回翻。本节把这些判据封装成一件独立的、可被任何城市取走即用的东西：**折返等价基准（Switchback Equivalence Baseline，SWB）v0.1**。它不是本方案的附录，而是本方案希望留下的公共产品——海淀能否成为人本智能策源地，最终取决于别的城市是否愿意用海淀写下的判据来检验自己 [source:AGENT-TASKBOOK]。机器可读规范位于 `visual/assets/swb-spec.json`，与本章逐项一致。

| 基准组件 | 规范内容 | 本包内机器可读位置 | 复用时必须一并接受的约束 |
| --- | --- | --- | --- |
| 折返点 schema | 每个点位声明 ai_off_path/human_handoff/gate_id/operating_mode/responsible_role 五字段，缺一不计覆盖 | `geometry/constraints.geojson` 中 constraint_type=ai_off_equivalence_point 的 8 个折返等价点（SWB-01..08）属性表 | 五字段必填非可选；ai_off_path 不得填写"引导至线上办理"一类仍依赖同一系统的路径 |
...
**版本治理已转动一次。** 这套治理不是写给别人看的条款，它在本方案内部转过一圈：把折返点判据写成可执行 schema 时撞到一处缺口——v8.1（K0）的折返点只在正文散文描述，constraints.geojson 未声明任何 constraint_type=ai_off_equivalence_point 的折返等价点要素，"AI关闭后仍可用"无法被机器逐点核对。该缺口被登记进变更回执 **CR-2026-08-14-001**（见 `visual/assets/governance-receipts.json`）而非就地绕过，随后按"变更须附失败样本登记与复算说明"带着证据交付：新增 8 点五字段、新增三项指标定义。本次为 schema 层变更，零既有指标数值受影响。这一段能证明的不是判据写得好，而是 K标版本治理是一个已经转动过的机制而不是一句承诺 [depth:kmarker_versioning]。

**复用场景。** 基准的价值只有在离开本项目之后才能被检验。中关村AI北纬社区初创团队可只取 node_schema 的 ai_off_path 与 human_handoff 两项，为早期产品做一次"AI关闭后仍可用"的自测，不需任何平台或授权；未来科学城能源/医药专业设备界面可只取评分口径中的服务等价差一项，把"专业设备只给专业人员用"这一默认前提放到可测位置；任意城市的政务大厅可只取 G0 至 G2 三级，不改造任何空间先完成一轮成对测试，得到本厅的服务等价差读数。这是区域协同章"输出可复用的测试方法学"一句的实物形态：不是一份意向，而是一个文件加一张表 [source:BJ-AI-INNOVATION-DISTRICTS-20260121]。基准本身不构成合规结论或认证资格，它只是把"怎样才算做到了"写成任何人都能逐项核对的形式；托管主体、发布渠道与许可仍待授权主体确认 [source:SOURCE-REGISTRY]。
...
本节是 SWB 评分口径的一次实物演示：在 SC-01 无障碍路径助手（折返点 SWB-01，五道口/原点社区）做一组配对测试，把"AI关闭时城市是否仍可用"从口号变成可逐案核对的数据。**这是方法学演示，使用合成测试用例，非真实世界普查基线**——其价值在于方法与数据公开，任何人可复算等价差读数，验证测量方法本身是否成立；真实基线在 K3-K6 一期普查建立后填入，建立前不冒充真实测量 [depth:metrics_recalculation]。
...
**复算读数（任何人可重算）：** AI开启完成率 12/12=1.0；AI关闭完成率 8/12=0.667；**等价差 0.333** [metric:ai_off_service_equivalence_gap]。缺口集中在 4 项需实时数据重算的路线（临时占用绕行、夜间无引导岗、雨后改道、施工大幅绕行）——AI关闭时引导岗未掌握实时占用/封路数据。这不是真实基线，但它把"哪些路线在AI关闭后仍可用、哪些不可"变成了可逐案核对的数据，而可逐案核对正是 SWB 要证明的事。真实运行后，实时数据补齐与引导岗扩窗是缩小该缺口的具体方向。复算脚本：`count(ai_on_completed==true)/12` 与 `count(ai_off_completed==true)/12`，分母含全部 12 案（含 4 个非完成），未删失败样本。
...
- 机器可读：geometry（含 8 个 SWB 折返点）、metrics（含 3 项等价指标）、risk（8项风险维度停止/恢复规则）、assumptions、sources、compliance/standard/depth matrices，以及 visual/assets 下的 swb-spec（折返等价基准规范）、governance-receipts（K标变更回执）、pilot-evidence（OP-01 开放可复演试点数据）。
