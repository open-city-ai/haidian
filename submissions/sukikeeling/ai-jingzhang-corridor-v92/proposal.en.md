...
### Switchback Node = Pull-the-Plug Point (AI-Off Equivalence Operational Layer)
A switchback node is not merely a "stop and review" point; it is a **pull-the-plug test point**: each scenario must declare at its switchback node five machine-checkable fields — `ai_off_path` (the equivalent path when AI is switched off; must not route to an online path that still depends on the same system), `human_handoff` (the human takeover role), `gate_id` (the current gate), `operating_mode` (mainline / siding / depot), `responsible_role` (the responsible profession); any point missing one of these fields does not count toward service coverage [data:geometry/constraints.geojson#SWB-01]. This proposal declares complete five-field bindings for 8 switchback nodes (covering gentle / medium / steep grades and the three states); both path-completeness and human-handoff-designation rates are 1.0, checkable point by point [metric:ai_off_path_completeness] [metric:human_handoff_designation_rate]. Turning the switchback node into a pull-the-plug point moves the "three meanings of the Ren character" from philosophy into data: arrival must be confirmed by a human, and the first piece of evidence for "confirmed by a human" is that the city still works when AI is off. The stop/resume rule set (8 risk dimensions' mitigation and human_review, trigger-then-stop, resume requires evidence not promises) is in `risk.json` [depth:risk_missing_data].

## Switchback Equivalence Baseline SWB (Take-and-Use Public Product)
The preceding sections scatter criteria across scenario cards, gates, indicators, and risk items; a reader has to flip among several places to piece together "what counts as having been done." This section packages those criteria into a standalone, take-and-use artifact: the **Switchback Equivalence Baseline (SWB) v0.1**. It is not an appendix to this proposal but the public product this proposal hopes to leave behind — whether Haidian becomes a cradle of human-centric intelligence ultimately depends on whether other cities are willing to test themselves against the criteria Haidian has written down [source:AGENT-TASKBOOK]. The machine-readable spec is at `visual/assets/swb-spec.json`, consistent with this section item by item.

| Component | Specification | Machine-readable location in this package | Constraint a reuser must accept |
| --- | --- | --- | --- |
| Node schema | Each point declares ai_off_path/human_handoff/gate_id/operating_mode/responsible_role; missing any field disqualifies the point from coverage | The 8 switchback-equivalence points (SWB-01..08, constraint_type=ai_off_equivalence_point) in `geometry/constraints.geojson` | The five fields are required, not optional; ai_off_path must not route to an online path still depending on the same system |
| Scoring criteria | Numerator/denominator definitions for equivalence and fallback indicators, covering path completeness, handoff designation, AI-on/off service equivalence gap | The numerator_definition/denominator_definition of three indicators in `metrics.json` | **The denominator must not drop failure cases**: withdrawals, technical faults, and non-completion reasons must be reported together with completions; dropping any category invalidates the measurement |
| Level definition | Four gates G0 data-permission / G1 reversible prototype / G2 closed paired testing / G3 limited-open and steady-state; each binds admission, stop, resume-evidence, and responsible party | gate_binding in `risk.json` + the gate table in the implementation roadmap | Levels are per "scenario × point"; downgrades allowed without extra procedural threshold; returning from depot to mainline requires switchback re-evaluation |
| Decision rules | Eight risk dimensions (data_privacy etc.), each with stop conditions (mitigation) and resume evidence (human_review) | The mitigation and human_review fields of the 8 dimensions in `risk.json` | A stop condition triggers an immediate stop, not a "rectify by deadline" substitute; resumption requires submitted evidence, not a promise |

**Version governance has turned once.** This governance is not a clause written for others to read; it has turned one full loop inside this proposal: when writing the switchback-node criteria into an executable schema, a gap surfaced — in v8.1 (K0) the switchback node was described only in prose, constraints.geojson declared no ai_off_equivalence_point feature, and "the city still works when AI is off" could not be checked point by point by machine. The gap was logged in change receipt **CR-2026-08-14-001** (see `visual/assets/governance-receipts.json`) rather than worked around on the spot, then delivered with evidence per the "a change must attach a failure-case log and a re-affected-indicator recalculation" rule: 8 five-field nodes and three indicator definitions were added. This is a schema-level change; zero existing indicator values were affected. What this passage proves is not that the criteria are well written, but that K-marker version governance is a mechanism that has actually turned, not a promise [depth:kmarker_versioning].

**Reuse scenarios.** The baseline's value can only be tested once it leaves this project. A startup team in the Zhongguancun AI Beilu community can take only the ai_off_path and human_handoff fields of the node schema and run a self-test of "still usable when AI is off" for an early product, with no platform or authorization needed; the Future Science City energy/medical equipment interface can take only the service-equivalence-gap indicator from the scoring criteria and put the default assumption that "professional equipment is only for professionals" into a measurable position; any city's government service hall can take only the G0–G2 three levels and, without altering any space, complete one round of paired testing to obtain its own service-equivalence-gap reading. This is the physical form of the regional-collaboration chapter's line "export a reusable testing methodology": not an intention, but a file plus a table [source:BJ-AI-INNOVATION-DISTRICTS-20260121]. The baseline itself does not constitute a compliance conclusion or certification; it only writes "what counts as having been done" into a form anyone can check item by item; the hosting body, release channel, and license remain subject to an authorized body's confirmation [source:SOURCE-REGISTRY].
...
|---|---|---|---|
|Singapore Smart Nation / AI Verify [source:CASE-AI-VERIFY]|Public-interest goals, standardized testing|Public AI evaluation protocol for urban AI|National identity and data conditions|
|Barcelona 22@ [source:CASE-22AT]|Old industrial mixed renewal, innovation network|Railway heritage + incremental activation of existing stock|Large-scale real-estate renewal|
|Eindhoven Brainport [source:CASE-BRAINPORT]|Enterprise–university–government collaboration|Task-based alliance of three cores and two wings|Single-champion dependence|
|Toronto Waterfront digital-governance debate [source:CASE-QUAYSIDE]|Learning from failure in data governance and public consent|Data impact assessment, turn-back and depot entry|Platform-enterprise-led governance|
|Boston Kendall Square [source:CASE-KENDALL]|High-density research commercialization and public space|Origin Community walkable innovation network|High-rent exclusion effect|
|Shenzhen Nanshan [source:CASE-NANSHAN]|Industrial chain, capital, rapid scenario iteration|Zhongzhi full-stack validation + Dazhongsi market feedback|Speed replacing safety evaluation|
...
## OP-01 Desktop Paired Pilot Archive (Methodology Demo)
This section is a physical demonstration of the SWB scoring criteria: a paired test on SC-01 Accessibility Route Assistant (switchback node SWB-01, Wudaokou / AI Origin Community) turns "is the city still usable when AI is off" from a slogan into checkable, case-by-case data. **This is a methodology demo using synthetic test cases, not a real-world surveyed baseline** — its value is that the method and data are public, so anyone can recompute the equivalence-gap reading and verify the measurement method itself; the real baseline is filled in after the K3–K6 first survey, and is not impersonated before then [depth:metrics_recalculation].

Method: the same 12 accessibility route requests are run under both (A) AI-on and (B) AI-off (ai_off_path = on-site accessibility guide + large-print paper block map + tactile guidance strip) conditions; whether each completes is recorded; withdrawals, technical faults, and non-completion reasons are registered together with completions, with no failure cases dropped. The full 12 cases and per-case results are in `visual/assets/pilot-evidence.json`, independently replayable.

**Recomputable reading (anyone can recalculate):** AI-on completion rate 12/12 = 1.0; AI-off completion rate 8/12 = 0.667; **equivalence gap 0.333** [metric:ai_off_service_equivalence_gap]. The gap concentrates in 4 routes requiring real-time data re-routing (temporary-occupation detour, no-guide night shift, post-rain reroute, large construction detour) — when AI is off, the guide does not hold real-time occupation/closure data. This is not a real baseline, but it turns "which routes remain usable when AI is off, and which do not" into checkable case-by-case data, and checkable case-by-case is exactly what SWB must prove. After real operation, real-time data completion and guide-hour expansion are concrete directions to close the gap. Recalculation script: `count(ai_on_completed==true)/12` and `count(ai_off_completed==true)/12`; the denominator includes all 12 cases (including 4 non-completions), with no failure cases dropped.
...
- Machine-readable: geometry (incl. 8 SWB switchback nodes), metrics (incl. 3 equivalence indicators), risk (8-dimension stop/resume rules), assumptions, sources, compliance/standard/depth matrices, plus visual/assets: swb-spec (switchback equivalence baseline spec), governance-receipts (K-marker change receipt), pilot-evidence (OP-01 open replayable pilot data).
