...
> **人字形三义。** 1909 年，詹天佑在青龙桥遇到 33‰ 极限坡度：直行必翻，他选择停车、换向、以退为进。人字形展线由此诞生——它既是**工程方案**（之字形爬升陡坡），也是「**人**」字（城市 AI 的每一次折返，都是回到"人"这个原点），更是**反求诸己**（出了问题先停、先查、先问自己，而不是沿原方向冲）。三重含义分别对应三条制度条款（见折返点制度条款 R1/R2/R3）。
...
**核心判断。** 百年京张留给海淀最稀缺的不是更多算力机房，而是一条能让居民、科研团队、企业、运营者和监管者共同判断"这项智能是否值得进入城市"的公共验收轨道。本方案以青龙桥人字形展线为制度原型，把京张遗址公园转译为一条可步行的**折返治理走廊**：北段众智园做全栈与安全评测，中段 AI 原点社区把科研原型转成可参与的街区服务，南段大钟寺检验公众体验与消费权益；中关村科技服务翼提供法务、资本、标准与人才接口，小月河场景翼提供交通、生态与机器人受控测试 [source:AGENT-TASKBOOK] [depth:three_level_scope_framework]。

**产业事实锚点（官方口径，2026 中关村论坛年会发布）。** 海淀区以全线长约 9 公里的京张铁路遗址公园为"创新链路"、以"三区两翼"为空间布局打造百年京张 AI 创新带；海淀聚集 AI 企业超 2000 家、独角兽企业 26 家、备案大模型 130 款，AI 核心产业规模超 3500 亿元、占全国 30% [source:HD-AI-CORRIDOR-FACTS-2026]。

2026 年海淀区政府工作报告明确"依托 AI 原点社区、AI 北纬社区和京张铁路遗址公园创新带，打造人工智能创新街区"，且京张铁路遗址公园沿线街区控规已通过技术审查、推进获批实施 [source:HD-GOV-WORK-REPORT-2026] [source:HD00-1601-CONTROL-PLAN-NOTICE]。本方案的三区两翼命名与官方空间布局直接对位，制度设计则补上官方口径未展开的一环：**由谁来判定 AI 是否值得进入城市**。

**制度闭环。** 任何城市 AI 场景进入公共空间前，必须在折返点经历"到站必停 → 回授门（述职—质询—表决）→ 放行/折返/入段"的完整回路：听不懂就折返，任何一方否决即强制折返，不设自动恢复 [depth:switchback_governance] [depth:teach_back_gate]。每一次折返、入段与复算都记入 K 标版本链，公众可查 [depth:kmarker_versioning]。这套"人字三义—折返点—回授门—K标链"是本案的原创闭环：空间不是技术容器，而是公众能够质疑、拒绝并复测城市 AI 的制度界面 [assumption:A-SWITCHBACK-HISTORY-001]。
...
|---|---|---|
| 与任务书是否贴合 | 六项任务均落到空间、折返制度、机器合同与验收物 | 政府已采纳或机构已承诺 |
| AI 与规划如何结合 | AI 运行必须穿过折返点、回授门、坡度准入与公众决定 | "装上传感器"即完成创新 |
| 如何实施 | 首期 90 天低后悔准备 → 3 可逆原型 → K标分阶段推进 | 已有预算、场地、采购与运营主体 |
| 谁获得公共利益 | 六类公众代表以回授门参与验收，最差组与申诉优先 | 已完成真实用户测试 |
| 如何面对失败 | 折返、入段、拒绝、接管与少数意见均记入 K 标链 | 零风险或性能保证 |
...
方法为"证据—折返—指标—版本"四联单：每项策略绑定来源、空间图层、折返条件、可复核指标与版本记录。所有城市AI遵循最小数据、可选择、可申诉、人工终审、日志留痕与独立评测，与国家《生成式人工智能服务管理暂行办法》的合规方向保持一致 [standard:GENERATIVE-AI-INTERIM-MEASURES]，无障碍参与与替代通道对齐《中华人民共和国无障碍环境建设法》[standard:BARRIER-FREE-ENVIRONMENT-LAW]。[standard:PROJECT-OFFICIAL-ANNOUNCEMENT] [depth:existing_conditions_diagnosis]
...
制度原型：京张铁路青龙桥人字形展线（1909 年，33‰ 极限坡度，列车在折返点停车换向、以退为进）[source:OFFICIAL-ANNOUNCEMENT] [assumption:A-SWITCHBACK-HISTORY-001]。据此设定三条可执行条款：

- **R1（形·工程方案）**：每个 AI 场景的运行轨道设固定折返点，场景到达折返点必须停车，不得沿原方向自动续行 [depth:switchback_governance] [depth:switch_states]。
- **R2（人·回到原点）**：折返点停车后，由场景责任人（决策）、专业复核（技术）、公众代表（权益）三方共同决定"放行 / 折返 / 入段"；任何一方持否决意见即强制折返 [depth:switchback_governance]。
- **R3（省·反求诸己）**：场景因折返或入段暂停后，须先自检问题所在并重新评估，方可再进入；不设自动恢复，从检修态回到正线须经折返点重新评估 [depth:switch_states] [depth:kmarker_versioning]。
...
- **回授门（Teach-Back Gate）**：折返点停车后先经回授三步再表决 [depth:teach_back_gate]——**述职**：责任人用**非技术语言**向公众代表解释该AI做了什么、可能错在哪、怎么停掉；**质询**：公众代表提问，责任人须回答到“听懂了”为止；**表决**：三方决定放行/折返/入段。公众未听懂即折返 [metric:public_issue_closure_rate] [assumption:A-TEACHBACK-001]。
- **折返点（Switchback Node）**：停车后先进入回授门，再由三方（责任人/专业复核/公众代表）决定"放行 / 折返 / 入段"；任何一方否决即强制折返 [depth:switchback_governance]。
- **否决权程序**：公众代表可提出否决动议（口头即可，秘书处代记）；动议后 48 小时内场景降级为侧线运行（限速限域限数据）；责任人可申请公开复议一次，公众代表席位不少于 5 人；每次否决记入 K 标版本链永久可查 [depth:switchback_governance] [depth:teach_back_gate]。
- **坡度分级准入（Grade-based Access）**：参照铁路33‰极限坡度，将场景按"爬坡难度"分三级——缓坡（普惠体验类，社区级准入）、中坡（行业验证类，需专业预审）、陡坡（科研攻坚类，需联合攻关协议）。坡度越高，准入复审越严。坡度分级参照铁路限坡逻辑：缓坡（普惠体验类，社区级准入）、中坡（行业验证类，需专业预审）、陡坡（科研攻坚类，需联合攻关协议）；坡度越高准入复审越严、回授要求越透 [depth:grade_based_access]。
...
## 三层范围工作框架
折返治理沿百年京张历史线展开为**公共价值折返走廊**：问题由市民与企业提出，原型在三区研发，在两翼获得要素与场景，进入公共测试带小规模验证，最终以公开结果决定放行、折返或入段 [data:geometry/public_space.geojson#PUB-001] [depth:public_participation]。
...
更新方式遵循《北京市城市更新条例》（2023-03-01 施行）[source:BJ-RENEWAL-REGULATION] 与住建部防止大拆大建要求（建科〔2021〕63 号）[source:MOHURD-63-NO-MASS-DEMOLITION]，并与《海淀区城市更新导则（2025 年版）》强调的人工智能创新街区、轨道站点一体化高效复合利用方向一致 [source:HD-RENEWAL-GUIDE-2025]；《北京市城市更新政策激励工具箱（1.0 版）》的轨道沿线等激励工具作为存量激活的政策背景 [source:BJ-RENEWAL-TOOLKIT-2026]。
...
- **大钟寺AI产业聚集区** [data:geometry/key_areas.geojson#PROV-KEY-003]：城市型智能经济街区，四象限缝合商业、数据要素剧场与公共体验，避免封闭园区化。区内蓝景丽家项目已由海淀规自分局官方案例确认：北京云月长石科技有限公司通过挂牌出让取得 HD00-1603-01、03A 两地块（总用地面积约 3.95 公顷），为海淀首例城市更新引入社会资本、助力重点企业落地案例 [source:HD-LANJINGLIJIA-CASE-2026]；本方案对地块周边只做概念公共界面组织，不延伸任何规划控制结论。
...
## 蓝绿空间、公共空间与城市风貌
**已建成事实与规划目标分层（可复核）：** 京张铁路遗址公园**一期已于 2023 年 6 月建成开放**，长 2.4 公里、面积 16.8 公顷，形成面向市民的公园、慢步道与骑行空间 [source:JZ-PARK-PHASE1-OPENED]，相关城市更新行动案例见 [source:JZ-PARK-PHASE1-REPORT]；五道口启动区于 2019 年 9 月完成约 800 米、约 1.7 公顷的绿化景观初步提升 [source:JZ-PARK-STARTUP-2019]。公园一期 2025 年举办 60 余场主题活动、接待游客 430 余万人次（运营方统计口径）[source:JZ-PARK-2025-EVENTS]。**二期用地面积 53.09 万平方米（官方项目口径）**，沿 13 号线南起西直门、北至北五环，南延北展打造 9 千米长的京张铁路遗址公园，采取"融、拆、退、补"四大策略 [source:JZ-PARK-PHASE2-53HA]；2024 年 9 月市发改委发布二期年内开工口径（支路打通、桥下空间活化、带动西郊冷冻厂/明光村更新）[source:JZ-PARK-PHASE2-PLAN-2024]；2026 年 7 月市园林绿化局发布二期配套项目完工 [source:JZ-PARK-PHASE2-COMPLETED-2026]。规划形成京张水韵、社区活力、京张遗址纪念、青年国际交往、自然休闲五个组团，"全长约 9 公里、服务沿线近 70 个社区约 45 万人"为规划服务目标，非已建成现状 [source:JZ-PARK-PHASE2-PLANNED]。本方案所有"约 9.7km 主轴"表述均指规划目标，空间证据以 provisional 边界为准。
...
**第一幕·自主建造（1909）**：京张铁路由詹天佑主持，以青龙桥人字形展线解决 33‰ 极限坡度——停车、换向、以退为进 [source:JZ-PARK-PHASE1-OPENED] [source:PROCESSED-FACT-PACK]。带内清华园车站旧址为市级文物保护单位，保护范围与建设控制地带已划定（京政发〔2025〕3 号）[source:HERITAGE-QINGHUAYUAN-STATION-SCOPE]，沿线文保相关概念一律以专项论证为前置条件。这份自主工程经验转化为城市 AI 的第一条原则：技术遇极限先停先查，不沿原方向硬冲 [depth:switchback_governance]。

**第二幕·开放试验（1980s—）**：中关村从电子一条街发展为自主创新示范区，实验室向街道开放、研究成果向社区流动 [source:PROCESSED-FACT-PACK]。对应城市 AI 第二条原则：创新以开放接口承接，不设封闭围墙 [depth:overall_spatial_structure]。

**第三幕·可验证协作（2026—）**：AI 进入城市公共空间后在街道上真实运行。对应城市 AI 第三条原则：能力须经回授确认后方可上线，城市 AI 只有被城市理解、监督和接管之后才算真正到达 [depth:teach_back_gate] [metric:independent_ai_evaluation_coverage]。
...
年度治理按四季节奏执行（对应农时播种—除草—收获—存证）[assumption:A-SEASONS-001]：
- **春（Q1·问题开源季）**：居民与基层运营者发布任务，形成公开问题单 [metric:open_task_count]。
- **夏（Q2·模型上街前挑战）**：围绕安全、公平、无障碍开展红队测试与复现 [metric:red_team_repro_count]。
- **秋（Q3·京张城市AI周）**：开放演示、国际对话、失败案例展与专业评审 [metric:annual_week_events]。
- **冬（Q4·公共价值复盘会）**：公布指标、折返、入段、事故、停用记录与下一年预算建议，归档存证 [metric:sunset_clause_trigger_count] [metric:public_issue_closure_rate]。
...
社区对"表演赛式"更新的实名批评（issue #1061）[source:PUBLIC-ISSUE-1061] 是回授门与否决程序的现实起点：参与不是展示环节，而是决定环节。
...
- [source:HD-AI-CORRIDOR-FACTS-2026] "三区两翼"打造世界级 AI 集聚地（市科委，2026-04-03）。
- [source:HD-GOV-WORK-REPORT-2026] 海淀区 2026 年政府工作报告（2026-01-12）。
- [source:HD00-1601-CONTROL-PLAN-NOTICE] 京张铁路遗址公园沿线街区控规（草案）公示采信通告（规自委海淀分局，2025-02-08）。
- [source:JZ-PARK-PHASE2-53HA] 京张二期用地面积 53.09 万平方米（海淀区政府门户，2025-02-20）。
- [source:JZ-PARK-PHASE2-COMPLETED-2026] 二期配套项目完工（市园林绿化局，2026-07-14）。
- [source:JZ-PARK-PHASE2-PLAN-2024] 二期年内开工口径（市发改委，2024-09-09）。
- [source:HD-LANJINGLIJIA-CASE-2026] 蓝景丽家项目城市更新案例（规自委海淀分局，2026-06-12）。
- [source:BJ-RENEWAL-REGULATION] 北京市城市更新条例（2023-03-01 施行）。
- [source:BJ-RENEWAL-TOOLKIT-2026] 北京市城市更新政策激励工具箱 1.0 版。
- [source:HD-RENEWAL-GUIDE-2025] 海淀区城市更新导则（2025 年版）。
- [source:HD-DISTRICT-PLAN-2035] 海淀分区规划（2017—2035）。
- [source:HERITAGE-QINGHUAYUAN-STATION-SCOPE] 清华园车站旧址市级文保及保护范围（京政发〔2025〕3 号）。
- [source:MOHURD-63-NO-MASS-DEMOLITION] 防止大拆大建通知（建科〔2021〕63 号）。
- [source:HD-AI-FIFTEENTH-FIVE-YEAR-2026] 北京备案模型 212 款与"十五五"AI 万亿集群（市科委，2026-01-28）。
- [source:HD-1X1-INDUSTRY-SYSTEM] 海淀"1+X+1"现代化产业体系（2026-03-03）。
- [source:PUBLIC-ISSUE-1061] 实名居民公开意见（issue #1061）。
