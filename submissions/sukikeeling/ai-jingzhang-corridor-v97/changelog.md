...
**依据**：v9.6（75 分）证实"在84基座加机器可读证据层"回落9分，与 v9.2/v9.3/v9.4 的"加内容必落75"规律一致。本版改走最小扰动路线：仅做 agent.4 术语对齐，零新文件、零增量 metrics，测试"只对齐不加内容"能否保住84。
**变更**：
- proposal 双语：将"三个公共节点"标注为"三个 AI 朝圣地标（agent.4 公共节点）"，对齐 agent_taskbook agent.4 的"不少于3个AI朝圣地标"术语。实质内容不变（开源穹顶/时序站/折返议事厅），仅让 taskbook 术语显式出现。
- K标维持 K3。
- 不动 geometry/metrics/操作工件/plot-conditions/cover/交互Canvas/架构。无新文件。
**待复核**：术语对齐微扰能否保住84（或证明84对正文措辞不敏感）。
