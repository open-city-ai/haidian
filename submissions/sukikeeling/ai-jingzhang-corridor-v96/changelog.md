...
## v9.6 - 2026-08-15
**机器可读证据层（当前版本）**
**基于**：v9.5（84 分，K3，正式评分通道，四门 formal-review-ready）。
**依据**：高分选手文件结构对比——Abreto(91)有 spatial.json（空间叙事索引，Portal展示用）；校验器有 simulation.json 一致性校验（可复演聚合指标）；007xf 有 reviewer-evidence-index.json（评审者证据索引）。三者 sukikeeling 均未用。
**变更**：
- 新增 `simulation.json`：将 OP-01 拔线测试的 12 个合成用例结构化为可复演 simulation tasks（outcome/dispatch_schema_valid/energy/audit_complete）；校验器可自动派生 simulation_success_rate=0.6667 / tool_schema_pass_rate=1.0 / audit_completeness=1.0，与 pilot-evidence.json 的 ai_off_completion_rate 一致。
- 新增 `spatial.json`：10 个空间叙事节点（1主轴+3区+2翼+4节点），linked_scenarios 关联 12 场景卡；把"人字形折返治理"的空间叙事结构化为 Portal 可展示的概念节点。
- 新增 `visual/assets/reviewer-evidence-index.json`：为七维评审各维度指向最相关证据文件，减少评审者翻找成本。
- metrics.json 升级：ai_off_service_equivalence_gap 从 unknown→known（0.3333, source=simulation.json）；新增 simulation_success_rate/tool_schema_pass_rate/audit_completeness/simulation_task_count 四项 known 指标。
- manifest 加 data_confidence=mixed_provisional_and_conceptual（诚实声明，Abreto=medium/baobao=low 的同字段）。
- K标维持 K3：机器可读证据层，未动 geometry/正文/操作工件/架构。
- 不动 v9.5 的 plot-conditions、cover、交互 Canvas、geometry、操作工件。
**待复核**：机器可读证据层对 ai_planning_innovation / expression_completeness / implementation_feasibility 维度的提升。
