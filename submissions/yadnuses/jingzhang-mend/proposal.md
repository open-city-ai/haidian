...
| 城市更新政策链 | 《北京市城市更新条例》（2023-03-01 施行）确立「留改拆并举、以保留利用提升为主」与联合审查公示不少于 15 个工作日 [source:BJ-URBAN-RENEWAL-REGULATION]；《海淀区城市更新导则（2025 年版）》（海政发〔2025〕4 号）明确「53 平方公里人工智能创新街区、重点聚焦五道口和大钟寺先导区、重点围绕京张铁路遗址公园沿线更新」 [source:HAIDIAN-RENEWAL-GUIDE-2025] | 中办发〔2025〕34 号原文已于本轮检索获取（gov.cn 全文，文号经两处官方文件引用交叉确认） [source:POLICY-ZFBAN-2025-34] | 无待办；引用以其红线条款原文为准 |
...
每张卡从「声明」升级为「照护契约」：在原有 users / context / data_inputs / public_value / risks / human_review 六字段之外，补齐四列与三要素。术语第一次出现时说明：硬停止条件=触发即暂停服务的具体失败情形；恢复证据=申请恢复服务时必须提交的材料；照护服务评估窗=到期必须接受评估的时间窗口；AI-OFF/人工等价路径=关掉 AI 后服务仍可由纯人工完成的替代方式；退出机制=服务终止后的数据处置与场地恢复约定。所有场景保留人工复核入口与居民叫停通道，空间载体落在具体图层 [data:geometry/public_space.geojson#PUBLIC-001] [data:geometry/roads.geojson#ROAD-001]；风险编号 R-xx 的量化触发器见第十二章。
...
| JZ-04 大钟寺站四象限连通 | 区交通部门+轨道运营方 | 站点改造立项 | 中（待测算） | 过街流量与绕行耗时（待核） | 站点条件与市政资料确认 | 退回地面标识与信号优化建议 | R-03、R-07 |
| JZ-05 Mend Lab 与清河界面 | 园区平台公司+科研机构 | 园区改造项目审批 | 中（待测算） | 创新活动频次基线 | 河道蓝线与生态条件确认 | 退回室内功能置换建议 | R-01、R-07 |
| JZ-07 算力驿站 | 园区平台公司 | 新型基础设施试点申报 | 中（待测算） | 社区算力需求调研 | 试点期能耗与安全评估 | 到期未续则设备撤场 | R-07、R-10 |
| JZ-08 社区共修工坊 | 街道+社区组织 | 公共空间使用备案 | 轻（待测算） | 居民参与意愿调查 | 首季参与度评估 | 转为线上共修与活动预约 | R-08、R-11 |
...
本节把方案的关键主张压成一张「一句话可检验」表：每条主张都能在指定文件或图层上用一段脚本或一次复算当场判真假，不依赖正文表述。数字与 `metrics.json` 逐字一致，R 编号与第十二章风险表一致；其中承重两行由机器工件 `visual/assets/care-contracts.json` 及其校验脚本（退出码 0 即全部断言通过）执行验证（「逐字一致」特指该工件与第十二章风险表之间，正文卡表为摘要版） [metric:land_use_total_area_sqm] [metric:green_ratio]。
...
1. 北京市规划和自然资源委员会海淀分局：《百年京张AI创新带城市设计国际方案征集资格预审公告》，2026-04-30 发布、2026-05-09 挂网。
