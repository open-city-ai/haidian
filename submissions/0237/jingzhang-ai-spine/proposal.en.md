...
**Mechanism verification (completed, independently re-checkable, actively falsifiable)**: all 12 AI scenario cards (SCN-01…12, each defined and spatially anchored to PROV-KEY-001/002/003 and other key nodes) ran the offline state machine "AI-auto → anomaly → dual-gate switch → signaler takeover → baseline return → handover record"; **12/12 scenarios and 48/48 assertions passed** (4 assertions per scenario: offline drillable / dual gate mandatory / return ticket mandatory / handover record mandatory). The verification is now a **zero-dependency executable**, `mechanism_state_machine.py` (a reproducible companion script in the contributor repository): running it locally yields 12/12, 48/48 (exit code 0); running `--inject-fault SCN-05` actively injects a fault and conditions ①② ③ all report "fail" with exit code 1 — proving the checks are real judgments, not hardcoded passes; evidence is persisted in `mechanism_verification_evidence.json` (with scenario_spec_sha256 and UTC timestamp) [metric:mechanism_verification_scenario_passed] [metric:mechanism_verification_assertion_passed] [source:AGENT-TASKBOOK].
...
- **Application templates**: exhibition banner band, web page header, report header, and scenario-card template have been designed; all graphics are original vector with rights-cleared fonts and imagery. Deliverables are in `assets/brand/`.
