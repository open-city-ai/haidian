...
图面规则与指标规则一致：`verified` 用空心来源锚点，`provisional` 用低对比短划线，`conceptual` 用半透明色块/概念线，`unknown` 不绘制确定几何。
...
已合入 main 的 #71「开源回路」保留为历史版本。本方案不是原方案的原地润色，而是完成空间诊断、坐标核验和对抗审查后的新包：

- 旧方案把"可验证"放在命名中心；本方案把核验过程放进证据层；
- 旧方案以回路为总体隐喻；本方案以剖面作为城市设计单元；
- 旧方案从概念开始；本方案从已开放绿廊、站点、蓝绿和重点区锚点矛盾开始。
...
视觉语法：不用人字形、轨道 Logo 或紫色科技渐变。视觉语言采用横向剖面线、三层色带、可核验坐标点、条件虚线和复算标记。正式 Logo 在后续命名/视觉任务中完成。
