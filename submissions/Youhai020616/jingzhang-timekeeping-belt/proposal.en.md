...
| How trustworthy is the data | All geometry uses a provisional rough boundary (0.11% off the announced figure); repository Issue #846 records a reproducible spatial discrepancy between the provisional boundary and the OSM-mapped park — evidence unfavourable to this proposal, disclosed anyway; full recalculation once official polygons arrive | Boundary status and recalculation trigger [source:BOUNDARY-SOURCE] [metric:provisional_site_area_difference_ratio] |
...
The logo direction has three elements: a horizontal baseline, a circular dial outline, and a deliberate gap in the dial. The gap means "there is always a next time check" — every pass is temporary. Colours: baseline ink-black, signal red (degrade and delay states only), on-time teal, brick-timber warm grey (culture and heritage interfaces). Signal red is never decorative. No unauthorised typefaces, trademarks or likenesses; trademark search and accessibility contrast remain for professional teams. [depth:height_massing_character]
...
| P6 University staff and students | Co-build baselines, course-based calibration practice | Near-campus hall, co-build room, open courses | Campus data needs authorisation; no minor profiling |
...
### The Time Check Protocol: ten fields and six gates

All scenarios share one **service timetable** (10 required fields): service id; public purpose; scope (users / spatial node / excluded uses); minimum data (with retention and a no-personal-data constant); responsible human role; non-AI equivalent path; baseline reference (version and establishment status); time-check window (interval capped at 180 days); degrade plan (triggers / steps / recovery); delay ledger (public, append-only). The schema and four samples ship with this package; no field is optional. [data:visual/assets/timetable.schema.json] [metric:timetable_field_count]
...
This proposal deliberately makes no traffic-provision claims: no ridership estimates, no parking counts, no transfer alignments. Before any traffic pilot enters the time-check flow, corridor baselines must exist (time-sliced counts, accessibility continuity audits, complaint baselines) — and the baselines themselves are governed by the protocol: expired data degrades its conclusions. Rail stations carry only the "information and walking continuity first" principle. [assumption:A-TRANSPORT-001]
...
| Timetable fields / gates | 10 / 6 | Protocol interfaces, third-party reusable [metric:timetable_field_count] |
| Tabletop drill | 6/6 pass | Input-hashed, byte-reproducible; real operation not authorized, not run |
...
The narrative, this English counterpart, all GeoJSON, metrics, matrices, the timetable schema, drill scripts, five figures, A3/A0 booklets and offline pages were originally generated for this submission by Claude Code (an Anthropic Claude model) under the human participant's authorisation; figures are drawn deterministically from structured data via matplotlib using system fonts, with no font files distributed. No external images, commercial maps, corporate marks, likenesses or paper figures are used; global cases cite only names and mechanism descriptions from public institutional pages. Full disclosure in `report/copyright_statement.md`. [self_check:COPYRIGHT_TRACE]
...
9. Urban design administrative measures; regulatory detailed planning measures; land-use classification guide; architectural design-depth provisions. [standard:MOHURD-URBAN-DESIGN-MEASURES] [standard:MOHURD-CONTROL-DETAILED-PLANNING]
10. BIPM, NPL, NIST, MLCommons, AI Singapore, Vector Institute public pages (background mechanism comparison). [source:CASE-BIPM]
