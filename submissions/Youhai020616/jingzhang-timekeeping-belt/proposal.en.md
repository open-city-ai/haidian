...
| National, 2025 | 16 high-value invention patents per 10,000 residents (Haidian: roughly 37.4×) [source:NBS-2025-STATISTICAL-COMMUNIQUE] | Haidian's innovation density leads the nation by a wide margin — what is scarce here is not innovation volume but trust infrastructure; a timekeeping regime that runs where innovation density is highest has passed the harshest possible test before export to other cities | That the two statistical levels are fully comparable in institutional attribution, or that the ratio works as a corridor target or ranking conclusion |
...
The proposal does not start from a single inspiration. Before design, three candidate scenarios were compared on seven equally weighted criteria (1–5 each): overall effect, public interest, AI nativeness, spatial translatability, long-term operation, verifiability of evidence, and robustness to data gaps. The comparison is an inferential framework that explains "why this one" — it replaces neither field investigation nor statutory planning.

| Candidate | Total of seven | Strengths | Weaknesses |
| --- | --- | --- | --- |
| A Smart display belt: devices and experience nodes along the spine | 19 | Direct to communicate, quick to show | Weak public loop; fast device depreciation; under data gaps it can only fake precision in siting |
| B Benchmark campus belt: test facilities in each of the three parks | 22 | Clear industrial logic, maps onto the three key areas | The public remains spectators; governance stops at campus walls; no institutional link between parks |
| C Public timekeeping regime: timetables, check points and a delay ledger | 33 | Closes verification, use and oversight into one loop; sites by institution rather than fake precision under data gaps; exit and accountability are built in | Depends on sustained human effort; slower to show results than display schemes |
