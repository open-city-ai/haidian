...
summary: "以任务—模型—空间逆向定径法，把公共任务硬条件、软硬件同题实测与城市空间包络连接起来；规则或人工已足够时，不部署模型也是有效结论。"
...
模型选择随后被翻译为五级空间包络：JE-0 无模型基线、JE-1 终端内嵌、JE-2 可换柜体、JE-3 独立后场、JE-X 负担过大拒绝。包络叠加设备外廓、供配电、散热声学、消防安全、维护动作以及人工与无 AI 服务空间，不用模型参数量或芯片 TDP 冒充工程尺寸。完整方法、桌面试点和空间包络见 `assets/research/alternative-concept-booklet.md`、`assets/research/evidence/desktop-pilot-results.json` 与 `assets/research/structured/spatial-envelopes.json`。[depth:three_key_area_detailed_design] [data:geometry/key_areas.geojson#PROV-KEY-001]

三处重点区分别承担不同的“尺”：北京 AI 原点社区以“人因原尺”签发任务合同；众智园以“定径台”完成模型、编译器、芯片和整机同题实测；大钟寺以“省下来的房间”检验合格组合与人工柜台、连续通行、公共活动和维护空间如何共存。中关村科技服务翼提供知识产权、采购中立和迁移合同，小月河场景赋能翼只提供经授权的环境工况校准。[data:geometry/key_areas.geojson#PROV-KEY-001] [data:geometry/key_areas.geojson#PROV-KEY-002] [data:geometry/key_areas.geojson#PROV-KEY-003]

方案已经形成 12 张场景卡、6 类用户画像、3 项产业测试和 5 类空间包络，机器可读记录位于 `assets/research/structured/task-matrix.json`。场景覆盖多语到达、无障碍路径、公共服务路由、遗产讲解、商户内容、低扰动活动、环境巡检、低速设备停机、紧急人工转接、设施维护、会议字幕摘要和设备关闭开放日。每项都允许得出“不用模型”“规则即可”“小模型”“后场模型”或“拒绝部署”的不同结论。[source:AGENT-TASKBOOK] [depth:ai_ecosystem_scenarios]
