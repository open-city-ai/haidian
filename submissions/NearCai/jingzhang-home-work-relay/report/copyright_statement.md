...
工具链仅用于本地生成与检查，不随投稿包再分发其二进制或源代码：ReportLab（BSD-3-Clause，用于 PDF 排版）、Pillow（HPND，用于位图绘制与检查）、Shapely（BSD-3-Clause，用于几何构造与拓扑校核）、PyProj（MIT，用于 EPSG:4548 投影与面积复算）及 Poppler（GPL-2.0-or-later，用于 `pdftoppm` 渲染检查）。这些许可证适用于各自软件，不自动适用于本投稿内容。

投稿内容采用 `COMMUNITY-DISPLAY-ONLY`：权利人允许主办方和本开源征集社区在本次征集的接收、机器校验、人工评审、非商业归档、讨论与展示语境中复制和公开呈现本包。该标记不自动授予商业利用、再许可、独立衍生、训练用途或脱离征集语境的再发布权；第三方来源仍受各自权利人的条款约束。方案是开放共创建议，不是正式规划、政府审定、工程设计或实施授权。
