...
**公共空间的包容性验收**(把"无障碍不是附加设施,是形式本身"落成可核对的动作):七处公共空间节点 [metric:public_space_node_count] 在开放前各走一遍同样的流程,验收人不是设计方,而是五类画像的代表与街道议事会 [source:SRC-HD-CENSUS7]。走查分四条线路各走一次——轮椅与婴儿车推行线、视障与听障导引线、老龄与适老步行休憩线、无智能终端到达线;四条中任一条走不通,该节点不进入开放清单。四条线路的共同判据只有一句:同起点、同终点、同一个到达,不设"专用入口"式的绕行安排,绕行本身就是一种排除。
