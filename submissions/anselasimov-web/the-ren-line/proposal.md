...
| 背景论证(29条经核验的公开政策、官方媒体与行业来源) | 只作趋势与本底判断,不支撑控制性结论 | 市级战略以《北京市"十五五"规划纲要》"沿交通廊道布局创新与产业设施载体、打造一批科创走廊"与"人工智能产业以海淀区为核心"为纲 [source:SRC-BJ-145];AI原生城市与空间计算试点出自京发改〔2024〕1081号 [source:SRC-AIPLUS-1081];国家层面总纲方向由国务院《关于深入实施"人工智能+"行动的意见》提供 [source:SRC-STATE-AIPLUS](政策脉络最上位一环,不直接推导本方案具体设计取值);"人工智能创新街区"主阵地定位出自中关村科学城全景赋能行动计划 [source:SRC-ZGC-AI-PANORAMA];"两区一带"与京张控规进度出自2026年海淀区政府工作报告 [source:SRC-HD-GOV-2026] |
...
选取8个案例,逐例提炼可转化机制 [source:SRC-KINGSCROSS-ULI] [source:SRC-XUHUI-MOSU] [source:SRC-SIDEWALK]。选取标准有二:一是空间可比性——须落在铁路存量用地更新、校城交界地区、既有楼宇改造三类情境之一;二是机制可移植性——案例必须留下一件可以写进协议或章程的东西(许可条款、会员制度、准入规则),只有形态没有机制的案例不选。同时说明三条不可照搬的边界:境外案例的土地产权与开发许可制度与我国不同,可借鉴的是机制逻辑而非条款本身;单体孵化器的规模不等于街区的规模;反例的教训优先于正例的经验。**证据链强度提示:以下五案例(案例2/3/4/5/7)数字转引自案例方官网与媒体公开报道,`sources.json`未逐条登记对应来源,本方案未独立核实,读者复核请自行查证原始出处。**八例如下:

1. **伦敦King's Cross知识区**(与京张情境最相似:27公顷铁路废弃地更新):三方有限合伙KCCLP+单一总体规划许可含**20%用途弹性**(总量74万平方米内业态可浮动,多年后才能为Google腾出9.3万平方米整栋)+临时公共空间前置招商+**Knowledge Quarter会员制运营机构**(100+机构、分档会费、双层治理)[source:SRC-KINGSCROSS-ULI]。机制细节有三处值得抄:KCCLP由开发商Argent持50%、国有铁路公司LCR持36.5%、物流企业DHL持13.5%,即"土地方以地入伙、专业开发商操盘"的单一总体开发主体,产权不散、决策不绕;规划许可把关键路径、公共空间、各区高度上下限"锁死",把单体设计与用途配比"放活",固定与弹性并置而不是一律从严;配套义务由S106协议承担,现金与实物两类贡献分别对应地方基础设施、公共空间与社区服务,把规划意图直接写进融资条款。KQ本身则是独立于开发商的会员制组织,范围为周边1英里半径,治理为"UCL加8家机构组成的董事会+其余成员组成的执行组"两层,会费按机构规模分档。转化:京张控规预留用途弹性区间的概念建议;新京张站务段采用KQ式会员制,并把配套义务清单化,作为区域综合性更新单元申报的附件。
...
交通慢行:五道口站与清华东路西口站双接驳 [data:geometry/roads.geojson#RD-014];13号线拆分工程(13A/13B)在建,是本区可达性的关键变量 [source:SRC-M13-SPLIT],"清华东路站转正并与15号线换乘"为本方案推断,尚待专项确认。在此之前,北侧校城界面的可达性主要靠步行与自行车,校墙一线的开口位置因此比新增车行通道更值得先讨论。公共空间:换向台·人字形大台阶(PS-001)+新站匾墙(PS-002),两处均位于本区西侧、正对中关村大街来向,是创新带对城市的正脸,到达路径按无障碍与适老标准同步设计。
...
**智能线三层架构**(概念建议,全部叠加在既有街道与市政设施上,不新增廊道 [data:geometry/constraints.geojson#CN-AISZ]):**感知层**是城市侧的眼睛——空间计算锚点(S10)与低密度环境传感,纪律前置于设备:先定数据目录与公开口径,再谈装什么;锚点数据按目录制公开 [source:SRC-AIPLUS-1081](源文件京发改〔2024〕1081号的政策接口);不做人脸识别常态部署,是本方案自设的治理约束,源文件未涉及此项,不借其背书。**算力层**是就近的一层——边缘算力路灯网(S12)顺着市政杆件把算力铺到街道边,枢纽节点设端侧机房承接时延敏感任务,与远端大规模训练算力分工;用能方向衔接京张走廊西端张家口的绿电与算力枢纽,让"算力从哪儿来"这一问有个可回答的方向(用能口径待专项研究)。**服务层**是对外开口的一层——面向开发者的Agent服务接口、面向机器人与低速设备的路权规则(S06)、面向企业的场景API [source:SRC-AVZONE-40],三类接口统一按"可申请、可计量、可退出"设计。
...
![12张AI场景卡空间锚点索引图(位置示意,坐标见public_space.geojson#SN-S01~S12)](assets/figures/scenario-index.png)
...
**轨道与站城一体化**:范围内轨道格局三个关键变量,证据链强度不同。其一,12号线大钟寺站已通车但与13号线出站换乘,约8分钟绕行是2024年12月开通首日实测 [source:SRC-DZS-M12-2024],本方案只当改善基线,不当现状定论;"四象限"步行连通是公告点名的任务本身 [source:SRC-PREQUAL-2026],"站内连通"是本方案自己的概念解法,公告未替其背书。其二,13号线拆分工程(13A/13B)在建为官方事实 [source:SRC-M13-SPLIT];"清华东路站转正并与15号线换乘"是本方案的推断,尚待专项确认。其三,清河站方向为自动驾驶示范区4.0任务点,官方规划确认 [source:SRC-AVZONE-40],证据链最硬。四条站点接驳概念线见 [data:geometry/roads.geojson#RD-013](大钟寺)、RD-014(五道口)、RD-015(清华东路西口)、RD-016(清河方向)[depth:traffic_rail_slow_parking]。

**轨道覆盖率,以及它说明的事**:范围内5座轨道站(北京北、大钟寺、五道口、清华东路西口、清河)按800米步行半径复算,覆盖提交边界的54.8% [metric:rail_station_800m_coverage_ratio]。这个数不高,方案也不打算把它说高。它同时说明两件事:中段与南段的轨道条件已经成立;北端的覆盖空缺要等13号线扩能(13A/13B,在建为官方事实 [source:SRC-M13-SPLIT])与清华东路站转正(本方案推断,尚待专项确认)之后才有实质改善,在此之前,北端可达性只能由站点接驳线与东西缝合线补上——这也是本方案把接驳线与缝合线放在近期、把北段组团放在中期的直接理由。覆盖率随官方站点坐标与新线开通一键重算 [depth:metrics_recalculation]。
...
**公共服务设施**:全区常住人口311.1万、地区生产总值13,691.4亿元 [source:SRC-HD-STATS-2025]——这组区级本底只说明海淀整体具备支撑创新型公共服务的人口与经济体量,不直接推导三类设施各自的类型、数量与布点,后者仍是概念层面判断:面向开发者的创新服务平台(政务+算力+法务一站式,置于换向站)、面向居民的社区服务设施(概念用地574,387平方米 [metric:land_use_0702_area_sqm],保证生活服务不被创新功能挤出)、面向全龄与老龄居民的健康驿站(S03,入口与流线按无障碍标准)。设施容量与布点均为概念建议,待专项数据核实。
...
**公共空间的包容性验收**(把"无障碍不是附加设施,是形式本身"落成可核对的动作):七处公共空间节点 [metric:public_space_node_count] 在开放前各走一遍同样的流程,验收人不是设计方,而是五类画像的代表与街道议事会。走查分四条线路各走一次——轮椅与婴儿车推行线、视障与听障导引线、老龄与适老步行休憩线、无智能终端到达线;四条中任一条走不通,该节点不进入开放清单。四条线路的共同判据只有一句:同起点、同终点、同一个到达,不设"专用入口"式的绕行安排,绕行本身就是一种排除。
...
**分期概算逻辑**:本方案不给出造价、投资规模与财政测算,只给出概算应当怎么组织。三期的资金强度并不均等,结构也不同。近期以荣誉体系、场景植入与运营组织为主,不占地、不动红线、不改权属,资金需求集中在轻量构件、活动组织与常设机构的启动运行,单位面积成本最低,且大部分支出可随时中止;中期进入组团建设与测试段基础设施,建设成本占比上升,并第一次出现算力、设备与安全员的持续运维支出,资金结构由一次性投入转为投入与运维并行;远期以存量社区渐进更新为主,建设强度下降而运营与维护支出成为主体,收益侧则依赖会员规模与场景付费是否已经成立。三期的概念面积口径 [metric:phase_1_area_sqm] · [metric:phase_2_area_sqm] · [metric:phase_3_area_sqm] 只用于说明工作量级,不得据以推算投资规模或作为资金申请依据。

**资金来源分档**(概念建议;三档并列而非依次替代,同一项目可由多档组合承接):

| 档位 | 主要来源(概念) | 适用的项目类型 | 使用边界(否定条件) |
|---|---|---|---|
| 一档·财政与政策工具 | 区级城市更新既有资金渠道、市级建筑规模指标激励等政策性支持 [source:SRC-BJ-RENEWAL-REG] [source:SRC-LJLJ-RENEWAL-2026] | 公共空间、无障碍与适老改造、荣誉体系与公共导览等公益属性投入 | 政策适用性未经主管部门确认,不得写入任何实施承诺;本方案不得被用作财政资金或专项资金的申请依据 |
| 二档·社会资本 | 城市合伙人出资、楼宇权属方与运营商投资、企业对场景与算力的投入 | 存量楼宇AI村化、智能原生消费界面、场景与测试段基础设施 | 社会资本投入不得以削减替代通道、无障碍设施或公共开放时段作为交换条件;不得以投资额换取指标承诺 |
| 三档·运营自平衡 | 站务段分档会费、场景API与试车段的计量付费、活动与展陈收益、载体租金 [source:SRC-KINGSCROSS-ULI] | 常设机构日常运行、年度活动体系、场景运维与走查抽查 | 档次与费率由站务段章程议定并公开,不得由单一主体设定;运营收益不得挪作偿付建设投入,以免运营被建设债务绑架 |
...
![分期实施地图(三期色阶+阶段门,与phasing.geojson一致)](assets/figures/phasing-plan.png)
...
- 其二,概念容积率、建筑密度、建筑基底与概念总规模四项为校验值而非设计目标,不得被引用为强度诉求或指标争取的依据;官方控规条件公布后,概念组团按官方条件反向重排 [depth:development_intensity_controls]。
- 其三,状态为unknown的两项(官方容积率、官方建筑高度)在官方数据公布前不得被赋值,也不得以概念测算值顶替;宁可留空,不得凑数。
- 其四,覆盖率、间距与长度类指标随几何一键重算 [depth:metrics_recalculation],版本号不一致的两组数值不得混用于同一张图或同一段论述,否则复算就失去校验意义。
...
- 其七,指标下降本身不是错误,说不清原因才是:任一比例较上一版下降而给不出理由的,不得出图;绿廊500米覆盖率 [metric:greenway_500m_coverage_ratio] 一旦下滑,该回头调的是用地,不是说法。
...
- 缓解:全部面积类指标由geometry一键复算 [depth:metrics_recalculation],替换边界文件即可重跑;6条边界性假设逐条登记于assumptions.json;正文每处敏感数值均标注为校验值而非目标值 [source:PROCESSED-FACT-PACK]。
...
- 缓解:全场景默认不采集个体数据,执行"自愿、本地、可撤回"三原则;锚点数据按目录制公开 [source:SRC-AIPLUS-1081];偏差审计季度公开,申诉入口与阈值同时公示。
...
- 缓解:文物建控地带要求前置引用并写入实施敏感点 [source:SRC-XZM-HERITAGE] [source:SRC-QHY-STATION];解说文本经文保与史料部门审核后锁定;体量与高度只作概念表达,不进入指标口径。
...
- 缓解:按"单元内先成协议、再谈体量"的顺序推进,把协议达成本身作为阶段门的一项 [source:SRC-DZS-LJLJ-PLAN-2025];对既有载体只做接口式介入,不改变其产权安排与建设时序。
...
- 缓解:测试段的物理边界与开放时段公示,越界即停;安全员与远程人工接管席位常设 [source:SRC-AVZONE-40];概念断面上低速设备通行边贴边设置,不与步行流线交叉。
- 退出:发生安全事故或越界即暂停路测并回滚至人工驾驶与人工配送;施工期的无障碍绕行路径未落实,相关工段不得开工。
- 残余风险:轨道扩能时序不由本方案决定,北端可达性在13号线扩能落地前始终偏弱 [source:SRC-M13-SPLIT];接驳线与缝合线只能部分补偿这一缺口,不能替代轨道。
...
- 缓解:阈值与判定程序由站务段理事会与属地街道议事会共同议定并公开;统计边界从第一天定清楚(模速空间口径混乱的反面教训 [source:SRC-XUHUI-MOSU]);运营数据与登记册年度公开。
- 退出:阈值与判定程序未公开,场景不得开通;口径不一致即停止对外发布该组数据并公开更正,不得以"以最新口径为准"含糊带过。
- 残余风险:公众信任建立得慢、失去得快,一次治理承诺说不清就足以让十亿美元级项目停摆(Sidewalk反例 [source:SRC-SIDEWALK]);这类风险无法用技术手段对冲,只能靠持续、可核对的公开来积累。
...
本方案的资料证明力沿用"设计依据与资料清单"章已定义的三级框架,不重复列出条文,只复述每层的证明力边界:formal任务依据层(预审公告、任务书、三项国家/行业标准)决定任务口径、范围与法定深度,可直接作设计依据;临时粗略边界层(总体范围与三处重点区临时polygon)只供概念生成、可视化与入库自检,不作面积或审批依据,官方红线公布后须整体替换;背景论证层(29条经核验的公开政策、官方媒体与行业来源)只证明趋势与本底条件,不支撑任何控制性结论。结构化清单如下,供逐条复核 [source:SITE-PACKAGE]:
