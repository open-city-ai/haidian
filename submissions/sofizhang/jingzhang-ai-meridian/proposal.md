...
**退役登记（decommissioning registry）。** 每件进入公共空间的AI设施在安装登记的同时，即登记三项退役信息——计划复审/退役日期、拆除与场地复原的责任主体、场地复原标准（恢复为无AI状态下仍然成立的公共场所，口径与治理运行表『退出与空间回归』列一致）。登记状态纳入站点证据牌公示，年度场景开放日逐站核对时一并核验退役登记项；未完成退役登记的设施不进入公共空间。机制为概念建议，供运营主体按程序采纳，不新增任何数据采集。退役在本方案中不是失败记录,而是治理闭环的常规一环——轨道常在,列车常新。
...
本方案的全部文本、图面、几何、指标与渲染均为独立生成,未复用任何同场方案的文本、图面、几何或 schema。青龙桥『人』字形折返线与京张里程桩号同属公共历史遗产与通用工程语汇:据同场方案公开披露,人字母题在本场已被 25 份以上方案采用(含已合入的《人字形·智轨 Λ·RAIL》),《京张人字轨》亦独立采用了 K0–K11 里程桩语汇。本方案不主张对任何母题的独占,并按贡献指引致谢在先贡献者;差异放在机制上——其一,桩号在本方案中即站点系统本身:K0–K9 十站每站都是服务、治理与证据公示的复合单元,而非线性缝合装置;其二,『零公里以北』提供方向性叙事,自 K0 零公里原点向北展开空间与实施的双重序列;其三,治理侧以退役登记与分时段服务承诺构成可核查的运行契约;其四,表达侧坚持双语图册与自研 CPU 光线追踪写实渲染,全程不使用照片、外部影像或图像生成模型。
