...
| 评审会问 | 本方案的回答 | 可核验的东西 |
|---|---|---|
| 核心命题是什么 | 信任不来自单次精度，来自**测得回来**：绕一周回到原点，闭合差超限则整段重测，不允许挑一站修正 | `node visual/assets/check_closure.js` 与 `run_s08_tabletop.js`——机制可被独立执行，8/8 用例含 6 个拒绝分支 |
| 为什么用在这两条赛道 | 因为读数错了会伤到人。**实测**机器人 18/354（5.1%，八条赛道中最薄）、AI公共服务 32 份（与青年友好并列次薄） | `visual/assets/field_map.json`，普查脚本可重跑 |
...
**四周后，回到 BM-0 复算。** 三站读数的最大偏离就是这一期的闭合差 f。它超过了这条场景的限差 F。按第 4 条，**整段路线退回复测，该场景降级为无AI等价路径**——窗口挂出纸质流程与人工转介，服务不中断。按第 5 条，**不允许只去修正表现最差的那一站**。
...
**本方案的定位声明。** 城市AI治理是本方案的**方法层**，不是卖点——把治理协议本身当成成果，是这次征集里最饱和的做法（最近一次实测：354 份已合入方案中 230 份申报治理赛道，「证据链/可复算」类表述出现在 31.1% 的方案里 [source:FIELD-CENSUS-2026-08]）。本方案把治理当工具，把工具用在覆盖最薄的赛道上：`robotics-autonomous-mobility` 按标签计 354 份中仅 **18 份（5.1%）**，是八条赛道里最薄的一条，而次薄位由青年友好公共空间与 AI 公共服务并列占据，各 32 份；`ai-public-services` **30 份（8.9%）**，仅次于它。这不是为了避开竞争而选空位——恰恰相反，闭合差机制在这两条赛道上才真正不可替代，因为只有在这里，一次未被复核的错误读数会落在具体某个人身上。
...
普查刻意不读 `submissions-data.js`——那是生成的展示索引，会滞后。**这条观察在本方案自己的复测下发生了反转，如实记录：** 8 月 8 日它只列 184 份，比仓库实际少 44 份（19.3%），且两次普查之间条目数完全未变，当时判断「滞后在扩大」；8 月 9 日复测为 292/298，滞后收窄到 **6 份（2.0%）**，当时写成「索引显然已被追平」。**同日第五次复测为 309/354，滞后又回到 45 份（12.7%）——因此「已被追平」同样是一次快照结论，也必须收回。** 四次索引读数按时间顺序为 31 → 44 → 6 → 45 份（此前这里把前两次写反了，而顺序恰恰是关键：滞后最初确实在扩大，这正是当时得出「滞后在扩大」的原因）：它既不在单调扩大，也没有被一次性追平，而是**随合并批次起伏**。本方案不据此邀功，也不据此指责——无法证明因果；写在这里是因为，一个只在对自己有利时才复测的机制不成立，而**连续两次拿单次快照下结论，说明这个毛病不是别人的**。**结论中仍然成立的那部分是方法层面的：评审仪器必须读权威源（git tree），而不是读一个可能滞后、且滞后幅度本身会变的派生索引。**
...
这不是评论主办方的工作，而是**交付一件他们此刻最缺的东西**。截至最近一次实测仓库已合入 354 份方案，PR 编号已过 1000，维护者在以每分钟数份的速度合并。此时最难的不是收稿，是**横读**：三百多份里哪些已经收敛、哪些位置还空着、哪些声明其实无法被聚合。这件事无法靠翻展示页完成，需要一台仪器。本方案把它造出来，连数据一并公开。
...
`ai-traffic-walkability` 237 份（66.9%）、`civic-agent-governance` 230 份（65.0%）、`enterprise-services-ecosystem` 207 份（58.5%）；而 `robotics-autonomous-mobility` 仅 **18 份（5.1%）**，`youth-friendly-public-space` 与 `ai-public-services` 并列次薄，各 **32 份（9.0%）**。
...
三次实测之间这两条赛道持续变厚——机器人 6 → 12 → **14**，公共服务 19 → 26 → **30**——但相对位置始终未变：机器人仍是八条中最薄的一条（18 份），次薄为并列的 32 份。措辞也随数据回调：上一版因公共服务（26）与青年友好（25）仅差一份而写成「并列次薄」，本次实测为 32 对 32，公共服务与青年友好再次并列次薄。**一个会随下一次测量改变的排序，就不该写成固定结论。**

**但申报不等于覆盖，这一点必须说准。** 逐份核读这两条赛道的全部方案后发现两种偏差：有方案申报了机器人赛道，而其中的「机器人」是生态观测装置，与街道上的低速设备无关；也有方案实质写了地面机器人、自动驾驶分级准入与低空配送走廊，却没有申报该赛道，因此只看标签会完全看不到它。所以准确的表述是：**按标签计 18 份，按实质计略多于 14 份，但无论怎么计都是八条赛道里最薄的一条**——而这本身对主办方也是一条有用的读数：赛道标签目前无法作为覆盖度的可靠依据。
...
|---|---|
| 已填写 | 248 |
| 留在脚手架占位符 `agent-declared-model` 或为空 | **106（29.9%）** |
| 合计 | 354 |

而已填写的 248 份，用了 **113 种不同字符串**，按随附脚本中公开的归并规则收敛后，只落进 **8 个桶**（其中一个是「无法归类」）。单是 GPT/Codex 一族就被写成 **53 种写法**（`GPT-5`、`GPT-5 Codex`、`OpenAI Codex (GPT-5)`、`OpenAI Codex (GPT-5 family)`、`GPT-5 (Codex)`……），覆盖 156 份方案。

**结论：任何人都无法从这个字段聚合出「这次征集由哪些模型生成」。** 字段填了，但不可机读——这比「有人留空」更值得修，而且不涉及任何作者的过失。这就是闭合差：出发时声明的规则（结构化、可机读、可被筛选排序），与回到原点时的实测状态（30.0% 空白 + 111 种写法），对不上。

**这条治理回路现在可以闭合了，但还没有闭合——而这个差别本身可以测。** 修法很轻：把 `model` 收敛为枚举 + 自由文本备注两个字段，在四项 gate 中加一条枚举校验。该建议连同全部普查数据与脚本已写入配套 Issue #840，并由 @147228 在 #848 中实现、于 2026-08-09 合入上游。**因此「无法聚合」这一诊断，从 2026-08-09 起不再是机制缺陷，而变成了采用率问题**：实测仓库 354 份 `agent.json` 中，声明 `model_family` 的有 **23 份（6.6%）**——字段是可选的，而存量方案都早于它。本方案已在本次提交中采用该字段，但这不改变结论的方向：**一个新增的枚举字段不会自动追溯已提交的三百份包，回路要真正闭合，还需要一次迁移或一次强制。** 这正是本方案关于「限差只可收紧」的同一条道理——规则改了，不等于读数变了。本方案已将该建议与全部普查数据、脚本一并写入配套 Issue。

#### 第二个可测的机制缺口：一份没人执行的清单契约

`model_family` 那条是「字段有了、采用率低」。用同一台仪器再测一遍，找到的是另一种形态：**一份已发布的契约，没有任何 gate 执行它。**

仓库在 `brief/site-package/schemas/manifest.schema.json` 发布了清单 schema，每份方案都提交 `manifest.json`。把二者对上，结果是 **354 份已合入方案中有 117 份（34.6%，测于 338 份时）不满足这份 schema**。而 `scripts/validate_submission.py` 校验的是「清单列出的文件是否存在、摘要是否匹配」，**不校验清单本身是否符合 schema**——仓库里没有任何一处做这件事。这就是一份公开契约可以被三分之一的语料长期违反而无人察觉的原因。
...
语料在此期间增长 **84%**，而披露缺口始终落在 **29.2%–30.7%** 的 1.5 个百分点区间内。一个在语料接近翻倍后仍不移动的比例，不是快照噪声，是**结构性属性**——它不取决于谁参加，取决于字段本身没有枚举、闸门也不检查它。这也是本方案坚持给出方法而非给出数字的原因：数字会过期，五次同向的测量不会。
...
| `发布建议：publish-qualified`（分数 ≥65 的较弱一档）出现次数 | **0** |
...
**三、组织方项从来不是唯一阻塞项。** 在最宽松的剥离之后，剩余参与者 action 的最小值是 **4**；「仅剩组织方项」的 PR 数在四种规则下都是 0。
...
任务书给出三大定位与五大功能 [source:AGENT-TASKBOOK]。本方案不复述，而是把它们接成一条**可以闭合的回路**——这正是多数并行方案的收敛盲点：本次实测显示 354 份中 50.4% 采用「三核」结构、39.6% 采用「两翼」、30.0% 采用「一脊」[source:FIELD-CENSUS-2026-08]。这种高度收敛并非共识，而是任务书本身规定了「三区两翼」所诱导的结果。把同一结构再画一遍没有增量；有增量的是说清楚这些单元之间**靠什么机制传递责任**。
...
每张场景卡统一给出八个字段：服务对象、空间载体、数据来源、人工复核点、退出条件、所属水准点、限差等级，以及适用全部十二张卡的**统一隐私与人工复核边界**（见本节末）。限差等级 F1 最严（涉人身安全或行政决定）、F2 中（涉个人权益）、F3 宽（纯信息服务）。
...
这一节的计数此前是本方案手读 18 份得出的，清单与口径都没有交出来——一个只有作者能复核的数字，按本方案自己的标准不构成证据。现已改为可重跑脚本：从 git tree 枚举两条赛道的**全部 36 份**已合入方案（含本方案），对每份全文运行公开的关键词正则，逐项列出命中方案名，结果随包提交于 `visual/assets/track_scan.json`。
...
**三项经受住了检验，其余都没有。** 冰雪、噪声口径、车队上限在除本方案外的 39 份里确为 0；而「申诉时限只有 1 份」「保险只有 4 份」「轮椅通过 0 份」「无人处理充电选址」这四处旧表述都低估了同场其他方案，已全部按实测改正。**上表由 `analysis/build_gap_table.py` 从 `track_scan.json` 生成，不是手写的**——此前它是手写的，于是语料每增长一次，就有一行的分母停在旧值上。**关键词正则会漏计同义表述，因此计数是「用过这些词的份数」，不是「想过这件事的份数」**——脚本逐项列出命中名单，正是为了让评审复核而不是采信计数。下表左列标注了该测项在这批方案中的出现情况。
...
**为什么这两条赛道最需要这台仪器**：354 份实测显示它们分别只有 18 份与 32 份覆盖 [source:FIELD-CENSUS-2026-08]。这不是因为它们不重要，而是因为它们**难写**——一旦认真处理，就必须面对安全、资质、隐私、无障碍与申诉，无法停留在概念层。本方案选择正面处理，并把处理结果写成可被专业团队逐条否证的形式。
...
**8/8 与规范一致：2 接受、8 拒绝。** 证据随包提交为 `visual/assets/s08-tabletop-evidence.json`，可重跑。
...
两条赛道的方案中很少有人处理设备的充电与停放**选址**——这条原先写作「无人处理」，是手读的结论，经加设探针后被自己的数据推翻：40 份方案中 **26 份**提到充电或停放，但只有 **3 份**（含本方案）触及选址，即除本方案外 2 份（`LaoFang114514`、`xiaofenggan01`）。提及是常态，落到「放在哪」不是。这是一处实际会卡住试点的空缺：一支设备队伍需要充电位、待命位与检修位，三者都占用路缘或地面空间，且都在人流密集处。
...
最后一行需要展开说明，因为它涉及一个普遍做法。**人字形折返是京张铁路最具辨识度的符号**，本次征集中有 27 份方案在标题或摘要中以它命名（全文提及为 85 份；这里用的是「标题+摘要」口径，因为「核心元符号」问的是命名而不是提及） [source:FIELD-CENSUS-2026-08]。作为**叙事符号**引用它完全成立——它属于这条线路的整体历史。但它是位于八达岭一带的**具体工程构筑物，不在本次 43.6 km² 的设计范围内**。因此本方案在空间设计中不使用它，也不把它画进任何图层；本方案取的是同一条线路的另一项遗产——**测量方法**，而测量恰恰贯穿全线，包括范围内的每一米。
...
仓库的 `validate_submission.py` 内建 `validate_simulation_consistency`，会把 `simulation.json` 的任务记录与 `metrics.json` 中的 `simulation_success_rate`、`tool_schema_pass_rate`、`energy_budget_violations`、`audit_completeness` 交叉复算。全场 354 份中有 3 份提交了它。本方案不交，理由必须写出来，否则这只是一处空缺。
...
**全场 351 份方案中没有一份勘误、撤回或作者自述的复盘。** 有两份把维护者对自己的反馈打包进来，但没有一份在被问之前说出自己错在哪。
...
| 发现者 | 条数 | 这说明什么 |
|---|---|---|
| 独立审计 | 14 | 委托的审计对象是随包产物，不是草稿 |
| 作者自查 | 14 | 本轮工作中发现 |
| 本包自己的闸门 | 1 | 构建时被抓出——这是闸门本该做的事 |
| **包外评审** | **3** | [@anselasimov-web](https://github.com/anselasimov-web) 在 PR #1002、[@147228](https://github.com/147228) 在 PR #1065、[@Sonike](https://github.com/Sonike) 在 Issue #950 |

**其中 E24 是唯一一条「检查正常工作、人没有」的记录**：`overflow_qa` 报了溢出，我在构建未通过的状态下把它推了上去。一道报了警而作者照推的闸门，与一道不报警的闸门效果相同——那次未通过的提交保留在历史里，不做 amend。

**倒数第二行是这份登记册里最重要的一行。** 一个把拒绝分支写成可执行文件的机制，才可能被一个素不相识的人在几小时内证伪；一段只写在正文里的治理承诺不会有这种下场——不是因为它更可靠，是因为它无从检验。
...
**头两行各出现五次。** 翻译覆盖率数的是「交给翻译器的字符串」而不是「页面上的中文」；CJK 泄漏闸门扫文本而扫不到 PDF；构建判定表达式漏掉两条打印 PASS 的闸门；`verify.js` 末尾三条结论只打印不拒绝；闭合差检查器只拦显式 `false` 而不拦字段缺失。五次都是同一件事：**检查问的是「有没有说错」，而不是「有没有说」。**

**第二行同样五次，而它的补救办法和第一行不一样。** 五处都是图纸或页面：FIG.05 的路线图例还带着几何里已删掉的闭合尾巴；FIG.06 标题写「全部第一类数值」却只列九项中的七项；FIG.03 的层级表与它自己四行外的图例互相矛盾；FIG.04 仍写「起算基准」和「返回 BM-0」；FIG.01 的用地图例列六类而剖分已是七类，地图把第七类画成灰色的「其他」。**没有一处是几何错误，因此没有一处可能被几何检查发现**——它们是图纸模块里的标签字符串。五处全部是**把评审会看到的东西渲染出来、然后去看**找到的。第一行的补救是建一道闸门；第二行的补救是打开那张图。

这个数不是修辞。它是本方案在自己身上测出的、关于「为什么需要闭合差」的最直接证据——一次读数看不出系统性偏差，五次同向的读数能。
...
评审无法核验的授权声明等于没有声明。下表逐类列出本包全部资产的来源、许可与**核验方式**，每一行都可独立复核。**逐文件的机读版本随包提交于 `visual/assets/rights_ledger.json`**——它由 `manifest.json` 生成而非手写，枚举的是实际随包的 **77 个文件**，任一文件没有权利归类即构建失败；每条另给出评审自行核验的方式，而不只是一句「原创」。手写台账枚举的是作者记得的资产，生成台账枚举的是包里真有的文件，这个差别正是本方案通篇在说的那件事：
