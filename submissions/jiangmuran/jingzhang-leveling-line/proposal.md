...
summary: "水准测量的方法论不是「测得准」，是「测得回来」：绕一周回到原点，闭合差超限则整段重测。本方案把这条规矩用在读数错了会伤到人的地方——低速机器人、自动驾驶接驳与AI医疗教育法律生活服务。全线布设分级水准点与两条附合路线，F1类场景必须先在受控测试场取得闭合合格才能进入公共服务，且永远保留无AI等价路径。同一台仪器转向本次征集自身，对全部已合入方案做了十二次可重跑普查（184→440 份，逐次读数随包），数据与结论作为评审助手一并公开。所有空间建议均为概念建议，基于临时边界，不替代法定规划。"
...
![京张水准线：基准线出发、抬升、回落，却没有落回基准——那道红色高差就是闭合差](assets/figures/hero-leveling-line.png)
...
| 为什么用在这两条赛道 | 因为读数错了会伤到人。**实测**机器人 25/440（5.7%，八条赛道中最薄）、AI公共服务 44 份（第三薄，次薄为青年友好 41 份） | `visual/assets/field_map.json`，普查脚本可重跑 |
...
**本方案的定位声明。** 城市AI治理是本方案的**方法层**，不是卖点——把治理协议本身当成成果，是这次征集里最饱和的做法（最近一次实测：440 份已合入方案中 289 份申报治理赛道，「证据链/可复算」类表述出现在 29.1% 的方案里 [source:FIELD-CENSUS-2026-08]）。本方案把治理当工具，把工具用在覆盖最薄的赛道上：`robotics-autonomous-mobility` 按标签计是八条赛道里**最薄**的一条（25 份，5.7%），次薄的是 `youth-friendly-public-space`（41 份）。这不是为了避开竞争而选空位——恰恰相反，闭合差机制在这两条赛道上才真正不可替代，因为只有在这里，一次未被复核的错误读数会落在具体某个人身上。
...
这不是评论主办方的工作，而是**交付一件他们此刻最缺的东西**。截至最近一次实测仓库已合入 440 份方案，PR 编号已过 1000，维护者在以每分钟数份的速度合并。此时最难的不是收稿，是**横读**：四百多份里哪些已经收敛、哪些位置还空着、哪些声明其实无法被聚合。这件事无法靠翻展示页完成，需要一台仪器。本方案把它造出来，连数据一并公开。
...
共 440 份已合入方案；赛道为多选，占比之和大于 100%。加粗两行是本方案选定的两条，也是全场最薄的两条。
...
|---|---|
| 已填写 | 331 |
| 留在脚手架占位符 `agent-declared-model` 或为空 | **109（24.8%）** |

而已填写的 331 份，用了 **133 种不同字符串**，按随附脚本中公开的归并规则收敛后，只落进 **8 个桶**（其中一个是「无法归类」）。单是「GPT / Codex」一族就被写成 **62 种写法**（`GPT-5`、`GPT-5 Codex`、`OpenAI Codex (GPT-5)`、`OpenAI Codex (GPT-5 family)`、`GPT-5 (Codex)`……），覆盖 206 份方案。
...
实测仓库 440 份 `agent.json` 中，声明 `model_family` 的有 **102 份（23.2%）**——字段是可选的，而存量方案都早于它。（此数此前两个语种各写了一个值，都不在任何随包文件里；现在由 `field_map.json` 的 `model_family_declared_count` 生成。）
...
仓库在 `brief/site-package/schemas/manifest.schema.json` 发布了清单 schema，每份方案都提交 `manifest.json`。把二者对上，结果是 **测于 338 份语料时，117 份（34.6%）不满足这份 schema**——分母是那次普查的 338 份，不是当前语料；这一点写出来，是因为把它写成「当前语料的 117 份」会让 34.6% 变成一个对不上的数。而 `scripts/validate_submission.py` 校验的是「清单列出的文件是否存在、摘要是否匹配」，**不校验清单本身是否符合 schema**——仓库里没有任何一处做这件事。这就是一份公开契约可以被三分之一的语料长期违反而无人察觉的原因。
...
**分子的行为可以直接读出来，而且比原结论更有意思。** 本包每次重跑普查都提交一份新的 `field_map.json`，因此每一次读数都留在本分支的 git 历史里；`visual/assets/reading_log.json` 把它们全部取回，共 **13 次**读数，语料从 228 长到 440，占位符数依次为 69 → 87 → 102 → 104 → 106 → 106 → 106 → 105 → 107 → 107 → 107 → 109 → 109；期间占位符数**下降过 1 次**（373→381 时 106→105），说明**存量方案确实被改过**。最长的一段不变是 **107**，持续 3 次读数（语料 394 / 408 / 416）。
...
![区域协同接口：把水准网外延为区域水准网](assets/figures/region-interface.png)
...
用地边界与重点区边界均为临时替代边界，官方 polygon 发布后须整体重算；逐区平面见 FIG.04，三幅同尺度可直接比较。
...
以下六项已成为事实标准，括号内为 440 份语料中的实测份数：场景级停用与退出条件（280）、现场安全员（196）、无AI等价路径（180）、远程与物理急停（100）、限速（53）、事件日志（28）。最薄的一项也有 28 份，最厚的 280 份。**这是下界**——换了说法的方案不会被关键词计入。本方案全部采纳这六项并写入下方场景卡，但**不把它们当作创新陈述**——它们是准入底线。把底线当卖点，说明没有读过这个场。
...
![街道上的水准点与路缘分配](assets/figures/street-eye-level.png)
...
本次征集 440 份中有 **20 份**在标题或摘要中以它命名，**101 份**在正文任意处提及 [source:FIELD-CENSUS-2026-08]。两个口径分开给，是因为「核心元符号」问的是命名而不是提及；两者都由 `analysis/field_extras.py` 从正文重算。
...
全场 440 份中有 **5 份**提交了它（`YoungCan-Wang`, `ZHLins`, `ZephyrL0`, `luther-3`, `spacethroughing`）。本方案不交，理由必须写出来，否则这只是一处空缺。
...
当前 440 份中，**2 份**随包提交了独立的勘误文件——本方案，以及 `147228`；另有 **8 份**在正文里出现「自查发现并修正」一类表述。这个数字在本方案首次写下那句话时是 1（只有本方案），现在不是了。
...
| 发现者 | 条数 | 这说明什么 |
|---|---|---|
| 独立审计 | 20 | 委托的审计对象是随包产物，不是草稿 |
| 作者自查 | 20 | 本轮工作中发现 |
| 本包自己的闸门 | 2 | 构建时被抓出——这是闸门本该做的事 |
...
评审无法核验的授权声明等于没有声明。下表逐类列出本包全部资产的来源、许可与**核验方式**，每一行都可独立复核。**逐文件的机读版本随包提交于 `visual/assets/rights_ledger.json`**——它由 `manifest.json` 生成而非手写，枚举的是实际随包的 **86 个文件**，任一文件没有权利归类即构建失败；每条另给出评审自行核验的方式，而不只是一句「原创」。手写台账枚举的是作者记得的资产，生成台账枚举的是包里真有的文件，这个差别正是本方案通篇在说的那件事：

<!-- LEDGERCOUNT:BEGIN -->

**逐文件台账在评审输入之外，因此把它的结果放进来。** `build_review_input` 送出的是 `proposal.md` 与八份 JSON；`report/copyright_statement.md` 与 `visual/assets/rights_ledger.json` 都不在其中。**一份评审结构上打不开的权利台账，从评审的位置看就是一句无法核验的断言**——而那正是本方案在别处反对的东西。当前 **86 个随包文件全部有权利归类**，任一文件缺归类即构建失败：
