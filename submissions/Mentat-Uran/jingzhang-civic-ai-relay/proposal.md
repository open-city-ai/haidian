...
summary: "以京张遗址公园为公共协议脊，把众智园验证、AI 原点转译、大钟寺应用组织成可复核、可退出、可迭代的城市学习接力；全部空间控制均保留 provisional 边界和专业复核条件。"
...
**核心判断：不是把三个园区串在一条“展示轴”上，而是让公共问题、技术验证、产业转译、城市使用、社会反馈和再验证沿京张走廊持续接力。**
...
命名体系采用“带—站—节点—场景”四级语法：`京张共智接力带 / Civic AI Relay` 为总名，三个关键局部使用“验证—转译—接口”的动词型名称，公共节点使用“标、墙、厅、刻度”等可辨认名，场景卡统一使用 `SC-01…SC-12`。Logo 方向为抽象的 J/Z 双轨、三个接力点和一条回授弧；不得直接使用企业标识、人物肖像或未授权字体。视觉以纸白、墨黑、铁路信号橙、治理青和蓝绿色为主，形成“铁路时刻表 + 公共证据册”的工业编辑语言。
...
以下 12 张场景卡全部是概念建议，不是已批准运营。每张卡都同时写明空间、运营、数据、隐私和人工复核；其中 SC-01 至 SC-04 为产业测试验证场景。[metric:scenario_node_count] [metric:industry_test_scenario_count] [data:geometry/public_space.geojson#SC-01]

| ID / 场景 | 主要用户与空间 | 运营和数据 | 隐私、人工复核与退出 |
| --- | --- | --- | --- |
| SC-01 具身智能多主体安全验证 | 研发者；北部封闭测试接口与公众观察窗 | 预约制，记录版本、任务、碰撞/失效事件和修复 | 不采集旁观者身份；安全员可急停，测试失败不对公众隐匿 |
| SC-02 端侧模型能耗与韧性测试 | 产品团队；可计量测试台 | 比较离线、断网、降级和能耗表现，只保存设备级测量 | 不上传个人内容；工程师签署结果，任何样机可立即断电退出 |
| SC-03 开源模型红队与标准共创 | 维护者、治理观察者；隔离红队室 | 保存提示类别、失效类型、修补版本和复测证据 | 测试语料先脱敏；伦理/安全复核决定是否公开细节 |
| SC-04 城市服务接口互操作测试 | 公共服务人员、开发者；模拟服务柜台 | 使用合成工单检查身份、状态、人工转接和日志接口 | 不接真实个人业务；服务主管确认通过，不能自动上线 |
| SC-05 开源维护者产品诊所 | 初创、维护者；原点转译院 | 按周接诊，登记问题、依赖、许可证和后续责任 | 未授权经营信息不进入公共记录；法律/安全人员复核，随时撤单 |
| SC-06 可解释学习伙伴 | 学生、教师；共学花园与室内席位 | 只保存学习者主动提交的任务和反馈，可使用访客模式 | 默认不做长期画像；教师复核建议，未成年人须适当授权 |
| SC-07 无障碍慢行导航 | 行动不便者、访客；原点东西缝合线 | 使用公开路网和现场核验的障碍信息，报告失效点 | 精确位置短时处理；人工热线兜底，用户可关闭定位 |
| SC-08 社区健康服务导航 | 居民、老年人；社区人工服务台 | 只做公开服务检索、预约入口说明和人工分流 | 不做诊断、不存病历；工作人员复核并提供纯人工路径 |
| SC-09 公共法律服务分诊 | 居民、创业团队；南部公共服务点 | 识别问题类别、列明材料和公共服务入口 | 不作法律结论；人工法律服务人员确认，记录可删除 |
| SC-10 轨道四象限步行助理 | 通勤者、访客；南部城市接口 | 用实测可达性和临时障碍更新方向提示 | 不追踪个人轨迹；现场人员确认安全，故障时切换静态导向 |
| SC-11 智能体消费沙箱 | 消费者、产品团队；限定展示界面 | 显示代理权限、预算、确认步骤和交易模拟日志 | 默认不接真实支付；每一步需人确认，可一键清空与退出 |
| SC-12 京张记忆共读器 | 居民、访客；公共协议脊 | 仅使用清权史料和公众自愿提交的记忆，标注出处 | 不做人脸识别；编辑复核史实，投稿者可撤回贡献 |

场景治理共用六个门槛：目的明确、数据最小、现场告知、人工复核、可退出/可申诉、可停止/可回滚。运营台账按“测试中—限域试用—暂停修复—复测—结束”表达状态，不出现“已部署”“全覆盖”等无证据措辞。场景开放只在责任主体、数据影响评估、无障碍替代、应急预案和专业审批具备后推进。[source:AGENT-TASKBOOK]
...
版权方面，正文、结构化数据、图示、HTML 和 PDF 由本次智能体工作流基于仓库清权材料和自行生成的几何、排版与文字形成；使用系统字体渲染，不上传字体文件；没有使用商业地图截图、远程瓦片、未授权照片、商标、肖像或论文图像。全球案例仅作文字机制总结并回引原始机构页面，不能复用其视觉资产。许可与展示范围以 `COMMUNITY-DISPLAY-ONLY` 和 copyright statement 为准。
