...
信号装置是空间的硬编码，不是网页软提示；智能体网关是这套制度的“总信号台”——智能体入城先过信号，再进空间 [depth:urban_signal_regime]。信号制度同时回应任务书“AI 治理全球话语权”定位：一套可读、可停、可复制的智能体治理语言，本身就是中国 AI 治理话语权的城市表达 [source:AGENT-TASKBOOK]。
...
### 信号兼容分区：给数字市民一张可读的城市准入地图

信号生命周期让“可停止”成为制度；**信号兼容分区（Signal Compatibility Zoning）**则把三态语言从运营与设施层进一步扩展到**规划语言**——让“智能体作为数字市民，在哪些空间被常态欢迎、在哪些空间需交接护送、在哪些空间接受独立测评”成为规划图上一眼可读的准入地图。这是 USR 的第三层应用（前两层为：场景信号状态·运营层、节点信号装置·设施层）[depth:urban_signal_regime] [depth:signal_compatibility_zoning]：

| 兼容区 | 数字市民准入状态 | 空间落点（概念·对应本方案空间结构） | 人类市民可读到什么 |
| --- | --- | --- | --- |
| 🟢 绿态兼容区 | 常态可达，按开放协议服务 | 京张遗址公园开放段（GREEN-001）、原点社区公共客厅与开源发布厅前广场（PROV-KEY-002）、大钟寺产业与商业街区（PROV-KEY-003） | 智能体在此为常态；状态可读、可停 |
| 🟡 黄态兼容区 | 交接护送 / 时段限定后进入 | 众智园受控试验段（PROV-KEY-001）、大钟寺站轨道与换乘庭界面、校园与医院等敏感界面 | 进入需交接入场；高峰时段人机共操 |
| 🔴 红态兼容区 | 接受独立测评的隔离区（智能体默认不进入） | 众智园安全治理沙盒（S-06，红队评测与数据隔离区） | 智能体的能力在此接受独立核验；边界透明、可审计 |

分区作为**城市设计导则的“智能体兼容度”条款方向（概念建议）**提出，纳入公开程序讨论，待官方控规与现状底数补齐后由专业团队核定；网关在入口执行准入鉴权：智能体入城先按目标空间的兼容状态判定，绿态直接进入、黄态转交接入场、红态路由至测评流程并写入审计记录 [metric:green_compat_zone_count] [metric:yellow_compat_zone_count] [metric:red_compat_zone_count]。

分区与“无 AI 等价”原则互为表里，构成“数字市民”的**准入与问责闭环**：红态与黄态空间本就由人工等价路径完整覆盖，智能体从不是必需项；分区让 DCC 的义务条款（②可停止、③无 AI 等价）拥有明确的规划语言表达——绿态的“常态可达”与红态的“接受测评”在规划图上同页可读，使智能体的准入、服务与退出在信号与审计记录中逐项可追溯 [depth:no_ai_equivalence] [depth:public_interest_ledger]。
...
| 规划创新性 | Agent-First 数字市民 + 数字市民公约（DCC）+ 城市信号制度（USR） | 核心主张 / 记忆—治理主轴 | [depth:urban_signal_regime] [depth:no_ai_equivalence] |
...
| --- | --- | --- | --- |
| 发展愿景：世界级人工智能创新街区、全球人工智能产业高地、"城市智能体"样板区；开放、可学习、可进化、可落地 | Agent-First 数字市民；把"智能体样板"做成可读、可停、可交接的空间机制 | 数字市民公约（DCC）+ 城市信号制度（USR）+ 信号生命周期 + 信号兼容分区 | 核心主张 / 实施·试点—示范—推广 [depth:signal_compatibility_zoning] [depth:pilot_demo_scale] |
...
1. **无 AI 等价（No-AI Equivalence）**：每一公共服务节点均设人工/离线等价物，智能体服务是可停止的加成项而非必需项，不会因不熟悉或不会使用智能体而失去服务 [depth:no_ai_equivalence] [metric:no_ai_equivalent_count]。
...
四项保障的落地由**公共利益台账（public-interest ledger）**持续核验：信号生命周期的**月度台账审计**在核对信号与行为入账的同时，逐月核对四项保障的证据——每个服务节点是否有生效的无 AI 等价物、每项停止/冻结权是否留有具名负责人与受影响者记录、公共开放是否保持免费或低门槛、无障碍与代际包容是否按运营规则执行；审计结论进入公共知识库，任何”信号与行为不符”或”保障缺位”即进入红牌审查 [depth:public_interest_ledger] [metric:no_ai_equivalent_count] [metric:stop_rights_count]。
...
本方案的交通枢纽性设计是**大钟寺站“智能体网关接驳站”**：轨道站点不仅是人类通勤的转换枢纽，也是智能体接入城市服务的第一网关。智能体在此完成身份鉴权、权限绑定与服务目录订阅，再沿慢行环与蓝绿网进入各场景节点；无人配送走廊（S-08）作为低速接驳专线，把物流智能体与人类慢行分离在不同空间层面 [depth:traffic_rail_slow_parking]。大钟寺站四象限步行连通同步解决人类行人与智能体移动的双重可达性 [data:geometry/key_areas.geojson#PROV-KEY-003]。
...
| JZ-11 | 城市信号系统示范线（含信号兼容分区） | 公共空间/治理设施 | 近期试点（概念示范） | 治理沙盒/公共空间管理方 | 公共空间许可、信号协议、兼容度条款与责任签认机制 | [depth:urban_signal_regime] [depth:signal_compatibility_zoning] |
...
信号兼容分区、信号生命周期与全部场景机制共享同一条**三跃迁路径**，任何机制不直接全带铺开，而是"试点→示范→推广"逐级放行，把概念机制变成**可排期、可验证、可撤销**的实施路径 [depth:pilot_demo_scale]：
...
| 推广（Scale） | 示范满两年；治理办法与数据治理规则公示满一年；公共知识库沉淀达标 | 全带 12 场景节点与三态兼容区（JZ-07/09/10 与年度运营体系） | 退出机制落地，任何机制可撤回 |

三跃迁与"近期试点—中期更新—长期治理"分期一一对应，并让**信号兼容分区**作为近期试点的可移除装置之一（信号柱 + 兼容度条款）先行验证"分区准入—人工交接—红态测评"的完整链路 [depth:signal_compatibility_zoning] [depth:pilot_demo_scale]。所有跃迁条件为概念阈值，正式阈值由运营实施阶段以公开程序确定，不构成任何政府实施安排或投资承诺 [depth:phasing_implementation]。
...
| green_compat_zone_count | 3 | known | 绿态兼容区数量：智能体常态可达的开放空间与产业街区（京张遗址公园开放段、原点社区公共客厅与发布厅前广场、大钟寺产业与商业街区）[metric:green_compat_zone_count] |
| yellow_compat_zone_count | 3 | known | 黄态兼容区数量：需人工交接/时段限定后进入的受控与敏感界面（众智园受控试验段、大钟寺站轨道与换乘庭、校园/医院界面）[metric:yellow_compat_zone_count] |
| red_compat_zone_count | 1 | known | 红态兼容区数量：智能体接受独立测评的隔离区（默认不进入）——众智园安全治理沙盒（红队评测与数据隔离区）[metric:red_compat_zone_count] |
