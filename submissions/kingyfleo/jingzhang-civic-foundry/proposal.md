...
summary: "以京张公共空间为底盘，以真实场地锚点、三种城市织体、五类街区断面、AI可审查技术栈和1+4+3公共价值验收合同，把文化证据、城市委托、公共样机与十二个月采用/退出组织成可复核的城市空间与公共产品转化链。"
...
### 5. 五向区域协同：京张做“转化接口”，不与其他创新区争同一块牌子
...
#### 样板节点：AI原点“委托前廊—共同作者院—可见工坊”

本次不是以效果图代替深化，而是把下一阶段的图纸、法定字段和验收任务锁定在同一个真实锚点上。样板节点以原点大厦周边既有街—巷—院关系为背景，当前仅作概念性原型；正式落位须在官方重点区polygon、地块权属、现状测绘和控规条件到位后确认。[data:visual/assets/ai-origin-prototype-control.json]

| 图纸尺度 | 必须回答的空间问题 | 必须绑定的验收证据 |
| --- | --- | --- |
| 1:1000总平面 | 现状建筑与街巷、保留/修补/再利用、全天公共通道、消防与装卸、树冠和雨洪、周边日常服务 | 地块ID、边界来源、拆改留分类、通道权利、现状/建议面积复算 |
| 1:500首层平面 | 委托前廊、人工服务窗、无需消费公共客厅、共同作者长桌、可见工坊、安静旁路和后勤入口 | 首层可读界面、无扫码路径、无障碍任务、工坊围界、值守与撤回点 |
| 1:200关键剖面 | 街道—廊—庭—坊的宽高关系、遮阴、树冠、门槛、视线、声环境、装卸与消防 | 实测宽度/坡度、夏冬遮阴、夜间照度、消防净空、撤场恢复方式 |
| 轴测与运行图 | 上午居民出题、下午共同制作、傍晚公众复验，以及春夏秋冬的空间切换 | 六观察窗、角色同意、时间许可、状态牌、投诉修复和A0/A1/A2记录 |

这个节点不是全线复制的标准建筑，而是一套可迁移的“图纸—任务—签字”接口：不同街区保留自己的尺度、生活和建筑类型，只复用证据字段、AI准入和公共价值验收方法。

![AI原点样板节点：从1:1000到1:200的图纸—任务—签字接口](assets/figures/ai-origin-prototype.png)
...
## 1+4+3公共价值验收合同：不是综合打分，而是逐项签字和失败处置

旧式“双账”容易把法定前提、空间体验、文化产出、AI运行和活动客流混成可以互相补分的总账。本案不再采用并列账目，而把验收改成一份公共合同：**1条不可折抵的法定安全底线、4类分别设最低保证和改善证据的公共价值、3个必须重新签字的验收时点。**法定条件不允许被体验、传播、客流或AI效果抵销；任何一类公共价值未达到最低保证，都不能靠其他类别的高分补过。[source:BEIJING-URBAN-RENEWAL-WHITE-PAPER-2024] [source:UK-MAGENTA-BOOK-2026]

### 相似案例转成京张规则，而不是停留在案例引用

| 可转用方法 | 案例启示 | 京张转译 |
| --- | --- | --- |
| 体检—更新—评估 | 北京城市更新把前期体检、实施和后评估连接起来 | A0基线签约、A1开放核验、A2使用复验，三个时点不得合并 |
| 设计检查与真实体验分开 | Healthy Streets不以图纸自证建成后的街道感受 | 每处样板节点按工作日/周末×上午/下午/傍晚形成六个观察窗，并在夏冬两季复测 |
| 公共生活数据可比较 | Public Life Data Protocol统一地点、时间、移动、停留和活动字段 | 每项空间结论同时记录分子、分母、样本窗口和观察位置 |
| 持续履约与整改 | 纽约POPS审计把开放义务、现场巡查、违规和整改连接起来 | 每项最低保证配置独立签字人、失败动作、复核期限和公开状态 |
| AI影响与技术性能分开 | 政府AI评价要求基线、正常服务对照和可归因的公共影响 | AI必须与无AI正常服务完成同任务比较，模型指标不能替代城市结果 |

### “1”：不可折抵的法定安全底线

所有适用法定字段取得正式来源、批复和效力日期，适用专业审查100%通过，建设关键 `unknown=0`。它是准入条件而不是绩效分：消防、无障碍、轨道、文保、道路、市政或产权任一阻断项未关闭，项目停止在相应质量门，不进入公共开放。[data:visual/assets/statutory-control-register.json]

### “4”：四类公共价值分别验收

| 公共价值 | 最低保证：不满足即失败 | 改善证据：与A0基线比较 | 独立签字人 |
| --- | --- | --- | --- |
| V1 空间与公共生活 | 无扫码连续路径100%；关键无障碍任务100%；日常可识别首层≥70%；夏季14:00关键停留区遮阴≥60% | 每节点每季完成工作日/周末×上午/下午/傍晚六个观察窗，比较通行、停留、求助、冲突和排斥 | 规划/建筑专业人员+无障碍共同测试者 |
| V2 文化生产与共同作者 | 来源、事实/转译、作者、许可、报酬或约定收益、撤回机制记录率100%；未决权利争议=0 | 每季度≥3项真实城市公共委托，其中≥1项来自一线公共问题；每项形成可复用内容包、体验模块或服务脚本，12个月内至少在2个独立场景复用 | 文化编辑+权利人/共同作者代表 |
| V3 普惠服务与可拒绝AI | 非数字等价路径100%；公共服务时段人工接管100%；拒绝账号、扫码、生物识别或AI不降低基本服务；未关闭一级/二级风险=0 | 同一任务比较“实体/人工正常服务”与“可选AI”：完成率、时间、纠错、排斥、一线工作量分别报告 | 一线服务责任人+独立公共利益评审人 |
| V4 韧性运维与可逆退出 | 维护人、资源、替代流程、回滚方案100%；基本公共功能24小时内恢复 | 记录故障、投诉、修复、能耗、成本和四季运行；任何固定化前先通过撤除与恢复演练 | 设施运维人+政府资产/项目代表 |

这里的“100%”只用于明确的清单型底线，并必须给出分子与分母。凡缺少基线、分母、样本窗口或签字人的指标，一律标记为 `pending_A0_baseline`，不得写成“已达标”。[data:visual/assets/lived-culture-dual-ledger.json]

### “3”：三个验收时点

- **A0 基线签约（G0—G1）**：冻结权威资料、对象、分子分母、正常服务对照、样本窗口、目标、签字人和失败动作。没有A0，不接受后补式“成绩”。
- **A1 开放核验（G3—G4）**：图纸、竣工和现场任务逐项一致；四类最低保证全部通过；AI完成红队、人工接管、断网、停机和恢复演练。
- **A2 使用复验（D30/D180/D365）**：形成十二个月证据包；公共结果和AI增量分开报告，决定采用、修正、缩小、迁移或退出。

每张验收卡固定九个字段：`outcome / baseline / normal_service_comparator / minimum_guarantee / improvement_target / numerator_denominator / sample_window / independent_signoff / failure_action`。这使“建成什么样、谁来验、没做到怎么办”在项目开始前就成立。

![人物、文化生产与1+4+3公共价值验收](assets/figures/metrics-evidence.png)

## AI技术架构：证据—模型—智能体—人工门—空间接口

AI不是安装在公园里的一块屏，也不是替政府、专业人员和一线人员作决定。本案把技术拆成六层可审查系统，并让每层都输出可复核证据。[source:AI-RAG-LEWIS] [source:AI-HUMAN-INTERACTION-AMERSHI] [source:NIST-AI-RMF-1]

| 层级 | 技术功能 | 人工责任 | 空间输出与停止条件 |
| --- | --- | --- | --- |
| L0 公共底座 | 实体导视、无障碍、人工窗口、安静旁路和断网运行 | 场地与公共服务运营者 | AI全部下线时基本公共功能仍完整 |
| L1 证据与权利 | 许可来源库、版本、作者、事实/转译标记；RAG仅检索已登记证据 | 文化编辑、权利人与数据责任人 | 无来源或权利未决内容不进入空间发布 |
| L2 模型与空间计算 | 模型注册、规则核验、场景推演、GIS/微气候/可达性计算 | 规划、建筑、工程与领域专家 | 模型建议不自动生成法定指标或拆改留决定 |
| L3 五类责任智能体 | 证据、空间、共同作者、运营、评效智能体各自检索、比较、提示和生成草案 | 五类责任人逐项复核，不允许一个总智能体越权放行 | 冲突和低置信度进入人工门，不向公众伪装为确定答案 |
| L4 人在回路与空间接口 | 红队、人工接管、实体/数字双通道、分级暴露与限时许可 | 一线人员、独立评审、无障碍共同测试者 | 无人接管、无拒绝路径或高风险未关即停机 |
| L5 证据闭环 | 时间戳日志、模型/数据卡、投诉、修复、能耗、回滚和A0/A1/A2比较 | 运维、审计与政府项目责任人 | 无法解释公共增量或无法回滚，不续期 |

AI只按“公共增量”获得续期：如果空间和人工服务工作正常，但AI没有相对正常服务的可验证改善，或增加排斥、错误和一线负担，就保留空间与人工服务、缩减或撤除AI；只有公共结果改善且AI增量成立，才允许扩展，并在每个新场景重新建立A0基线。[source:UK-AI-IMPACT-EVALUATION-2026]

![AI技术、空间与公共价值的可审查架构](assets/figures/ai-spatial-stack.png)
...
### 1. 文化生产如何进入公共价值，而不是再增加一套概念

文化生产不再单列成一笔可以自我加分的“文化账”，而是贯穿1+4+3验收：它在V1决定空间是否真的支持日常制作、观看、修改和安静退出；在V2确认事实、作者、许可、报酬和跨场景复用；在V3确保技术解释、人工服务和拒绝权；在V4确认内容更新、维护和退出后谁继续负责。[source:AUTHOR-CULTURE-METHOD]

每份城市委托后台仍保存公共价值、文化真实性、权利来源、安全环境、运营回投五类原始记录，以兼容审计与机器检查，但它们只是证据字段，不再成为公众需要记忆的第五套口号。[metric:five_ledger_count] 公众只需理解一句话：**在真实空间中共同生产，并由真实使用、独立签字和十二个月复验决定是否留下。**
...
上述五维复盘是A2使用复验的分析维度，不与1+4+3另行并列计分；公共结果与AI增量始终分开报告。
...
风险控制不靠一句“合规”兜底，而以四条可核查边界执行：资料只使用公开或已获授权的内容，公开状态不明时停止引用；个人信息坚持最小必要、目的限定和可撤回，隐私数据不进入公开展示；文化内容逐项记录来源、作者、版权与许可，AI生成内容不得伪装为史实；涉及公共服务和空间部署的模型输出必须经过人工复核、专业合规审查和责任主体确认。实施风险、数据风险、权利风险与财政风险分别进入转化责任链和1+4+3验收，不以技术团队自评替代独立复核。[source:SOURCE-REGISTRY] [standard:PROJECT-OFFICIAL-ANNOUNCEMENT] [depth:risk_missing_data]
...
本版新增的转化责任链、五道门、三级准入、1+4+3公共价值验收、AI可审查技术栈、公共保障、资金渠道、样板节点和交付包，均进入结构化证据；正文只保留与判断直接相关的代表性锚点。[metric:conversion_accountability_chain_schema_count] [metric:first_100_day_action_count] [metric:delivery_package_count] 完整定义、公式、来源文件、置信度和假设统一保存在 `metrics.json`，避免让机器索引干扰专业阅读。
