---
title: "Jing-Zhang Civic Spine: An AI Civic Innovation Corridor on a Century-Old Railway Heritage Axis"
author_github: "awaxiaoyu"
language: "en"
...
### v0.6 One-Page Scoring Evidence Table

| AI Review Dimension | Direct v0.6 repair | Visible evidence entry | Cannot prove |
| --- | --- | --- | --- |
| Brief alignment | agent.1-agent.6, the three scope levels, three key areas and three-area/two-wing loop are compressed into 10 visible claims linked back to proposal, metrics, GeoJSON, visual pages and self-check | `civic-spine-claim-provenance-audit.json`; `compliance_matrix.json`; `taskbook-delivery-checklist.json` | official selection or implementation authorization |
| Originality | Jing-Zhang Civic Spine becomes a civic operating system with claim provenance and delivery audit, not only a naming concept | `civic-spine-claim-provenance.json`; `score-repair-map.json`; six spatial prototypes | adoption by the organizer |
...
| Expression completeness | CN/EN proposal, report HTML, visual HTML, manifest, sources and persisted audits all expose the same v0.6 high-water evidence | `proposal.md` / `proposal.en.md`; `report/proposal.html`; `visual/index.html` | gallery publication or final score |
...
![Source and evidence chain](assets/figures/site-overview.en.png)
...
![Mobility, rail, blue-green and public-space system](assets/figures/mobility-bluegreen.en.png)
...
**Open-issue check on 2026-08-16.** Issue #846 and the merged provisional-boundary basis note add three public-background cautions: the OSM-mapped Jing-Zhang Railway Heritage Park does not intersect `PROV-SITE-001` and is about 412.5 m away at its nearest point; a peer reproduction flags a roughly 2 km north-south anchor discrepancy between provisional `PROV-KEY-003` and the Dazhongsi station / Jueshengsi anchor; and the four announcement-named street centerlines sit about 533-898 m west of the current `PROV-SITE-001` edge under one OSM-derived comparison [source:ISSUE-846-PROVISIONAL-GEOMETRY-CHECK] [source:PROVISIONAL-BOUNDARIES-BASIS]. This package treats those readings only as risk triggers, not as official polygon evidence. The Dazhongsi content should therefore be read as a station-service and adoption-return operating-section proxy; once official key-area polygons, road-redline data, or cleared anchor coordinates arrive, station frontage, four-quadrant walkability, green-space reuse, phasing, and the three core metrics must be relocated, recomputed, and redrawn together.
