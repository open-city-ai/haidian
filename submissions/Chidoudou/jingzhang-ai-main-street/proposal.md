...
summary: "以京张遗址公园为公众可叫停的首发试验线，用一张上街票、三次空间盖章与七项首用权补齐AI成果上街的最后500米；v2.4以大钟寺现场证据把泛化店前市集纠正为A口—1733—D口首用换乘环。"
...
合成、非识别性角色“林岚”是一位大钟寺通勤者、照护者与小组织协作者。她希望把13号线A口—1733—12号线D口的多语换乘和无障碍到达信息说清楚，但不上传乘客身份。07:30，她先在京张遗址公园使用无 App 导向、实体地图、遮荫座椅与人工求助；09:30，问题到众智园，只有测试范围、实体急停、近失记录与撤场线清楚才进入 `PROVE`；12:30，在 AI 原点社区，只有她能用普通语言复述用途、局限、人工责任和撤回方法才获得 `TRANSLATE`；16:30，在大钟寺，A—1733—D须先靠实体图和人工服务走通，AI才可作为自愿、限时、可关闭的侧层完成 `FIRST USE`；20:00，成功、暂停和失败以同一张结果卡回到公共账本。[metric:journey_stage_count] [data:geometry/public_space.geojson#SCN-03]
...
## 现场证据叫停了旧假设：大钟寺从“店前市集”变为“首用换乘环”

2026年8月13日18:07—19:44，人工贡献者从13号线大钟寺站A口方向出发，经地面路径、1733地面/B1/B2、12号线D口，再到北三环西路桥下，从公共视点采集30张Live Photo与2段短视频。v2.4只提交4张重新编码、移除EXIF和动态负载的静帧；不提交原始定位轨迹或视频。[source:USER-FIELD-DAZHONGSI-20260813]

这次观察使方案执行了自己的“暂停—纠正—恢复”规则：现场并不是旧版假设中的连续街边小店，而是由高架13号线、地面步行、1733多层商业/景观界面、12号线D口和桥下非机动车边缘组成的垂直换乘系统。因此叫停“大钟寺店前首用市集”，改为“A口—1733—D口首用换乘环”。官方任务书恰好要求优化大钟寺站一体化、四象限步行连通和非机动车停放；现状证据让这一要求从泛化口号变成可被证伪的第一段到达。[standard:PROJECT-OFFICIAL-ANNOUNCEMENT] [source:OFFICIAL-BEIJING-SUBWAY-L13-DAZHONGSI]

| 现场所见 | 对设计的直接修改 | 尚不能推出什么 |
| --- | --- | --- |
| A口邻近地面路径同时出现高架站体、施工/围挡、台阶和指向12号线的导向 | C1实体状态、C2人工问路和经核验的无障碍替代先于App建议 | 不能仅凭照片判断施工边界、通道权属或无障碍合规 |
| 1733地面、B1、B2与D口形成多层公共商业界面 | 首用从“临时市集”改为“普通换乘+既有商业嵌入”，运营和维护成为第一责任 | 不能由一次傍晚观察推断全天客流、商业经营规律或因果 |
...
![大钟寺现场证据：一次观察如何纠正空间原型](assets/figures/dazhongsi-field-evidence.png)
...
空间骨架不只是一条南北线：京张铁路遗址的连续公共底板负责“看见与到达”，东西缝合负责把高校、园区、社区和轨道方向的真实问题送到主线，公共首层负责把研发界面变成可进入的花园、街屋、廊院与多层换乘商业，蓝绿和维护界面负责让遮荫、雨洪、座椅、检修与退场同时成立。三个首发场不是同一模块换名，而分别固定为“验证环、转译廊院、首用换乘环”三种空间原型。[data:geometry/public_space.geojson#SCN-04] [depth:overall_spatial_structure]
...
大钟寺承担“第一次真实到达和采用”。现场证据把重点从假想店前转到真实的A口—地面—1733—D口多层换乘界面：乘客、照护者、商户/运营者、配送骑手和维护人员共享同一段到达压力。四类首用接口相应改为换乘/无障碍说明、内容与多语服务、既有商业运营、社区维修与维护；小组织仍可带着脱敏问题进入首用链，但不能占用盲道、楼梯口、通勤主线或把数字服务变成换乘前提。是否使用1733或其他存量载体，需建议对接公开更新统筹主体、轨道运营方与场地管理方并纳入项目库、公益—经营平衡和近中远期审查。[data:geometry/key_areas.geojson#PROV-KEY-003] [source:USER-FIELD-DAZHONGSI-20260813]

代表性100米概念序列按“A口到达—地面连续—1733地面/B1/B2—D口换乘—桥下边缘”组织：C1显示线路/通道运营状态和非AI入口，C2提供人工问路与纸面回执，C3实体图给出普通路线和经复核的无障碍替代，C4只作为不占主线的可逆多语/到达试用口袋，C5/C6负责关闭、维护与恢复普通换乘。AI关闭后，实体导向、连续通行、等候、既有商业与人工服务继续。实体导向不可用、推荐未经核验的无障碍路线、占用盲道/楼梯口/通勤主线、持续身份追踪、非机动车冲突无法隔离或没有维护责任人，任一项都不得盖 `FIRST USE` 章。[data:geometry/key_areas.geojson#PROV-KEY-003] [source:OFFICIAL-DAZHONGSI-RENEWAL]

![大钟寺首用换乘环：A口—1733—D口概念序列、剖面与四态](assets/figures/prototype-dazhongsi.png)
...
十项概念更新项目形成“空间小投入、制度先到位”的组合：1 上街主线连续性审计；2 上街票协议与公共状态板；3 众智园公开验证庭；4 原点社区成果转译窗；5 大钟寺首用换乘环；6 十二站统一服务组件；7 无障碍同行最小试验；8 小组织首用最小试验；9 具身智能共路最小试验；10 可纠错贡献里程碑与运维、申诉、退场公开账本。[metric:renewal_project_count] 每项都须在现状、权属、文保、交通、市政、运营授权和公众影响核验后才能进入专业深化。[depth:renewal_project_list]
...
| G3 大钟寺换乘与存量更新 | 13/12号线跨线运营状态、A—1733—D分时路线、连续无障碍、非机动车/公交冲突、场地权属消防、1733运营维护、更新项目库接口 | 哪段可形成普通换乘基线、首用口袋放在哪里、谁可被授权维护、何时退出 | 只做实体导向与人工问路；不发布未经核验的无障碍路线，不作站城工程、拆改留、招商或投资判断 |
...
| SP-04 大钟寺首用换乘环 | A—1733—D实体图、人工台、运营状态板、可逆多语/无障碍建议口袋 | 轨道/场地/片区可被授权运营者 / 使用者、无障碍、交通、消防、维护和数据专业方 | 普通路线或人工服务不成立、无障碍路线未核验、占主线或无维护人即停止 | 到达完成、人工纠正、错误路线、暂停回执、撤场恢复 |
...
90天只是一段概念决策序列，不是获批工期、预算、采购或真实试点；具体责任人、点位、尺寸与时段均待授权和专业复核。其价值是让专业团队知道“第一天做什么、什么证据才进入下一步、失败后交还什么”；结构化决策门随包保存在 `visual/assets/dazhongsi-90-day-gates.json`。
...
本包的指标分三类。第一类是从提交几何复算的内部一致性指标：临时 site area、绿地和公共空间面积/比例、概念建筑基底、慢行线长、三处重点区与三期面积；第二类是计数指标：12 个场景节点、10 项概念更新项目、3 处重点区、6 个全球案例、3 次街票盖章、7 项首用权、3 张最小试验单与 4 项通过的自检闸门 [metric:validation_gate_pass_count]。v2.4延续3处100米概念窗口 [metric:prototype_study_count]、6类可逆构件 [metric:reversible_component_type_count]、5段公共旅程 [metric:journey_stage_count]、4类共评角色 [metric:public_co_review_role_count]、4张证据卡 [metric:public_evidence_card_count]和3个交付窗口 [metric:delivery_horizon_count]，并新增大钟寺4张清权现场静帧与5段90天证伪门；后两者是材料/阶段计数，不冒充城市绩效。第三类是必须保持待正式数据补齐的法定或工程指标：总建筑面积、容积率、建筑高度和道路面积率。指标的 status、value、unit、source_files、formula、confidence 和 assumptions 全部记录于 metrics.json。[depth:metrics_recalculation]
...
版权方面，核心空间图由本包 GeoJSON、metrics、矩阵、`visual/assets/spatial-prototypes.json` 与现场证据数据通过本地确定性生成流程重建；双语源数据、图件和方法说明随投稿包开放以便复核。原创 Logo 与街票协议以 SVG 提交。两张氛围图仍由 OpenAI 内置图像生成工具制作，只表达众智园与原点的概念体验；大钟寺主视觉和封面改用人工贡献者在公共视点拍摄、重新编码、移除EXIF/动态负载的清权静帧，单次观察不作为长期客流或工程证据。使用 Microsoft YaHei 仅用于本地排版生成，不在仓库分发字体文件；没有使用商业地图瓦片、新闻图片、企业 Logo、可识别人物肖像或第三方论文图像。外部案例文字以官方机构页面为背景来源，仅做简短机制概括。投稿知识产权与展示使用遵循公告和 `COMMUNITY-DISPLAY-ONLY` 声明，详细见 `report/copyright_statement.md`。
