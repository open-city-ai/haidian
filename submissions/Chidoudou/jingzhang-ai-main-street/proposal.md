...
| 城市设计在哪里 | 一条上街线、三座首发场、两翼服务网、十二个街角站；每处都有“日常公共基座 + 可逆 AI 层” | 五张核心图、九类 GeoJSON、三处重点区章节 |
| 产业如何转化 | 以“问题库—验证场—转译台—首用窗—责任账本”连接全栈验证、科研转化、智能体/内容消费/智能终端与两翼服务 | 三区两翼产业接口、12 张场景卡、3 张试验单 |
| 首期交付什么 | 8 个 SP 首用行动包；先做状态板、人工窗口、慢行/无障碍和三张可证伪试验单，再由证据决定扩展或退场 | 行动包表、`visual/assets/pilot-matrix.json` |
...
“首用街角剖面”从公共侧到研发侧依次为：连续无障碍通行带、日常树荫/座椅基座、可读状态板与人工安全员窗口、可逆测试口袋、后场急停与检修接口。AI 设备全部撤走后，前两层仍是一处完整的花园型公共空间；没有安全员、急停演练和冲突观察数据时，验证环不得向公共慢行主线扩大。
...
其街角剖面由无 App 咨询台、公开模型卡架、可预约的小样本共测桌、隔离的敏感信息处理空间和可直接返回普通社区服务的出口组成。公共基座是人才/居民都可使用的咨询、休息、社区活动和无障碍院落；AI 层只占可逆工位与限定时段。任何成果若不能用普通语言说明用途、局限、人工责任与撤回方法，就不能获得 `TRANSLATE` 章。
...
其街角剖面由连续店前步行带、可坐可等的日常公共基座、纸面/人工需求拆解台、限时 AI 试用桌和后场商户确认/撤回窗口组成。试用桌撤走后仍是一处可经营、可会面、可维修的普通首层；没有经营者最终确认、数据删除回执和非 AI 经营模板时，不得获得 `FIRST USE` 章。
...
六个测试验证场景采用统一门禁：测试前公开目标、范围、责任人、非 AI 方案与停止阈值；测试中记录误报、漏报、接管和近失事件；测试后由受影响用户与专业人员共同评估，未达公共价值或运维能力即退场。场景不使用秘密地图、企业内部数据或个人隐私，不锁定供应商，不把原型写成已可全面部署。
