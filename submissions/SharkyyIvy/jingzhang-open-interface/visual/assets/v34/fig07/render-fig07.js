...
const renderer = path.join(sourceDir, 'fig07-bluegreen-public-realm.svg');
...
if (!edge) throw new Error('Microsoft Edge is required to render canonical FIG-07.');
if (!fs.existsSync(renderer)) throw new Error(`Canonical FIG-07 renderer is missing: ${renderer}`);
...
function waitForStableFile(file) {
  ...
  if (stableSamples < 3) throw new Error(`Timed out waiting for FIG-07 render: ${file}`);
}
...
function assertPngDimensions(file) {
  ...
  if (width !== 2400 || height !== 1500) throw new Error(`FIG-07 must be 2400x1500; got ${width}x${height}.`);
}
...
const zhOutput = renderLocale('zh', 'fig-07.png');
const enOutput = renderLocale('en', 'fig-07.en.png');
const zhAliases = ['fig-07.zh.png'];
const enAliases = ['fig-07.zh.en.png'];
...
if (zhHash === enHash) throw new Error('FIG-07 ZH and EN renders must be independently localized.');

console.log(JSON.stringify({
  renderer,
  width: 2400,
  height: 1500,
  zh: { sha256: zhHash, outputs: ['fig-07.png', ...zhAliases] },
  en: { sha256: enHash, outputs: ['fig-07.en.png', ...enAliases] },
}, null, 2));
