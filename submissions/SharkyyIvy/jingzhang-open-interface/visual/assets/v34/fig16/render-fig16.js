...
const renderer = path.join(sourceDir, 'fig16-metrics-evidence.svg');
...
if (!edge) throw new Error('Microsoft Edge is required to render canonical FIG-16.');
if (!fs.existsSync(renderer)) throw new Error(`Canonical FIG-16 renderer is missing: ${renderer}`);
...
function waitForStableFile(file) {
  ...
  if (stableSamples < 3) throw new Error(`Timed out waiting for FIG-16 render: ${file}`);
}
...
function assertPngDimensions(file) {
  ...
  if (width !== 2400 || height !== 1500) throw new Error(`FIG-16 must be 2400x1500; got ${width}x${height}.`);
}
...
const zhOutput = renderLocale('zh', 'metrics-evidence.png');
const enOutput = renderLocale('en', 'metrics-evidence.en.png');
const zhAliases = ['fig-16.png', 'fig-16.zh.png'];
const enAliases = ['fig-16.en.png', 'fig-16.zh.en.png'];
...
if (zhHash === enHash) throw new Error('Canonical FIG-16 ZH and EN renders must be genuinely localized.');

console.log(JSON.stringify({
  renderer,
  width: 2400,
  height: 1500,
  zh: { sha256: zhHash, outputs: ['metrics-evidence.png', ...zhAliases] },
  en: { sha256: enHash, outputs: ['metrics-evidence.en.png', ...enAliases] },
}, null, 2));
