...
const renderer = path.join(sourceDir, 'fig06-mobility-bluegreen.svg');
...
if (!edge) throw new Error('Microsoft Edge is required to render canonical FIG-06.');
if (!fs.existsSync(renderer)) throw new Error(`Canonical FIG-06 renderer is missing: ${renderer}`);
...
function waitForStableFile(file) {
  ...
  if (stableSamples < 3) throw new Error(`Timed out waiting for FIG-06 render: ${file}`);
}
...
function assertPngDimensions(file) {
  ...
  if (width !== 2400 || height !== 1500) {
    throw new Error(`Canonical FIG-06 must be 2400x1500; got ${width}x${height}.`);
  }
}
...
const zhOutput = renderLocale('zh', 'mobility-bluegreen.png');
const enOutput = renderLocale('en', 'mobility-bluegreen.en.png');
const zhAliases = ['fig-06.png', 'fig-06.zh.png'];
const enAliases = ['fig-06.en.png', 'fig-06.zh.en.png'];
...
if (zhHash === enHash) {
  throw new Error('Canonical FIG-06 ZH and EN renders must be genuinely localized.');
}
...
console.log(JSON.stringify({
  ...
  width: 2400,
  height: 1500,
  zh: { sha256: zhHash, outputs: ['mobility-bluegreen.png', ...zhAliases] },
  en: { sha256: enHash, outputs: ['mobility-bluegreen.en.png', ...enAliases] },
}, null, 2));
