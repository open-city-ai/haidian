...
![Six-task delivery spine: concept, ecosystem, twelve scenario cards, public-space culture and annual operation](assets/figures/taskbook-spine.en.png)
...
![Twelve scenario cards: three deep validations and nine ordinary-space or NO-BUILD outcomes, each with human authority and stop/reset](assets/figures/scenario-contracts.en.png)
...
![Jing-Zhang spatial judgment and conditional regional interface with scale, north, provisional warning, three differentiated key areas and five evidence-feedback rows](assets/figures/site-overview.en.png)
...
![Land use, action patches, and urban stitching](assets/figures/land-use-structure.en.png)
...
### Implementation Closure: Four Reviewer Questions

**Q1 | Phasing: what is the path?** `G0-G4` routes evidence and decisions through participant design freeze, G1 professional/input verification, carrier and role readiness, stop-reset rehearsal, and a future bounded release or ordinary-baseline closeout. The more exact `R0-R6` model says only `R0` design-package completion, `R1` desk verification, and `R2` professional-review-package readiness are current; `R3-R6` have not started and remain HOLD. Any failure returns to lower-tech/ordinary space or NO-BUILD; it never bypasses a gate. [data:visual/assets/implementation-design-freeze.json#release_state_model]

**Q2 | Pilot: what is frozen?** The frozen item is an S01 zero-coordinate, carrier-neutral reference typology - not a selected site: `Z0` ordinary route -> `Z1` human handoff -> `Z2` bounded test -> `Z4` booked observation; `Z3` keeps operation and safety separate, `Z5` carries stop, isolation, removal and reset, and the professional buffer in `Z6` remains `null`. Ordinary city/NO-BUILD is the default and a lower-tech reversible solution is the first response; P0 can be considered only after genuine physical necessity survives future admission. [data:visual/assets/implementation-design-freeze.json#canonical_s01_participant_reference_topology] [data:visual/assets/implementation-design-freeze.json#authoritative_alternative_hierarchy]

**Q3 | Actors: who participates and decides?** The participant has frozen **role types** and separation: operator, independent human safety authority, public/service handoff steward, reset custodian and independent witness. G0 confirms participant design only; future G1-G4 require applicable professional, rights/carrier, authority, independent-witness and human-stop receipts that are current and scope-matched. No real person, institution or operator is appointed by this submission. [data:visual/assets/implementation-design-freeze.json#gate_authority_topology] [data:visual/assets/implementation-design-freeze.json#role_phase_pattern]

**Q4 | Measurable indicators: how will performance be measured?** Eight compact D0 methods define future package completeness, S01 route/stop/reset, S04 handoff, S07 appeal, evidence freshness and alternative decisions. `A` accepts only participant-decidable design completeness now; `B` professional, `C` field and `D` authority release all remain HOLD. Every method's actual sample and actual value are `null`, with missingness, failure, escalation, expiry and reset as hard-stop semantics rather than invented performance results. [data:visual/assets/implementation-design-freeze.json#d0_method_book] [data:visual/assets/implementation-design-freeze.json#acceptance_hierarchy]

**Truth boundary.** Official geometry, real carrier rights, appointments, professional findings, field observations/results, quotations, approved budget/funding and activation remain future external inputs and are not claimed here. The seven #4344 data gaps and six conditional follow-ups are now explicit receive, rebind, invalidation and fallback contracts. The current readable verdict is only: **PARTICIPANT REFERENCE DESIGN FROZEN -> PARTICIPANT METHOD DESIGN COMPLETE -> READY FOR G1 / PROFESSIONAL VERIFICATION -> EXTERNAL RELEASES HOLD -> NO PROCUREMENT OR FIELD ACTIVATION.** `P0_GO=false`. [data:visual/assets/implementation-external-input-contract.json#reviewer_gaps] [data:visual/assets/implementation-external-input-contract.json#conditional_followups]
...
![Mobility, slow traffic, services, and blue-green network](assets/figures/mobility-bluegreen.en.png)
