...
本方案建议的总体概念为“京张智脉·开放实验带（JingZhang Open Lab Belt）”：以京张遗址公园为历史与公共空间主轴，以众智园、北京AI原点社区、大钟寺三处重点片区为创新锚点，以高校、企业、社区和轨道站点为日常网络，形成“一带三核、多点场景、蓝绿慢行复合环”的空间组织。它把城市理解为可被居民、开发者、专业团队和公共部门共同提出问题、制作原型、测试、人工复核与迭代的开放实验室，而不是把居民变成被动的数据来源。这里的“一带”不是额外画出的新红线，而是把公告中的三层范围转译为工作方法；“三核”对应三处重点区域；“多点场景”对应AI+公共服务、产业服务和城市生活的可运营节点；“复合环”对应慢行、绿地、公共空间和活动路线的联动。
...
| 11 城市耳朵与城市回应 | 座椅、路灯、垃圾分类点和社区服务界面 | 可选择使用的语言交互、问题提交与人工审核闭环；不采集连续轨迹或人脸数据 |
| 12 Transformer 公园学习路径 | 京张遗址公园活力带 | 以输入、注意力、层、人工复核、输出组织无障碍体验路径，把模型知识转化为可讨论的公共学习体验 |
| 13 动能—推理 Token 可视化站 | 运动单车、步行与康复设施周边 | 将自愿运动微发电、储能/能耗监测与公共语言模型输入/输出 token 估算并列展示；换算公开假设，居民可自愿匿名或署名参与贡献榜，绝不作为公共服务门槛 |

### 公共交互微节点：把建议送回城市

作为“开放实验室”的第一组可落地原型，本方案提出“城市耳朵与城市回应”微节点网络：在座椅旁、路灯杆旁、垃圾分类点、运动设施和社区服务界面设置可选择使用的本地交互终端。终端使用语言模型提供导览、无障碍提示、公共服务指引、设施报修线索和居民建议收集；每条建议必须显示用途、保存期限、是否匿名、人工处理责任人与可撤回入口。它不采集连续个人轨迹、不进行人脸识别、不把对话直接用于商业画像，也不把模型回答视为公共政策或紧急服务指令。经脱敏与人工复核后，聚合的问题可进入“城市实验议题板”，由居民、运营者和专业团队共同评估是否形成下一轮小规模测试。这是一项概念建议，需由隐私、网络安全、无障碍、设施运维和公共管理团队深化 [source:AGENT-TASKBOOK] [data:geometry/public_space.geojson#PUBLIC-001]。

### 动能—推理 Token 可视化：让运动、能源与公共 AI 对话形成可解释链路

用户提出的“运动发 token”在本方案中不是加密货币、积分或公共资源兑换凭证，而是对**大语言模型推理 token** 的可解释性表达：居民自愿使用运动单车等微发电设施所产生的电能，经由可计量的本地储能与能耗监测，支持公共语言交互节点的部分真实用电；系统将当日可核验的发电量、设备效率、储能损耗与语言模型实际输入/输出 token 量并列展示，说明“多少瓦时支持了多少次对话或多少 token 的推理能耗”。两者绝不被伪装为一一等值关系：模型 token 是计算量/文本单元，电能是物理量，换算必须公开模型、设备、时间窗、PUE/网络/边缘设备等假设，并标注为教育性、运行性估算而非固定承诺。

“动能供给 AI”是一个解释型公共智能体：它向居民说明某一次运动、一次语言交互和一次模型推理之间的真实能源边界，展示输入 token、输出 token、推理耗能估算与设备状态，并提示公共交互系统仍依赖安全、稳定的市政或合法清洁电力。运动发电不能替代城市供电、也不应成为获得公共服务的门槛。可设置**自愿、默认匿名、可退出**的“城市 Token 贡献榜”：只有居民主动选择署名或昵称后才展示累计的经核验发电量及其对应的“支持推理 token 估算量”；不显示健康数据、不做强制排名、不与公共服务、信用、就业或商业画像挂钩，并提供不参与排名但仍可使用设施的同等路径。排行榜的目的只是让每位居民看见自己与城市公共 AI 对话之间的可解释贡献，而不是把身体活动转化为竞争性数据资产。这一机制属于概念原型，须经电气安全、健康安全、隐私、无障碍、网络安全、算法能耗计量和公共管理专业审查后再考虑试点。

### Transformer 公园：步行完成“一句话进入模型、生成一句话”的可体验计算旅程

本方案提出“Transformer 公园”作为京张遗址公园活力带的概念性 AI 朝圣地标与公共学习空间。它的空间叙事不是笼统地介绍 AI，而是让居民从入口到出口，以一条游览路线**模拟一句自然语言输入进入大模型并逐步生成输出的中间计算过程**。公园不复制某一企业模型、训练数据或商标；它以可公开讲解的通用 Transformer 原理为基础，并把每一步明确标为简化教学隐喻，而非真实模型内部的等比例复刻。

体验路径可由六段组成：

1. **一句话入口／分词广场**：访客输入或选择一句关于城市的问题，例如“怎样让夜间步行更安心？”；地面文字单元把句子拆成可移动的词元，说明模型首先处理 token 序列。
2. **位置花园／编码步道**：不同位置、方向和节奏的种植、铺装或光带提示相同词出现在不同位置会产生不同语义关系，解释位置编码。
3. **注意力廊道**：多条可视化连线连接句中关键词；访客选择关注对象，看到“夜间”“步行”“安心”等词如何建立不同权重关系，理解 self-attention 并非简单关键词搜索。
4. **多头观察台与层叠坡道**：若干平行的小型观察框展示不同“注意力头”从安全、无障碍、照明、活动、环境等角度读取同一句话；随后经过层叠坡道，说明信息会经多层变换和残差连接不断更新，而不是一次得出答案。
5. **人工复核台**：输出前必须经过可讨论的事实、隐私、偏见与公共安全提示；它强调大模型生成并不等于真相、规划结论或自动执行命令，人类需要判断、补充证据并承担责任。
6. **逐词生成剧场／输出广场**：公共屏幕以“下一个 token”的节奏生成一个带不确定性提示的建议草案；访客可比较不同提示语产生的差异，并把经过人工审核的城市问题送往开放实验议题板。

在这条路径中，居民不是被系统追踪或评分的 token；居民是有知情、同意、暂停、修改和退出权利的学习者与共同研究者。token 仅是帮助理解文本计算的教学角色，绝不对应真实身份、信用评价或算法排序。公园应提供无障碍绕行、低刺激模式、非屏幕讲解、儿童与长者版本，并且不以输入内容训练模型或保存个人对话，除非获得明确、可撤回的同意。

Transformer 公园与三处重点区形成三类地标协作：众智园的“可解释性工坊”面向开发者与治理讨论，北京 AI 原点社区的“开源记忆台”承载匿名、可撤回的城市问题与贡献归档，大钟寺的“输出客厅”用于展示经过人工审核的原型、社区反馈和国际交流。三者共同构成“输入—理解—反馈”的公共学习回路，同时保持京张文化、中关村创新文化和 AI 新文化的连续叙事。具体选址、设施形态、工程安全、文保影响及接入条件均待专业团队与主管部门核实，不构成建设承诺 [source:AGENT-TASKBOOK] [depth:blue_green_public_space] [data:geometry/green_space.geojson#GREEN-001]。
