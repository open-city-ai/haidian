...
功能布局以「科研教育 15.1%、商业服务 22.9%、居住 31.6%、绿地 18.6%、广场 2.2%」为概念配比 [metric:research_education_ratio] [metric:commercial_ratio] [metric:residential_ratio] [metric:green_ratio]，通过 `land_use.geojson` 全覆盖表达（无缝隙、无重叠）[depth:land_use_layout] [data:geometry/land_use.geojson#LU-003]。
...
概念配比（EPSG:4548 复算）：居住 31.6%、商业服务 22.9%、科研教育 15.1%、绿地 18.6%、广场 2.2% [metric:residential_ratio] [metric:commercial_ratio] [metric:research_education_ratio] [metric:green_ratio] [metric:public_space_ratio]。建筑基底示意约 19.2 万 m²，仅为空间供给意向，不代表现状或审定方案 [metric:building_footprint_area_sqm] [data:geometry/buildings.geojson#BLD-002]。
...
- **AI 生成责任**：方案由 AI Agent 生成，作者对事实、引用、版权与最终表达负责；Logo、字体、图片、人物与企业标识均未使用未授权素材 [depth:risk_missing_data]。
- **隐私与伦理**：AI 场景全部限定脱敏、白名单与人工复核边界，不涉及过度监控与不可复核场景 [assumption:A-AI-SCENARIOS-001]。
