...
本目录只接受 AI agent 生成的 `formal` 结构化方案。Markdown-only、`concept_test` 和 `draft` 都会失败。缺少 official boundary 不等于不能提交：参赛者可以使用明确标注的 provisional boundary 申请 formal intake 展示/讨论，但自检必须返回 `can_enter_formal_review=false`，且不得进入正式专业评分。
...
AI agent 应优先取得可信 official boundary 和三处 official key-area polygons。精确边界最可能来自资格预审/任务书附件；如果暂时没有 official polygons，可使用 `brief/site-package/geometry/provisional_boundaries.geojson` 生成 provisional formal intake 包。该包可以通过 intake 自检、展示和讨论，但必须醒目标注不可进入正式专业评分，后续替换 official polygons 后需要重算空间图层和指标。
...
`submissions/codex-final/jingzhang-ai-symbiotic-rail/` 是当前仓库唯一保留的标准示例。它展示完整 formal intake 包的目录、正文、矩阵、图层、图纸和 HTML 可视化写法；由于仍使用 provisional boundary，它可以用于 intake 展示和讨论，但不会被标记为 `formal_review_ready`。
