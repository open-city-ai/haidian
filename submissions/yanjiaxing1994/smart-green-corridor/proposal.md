...
证据引用：[source:OFFICIAL-ANNOUNCEMENT]、[source:AGENT-TASKBOOK] 及 [source:BOUNDARY-SOURCE]、[depth:existing_conditions_diagnosis]，并对照 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] 组织。
...
**核心机制——"微更新→强链接→运营激活"闭环。** 本方案不是一次性的空间改造，而是一套可长期运行的城市更新机制，由三步构成首尾相连的闭环：

1. **微更新（低成本低风险介入）**：以首层灰空间、慢行贯通、口袋设施和轻量单层构筑物为介入手段（19项更新项目全部为保留基础上的低干预改造，见§7.1-7.3），规避大拆大建的高资金、高审批与高社会成本。
2. **强链接（以绿廊为骨架串联成网）**：以9公里京张绿廊"三道一绿"为物理骨架，把众智园、五道口、大钟寺三片区以及高校、园区、社区、轨道站点连成连续网络；再以AI场景数据流（体测、活动、客流、能耗）建立跨片区的"数字链接"，使三片区不再是孤立节点而是一个可运营的系统（对应RACI跨部门协同，§7.6）。
3. **运营激活（使用→反馈→迭代的正反馈）**：以月度≥8场活动、三片区联赛制度、开源社区和品牌IP让空间持续被使用（§年度活动体系与运营机制），以运营收入（政府60%/社会30%/校企10%）反哺维护（§7.7），以活动满意度与使用频次作为下一轮更新的输入参数，形成"使用-反馈-迭代"的自更新回路。

**差异化主张。** 区别于"大拆大建式更新"（高投入、长周期、高社会扰动）与"纯景观美化式更新"（不可运营、不可量化、一次投入后衰减），本方案主张**"轻资产、可运营、可量化的线性AI生态更新"**：其一，方案落在9公里铁路遗址公园这一**线性空间**上，把AI创新生态（策源-转化-开源-体验）沿绿廊展开，区别于单点园区或新建科技城的"点状/面状"更新，是全球少见的线性AI创新生态系统（与§全球AI创新区案例对比结论一致）；其二，所有空间介入均绑定运营机制与可衡量指标（成本框架、RACI、DPIA），可被专业团队继续深化、运营团队继续执行、评审者逐项核验，实现"概念-设计-运营"三阶段的连续可转化。

证据引用：[source:OFFICIAL-ANNOUNCEMENT]、[source:AGENT-TASKBOOK]，深度证据见 [depth:existing_conditions_diagnosis]、[depth:overall_spatial_structure]、[depth:implementation_feasibility]
...
| 类别 | 数量 | 说明 |
|------|------|------|
| **保留建筑** | 15栋 | 众智园5栋科研楼、清华科技园主楼群、智源大厦、东升大厦、大钟寺体育公园服务中心（含5处现状公园绿地） |
| **改造空间** | 8处 | 众智园首层灰空间优化（5栋）、清华科技园首层开放界面、东升大厦首层商业激活、大钟寺体育公园跑道优化 |
| **新增设施** | 11处 | 铁路记忆科创互动馆、开源社区发布厅、古钟文化驿站、青年活动草坪服务站、下沉广场青年中心、碗池滑板场服务亭等轻量构筑物（其中6处为独立建筑体量，5处为公共空间附属服务设施） |

![拆改留总图与三片区空间关系](assets/figures/site-overview.png)

### 1.3 数据依据与边界声明

本方案以北京市规划和自然资源委员会海淀分局发布的资格预审公告为第一依据。三处重点区的 polygon 均为 `provisional_constraint` (`official_boundary=false`)，仅用于方案生成与设计讨论。Official polygons 发布后，所有空间指标需复算 [source:BOUNDARY-SOURCE]。
...
方案品牌英文名定为 **"Jing-Zhang AI Greenway"**（简称 Jing-Zhang AI Greenway / 京张AI绿廊），品牌主标识中文名为"京张AI绿廊"。Greenway 传达京张铁路遗址公园9公里城市绿廊作为方案空间主轴的核心意象，同时涵盖"绿廊+AI节点网络+青年运动社交带"的多义叠加。"Nexus"为内部方案演进代号，不作为对外品牌名使用。
...
| **AI技术支撑** | 智能体测站采用计算机视觉姿态分析——分析人体骨骼关键点（关节点坐标）以计算跑姿对称性、步频和触地时间；该过程在设备端实时完成，不采集、不存储人脸图像，骨骼关键点数据在输出跑姿反馈后即时丢弃。⚠ 姿态分析虽然不进行人脸识别，但在法律上仍属于生物特征数据的处理——骨骼关键点轨迹在特定条件下可能与其他数据组合而具有识别性。体测站须在醒目位置张贴"正在运行计算机视觉姿态分析"的公示，并提供物理开关允许用户暂停分析（切换为仅提供基础计时功能）。跑道沿线动态配速屏以匿名方式显示当前时段跑者平均配速，激励参与；企业跑团匹配算法——根据企业位置和午间可用时段自动推荐最优集结时间和路线（不使用个人日程或位置数据） |
...
| **隐私边界** | Web AR无需下载app，仅请求相机权限用于图像Marker识别（浏览器标准权限弹窗，用户可拒绝）；不采集GPS位置数据（AR内容基于图像Marker识别触发）；语音导览为设备端Web Speech API合成（不传输用户语音至服务器、不记录用户提问）；导览使用统计仅记录"各触发点扫描次数"聚合数据（不关联设备或用户）。⚠ Web AR的相机权限为必要权限——用户在首次使用时会看到浏览器标准权限提示，可选择"允许"或"拒绝"。建议在导览入口处明示权限说明并提供不涉及摄像头的替代体验方案（实体图文解说牌+纸质导览手册）。 |
...
| **AI技术支撑** | 智能体测站配置分段计时芯片读取（选手佩戴轻量可复用RFID计时芯片，赛后回收消毒）；AI跑步姿态分析（同场景卡01技术栈——计算机视觉分析跑姿，不采集人脸）；夜光涂层为被动蓄光型材料（日间吸收光能、夜间被动释放，释放强度随时间衰减，不可主动调节）。阴天/深冬季日照不足时，由环境光传感器触发LED辅助照明自动补充（与蓄光涂层独立工作）。⚠ 蓄光材料本身不具备主动调节释放强度的能力——此为被动发光技术。；赛道LED环场灯带随领先者实时动态变色（基于最快圈速芯片数据，非个体识别） |
...
| **预期频次** | 每周三晚间计时赛（18:30-20:30，四季运营）；月度企业/社区接力赛（每月最后一个周五晚）；年度"北京城市夜跑联赛大钟寺站"（1次/年，建议申报纳入北京市全民健身赛事体系，须经属地体育主管部门审批）；元旦"跨年夜跑"特别活动（12月31日23:00-1月1日0:30） |
...
| **数据来源** | 活动注册数据（姓名/单位/邮箱——仅限活动组织使用，GDPR兼容）；AI导览使用统计（匿名聚合——各节点访问量、平均停留时长、推荐路线采纳率）；排队预测数据（蓝牙信标计数——统计探测到的设备数量；MAC地址在设备端哈希化后仅保留前8位用于去重计数，完整哈希不存储、不上传。⚠ MAC哈希化可降低但无法完全消除重识别风险——在人口密度较低的时段和区域，边缘情况仍可能存在。建议在导览入口提供"选择不参与蓝牙计数"的选项，并考虑采用纯红外线断面计数作为不依赖个人设备的替代方案）；各节点互动体验数据（本地处理，不保存用户输入） |
...
#### AI场景卡隐私与数据保护影响评估（DPIA Checklist）

10张AI场景卡均涉及个人数据处理或传感器数据采集。依据《中华人民共和国个人信息保护法》（2021）和GB/T 39335-2020《信息安全技术 个人信息安全影响评估指南》，逐项完成以下DPIA Checklist。下表为方案阶段自我声明，正式运营前须由数据保护官（DPO）或第三方合规机构独立审计。

| 场景卡 | 数据采集类型 | 是否涉及个人信息 | 法律依据 | 数据最小化措施 | 存储与保留 | 用户控制 | 人工替代方案 |
|--------|------------|----------------|---------|-------------|-----------|---------|------------|
| 01 AI跑步俱乐部 | 跑步姿态骨架坐标（非人脸）、配速/心率（用户自选手环共享）、跑步时段统计 | 骨架坐标为匿名生物特征（不采集人脸，不具识别能力）；心率数据若关联手环ID可能间接识别 | 用户主动开启手环共享=同意；匿名骨架坐标=不构成个人信息处理 | 姿态分析仅提取运动学参数（步频/步幅/触地时间），前端推理后即时丢弃视频帧，不上传云端 | 不存储原始视频帧；聚合统计数据（非个人级）保留至赛季末；个人运动数据仅在用户设备本地 | 用户可在体测站或小程序中随时删除个人运动记录；可关闭骨架分析功能（仅使用手环数据） | 不连接任何设备的跑者仍可使用跑道计时（RFID芯片仅在赛事中发放，赛后回收；日常训练不强制使用任何设备） |
| 02 黑客松 | 报名信息（姓名/机构/邮箱）、代码提交（公开仓库URL）、项目演示视频 | 是（姓名+联系方式） | 参赛者主动报名=同意；赛事公示结果=合法公开 | 仅收集必要报名信息（不允许要求身份证号）；代码演示为公开性质 | 报名数据赛后12个月删除（除非获奖项目需长期公示）；公示仅保留团队名和项目摘要 | 报名时明确告知数据用途；参赛者可选择匿名参赛（使用团队代号） | 参赛者可在现场以实物展板+口头讲解方式展示项目，不强制录制视频上传 |
| 03 屋顶运动联赛 | 运动影像（同01）、比赛成绩、团队信息 | 比赛成绩按团队记录，不直接关联个人身份信息 | 参赛者报名=同意 | 比赛期间仅在赛场范围内采集；联赛排名仅发布团队名+得分，不发布个人身体数据 | 单场比赛录像赛后48小时自动删除（除非涉及争议判罚须保留至仲裁结束）；赛季积分榜保留至下一赛季开始 | 参赛者可要求比赛影像仅供裁判回放使用，不公开发布 | 观众区独立于数据采集区，普通观众不进入摄像头覆盖范围 |
| 04 露天电影夜 | 观影人数统计（红外计数器，不采集图像）、观众投票（匿名） | 否（人数统计为完全匿名计数；投票无身份关联） | 不涉及个人信息，无需法律依据 | 红外线计数器仅输出"通过/未通过"脉冲信号，不记录任何图像或生物特征；投票为举手或扫码匿名提交 | 人数统计数据仅用于活动效果评估（每场总人数），不作个体追踪 | 不需要（无个人信息采集） | 完全线下参与 |
| 05 铁路记忆AR | 相机实时画面（用于AR Marker识别和SLAM定位）、设备型号和浏览器版本 | 是（相机权限触及敏感权限——Android/iOS均将相机归类为敏感权限） | 用户主动打开AR功能=知情同意；首次使用弹窗授权；14岁以下儿童须监护人同意 | AR渲染在手机端侧完成（WebAR），不将相机画面或SLAM特征点上传至服务器；Marker识别为本地模型匹配，仅向下加载AR内容渲染所需的3D模型文件（不含用户数据） | 相机画面即时处理且不上传、不存储；用户设备类型的匿名统计（Chrome/Safari + iOS/Android比例）保留用于兼容性优化 | 用户可随时关闭相机权限（在浏览器或系统设置中）；AR页面提供退出按钮 | 纸质导览手册（三折页图文并茂，在科创互动馆和绿廊入口免费取阅）；实体解说牌（每处遗存点设30cm×40cm双语解说牌，含历史照片和文字说明） |
| 06 宠物市集 | 市集摊位报名信息（姓名/联系方式）、宠物基本信息（品种/体型/疫苗证明） | 是（摊主联系方式+宠物疫苗记录） | 摊主报名=同意；疫苗证明=合法合规要求（动物防疫法） | 仅收集与市集运营和安全直接相关的信息；不收集宠物主人身份证号或宠物芯片号 | 报名数据活动结束后3个月删除（保留至活动结算和投诉处理完毕） | 摊主可选择不公开个人联系方式（通过市集组织方中转沟通） | 宠物自由玩耍区不使用任何数据采集设备（仅设围栏和饮水设施） |
| 07 极限运动联赛 | 比赛影像（用于裁判评分和直播）、参赛者报名信息、排名积分 | 是（参赛者身份+比赛影像） | 参赛者报名=同意；赛事公开转播为行业惯例且选手知情 | 比赛影像仅覆盖赛场区域（非赛场周边行人自动模糊）；裁判评分完成后原始多机位录像保留至仲裁期结束 | 比赛影像常规保留12个月（用于赛季回顾和争议仲裁）；选手可要求特定比赛场次的录像不公开发布 | 参赛者报名时选择是否同意比赛影像公开直播/发布（选择"不同意"的选手仅限线下参赛，其比赛视频仅供裁判评分使用，不公开） | 非参赛者可在现场观赛，观赛区域独立于数据采集区 |
| 08 夜光跑道计时赛 | RFID芯片计时数据、智能体测站骨架坐标（同01）、LED灯光颜色（基于最快圈速触发） | RFID芯片在赛事中关联参赛者编号（可间接识别）；骨架坐标匿名 | 参赛者佩戴RFID芯片=同意；骨架同场景卡01 | RFID芯片仅记录分段时间+芯片ID，不记录GPS位置；跑姿分析同场景卡01 | RFID芯片赛后由工作人员统一回收消毒；比赛成绩（选手名/芯片ID/时段）保留至赛季积分榜发布；非赛事日跑道不启动计时功能 | 参赛者可使用自编参赛号代替真名参赛；成绩发布时使用参赛号而非姓名 | 非赛事时段跑道为普通公共跑道，无计时和数据采集功能 |
| 09 古钟声音艺术节 | 现场录音（环境录音，非定向拾音）、观众人数统计（红外计数器）、互动装置操作日志 | 环境录音若偶然拾取到可辨识的个人对话，可能构成个人信息处理 | 观众购票/预约=同意；现场张贴录音告知标识；演出=公开表演（无隐私合理预期） | 环境录音采用全指向麦克风阵列，以声场重建为目的，不进行个体语音识别或声纹提取；红外计数器同场景卡04 | 环境录音原始素材保留12个月（用于艺术档案和年度回顾剪辑），发布版经人工剪辑（剔除可辨识的个体声音片段） | 观众可在指定"静音观演区"（远离麦克风阵列覆盖范围）享受演出 | 在演出区外围（静音观演区）或通过线上直播观看（全程不涉及录音采集） |
| 10 全球AI开放日 | 活动报名（姓名/机构/国籍/联系方式）、现场摄影/摄像（用于活动宣传和媒体报道）、人流统计（红外计数器+WiFi计数） | 是（报名身份信息+活动影像） | 报名=同意；活动现场张贴摄影摄像告知；新闻媒体报道=合法公开（新闻传播） | 人脸识别技术评估：WiFi Probe Request计数以MAC地址哈希值为基准（每15分钟随机旋转盐值，不存储原始MAC地址）。⚠ 注意：MAC地址哈希化虽降低直接可追溯性，但结合时空模式和辅助信息仍存在重识别风险——方案建议以红外线计数+人工目测计数为主要人流量统计手段，WiFi计数仅作为交叉验证参考。除WiFi计数外任一项数据采集均不涉及MAC地址。 | 报名数据保留至活动结算完成（活动后6个月）；活动影像公开发布前经人工筛选（不发布可辨识个人且未经同意的特写镜头） | 报名时提供"不同意公开发布的活动影像"选项（选择此项的参与者将获得不同颜色的胸牌，摄影师和摄像师将避免对其拍摄特写）；参与者在入口处可见摄影告知标识 | 部分开放日内容（主旨演讲、颁奖、成果发布）以线上直播为主，线上参与者完全不涉及线下数据采集；AI运动体验区的运动数据采集规则同场景卡01和03 |

**DPIA 总体自我声明**（方案阶段，待独立审计）：
- 本方案10张AI场景卡的数据处理符合"合法、正当、必要"原则（《个人信息保护法》第5-7条）
- ✅ 已落实数据最小化：仅在功能必需时采集，优先使用匿名/聚合/端侧处理
- ✅ 已落实目的限制：每项数据采集均绑定明确目的，不用于声称目的之外的用途
- ✅ 已落实存储限制：原始数据保留期限明确，到期自动删除
- ✅ 已落实用户控制：所有场景卡均提供opt-out机制和人工替代方案
- ⚠ 待审计确认：RFID、骨架坐标、MAC地址哈希和WebAR SLAM技术的实际隐私影响须由独立安全审计验证
- ⚠ 待补充：正式运营前须完成全部场景的完整DPIA报告（非仅Checklist），由DPO或第三方合规机构签署
- ⚠ 儿童保护：14岁以下儿童的AR功能须监护人设备端授权；儿童参与的所有AI交互场景（科创互动馆、草坪AI互动）不采集儿童生物特征信息
...
| **成功标准** | 单次评测周期覆盖1000+公众测试者（30天，建议目标）；模型安全性评分参照GB/T 42574-2023等国家标准方法论（方案建议参照，非已获认证）；评测流程和结果建议参照ISO/IEC 42001 AI管理体系框架进行流程设计（建议方向，非已获认证）；CNAS认可评测资质为远期建议目标，须独立申请和评审。运营模式：评测沙盒建议由海淀区AI企业预约使用（按次收费覆盖运营成本），评测报告可作为模型上线前合规自查参考（非监管部门认可的正式合规证明）。⚠ 以上认证、标准和资质均为方案建议方向，不构成已获得或将被授予的承诺。 |
...
| **数据治理** | 仅采集聚合统计数据：人流密度（红外线断面计数，不涉及个人设备标识）、平均移动速度（建议使用红外断面+视觉计数作为主要数据源；WiFi Probe Request 探测仅在获得属地主管部门许可且张贴公示后启用，探测数据在边缘节点即时聚合为计数级统计后丢弃原始报文——⚠ WiFi Probe Request 包含 MAC 地址等设备标识符，即使即时聚合仍涉及个人数据，须完成数据保护影响评估后方可启用。方案建议优先采用不依赖个人设备的红外断面+视觉方案）、骑行道占用率（视觉匿名计数，逐帧处理不存储视频）、共享单车周转率（站点级统计，不关联个人行程）；智能体测站健康数据仅在本地参与联邦学习（数据不出站，仅梯度信息参与模型更新，差分隐私保护ε<1.0）；导航推荐算法仅使用用户自选偏好（最快/最舒适/最安静/无障碍），不跟踪个体路径历史，不建立用户画像；实验周期内聚合数据保留7天仅用于算法验证，7天后按计划自动清除；实验全程在绿廊入口、体测站和导航桩张贴"正在进行交通优化算法测试"的公示 |
...
| 保留建筑数量 | 15栋（另保留5处现状公园绿地） | §1.2 拆改留清单 | high |
| 改造空间数量 | 8处（首层+界面） | §1.2 拆改留清单 | medium |
| 新增设施总数 | 11处（6栋独立建筑+5处附属设施） | §1.2 拆改留清单 | medium |
...
方案形成可审查的更新项目清单共**19项**，项目位置、类型、功能、责任主体、依赖条件和实施阶段在分期表中逐一说明 [depth:renewal_project_list] [depth:phasing_implementation]。分期空间证据为 [data:geometry/phasing.geojson#PHASE-1]。

**可实施性摘要（阶段×主体×指标）：**

| 阶段 | 启动项目（举例） | 主要参与主体 | 可衡量指标 |
|------|----------------|-------------|-----------|
| **近期（0-2年）** | 慢行全线贯通、3座滑板场、三片区宠物友好区、青年草坪、下沉广场青年中心（JZ-01~JZ-07） | 场地管理方、属地街道办、区体育局、专业设计机构、施工单位 | 9km慢行道贯通率100%、滑板场建成3座、成本2,800-4,700万元 |
| **中期（2-5年）** | 首层灰空间优化、科创互动馆、开源发布厅、古钟文化驿站、夜光跑道（JZ-08~JZ-14） | 区政府各部门（规划/体育/园林/文旅）、文物部门、专业机构、场地管理方 | 建筑改造8处、文化设施建成3座、专项论证完成7项、成本2,150-3,780万元 |
| **远期（5年+）** | 联赛品牌化、全球AI活动周路线、宠物体系贯通、文化解说系统（JZ-15~JZ-19） | 第三方运营机构、区体育局、文旅局、社区/高校/企业代表 | 月度活动≥8场、年运营支出800-1,200万元、满意度≥80%（运营策划时核定） |
...
实施路径由四项支撑共同保证可行性：① **分期与依赖分级**（§7.1-7.4，19项项目按近中远三期与轻量试点/专项论证/依赖正式资料三级分级）；② **工程结构论证**（§7.5，6项关键工程逐项给出初步技术判断与待论证要点）；③ **责任矩阵**（§7.6，11方参与主体×三期RACI分工）；④ **成本框架**（§7.7，分期汇总5,250-9,080万元及资金来源设想）。四项均标注"方案建议、须属地政府审批"，不构成行政审批结论，深度证据见 [depth:implementation_feasibility]、[depth:engineering_verification]，并辅以 [depth:implementation_actors]、[depth:operational_governance]、[depth:cost_framework]。
...
19项更新项目按前置条件和论证深度分为三级：

| 分级 | 项目编号 | 说明 |
|------|---------|------|
| **轻量试点**（无重大前置依赖，建议优先启动） | JZ-01, JZ-02, JZ-03, JZ-04, JZ-05, JZ-06, JZ-14, JZ-15, JZ-17, JZ-19 | 滑板场、宠物区、慢行贯通、品牌运营和文化解说——以现有用地和设施为基础，主要涉及地面铺装、临时构筑物、标识系统和运营组织。须属地管理方同意，但无需大规模工程审批 |
| **专项论证**（须完成结构/消防/文保/防洪/噪声等专业论证） | JZ-07, JZ-08, JZ-09, JZ-10, JZ-11, JZ-12, JZ-13 | 建筑改造（首层灰空间、开放界面）、构筑物（科创互动馆、古钟文化驿站、开源发布厅）、滨水步道——涉及结构荷载、消防疏散、河道管理、文物保护、噪声控制等。须委托持证专业机构完成论证后方可进入施工图设计 |
| **依赖正式资料**（正式边界/控规/权属/市政发布后方可启动） | JZ-16, JZ-18 | 全球AI活动周路线固化、骑行道评级——依赖正式用地边界、道路红线、权属确认和市政管线综合规划。当前仅能给出概念方案，正式几何发布后须整体复算 |

⚠ 所有项目均须属地政府主管部门审批后方可实施。轻量试点项目虽前置依赖较少，仍须取得场地管理方书面同意、完成安全评估和公众公示。以上分级为方案建议，不等同于行政审批结果。
...
|--------|---------|-------------|-------------|
| 众智园屋顶运动场（空中花园） | JZ-07（下沉广场青年中心） | 学北园裙房屋顶按绿建三星标准设计，现有结构荷载具备设置轻量运动场的潜力（篮球半场+瑜伽区+观赛草坡，面层荷载≤3kN/㎡）。需原结构设计单位复核 | 屋面活荷载余量、防水层保护、排水坡度改造、护栏高度（≥1.2m）、噪声对顶层办公的影响 |
| 京张绿廊跨环路桥下贯通 | JZ-01（慢行贯通） | 北五环/北四环/北三环跨线桥下净高≥2.5m，可利用桥下空间实现骑行和跑步连续贯通。涉及桥下空间权属为市政部门 | 桥梁结构安全距离、防撞护栏设置、桥下照明和排水、防汛要求、与现行交通组织协调 |
| 古钟文化驿站铜钟互动装置（约1.2吨） | JZ-11（古钟文化驿站） | 按明代永乐铜钟典型重量估算，建议采用按原钟比例缩小（约1:4）的可敲击铸造铜合金钟。基础须独立设计 | 地基承载力、钟架钢结构抗震设计、敲击声压级控制（建议≤75dB @10m）、振动对邻近建筑的影响 |
| 大钟寺碗池滑板场（深度2.0m） | JZ-04（大钟寺碗池） | 碗池为半地下结构，开挖深度约2.5m（含基础和排水层），底面高出地下水位。大钟寺体育公园现有地形平坦 | 地下管线避让、排水设计（防止积水）、地下车库顶板荷载（若上方）、噪声对周边居民影响评估 |
| 铁路记忆科创互动馆（约200㎡轻量构筑物） | JZ-10（科创互动馆） | 单层钢框架+幕墙体系，基础浅埋，属临时/半永久构筑物。位置建议在五道口京张绿廊段已腾退地块 | 地基处理方案、消防疏散距离、临时建筑使用年限（建议按15年设计）、京张铁路文保控制线退让要求 |
| 定向声景装置（超指向性扬声器） | JZ-19（文化解说系统） | 超声波载波技术，声束宽度约15°，5m半径外音量自然衰减至背景噪声水平。运行时段8:00-20:00 | 超声波对人体的长期暴露安全性（须符合GB 3096声环境质量标准）、设备防水防尘等级（户外IP65+）、电磁兼容性 |
...
|--------|------|---------------------------|---------------------------|---------------------------|
| **海淀区政府（建议牵头部门：区规划自然资源分局）** | 规划审批与用地协调 | A（审批项目选址和用地） | A（审批建筑改造和用地变更） | C（品牌和运营方案征求意见） |
| **区体育局（建议）** | 运动设施建设与运营 | C（滑板场设计标准咨询） | C（极限运动设施专项咨询） | R/A（三片区联赛运营、极限运动网络管理） |
| **区园林绿化局（建议）** | 公园绿地管理 | A（宠物区和草坪的绿地兼容性审批） | C（滨水步道绿化设计审查） | I（运营阶段绿地养护监督） |
| **区文化和旅游局（建议）** | 文化设施建设 | I（近期不涉及） | C（文化驿站和铁路遗存解说内容审查） | A（铁路工业遗存文化解说系统审批） |
| **场地管理方（众智园/清华科技园/东升大厦）** | 场地提供与日常管理 | C（场地使用同意） / I | R（首层改造和灰空间优化实施） | C（品牌触点植入审批） |
| **属地街道办（中关村/五道口/大钟寺）** | 社区协调与公众沟通 | C（社区居民意见征集） / I | C（施工期间居民沟通和噪声公示） | C（运营期间社区反馈收集） |
| **专业设计机构（建筑/结构/景观/市政）** | 专业设计与论证 | R（方案深化和施工图） | R（专项论证报告编制） | I（运营阶段技术咨询） |
| **施工单位（公开招标）** | 施工实施 | R（轻量构筑物和地面铺装施工） | R（建筑改造和文化设施施工） | — |
| **第三方运营机构（建议公开招募）** | 品牌运营与活动执行 | — | I（设施建设进度了解） | R（品牌活动策划执行、设施日常维护、商业合作拓展） |
| **社区/高校/企业代表** | 使用方与反馈 | C（设计阶段需求征集） / I | C（改造方案意见征询） / I | C（运营满意度反馈） |
| **文物部门（建议北京市文物局）** | 文保审查 | I（近期不涉及文物控制线内施工） | A（铁路遗存和古钟文化驿站的文保审批） | C（文化解说系统内容审查） |
| **公安/消防/应急部门** | 安全审查 | A（滑板场和宠物区安全方案审批） | A（构筑物消防审批、大型活动安保方案） | C（联赛和大型活动安全方案审查） |

**运营机制建议**：
- **近期运营（0-2年）**：滑板场和宠物区由场地管理方直接管理（无需新增运营主体），AI跑团、周末市集等轻量活动由社区自治组织+志愿者运营
- **中期运营（2-5年）**：文化设施（科创互动馆、古钟文化驿站、开源发布厅）建议成立"京张AI绿廊运营管理办公室"（建议为区属事业单位或政府购买服务的第三方运营机构），统一管理三片区的文化设施预约、讲解服务、活动排期和设备维护
- **远期运营（5年+）**：品牌活动（全球AI开放日、极限运动联赛、古钟文化声音艺术节）由运营办公室主办，引入企业冠名赞助和票务收入。运营成本来源建议为：政府补贴50% + 商业合作30% + 票务/服务收入20%。年度运营支出估算（远期）：约800-1200万元/年（基于类似规模和密度的城市公共空间运营数据——参照地坛公园文化季、上海西岸公共活动运营和深圳人才公园运营年报，具体金额须正式运营策划时核定）
...
|------|------|-------------|---------|----------------|
| **地面铺装与慢行** | 彩色沥青骑行道（京张蓝） | 300-500元/㎡ | 9km×3m=27000㎡ | 810-1350 |
| | 塑胶跑步道 | 200-350元/㎡ | 9km×2m=18000㎡ | 360-630 |
| | 透水砖漫步道 | 150-250元/㎡ | 9km×2.5m=22500㎡ | 340-560 |
| **滑板场** | 初级泵道（沥青铺装） | 800-1200元/㎡ | 800㎡ | 65-100 |
| | 街头滑板plaza（混凝土） | 1500-2500元/㎡ | 1200㎡ | 180-300 |
| | 碗池（混凝土+钢结构护栏） | 2500-4000元/㎡ | 1500㎡ | 375-600 |
| **建筑改造** | 首层灰空间优化（幕墙+地面+灯光） | 3000-5000元/㎡ | 5栋×约200㎡=1000㎡ | 300-500 |
| | 跑道夜光升级（蓄光涂层） | 200-300元/㎡ | 500m×2m=1000㎡ | 20-30 |
| **轻量构筑物** | 单层钢框架+幕墙（约200㎡） | 6000-9000元/㎡ | 科创互动馆+开源发布厅+古钟文化驿站 3×200=600㎡ | 360-540 |
| | 模块化服务亭/集装箱改造 | 10-20万元/个 | 5处附属设施+3处体测站+可移动看台 | 100-200 |
| **景观与设施** | 雨水花园模块 | 500-800元/㎡ | 45处×约30㎡=1350㎡ | 70-110 |
| | 宠物友好区（围栏+地面+饮水设施） | 400-600元/㎡ | 2100㎡ | 85-130 |
| | 青年活动草坪（含地下排水层） | 300-500元/㎡ | 13000㎡ | 390-650 |
| | 智能体测站（含太阳能+传感器+LCD屏幕） | 8-15万元/套 | 5套 | 40-75 |
| | 定向声景装置（超声波扬声器+防水箱体） | 5-8万元/套 | 2套 | 10-16 |
| | 骑行驿站模块化构筑物 | 3-5万元/处 | 5处 | 15-25 |
| | 语音导航桩（太阳能+蓝牙+防水） | 1.5-2.5万元/处 | 18处 | 27-45 |
| **临时设施** | 端侧算力节点（20㎡+设备+散热+交互屏） | 15-25万元/处 | 5处 | 75-125 |
| **品牌标识** | 品牌触点（标牌/导视/地面标识） | 2000-5000元/处 | 约200处 | 40-100 |
| **前期与软成本** | 规划设计费（方案深化+初设+施工图） | — | 建安费的8-12% | 300-500 |
| | 专项论证费（结构/消防/文保/环评等） | — | 7项×各20-40万 | 140-280 |

**分期汇总（参考量级，万元）**：

| 阶段 | 建安成本 | 前期与软成本 | 合计 |
|------|---------|-------------|------|
| 近期（0-2年）JZ-01~JZ-07 | 2,500-4,200 | 300-500 | **2,800-4,700** |
| 中期（2-5年）JZ-08~JZ-14 | 2,000-3,500 | 150-280 | **2,150-3,780** |
| 远期（5年+）JZ-15~JZ-19 | 300-600（主要为品牌运营和设施维护） | — | **300-600** |
| **总计** | | | **5,250-9,080** |

⚠ **资金来源建议**（方案阶段设想，待政府确认）：
- 政府投资（市政基础设施+公共服务设施）：约60%（慢行系统、公共绿地、无障碍设施、解说系统等公益性项目）
- 社会资本（运营设施+品牌活动）：约30%（滑板场商业运营、临时商业设施、品牌赛事赞助）
- 高校/企业配建（众智园和五道口片区）：约10%（众智园首层改造由园区自有资金配建，清华科技园界面改造由物业自有资金配建）

以上所有金额为方案阶段参考量级，误差范围±30-50%。实际造价受市场材料价格波动、施工条件、招标方式等因素影响，须在各阶段概算中逐项核定。资金来源和分担比例须与各主管部门协商确认，不构成资金承诺。
...
京张AI创新带是海淀区"三区两翼"创新空间格局的核心纵向主轴。按任务书定义，"三区"为海淀核心创新区、中关村科技服务翼、小月河场景赋能翼。本方案所在为**海淀核心创新区**（沿京张绿廊），承担AI策源、成果转化、开源协作和国际交往的核心功能。方案同时在以下三个方向识别区域产业协同关系（以下方向为区域协同圈层，非任务书"两翼"术语）：

### 三大定位·五大功能·三区两翼协同表

| 维度 | 内容 |
|------|------|
| **三大定位** | ① 世界级AI创新策源地——沿京张绿廊串联高校、科研院所、头部企业和初创社区，形成基础研究到产业应用的全链条；② 国际AI产业集聚区——以大钟寺片区和五道口开放界面吸引全球AI企业研发中心和区域总部（此为建议方向，实际落地须属地政府主导）；③ 未来城市AI应用示范区——以京张绿廊全线为真实环境测试场，展示AI如何在公共空间中服务市民（此为概念方案，非政府审定事项） |
| **五大功能** | ① AI基础研究与策源（清华/北大/中科院+智源研究院）；② AI产业孵化与成果转化（众智园+东升大厦+清华科技园）；③ 开源社区与国际交往（五道口开源发布厅+全球AI开放日，全球AI开放日为方案建议活动，非已确定赛事）；④ AI公共服务与市民体验（三片区10张AI场景卡+3处产业测试场）；⑤ 城市更新与生态修复示范（京张绿廊"三道一绿"+清河滨水+铁路遗存活化） |
...
以下三个区域协同方向为方案基于地理邻近性和产业互补性提出的建议方向，不等同于任务书"两翼"概念，也不构成政府间协定或正式产业规划：

| 协同方向 | 组团 | 产业分工 | 交通连接（估算） |
|----------|------|----------|-----------------|
| **北向·算力与能源协同** | 北纬社区+未来科学城 | AI算力基础设施、大模型训练集群、能源AI和自动驾驶封闭测试场等重资产环节（建议协作方向，待双方确认） | 京新高速/G6走廊（车程约30分钟） |
| **东向·基础科学交叉** | 怀柔科学城 | 大科学装置（高能同步辐射光源、综合极端条件实验装置等）与AI基础理论交叉研究（建议协作方向，待双方确认） | 京承高速（车程约60分钟） |
| **南向·制造与产业化** | 北京经济技术开发区 | AI+智能制造、具身智能机器人量产、商业航天产业化和生物医药AI应用（建议协作方向，待双方确认） | 地铁亦庄线+京沪高速（车程约45分钟） |

```
北端：未来科学城（算力+能源AI+大科学装置）[建议协作方向]
  │  [京新高速/G6走廊 ~30min]
  │
  ├── 京张AI创新带（本方案）← 策源+转化+开源+国际交往 [建议定位]
  │     ├── 众智园·清河北岸创新港：自主创新加速
  ...
  │  [地铁4号线/10号线]
  │
  ├── 丰台科技园（AI+轨道交通、航空航天）[建议协作方向]
  │  [地铁9号线/房山线]
  │
  ├── 雄安新区（数字城市AI应用、智能基础设施）[建议协作方向]
  │  [京雄城际 ~50min]
  │
南端：北京经济技术开发区（AI+制造、机器人、商业航天）[建议协作方向]
  [地铁亦庄线+京沪高速 ~45min]

⚠ 以上协作关系均为方案建议方向，不等同于政府间协定或已确定的合作安排。
```

### 产业分工矩阵

| 创新组团 | 产业分工（建议方向） | 与京张带的关系（建议） | 协同机制（方案建议，待各相关方确认） |
|----------|---------------------|----------------------|----------------------------------|
| 北纬社区/未来科学城 | 算力基础设施、大模型训练集群、能源AI、自动驾驶测试（建议） | 京张带提供算法和应用场景需求，未来城提供算力支撑和测试场地（建议方向） | 算力券互通（建议）、联合实验室共建（建议）、人才互访计划（建议）、季度产学研对接会（建议） |
| 怀柔科学城 | 大科学装置（高能光源、极端条件装置）、基础理论、量子计算与AI交叉（建议） | 京张带提供应用场景和产业转化通道，怀柔提供基础理论突破和大科学装置共享（建议方向） | 联合博士生培养（建议）、大装置远程共享预约（建议）、交叉研究基金（建议）、年度AI+大科学研讨会（建议） |
| 北京经开区 | AI+智能制造、具身智能机器人、生物医药AI、商业航天（建议） | 京张带提供孵化成果和技术团队，经开区提供量产条件和中试基地（建议方向） | 成果转化绿色通道（建议）、"海淀研发-亦庄制造"产业链对接（建议）、中试基地共享（建议） |
| 中关村软件园/上地 | 成熟AI企业总部（百度/腾讯/网易等）、信息服务、云计算 | 京张带作为"延伸创新走廊"，承接外溢和创新展示功能（建议方向） | 企业Open Day、成果展示窗、人才交流、慢行通勤连接（均为建议活动） |
| 雄安新区 | 数字孪生城市、智能基础设施AI、数据要素市场化试点 | 京张带提供技术输出和人才培训，雄安提供超大规模应用场景（建议方向） | 京津冀AI产业协同规划（建议）、数据跨境/跨区域流动试点（建议）、干部交流挂职（建议） |

⚠ 上表协同机制均为方案概念建议，不等同于已签署的政府间协议、已设立基金或已获批政策试点。
...
小月河体验路径作为"文化辅线"，其线形沿现状小月河河道与绿地布局，纳入蓝绿慢行网络统筹；具体坐标待确认 [data:geometry/green_space.geojson] [data:geometry/roads.geojson] [depth:blue_green_public_space]
...
| **国际AI城市设计论坛** (International AI Urban Design Forum) | 每年10月（与全球AI开放日同期，为方案建议活动） | 众智园报告厅（主会场）+ 大钟寺国际路演客厅（分会场）（建议场地） | 500-1000人（预估） | 建议邀请全球AI创新区（伦敦国王十字、波士顿肯德尔广场、新加坡纬壹科技城、巴塞罗那22@区、上海西岸等）运营方和规划师参与——主题演讲、圆桌对话、建议发起"京张共识"（AI创新区城市设计原则讨论文件——由京张AI创新带作为发起方之一，邀请各国际案例方作为讨论参与方，非正式国际协定或联署文件） |

### 运营主体建议

⚠ 以下运营主体均为方案建议，不等同于政府部门已确认的职能安排。实际运营主体和机制须经各相关方协商确定。

| 运营域 | 建议运营主体 | 具体职责 |
|--------|-------------|----------|
| **运动设施与赛事** | 建议：属地体育主管部门（指导）+ 第三方赛事运营（招标产生） | 三片区滑板场、屋顶运动区、跑步道和智能体测站的日常维护标准制定和巡检（建议）；极限运动联赛、夜跑联赛、屋顶运动联盟联赛的审批、裁判认证、安全指导（建议机制）；年度"北京城市夜跑联赛大钟寺站"建议申报纳入全民健身赛事体系（须经主管部门审批） |
| **宠物友好体系** | 建议：第三方专业运营机构（招标产生） | 三片区宠物友好区和京张绿廊沿线补给点的日常保洁、设施维护、安全巡检（建议标准）；宠物社交周末市集的策划与现场管理（建议活动）；年度"爪印绿廊"宠物嘉年华的策划和执行（建议活动） |
| **文化活动与地标** | 建议：属地文旅主管部门（指导）+ 第三方文化运营（招标产生） | 铁路记忆科创互动馆和古钟文化驿站的日常运营（建议）；季度声音艺术节和年度铁路文化周的策展和执行（建议活动）；开源之光LED矩阵和古钟新声投影系统的内容审核和播放管理（建议机制） |
| **开源社区与创新活动** | 建议：社区自治委员会（高校开源社团+企业开源办公室+居民代表） | 开源发布厅的预约管理和使用规则制定（建议）；月度开源meetup和季度Hackathon的策划和组织（建议活动）；全球AI开放日开源展区的策展（建议活动） |
| **品牌与传播** | 建议：联合品牌运营委员会（多方代表协商组成） | "京张AI绿廊"统一品牌标识体系的管理（建议）；年度活动日历的统筹协调（建议）；全球AI开放日和国际论坛的品牌传播（建议）；"京张共识"的推广和国际伙伴拓展（建议方向） |
| **产业测试验证场景** | 建议：属地科技主管部门（指导）+ AI企业联盟（项目组织）+ 各场地管理方 | 自主模型安全评测沙盒的预约管理和运营（建议机制）；端侧AI算力服务驿站压力测试（建议项目）；城市智能体交通优化实验的联合立项（建议方向） |
...
- **品牌传播目标**（运营启动后第3年，为方案建议的参考指标，非承诺性KPI）：小红书"京张AI绿廊"相关笔记≥3000篇（建议参考值）、微信公众号/服务号关注≥10万（建议参考值）、全球AI开放日媒体触达≥5000万（建议参考值）
...
完整机器索引见 `sources.json`、`metrics.json`、`compliance_matrix.json`、`standard_matrix.json`、`design_depth_matrix.json`（均为本方案包内生成的机器可读证据，非外部来源）。
