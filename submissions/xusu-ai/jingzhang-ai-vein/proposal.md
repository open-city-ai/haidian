...
summary: "以京张遗址公园带为城市中轴（x=0），土地性质与地块划分重大调整：12个建设用地6对镜像对称双翼+3段中轴绿带，**全部建筑以中轴x=0左右镜像排布（两翼楼栋对称，设计意图）**。居住地块纯住宅：统一60×14m板楼5排南低北高（10-15层，北京阳光自南射入，南排30m矮、北排45m高、每排+3m，设计意图为逐排南低北高减少遮挡（正式日照需专业复核），参照万柳书院/使馆壹号院/融创北京壹号院；每排栋数按两翼边界较窄者自适应、外端离边界5m贴紧，无点式塔楼/教育/底商），排距380m≈8.4倍最高楼高满足大寒日≥2h日照；科研地块采用大基底点式写字楼（56-75m见方/单层3000-5500㎡，对标中国尊/正大中心/国贸三期单层面积，45-80m，最高仅为中国尊528m的15%，塔楼均匀疏散铺满地块、外侧贴翼边界），以4.0%低密度支撑0.35容积率（高度换面积），层高3.6-4.2m符合北京办公标准；商业综合体参照清河万象汇（8层/36m），底商≤3层；文化馆参照首都博物馆/中国科技馆。所有建筑体块均以北京案例为设计参照，非凭空体块（案例适用性需逐项专业确认）。"
...
**V3 方案核心转向**：在 V2「中轴对称」基础上，本版对**土地性质、地块划分与建筑体块**做重大调整——继续以城市中轴线（x=0，京张遗址公园带）为对称轴，**全部 12 个建设用地 6 对地块内的所有建筑以中轴 x=0 左右镜像排布（两翼楼栋对称，设计意图）**；**建筑体块以北京真实案例为设计参照，高度以中国尊为上限约束、层高对应北京标准（案例适用性待专业确认）**：居住地块为**纯住宅、统一 60×14m 板楼**（5 排南低北高——北京在北半球阳光自南射入，南排 30m、北排 45m、每排 +3m 递增，设计意图为逐排南低北高减少遮挡（正式日照需专业复核），参照万柳书院/使馆壹号院/融创北京壹号院；每排栋数按两翼边界较窄者自适应、外端离边界 5m 贴紧，无点式塔楼/教育/底商）并作概念间距校核（排距≈8.4×楼高，正式日照需专业逐时模拟复核），科研地块采用大基底点式写字楼（56-75m 见方/单层 3000-5500㎡，对标中国尊/正大中心/国贸三期单层面积，45-80m，最高仅为中国尊528m的15%，层高3.6-4.2m）以高度换容积率、大幅降低土地覆盖率，商业地块采用综合体（参照清河万象汇/西红门荟聚）与≤3层底商，文化地块采用大型文化馆（参照首都博物馆/中国科技馆）。设计意图：中轴作为京张铁路遗产的文化叙事主轴与城市通风廊道，双翼镜像体现城市服务均衡与天际线对称的设计意图，建筑体块基于北京案例参照、高度与层高对应北京北五环外城市肌理。
...
**设计意图**：中轴绿带（宽 220m）既是文化叙事主轴（京张铁路遗产），也是城市通风廊道与景观对称轴；双翼功能配对体现城市服务均衡的设计意图——居民在任何一侧都能获得对称的科研/文化/居住/商业供给。用地结构由 `geometry/land_use.geojson` 表达：15 个地块在 EPSG:4548 下无缝隙无重叠全覆盖场地（[data:geometry/land_use.geojson]），`land_use_area_by_code` 指标在 `metrics.json` 中复算（[metric:land_use_area_by_code]）。

**V3 建筑体块策略（案例为设计参照，高度以中国尊 528m 为上限约束）**：
...
- **南低北高梯度说明**：北半球住宅地块标准做法——南排最矮（10 层/30m）、北排最高（15 层/45m）、每排 +3m 递增。阳光自南射入时，南排 30m 不挡第 2 排，第 2 排 33m 不挡第 3 排……依此类推，**概念校核各排在大寒日可获得 ≥2h 日照（正式结论需专业日照软件逐时模拟复核）**。每排栋数按两翼边界较窄者自适应、楼群外端离边界 5m 贴紧（四角自然有楼贴边），**两翼以中轴 x=0 左右镜像（设计意图）**，与北京高端住宅社区（万柳书院 7-15 层、融创北京壹号院 12-18 层、使馆壹号院 12-18 层）的南低北高布置逻辑一致（[standard:RESIDENTIAL-SUNLIGHT-STANDARD]）。
...
- **建筑排布**：科研地块每翼 10 栋大基底点式写字楼（56-75m 见方，单层 3000-5500㎡ 超中国尊级，45-80m，层高 3.6-4.2m）**均匀疏散铺满地块（y 向 0.06~0.94 贴南北边界，外侧塔贴翼边界 2-7m）**+ 研发裙房；文化地块每翼 2 座大型文化馆 + 商业配套；居住地块为**纯住宅、统一 60×14m 板楼**（每翼 5 排南低北高 zigzag 错位铺满 1.6km 纵深，南排 30m → 北排 45m 每排 +3m；每排栋数按两翼边界较窄者自适应、外端离边界 5m 贴紧，无点式塔楼/教育/底商）；商业地块每翼综合体 + 副楼 + 底商裙房（均贴翼边界）。**全部 6 对地块建筑以中轴 x=0 左右镜像（设计意图）**（[data:geometry/buildings.geojson]）。
...
- **日照专项**：居住地块为**纯住宅、统一 60×14m 板楼**（5 排/10-15 层/30-45m，层高 3.0m，参照万柳书院/使馆壹号院/融创北京壹号院），**南低北高、中轴镜像排布**（5 排 zigzag 错位铺满 1.6km 纵深，南排 30m → 北排 45m 每排 +3m；每排栋数按两翼边界较窄者自适应、外端离边界 5m 贴紧，两翼左右镜像（设计意图）），按 4.2 节间距校核（排距 380m ≈ 8.4×最高楼高，概念校核；正式日照需专业复核）（[standard:RESIDENTIAL-SUNLIGHT-STANDARD]）
...
### 6.2 场景卡明细（含产业测试验证场景，agent.4 响应）

**01 开源发布厅**（众智园西翼）
- 输入数据：模型评测数据/开源仓库元数据/发布日志
- AI 能力：成果检索与智能推荐、评测结果自动生成
- 物理设施：发布厅/评测机房/路演区/直播系统
- 服务流程：成果登记→AI 生成评测摘要→发布→观众反馈收集
- 运营者：园区运营公司（产业平台）
- 人类复核：AI 评测摘要须专家抽检后对外发布；每年 4 次人工审计
- 隐私最小化：公开数据为主，涉企数据脱敏；不采集个人生物信息
- 失败回退：评测失败自动降级为人工路演；系统故障转线下登记
- 试验周期：6 个月试点（2026H2）
- 成效指标：发布数量≥30 项/年、评测覆盖率 100%、观众满意度≥4/5

**02 城市智能体沙盒**（众智园东翼）
- 输入数据：交通流量/公共服务工单/模拟环境参数（脱敏）
- AI 能力：智能体行为仿真、冲突检测、安全评分
- 物理设施：封闭测试场地/仿真服务器/监管观测室
- 服务流程：场景申报→沙盒隔离部署→自动仿真→安全评审→放行
- 运营者：测试验证平台（政府+企业共建）
- 人类复核：放行前安全评审委员会签字；每轮测试留痕
- 隐私最小化：仿真数据最小化，真实数据仅限脱敏聚合；测试期数据自动清除
- 失败回退：仿真超限自动熔断；智能体异常行为即时下线
- 试验周期：12 个月试点（2026H2-2027H1）
- 成效指标：测试场景≥10 个、安全事件 0、通过率≥80%

**03 慢行断点诊断**（中轴绿带）
- 输入数据：慢行路径 GPS 轨迹/POI/设施数据（匿名）
- AI 能力：断点识别、拥堵预测、路径推荐
- 物理设施：感知摄像头/边缘计算节点/信息屏
- 服务流程：数据采集→AI 识别断点→人工现场复核→工程整改→复测
- 运营者：市政管理单位+第三方运营
- 人类复核：断点清单人工复核率 100%；整改后复测闭环
- 隐私最小化：轨迹匿名化，不追踪个体；仅存聚合热力
- 失败回退：感知故障降级人工巡检；预测失败回退历史均值
- 试验周期：9 个月试点（2026H3-2027H2）
- 成效指标：断点识别召回率≥85%、整改率≥70%、步行满意度+10%

**04 人才生活管家**（AI原点社区双翼）
- 输入数据：居住/通勤/文体服务数据（用户授权）
- AI 能力：意图理解、服务编排、行程规划
- 物理设施：社区服务终端/App/物业联动接口
- 服务流程：需求输入→AI 匹配服务→人工确认→执行→反馈闭环
- 运营者：社区物业+生活服务商
- 人类复核：高敏操作（医疗/金融）必须人工确认
- 隐私最小化：最小必要采集，可随时撤回授权；敏感数据本地化
- 失败回退：AI 无法响应转人工客服；推荐失败回退标准服务目录
- 试验周期：6 个月试点（2027H1）
- 成效指标：服务响应<5min、用户撤回<5%、满意度≥4/5

**05 AI 安全治理廊**（中轴北段）
- 输入数据：治理案例库/标准文件/评测报告（公开）
- AI 能力：风险分级、合规检索、评测报告生成
- 物理设施：展示长廊/政策研讨室/标准成果展区
- 服务流程：案例入库→AI 风险分级→专家评审→公开展示
- 运营者：治理委员会（产学研政）
- 人类复核：分级结论须专家会签；敏感案例脱敏展示
- 隐私最小化：只展示公开或已脱敏案例；不采集访客身份
- 失败回退：检索失败转人工咨询；分级争议走仲裁流程
- 试验周期：持续运营（2027 起）
- 成效指标：案例库≥50 项、专家会签率 100%、年更新≥2 次

**06 校企转化客厅**（AI原点社区西翼）
- 输入数据：高校专利/成果/企业需求（授权）
- AI 能力：成果-需求匹配、转化路径建议
- 物理设施：会客厅/路演厅/知识产权服务窗口
- 服务流程：成果登记→AI 匹配→对接撮合→签约落地→跟踪
- 运营者：成果转化服务平台（高校+园区共建）
- 人类复核：签约前法务审查；匹配建议仅供参考
- 隐私最小化：专利数据授权范围内使用；不披露未公开成果
- 失败回退：匹配失败转人工对接；系统故障线下撮合
- 试验周期：12 个月试点（2026H2-2027H1）
- 成效指标：年对接≥50 次、转化落地≥10 项、满意度≥4/5

**07 数据要素剧场**（大钟寺西翼）
- 输入数据：数据资产目录/交易案例/合规样例
- AI 能力：可视化叙事生成、合规检测
- 物理设施：沉浸展示厅/数据资产大屏/互动终端
- 服务流程：选题→AI 生成可视化→合规审核→公开展示
- 运营者：数据交易机构+园区运营
- 人类复核：对外展示内容合规审核率 100%
- 隐私最小化：展示数据全部脱敏；不展示原始交易数据
- 失败回退：渲染失败回退静态展板；内容争议下架审核
- 试验周期：6 个月试点（2027H1）
- 成效指标：展示内容≥20 项、参观人次≥5万/年、合规 0 事故

**08 低碳算力驿站**（中轴南段）
- 输入数据：能源负荷/算力调度/天气数据
- AI 能力：负载预测、绿电调度优化、碳排放核算
- 物理设施：分布式能源站/边缘算力舱/储能设施
- 服务流程：负荷采集→AI 调度→绿电优先→碳核算→公示
- 运营者：能源服务商+算力运营方
- 人类复核：调度策略月度人工审计；异常告警人工处置
- 隐私最小化：用能数据聚合统计，不采集个体行为
- 失败回退：电网故障自动切换备电；预测失效回退规则调度
- 试验周期：12 个月试点（2026H2-2027H1）
- 成效指标：绿电占比≥30%、PUE≤1.3、碳核算覆盖率 100%

**09 京张记忆线路**（中轴全线）
- 输入数据：文保资料/历史档案/游客流量（公开/授权）
- AI 能力：AR 导览、文化叙事生成、流量预测
- 物理设施：AR 终端/标识系统/文化驿站
- 服务流程：打卡→AI 生成讲解→文化互动→反馈收集
- 运营者：文旅运营公司+文保单位
- 人类复核：文保内容专家审核；AR 内容不篡改史实
- 隐私最小化：位置服务仅存聚合热力；游客画像匿名
- 失败回退：AR 故障回退纸质导览；内容争议下架复核
- 试验周期：9 个月试点（2027H1-H3）
- 成效指标：导览覆盖率 100%、游客停留+20%、满意度≥4.5/5

**10 全球 AI 活动周路线**（三区串联）
- 输入数据：活动报名/场次/人流（脱敏）
- AI 能力：路线编排、人流预测、多语种内容生成
- 物理设施：活动场地/接驳/多语种服务台
- 服务流程：报名→AI 排程→分流引导→效果评估
- 运营者：活动组委会（园区+国际机构）
- 人类复核：涉外内容人工审核；安全预案人工启动
- 隐私最小化：报名信息最小收集；跨境数据合规处理
- 失败回退：人流超限自动限流；线上故障转线下签到
- 试验周期：年度活动（2027 起每年）
- 成效指标：参会≥1万人次/年、涉外活动≥10 场、满意度≥4/5



### 6.2b 全球案例比较矩阵（agent.2 响应）

| 案例 | 地区 | 核心机制 | 对本方案启示 | 适用性评估 |
|------|------|----------|--------------|-----------|
| 硅谷 101 公路创新走廊 | 美国 | 高校-资本-产业沿走廊聚集 | 中轴作为创新主廊串联三区 | 高（尺度可比） |
| 伦敦国王十字更新 | 英国 | 铁路遗产更新 + 知识经济导入 | 京张铁路遗产带更新模式 | 高（遗产类型相似） |
| 东京丸之内再生 | 日本 | 商务区 TOD + 混合功能更新 | 大钟寺 TOD 一体化 | 中高（密度更高） |
| 新加坡纬壹科技城 | 新加坡 | 园区-社区-生态三位一体 | 双翼园区+绿带社区结构 | 中（气候差异） |
| 首尔 DMC 数字媒体城 | 韩国 | 内容产业集聚 + 国际传播 | 全球 AI 活动周路线组织 | 中（产业类型相近） |
| 深圳大沙河生态长廊 | 中国 | 河流生态修复 + 科创走廊 | 小月河生态廊道 + 慢行系统 | 高（国内可借鉴） |

> 案例机制为公开资料综述，非逐项许可引用的正式比较研究；正式设计阶段需补充来源可核查的比较矩阵。

### 6.2c 公共空间组件库（agent.4 响应，组件清单）

| 组件 | 位置 | 功能 | 规格（概念） |
|------|------|------|-------------|
| 中轴绿带 220m | 京张遗址公园带 | 文化慢行主廊 | 三段贯通，含记忆步道/活动草坪/雨水花园 |
| 双翼广场 | 科研/商业地块 | 产业展示/集会 | 5000-8000㎡，含 AI 展示节点 |
| 社区口袋公园 | 居住双翼 | 5 分钟生活圈 | 3000-5000㎡/处，8 处 |
| 慢行断点节点 | 横轴交叉口 | 过街优先 | 无障碍坡道+信号优先 |
| 路演客厅 | 校企转化客厅前 | 户外路演 | 可移动舞台+观众阶梯 |
| 滨水活力带 | 小月河东翼 | 亲水休闲 | 3-4m 宽步道+生态驳岸 |

### 6.2d 导视与荣誉展示体系（agent.5 响应，清单）

- **三级导视**：中轴总览牌（三区叙事）→ 区段导视（功能指引）→ 节点标识（朝圣节点），双语（中/EN）。
- **荣誉展示**：众智园「开源圣火台」年度开源贡献榜；原点社区「创新墙」成果转化案例；大钟寺「数据方舟」产业荣誉展。均为规划建议，正式方案需与文保、市政协调。

### 6.2e 长期运营转化机制（agent.6 响应，框架）

- **主体结构**：园区运营公司（日常）+ 场景运营方（分场景委托）+ 政府专班（监管与标准）+ 社区理事会（民意反馈）。责任、成本、收益与风险分担以 PPP 式协议细化为准。
- **转化路径**：试点（6-12 月）→ 评估（KPI 达标）→ 规模化（纳入控规实施）→ 复制（跨区推广）。
...
**道路骨架**（[data:geometry/roads.geojson]）：**中轴大道**（智脉纵轴，主干）+ 东西翼纵路（对称）+ 六条横轴（南北对称等距），总长约 37.2 km（[metric:road_length_m]）。设计意图：中轴大道承担文化慢行主廊功能，翼纵路分流双翼车流，横轴连接三区与外部路网，对称路网体现两侧可达性均衡的设计意图。
...
**公共服务**：双翼镜像配置社区中心/教育/医疗/文化设施，目标为服务半径均衡（需现状设施核查）；重点区各设 1 处 AI 展示与服务中心。**数据缺口**：现状市政管线、学校医院分布待补充（`missing-data.md` 已记录）。
...
| total_floor_area_sqm | 3,956,083 | sum(footprint × floors_concept) |
| floor_area_ratio | 0.347 | total_floor / site_area |
| building_density | 0.040 | footprint / site_area |
| green_ratio | 0.187 | green_space / site_area |
| public_space_ratio | 0.015 | public_space / site_area |
| road_length_m | 37,186 | sum(line_length) |
| building_count | 110 | count(features) |
| key_area_total | 369.2 ha | sum(key_area polygons)，几何对齐公告（192.9/104.3/72.0 ha） |
