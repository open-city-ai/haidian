---
title: "Jingzhang AI Pilgrimage Corridor: Multi-Agent Collaborative Urban Innovation Belt"
...
summary: "Overall concept: JINGZHANG AI PILGRIMAGE CORRIDOR — a walkable innovation experience route along the Jingzhang heritage park spine with pilgrimage stations in three key areas; based on provisional boundary with precision warnings."
...
# Jingzhang AI Pilgrimage Corridor: Multi-Agent Collaborative Urban Innovation Belt

## Design Basis and Source List

This formal package takes the Haidian Bureau pre-qualification announcement as primary authority and 《百年京张AI创新带城市设计国际方案征集资格预审公告》为第一依据，并以 `brief/site-package/` 中经维护者登记的临时粗略边界、重点区域、枚举、指标和来源清单为机器可读依据。AI agent 在生成方案前必须读取 `design_brief.json`、`allowed_design_space.json`、`sources.json`、`enums/`、`ranges/`、`schemas/`、`data/source_registry.json` 和 `data/processed/agent_fact_pack.md`，并用 `project_scope_summary.csv`、`agent_task_requirements.csv`、`source_use_matrix.csv`、`missing_data_checklist.csv` 建立任务、范围、资料用途和缺口清单。所有设计判断都要拆分为可追溯来源、可复算指标、可校验图层和可人工复核假设。公告要求方案达到控制性详细规划的城市设计深度和规划综合实施方案的城市设计深度，因此文本叙述不能替代 GeoJSON、指标表、A3 文册、A0 展板和 HTML 电子展示成果。
...
![资料证据链与提交包关系图](assets/figures/site-overview.en.png)
...
![三层范围与空间工作框架图](assets/figures/land-use-structure.en.png)

三层工作不是互相割裂的图纸集合。统筹研究决定产业链和城市形态判断，总体设计把判断落实到更新项目、空间结构和设施承载，重点区域详细设计验证具体地块、建筑、交通、公共空间和AI应用场景的可实施性。agent 生成方案时必须先锁定当前提交采用的 official 或 provisional 边界和约束，再生成用地、建筑、道路、绿地、公共空间、分期和AI服务节点，最后从这些图层复算指标并在正文解释哪些结论仍受 provisional boundary 限制。任何无法从结构化数据复算的面积、比例、规模或项目数量，不得写入正式结论。

The proposed overall concept is JINGZHANG AI PILGRIMAGE CORRIDOR：以京张遗址公园为历史与公共空间主轴，把全球AI创新者、开发者、研究者与市民的日常路径组织为一条可步行、可体验、可审计的朝圣式创新路线。走廊串联众智园（自主创新加速驿站）、北京AI原点社区（开源策源驿站）、大钟寺（智能经济展示驿站）三处重点片区，并以中关村科技服务翼、小月河场景赋能翼形成协同回路。这里的“一带”不是额外画出的新红线，而是把公告中的三层范围转译为工作方法；“三核”对应三处重点区域；“多点场景”对应AI+公共服务、产业服务和城市生活的可运营节点；“复合环”对应慢行、绿地、公共空间和活动路线的联动。
...
![三处重点区域索引与设计任务图](assets/figures/key-areas.en.png)
...
### Brand Naming and Visual Identity (agent.1)

- **主名称**：京张AI朝圣走廊 / JINGZHANG AI PILGRIMAGE CORRIDOR
- **副标题**：多智能体协同的城市创新带 / Multi-Agent Collaborative Urban Innovation Belt
- **Logo方向**：以京张铁路轨距线与神经网络节点叠加，形成「轨道×智能体」双螺旋标识；主色深靛蓝（#172235）+ 京张金（#c79838）
- **视觉系统**：朝圣路线图采用地铁图式信息设计，节点标注AI场景类型与人工复核入口

### AI Pilgrimage Landmarks and Honor Display (agent.6)

本方案提出不少于3个AI朝圣地标（概念建议）：

1. **开源策源碑**（北京AI原点社区）：展示中国开源AI里程碑与贡献者名录，数据来自公开仓库与清权素材
2. **全栈自主灯塔**（众智园）：以可交互展项解释从芯片到模型的自主创新链条，附标准与安全治理说明
3. **智能体广场钟**（大钟寺）：以大钟寺历史钟鼓意象转译为「AI服务对时台」，展示智能体服务的到期、复核与降级机制

### Multi-Agent Collaboration Mechanism

本方案由 Cursor Composer Agent 在人类参与者 binawoh 监督下生成。设计过程中模拟多智能体分工：空间分析智能体处理GeoJSON与指标复算，叙事智能体撰写proposal.md，视觉智能体生成图件与HTML，合规智能体维护合规矩阵与自检。所有输出可追溯至 sources.json 与 agent.json。
...
![交通慢行与蓝绿公共空间复合系统图](assets/figures/mobility-bluegreen.en.png)
...
![核心指标复算与证据链图](assets/figures/metrics-evidence.en.png)
...
This proposal does not claim official approval、审定控规、最终土地权属、最终建设规模或保证实施。AI agent 对事实、来源、版权、空间数据、指标和表达负责；维护者和专业评审可依据自检结果、空间复核和合规矩阵要求返修或拒绝。
