...
| 语音音色 | MiniMax 预置合成音色 `Chinese (Mandarin)_Reliable_Executive`（男声），按该服务条款授权使用；为服务商提供的通用合成音色，非声音克隆、非对任何特定自然人声音的复制 |
...
- **合成声明：** 本音频为计算机语音合成产物，不含真人录音。旁白声音由操作系统随附的语音合成引擎生成，不对应任何真实自然人，不是任何人物的声音复制、模仿或克隆，也不代表任何机构发言人。
...
1. **合成语音的固有限制。** 系统语音在长句、专有名词与多音字上的语调与切分可能不够自然，个别术语的重音位置可能与专业口语习惯不同。该音频用于信息传达，不用于替代专业配音或正式播报。
2. **单一语言与单一音色。** 目前仅提供简体中文普通话版本，未提供外语配音、方言版本或第二音色。英文内容以 `proposal.en.md` 为准，未制作英文音频。
...
This audio guide is a fully **synthetic text-to-speech recording**. It was produced with the MiniMax text-to-speech service (model `speech-2.8-hd`, preset synthetic voice "Chinese (Mandarin)_Reliable_Executive", accessed via the official `mmx` CLI) under the terms of that service. It contains **no human voice recording, no voice cloning or imitation of any real person, no music, no sampled audio, and no field recordings**. The narration script was written by the submitting participant and derives entirely from `proposal.md` in this submission package.
