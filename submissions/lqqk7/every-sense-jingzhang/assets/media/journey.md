# 万感京张 多模态短片 · 分镜、旁白文字稿与权利记录

**关联媒体：** `assets/media/journey.mp4`（视频，H.264 High@5.0 + AAC-LC，1920×1080，30 fps，时长 63.477 秒）
**代表帧：** `assets/media/journey-poster.webp`（片头帧，1920×1080，无损 WebP）
**同源字幕：** `assets/media/journey.vtt`（UTF-8 WebVTT，11 条字幕，时间轴由实测合成结果构造）
**英文字幕轨：** `assets/media/journey-en.vtt`（UTF-8 WebVTT，11 条字幕，时间轴与 `journey.vtt` 同源，文本为同一旁白的英文翻译；无英文配音）/ **English caption track:** `assets/media/journey-en.vtt` (UTF-8 WebVTT, 11 cues, cue timing identical to `journey.vtt`, text translated from the same narration; no English voice-over).
**内容语言：** 旁白与字幕为简体中文；画面关键标注为中英双语
...
本方案的底线原则是「AI 开启与 AI 关闭，服务应当等价」，并要求关键信息不得只存在于单一感官通道。该主张若不作用于方案自身的交付物，即为空话。因此本片按同一原则组织，同一段内容通过三条通道等价提供：

| 通道 | 载体 | 适用情形 | 是否依赖声音 |
| --- | --- | --- | --- |
| 看 | 画面内的双语标注与画面底部的烧录中文字幕 | 听觉受限、静音环境、嘈杂环境 | 否 |
| 听 | 中文合成旁白 | 视觉受限、阅读困难、不便注视屏幕 | 是 |
| 读 | 本文件第二、三节（分镜表与旁白全文） | 无播放条件、断网离线、需要检索或引用、屏幕阅读器逐段朗读 | 否 |

三条通道承载相同表述，不存在「视频版更完整」或「文字版为摘要」的层级差。视频的中文字幕同时以两种方式提供：一是烧录在画面上，播放器不支持字幕轨时仍然可见；二是外挂 `journey.vtt`，便于逐句定位、复制与检索。视频不设自动播放，须由使用者主动触发；播放控件应可用键盘操作。本文件在没有任何播放能力的环境下（纯文本终端、打印稿、屏幕阅读器、离线只读）仍然完整可用，构成视频内容的等价退出路径。

---

## 二、分镜与画面文字对照

时间码格式为 `分:秒.毫秒`。镜头内的图件均为本投稿包自制图版的局部推移，画面文字为本片新增的叠加标注。

### 镜头 01 · 片头 00:00.00 — 00:05.21

- **画面：** 深青底（#10383d）上，品牌图形、标题与副题依次淡入。
- **画面文字：** 万感京张 / EVERY SENSE JING-ZHANG；每一种身体都能独立使用的 AI 城市 / An AI city that every kind of body can use independently；概念建议 / CONCEPT ONLY。
- **对应旁白：** 第 1 句。
...
- **画面：** 深青底，国际传播句、中文表达、边界声明与品牌图形依次淡入并停留至片尾。
- **画面文字：** One city. Many ways to sense it.；一座城市，多种感知，共同行走；**概念建议 · 非法定规划 · 非工程线位 · 非实景记录 / CONCEPT ONLY · NOT A STATUTORY PLAN · NOT ENGINEERING ALIGNMENTS · NOT A RECORD OF SITE CONDITIONS**；旁白由计算机语音合成，无真人录音、无音乐、无实地录音；画面全部来自本投稿包的自有图件与自制动画 / Synthetic narration, no human recording, no music, no field audio. All visuals are original assets of this submission package.
- **对应旁白：** 第 10、11 句。
...
时间码为每句语音的实际起点，对应 `journey.mp4` 与 `journey.vtt`。

| # | 起点 | 旁白 |
| --- | --- | --- |
| 1 | 00:01.20 | 万感京张：一座每一种身体都能独立使用的 AI 城市。 |
| 2 | 00:05.58 | 城市的智能，不以设备数量衡量。 |
| 3 | 00:08.39 | 而以任何人能否理解、选择、完成并退出同一项公共服务来检验。 |
| 4 | 00:14.54 | 总体骨架是一条通用可达底线、三座多感官实验院、十个独立完成点。 |
...
| 旁白句 | `proposal.md` 对应内容 |
| --- | --- |
| 1 | 方案主题句「每一种身体都能独立使用的 AI 城市」 |
| 2—3 | 《设计依据与资料清单》开篇的核心判断：城市的智能不以设备数量衡量，而以任何人能否理解、选择、完成并退出同一项公共服务来检验 |
...
旁白与画面标注中出现的机构名称、区域名称与功能表述，均来自项目公告与智能体任务书在 `proposal.md` 中的引用；其中「本方案对应角色」属于设计判断，不代表官方分工或既定安排。本片未引用任何第三方文字、影像、访谈、口述或档案材料。
...
| 项目 | 记录 |
| --- | --- |
| 画面渲染 | 为本片手写的 HTML/CSS 排版页，经 Playwright 驱动的 Chromium 无头浏览器逐帧截图，1920×1080，30 fps，共 1905 帧。每帧的镜头位置由时间参数确定性计算，不依赖实时动画，因此不存在掉帧或快慢不均。v5.9 重摄只重新渲染受影响的两段帧号区间（第 152—1054 帧即镜头 02 至 04 及其前后转场、第 1392—1572 帧即镜头 06 及其前后转场，合计 1084 帧）；片头、镜头 05 交互实录与片尾的 821 帧逐字节复用上一版渲染结果，未重新渲染。编码封装以音轨 63.477 秒经 `-shortest` 收尾，成片实际写入 1904 帧，渲染序列的最后一帧不进入成片 |
...
| 视频编码 | 首版为 H.264（x264 两遍编码，目标 590 kbps，preset veryslow，tune stillimage，关键帧间隔 60），High Profile Level 4.0，yuv420p，有限色域（tv range，BT.709），`moov` 前置（faststart）。v7.7 为压减包体，以该成片为源用 libx264 CRF 33 单遍重编码一次，音频基本流原样复制（`-c:a copy`），画面尺寸、帧率、帧数、时长与色域参数不变，容器仍 `moov` 前置；重编码后 H.264 Level 由 4.0 变为 5.0，与首版成片的全片 SSIM（全平面）实测 0.9896 |
| 音频编码 | AAC-LC，单声道 32 kHz，64 kbps |
| 成片规格 | 1920×1080，30 fps，视频 1904 帧（63.467 秒），音频 32 kHz 单声道（63.477 秒），容器时长 63.476 秒，3,303,310 字节。v5.9 重摄沿用同一音轨与同一编码参数重出，音频基本流逐字节与上一版一致，全片时间轴、字幕时间码与各句起止时刻均未变化；v7.7 CRF 33 重编码后帧数、帧率、音轨与字幕时间码同样未变，仅文件体积由 5,055,781 字节降至 3,303,310 字节 |
...
- **合成声明：** 本片旁白为计算机语音合成产物，不含真人录音，不对应任何真实自然人，不是任何人物的声音复制、模仿或克隆，也不代表任何机构发言人。
- **人物与肖像：** 全片不出现任何可识别的自然人、人脸、人声、车牌或私人室内空间，不涉及肖像权、声音权或个人信息。
- **音乐与素材：** 不使用任何第三方音乐、音效库、采样素材、影像素材或受版权保护的录音，因此不存在音乐、录音或影像的授权问题。
- **图件与字体权利：** 画面所用图件均为本投稿包自制；地图底图为 OpenStreetMap 公开背景，按 ODbL 署名，仅作背景参照，相关署名已印在原图版内并随图件一并出现在画面中。画面文字使用运行环境预装字体渲染，未嵌入或再分发字体文件。
- **文本权利：** 旁白与画面标注文本由投稿参与者原创，与 `proposal.md` 同源，不含第三方受版权保护文字的引用或改写。
- **商标与名称：** 片中提及的区域与机构名称仅用于说明方案对象与设计判断，不构成商标使用、合作关系声明或授权背书。
- **许可：** 随本投稿包整体适用 `COMMUNITY-DISPLAY-ONLY`，即仅用于本次征集的社区展示与评审场景。
- **容器元数据（2026-08-15 补）：** 成片与两条音频的媒体容器内嵌了标题、作者、合成声明与许可指针（`title`/`artist`/`comment`/`date` 标签），使文件脱离投稿包单独流转时仍携带溯源与「计算机合成、非官方规划」声明；重封装仅复制流不重编码，时长与音画内容逐字节不变，成片体积随容器结构微调。
- **非观测声明：** 本片内容是设计陈述，不是现场观测记录、不是效果图承诺、不是实测数据、不是居民意见征集结果，不得作为现状证据引用。片中出现的边界不是官方红线，面积与数量不是法定或精确指标。片中「AI 开启或关闭，公共任务都能独立完成」及同类语句为**设计验收目标的愿景表述**，不是已达成状态的陈述：截至制作时全部包容性指标未实测（保持 unknown），AI 关闭等价性的当前证据等级为声明完整度与桌面复演，见正文指标章与 A0 板五「验收目标」口径。
...
5. **内容为概念阶段结论。** 片中的空间骨架、区域分工、节点定义与读数均属概念建议。截至制作时，本方案没有官方精确边界、现场无障碍审计数据与真实用户任务基线。
6. **不构成承诺。** 本片不代表任何政府部门、运营主体或第三方的决策、批准、采纳或实施安排。
7. **播放方式。** 本片不得自动播放，须由使用者主动触发；播放器应提供可见控件、键盘操作与字幕开关。
