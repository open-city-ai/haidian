...
| 品牌落地字体策略 | 衬线标题、无衬线正文、等宽数据三分工，具体字体待定 | 待定 | 逐项核验授权，未清权字体不进入公开成果 | 尚未选定。品牌图版（`brand-system.png`）所列字体分工为**设计意向规范**，非发布物嵌入清单；本包唯一嵌入字体程序的载体是四册 PDF（见首行），图版与页面均不分发字体文件 | `proposal.md` 品牌章与「版权与授权」段 |
