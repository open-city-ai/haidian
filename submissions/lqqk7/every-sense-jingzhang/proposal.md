...
万感京张是一条线性走廊的方案。京张铁路遗址公园自西直门向北至北五环，已建成并开放约 9 公里带状公共空间，本方案的主脊即沿这条已建成公共基底组织 [source:JZRP-PHASE2-OPENING-20260806]。走廊上分布众智园、AI 原点社区与大钟寺三处重点区。方案在这条带上做四件事：一条通用可达底线沿公园南北贯通全带，任一感官通道单独可用即可走完；一条低刺激安静替代路径与之并行，供需回避噪声、人流与强光者使用；三处横向接口缝合长期被轨道与大院隔开的东西两侧；十个独立完成节点沿线布点，把服务承诺落成可识别、可停留、可测试、可退出的实体界面。中关村科技服务翼与小月河场景赋能翼分别向走廊供给专业服务与开放场景。三处重点区各有一句骨架判词：众智园双带并列、原点社区一街两院、大钟寺四向一厅。

**核心矛盾。** 本包的现状肌理读数把这条走廊的问题指向同一处：超过四公顷的超大封闭街块占全部街块面积的 52.3%，而场地内跨在营轨道的穿越主证据为零。「封闭」在此只指快照的可步行网络在该街块内未进一步细分这一拓扑事实，不等同于产权封闭或物理围墙；两项读数均为 2026-08-15 定界快照的社区数据计算值，属快照口径，非现场测量、非官方数据。合起来读，这条创新走廊的空间底盘由互不连通的大院构成：纵向贯通与横向缝合是同一个问题的两个方向。本方案的全部设计动作由此出发 [data:visual/assets/osm-fabric/osm-fabric-readings.json] [source:OSM-FABRIC-SNAPSHOT-20260815]。
...
**其一，OP-04 这个节点被本方案自己的证据链退回。** 十个独立完成节点中，OP-04 的 `ai_off_path` 字段写着一条 AI 关闭后的等价路径：字段填得完整、语法合法，两个互不共享代码的实现给出同一判定。本方案随后按自设判据在提交几何上复算这条路径，结果是它依托的东西向连续通道在空间上并不存在——字段合规与空间成立是两件事，本次是后者第一次被检出。处置是把该节点退回未证成立状态：`ai_off_continuity_status` 记为 `premise_unproven_pending_site_audit`，而原 `ai_off_path` 一字未改，因为一旦改写字段，这条链路本身就消失了，剩下的只是一个看起来一直很正确的数据文件。全过程数据、复演脚本与变更回执随包提交，退出码可当场复现 [data:geometry/constraints.geojson#OP-04]。

**其二，万感服务桌的四个位序不可调换。** 大钟寺包容采用市场的最小物理载体是一张万感服务桌，四个位序依次为人工窗口、多通道信息位、AI 开关、退出回执位。顺序是判据而不是布置建议：人工窗口先于一切 AI 构件设立，退出回执位永远在最后一步之后仍然在场。同一张桌子还承诺「不消费也能停留」——休息、如厕、问询与等候不以消费为前提，停留权先于商业界面成立；该承诺已作为属性登记于两处前厅类公共空间要素，深化与运营阶段按要素逐处核验 [data:geometry/public_space.geojson#PUBLIC-007]。服务桌是这套制度里最容易被别的城市直接带走的一件东西。开办这张桌子的七项条件——场地授权、责任角色表、许可与保险、建设与运维两类预算、维护与数据删除、人工兜底、回滚演练——逐项登记于开办条件档案，当前结论如实写作 no-go：七项无一具备，每个空缺字段写明由谁在什么条件下填 [data:visual/assets/service-desk-gonogo.json]。

**其三，点位否决权归属于直接受影响者。** 对于安装在其日常出入口直接相邻范围内的可撤除构件，沿线居民与在地小微商户拥有否决权；行使否决不需要说明理由，也不需要证明自己属于任何一类画像；否决一经登记，该点位的构件在本轮内移除或迁移。与之配套的是异议的去向：异议不进入意见箱，而进入闸门——任何一条异议登记后直接排入最近一次 G3 独立复核的议程，由未参与制作的专业人员与参与者代表逐条给出结论与理由并生成可带走的公开回执；未在 G3 给出结论的异议，视同该点位本轮未通过复核。否决权只适用于可逆构件与限定开放测试，不延伸到法定审批事项 [data:geometry/constraints.geojson#OP-03]。

**官方七项设计要求索引。** 下表只做定位不重述内容，每行指向本文章节与包内可直接打开的证据。
...
**命名对照卡。** 全文与结构化文件反复出现六个前缀，下表一次给出各自的含义与清单位置，读者无须从上下文推断编号体系。

| 前缀 | 含义 | 清单位置 |
| --- | --- | --- |
| OP | 十个独立完成节点 | `geometry/constraints.geojson` 中的十个点位要素 |
| S | 十六张 AI 场景卡 | 本文「AI 创新生态、人才画像与 AI+ 场景」章的场景卡表 S01—S16 |
| JZ | 六个更新工作包 | 本文「更新项目清单、实施政策与分期计划」章的概念项目表 JZ-01—JZ-06 |
| R | 十二条风险条目 | `risk.json` 中的 R-01—R-12 |
| G | 四道证据闸门 | 本文「更新项目清单、实施政策与分期计划」章的 90 天零阶段段落与开放机制表 |
| L | 六级开放层级 | 同章开放机制表的 L0、L1、L2、L3、L4a、L4b 六行 |
...
**现状起点与主动勘误。** 两项与场地直接相关的公开事实在此如实登记。其一，据官方媒体报道，京张铁路遗址公共空间改造提升工程（二期）已于 2026 年 8 月 6 日建成并向公众开放，与一期衔接形成西直门至北五环全长约 9 公里的带状公共空间，直接服务沿线约 70 个社区、约 45 万居民 [source:JZRP-PHASE2-OPENING-20260806]——因此本方案的主脊不是一条待建走廊，而是**已建成公共基底上的可达性补足与接口缝合**：在已经开放的公园上检验「AI 关闭后是否仍能独立完成」，正是本命题最自然的试验场。其二，仓库公开议题已登记两项临时几何的空间不确定性：其一为临时「大钟寺」重点区质心落在北京北站一带、距大钟寺站约 2.26 公里（issue #1029）；其二为临时「总体设计范围」多边形与 OSM 测绘的遗址公园已建段不相交、最近距离约 412.5 米（issue #846）。本方案对前者的处置是**已在自身提交几何中完成修正 [assumption:A-10]**——v6.4 版将该重点区连同其节点、接口与概念体量整体归位至真实大钟寺板块，修正过程与全量重算记录见 `changelog.md`；对后者，本方案「沿京张铁路遗址公园组织」的表述指向公告文本所述的遗址公园沿线，与继承自任务包的临时总体范围几何之间存在上述已登记偏差，具体衔接待官方边界确认后整体重算 [assumption:A-11]。两项议题均已写入前提与风险登记。
...
品牌采用 **EVERY SENSE JING-ZHANG**，国际传播句为 **One city. Many ways to sense it.**，中文表达为“**一座城市，多种感知，共同行走**”。品牌的判词只有一句：**城市的智能，由最不便利的那个人来证明。** 面向公众的传播语是**万感四问**——看得懂、挑得动、做得完、**走得掉**：前三问问的是通道，最后一问问的是权利，「走得掉」是传播上的第一锚点，因为它最难做到也最少被承诺。视觉语法从铁路“统一轨距”提炼为原创的平行感知线与开放节点：标志、导视与活动视觉不借用企业商标、人物肖像或未清权字体；文化比喻也不替代京张历史研究。

品牌系统按四项展开，图形承载以 `assets/logo.svg` 与品牌系统图为准，均为概念建议。**配色体系**沿用六色板并规定语义：深青 `#10383d` 为铁轨与夜行底色，用于深色底与主文字；水鸭青 `#177c78` 表示通用可达底线与“可用”状态；珊瑚 `#d76450` 只用于人工接管、急停与求助等需要人介入的状态；紫 `#6b5b95` 标记共同设计与参与者证据；金 `#d6a13b` 标记文化与遗产层；米白 `#f2efe7` 为基底与留白，控制眩光并保证对比。颜色不得单独承载信息，任何以颜色区分的状态必须同时具备形状、文字或触觉冗余，在色觉差异与低照度条件下仍可识别。**字体策略**采用衬线标题、无衬线正文与等宽数据的双语分工：中西文衬线体承担标题与文化叙事，中西文无衬线体承担正文、导视与界面，等宽体承担指标、编号与代码；双语并置以基线对齐和等高比例控制，中文不作人为压缩，西文不使用全大写长句，所有字体逐项核验授权，未清权字体不进入公开成果。**延展规则**以平行感知线表示“同一目的地的多种通道”，以开放节点表示“可停留、可求助、可退出”：导视上以浮雕与高对比印刷双载，界面上作为布局网格与状态条而非装饰性动效，印刷物上作为版心与分栏依据；线条不得改造为企业商标形态或人物肖像。**应用场景**覆盖站点导视、触觉铺装、活动视觉与国际传播物料四类，其中活动物料须可撤除、可维修，传播物料须同时提供易读与高对比版本。具体尺寸、材质与现场参数待现场共创和专业审查后确定；交付物自身的字号与对比度已按包内色板实测（正文对比度 12.69、警示行 7.13，满足 WCAG 2.1 AA；核心图最小字号 12.1—12.3 px、A3 页面文本层最小 9.07 pt），实测方法与逐项结果见质量自查记录 [data:visual/assets/qa-proofing-record.json]。
...
更新动作采用“先审计、再可逆试验、后专业深化”。连续路径优先识别信息断点、缺少休息与人工求助的位置、单一手机或账户依赖、感官负荷突然变化和跨街联系不足；三处接口布置可移动的多通道导视、安静停靠、可调高度交互台、人工接管灯和无账户服务台。建筑层面只提出首层公共界面、共享测试空间与可逆适配方向，不判断具体建筑保留、改造、拆除或新建 [assumption:A-BUILDINGS-AND-OWNERSHIP-001]。
...
空间动作与角色严格匹配。众智园设置验证门槛、状态可视界面和人工接管点，避免未成熟技术直接进入开放空间；原点社区设置低刺激路径、共同设计室、易读服务前厅和真实反馈台，任何参与都须自愿、可理解并可退出；大钟寺设置多通道消费界面、现金与人工替代、公开采用回执和维护入口；其最小物理载体是一张**万感服务桌**——四个位序为人工窗口、多通道信息位、AI 开关、退出回执位，顺序不可调换：人工窗口先于一切 AI 构件设立，退出回执位永远在最后一步之后仍然在场。服务桌是制度里最容易被别的城市直接带走的一件东西。采用市场同时承诺**不消费也能停留**：休息、如厕、问询与等候不以消费为前提，停留权先于商业界面成立——该承诺已作为属性登记于两处前厅类公共空间要素，深化与运营阶段按要素逐处核验 [data:geometry/public_space.geojson#PUBLIC-007]。三处通过公园主脊和两翼支持相连，形成技术问题向生活任务转译、生活证据向产品和运营反馈的循环。
...
六类画像用于组织需求与测试，不是诊断标签、商业分群或已经完成的用户研究 [metric:persona_count]。每类画像都对应空间动作、设计提出的服务等价目标和隐私边界；真实共同设计须由参与者自愿加入，获得合理便利，理解数据用途并随时退出 [assumption:A-CO-DESIGN-001]，共同设计席位是否覆盖必需角色由角色覆盖率衡量而非由人数衡量 [metric:co_design_role_coverage_ratio]。中国无障碍环境建设法支持平等参与与无障碍信息交流的方向，但不能替代具体产品、建筑或交通的专业符合性评价，也不把本方案的“等价服务”概念自动转化为具体法定义务 [source:ACCESSIBILITY-LAW-CN]。该法关于现场指导与人工办理的要求，严格限于其条文列举的医疗、社会保障、金融、生活缴费等公共服务事项；六类画像与合规矩阵按此边界组织，方案其余的人工兜底均标记为概念设计建议 [standard:BARRIER-FREE-ENVIRONMENT-LAW]。换一句更准确的说法：下表是任务约束维度表而不是人群分类表——每一行给出的是一组任务约束（信息获取通道、沟通方式、行动方式、认知与感官处理、语言与数字门槛、服务提供与维护），不用于识别、标注或统计任何人。
...
前面各章把判据分散在场景卡、节点属性、指标口径、风险条目与开放层级里，读者要拼出“怎样才算做到了”，需要在五处之间来回翻。本章把这些判据封装成一件独立的、可被任何城市取走就用的东西：**服务等价基准（Service Equivalence Baseline，SEB）v0.4**。判据的价值最终由外部采用检验，两例采用登记见下 [source:AGENT-TASKBOOK] [metric:op_node_count]。

基准由五个组件构成（v0.4 起）。每个组件在本包内都有对应的机器可读位置，复用者不需要重新发明，只需要接受同一套约束。
...
**推演对象与工具。** 推演以场景 S01 多模态无障碍寻路在 OP-01 多模态到达校正点上的一次完整闸门过程为对象 [data:geometry/constraints.geojson#OP-01]。三件工具随包提交，均为零依赖的离线文件：`visual/assets/seb-tabletop-run.js` 是校验器，读取基准与样例并逐条判定；`visual/assets/seb-tabletop-fixtures.json` 是四十条样例，正例十三条、反例二十七条（v0.4 扩充二十条覆盖生命周期、权限边界与测量声明类判据的正反分支），每条自带预期判定与双语说明；`visual/assets/seb-change-receipt-sample.json` 是变更回执的格式与两条回执。在 `visual/assets` 目录下执行 `node seb-tabletop-run.js` 可复现本章全部结论，退出码为零表示四十条样例的判定与预期完全一致；校验器在判定之前先校验基准版本与样例声明是否匹配——样例声明的版本须与所用规范快照严格一致，不一致即以退出码二拒绝运行而不作任何判定，采用方按「快照入包」方式采用时快照与样例自然同版，无须随本包演进重跑 [source:PACKAGE-STRUCTURED-EVIDENCE-INDEX]。样例不含任何计数或比例，分母只以“哪些类别被声明”的方式表达，因此工具无法、也不会生成读数 [metric:op_node_count]。
...
| 四级 G3 复核 | 同一停止条件重复触发，或异议未在本轮得到结论 | 建议排入最近一次复核 | 由未参与制作的专业人员与参与者代表复核并出具公开回执 | 结论按基准 v0.4 四选一：通过、有条件通过、退回整改，或关闭 AI 层保留服务 |
...
**环节二的第一个结果，是一个此前没有人算过的数字。** 按上述规则切出的 AI 关闭态网络总长约 17.3 公里，但它不是一张网，而是**两个互不相连的分量** [metric:ai_off_declared_network_length_m] [metric:ai_off_network_component_count]。通用可达底线与低刺激安静替代路线是两条平行的南北线，三条横向接口是它们之间仅有的连接，而三条都未声明 AI 关闭后的等价性。这句话的空间含义是：在大钟寺片区，一旦自动化关闭，一个人可以沿南北方向走很远，却无法向东或向西跨过去。

**环节二的第二个结果，是两项任务都不可达。** 两项任务取自 S04 场景的日常形态：由 OP-04 前往同一片区内的 OP-08 无账户消费与维护退出点，以及前往 OP-09 活动感官负荷退出点。AI 开启状态下两段行程分别为 244 米与 471 米；AI 关闭状态下两段**均不可达**。档案 v0.3.0 起的判定口径是接口固定：每个点位的物理挂接线在全部五条提交中心线上确定一次——门前的路不会因为 AI 开关而换成另一条——两个点位的接口线都是大钟寺四象限包容采用接口，而这条线未声明 AI 关闭后的等价性，于是关闭态下两点**连接入都没有**，不是绕远，是接口本身未被证明 [data:geometry/roads.geojson#ROAD-004]。接入距离随之只有一组可比读数：AI 开启态 OP-08 为 55.5 米、OP-09 为 189 米；关闭态接口缺席，无接入可言。
...
| --- | --- | --- | --- | --- | --- |
| A 直接路径改线 | 把大钟寺四象限包容采用接口自与通用可达底线的交点起、沿接口向市场一侧延伸至覆盖两个点位接入点的一段，升级为固定多通道导视、连续无障碍地面与有人求助位 | 282 米，改造既有中心线，不新增几何 | 轨道运营、市政、道路三类主体 | 244 米／471 米 | 1.00／1.00 |
| B 安静替代路径增补 | 自通用可达底线于 OP-09 同纬度处向西增补一段地面连接，伸至市场组团西缘接入点；该段位于横向接口以南，不穿越站点范围 | 282 米，新增概念连接，与 A 相当 | 用地权属、地面连续性两类 | 616 米／469 米 | 2.53／0.996 |

两个备选的差异是真实的取舍，不是优劣：两案工程量相当（均约 282 米），分野全在依赖与形态——备选 A 行程与 AI 开启态完全等价，但其可用性明写在数据里，该横向接口的 `design_role` 是“待官方道路、轨道与市政资料”，本方案无权代这三类主体承诺；备选 B 只引入用地权属与地面连续性两类核验，且 OP-09 恰落在新线上（T2 全程甚至比开启态短 2.10 米），代价是 OP-08 需向南绕行、T1 行程增加 373 米。
...
**人工复核以下述三条准则进行，结论均为不通过；三条均登记为方案方桌面推演，不代表参与者、专业机构或使用者代表的意见。** 其一，接入段按直线处理且假定可通行，这一假定未经任何审计，是本次核算最大的不确定性来源，而备选 B 下 OP-08 的 244 米接入段完全建立在它之上。其二，OP-08 是无账户消费与维护退出点，其功能定义要求就近可达，备选 B 把其接入距离拉长至 AI 开启态的 4.39 倍是否可接受，属于使用者的判断——**本方案不设阈值、不代答，把该问题原样交给现场共测**。其三，备选 A 的可用性取决于轨道运营、市政与道路三类主体的资料与授权，方案不得把未取得的授权写成已成立的路径，这与回执机制中“未采纳理由不得引用尚未存在的授权”属同一条纪律 [source:W3C-ACCESSIBILITY-PRINCIPLES]。
...
[4] 环节四 AI 关闭反事实 / Stage 4 AI-off counterfactual
    BASE_ON T1 OP-04→OP-08            全程 / total 243.514 m（接入 55.505 m，挂接 ROAD-004）
    BASE_OFF T1 OP-04→OP-08           不可达 / unreachable（接口线不在 AI 关闭边集）
    BASE_OFF T2 OP-04→OP-09           不可达 / unreachable（接口线不在 AI 关闭边集）
    ALT_B_OFF T1 OP-04→OP-08          全程 / total 616.487 m（接入 243.585 m，挂接 ALT-B-LINK）
    T1 行程比 / trip ratio : ALT-A 1.000 · ALT-B 2.532（+372.973 m）
    OP-08 ALT-B 接入距离比 对开启 / ALT-B approach ratio over AI-on : 4.389
...
叙事的历史锚点取公有领域通识史实：京张铁路于 1905—1909 年由詹天佑主持设计建造，是首条由中国工程师自行设计施工的干线铁路；青龙桥站以「人」字形折返线闻名，其线路工区自通车起持续运转逾百年 [source:JINGZHANG-RAILWAY-QINGLONGQIAO-NRA]。这三件事实只用于文化叙事定位——「自主设计」对应本方案「自主可核对」的生态判据，「人字形以展线换坡度」（延长运行距离以换取可行坡度）对应「以等价通道换包容」的设计转译，「逾百年持续运转的工区」对应「维护与纠错作为常设公共职能」的运营主张——不携带保护等级、边界或规划结论；其中 1909 年建成通车与首条自主设计施工干线铁路属公有领域通识史实，人字形折返与百年工区见所挂官方页面，文保判断一律以官方档案为准。
...
空间文化系统按三级组织，并与既有空间骨架同构。**主脊叙事线**沿京张遗址公园连续布置触觉文化带，任何一段都可停下、可跳过、可重复；**三处地标**分别承担三种公开记录职能：众智园“共轨门”公开通用接口的验证状态，AI 原点社区“万感原点”公开共同设计记录与未解决障碍，大钟寺“百感台”公开采用回执与事故退出反馈；**节点级微叙事**为十个独立完成点各配一段简短、默认关闭、可自行开启的解说。文化表达一律遵守 AI ON = AI OFF 原则：断网、断电或用户拒绝授权时，铸刻文字、盲文、触觉浮雕与固定图形仍可读，历史内容不会因系统关闭而消失 [depth:blue_green_public_space]。
...
公共界面不以自发光动效作为唯一状态提示，不设置拟人化机器人形象作为服务入口，不以技术贴图替代文化内容——三条均为现场可逐项核对的二值判据。国际传播以 **One city. Many ways to sense it.** 为主句，中文表达为“一座城市，多种感知，共同行走”，并按受众分层：面向国际专业受众公开测试方法学、协议与失败样本；面向国际公众以 AI 开启与关闭的成对对照说明什么叫服务等价；面向国内公众与沿线居民提供易读中文、手语与字幕版本；面向开发者社区开放问题库与场景测试规程。所有对外表述保留人工校订与纠错，不以机器翻译结果作为正式版本；多语言扩展在具备母语校订与相应使用者评估之前不发布。本章涉及的活动、合作、传播计划与地标名称均为概念建议，不构成审定、批准或既定安排。
...
公共空间组件形成可复用而非固定工程的工具箱：多通道导视柱在触觉、声音、图形和易读文字间提供冗余；安静停靠点允许恢复注意力；可调高度交互台服务不同身高与姿态；触觉文化带连接可追溯史料；人工接管灯公开系统状态；无账户服务台保证拒绝授权者仍可获得基本帮助。组件成效以独立使用、理解、错误恢复和维护可达衡量，具体尺寸与材料须由真实参与者和专业团队共同测试。组件库的可移动、可维修与低成本试用特性同时为青年创业团队提供低门槛参与入口——不需要场地授权的仅限桌面推演、非现场的自有设备样机测试与不占用公共空间的自测；一旦进入指定点位或任何公共空间，场地授权、保险、无障碍与安全闸门（L1 准入条件）全部适用，测试记录进入公开问题库。主脊沿线的安静停靠点与可调高度工作台在非测试时段默认对公众开放，不设预约、注册或消费门槛——青年友好的第一条判据是「不消费也能停留」，而不是新增多少青年空间。

三处公开记录节点承载文化和运营：众智园“**共轨门 / Common Gauge Gate**”公开通用接口的验证状态；原点社区“**万感原点 / Every Sense Origin**”记录共同设计者、问题、修订和未解决障碍；大钟寺“**百感台 / Hundred-Sense Forum**”公开采用、维护、事故和退出反馈。三处地标以公共空间图层中的概念界面承载，不是已选址、已批准或已清权的建成构筑物 [metric:landmark_count] [data:geometry/public_space.geojson#PUBLIC-001]。京张历史、中关村创新史、字体、图像、声音与人物材料均须逐项溯源和清权 [source:AGENT-TASKBOOK]。
...
首个 90 天只建立“零阶段证据包”，不承诺开工或开放运营 [assumption:A-IMPLEMENTATION-001]。第 0—30 天进入 **G0 资料与许可闸门**：核对官方范围、现场路线与服务清单，形成伦理、同意和数据处置协议，并把十六个场景改写成可复测任务脚本；R-01、R-02、R-05、R-06 任一责任或资料边界不清即暂停。第 31—60 天进入 **G1 可逆原型闸门** 与 **G2 封闭成对测试闸门**：制作可移除组件，在受控环境完成 AI 开启/关闭成对测试，交付问题日志、维护卡和匿名化测试证据；G2 收尾设 **AI 关闭日演练**——连续一个完整测试日全程保持关闭态运行，全部任务只经固定导视、触觉引导与人工通道尝试并完整记录，演练记录（含未完成任务及原因，不预设全部完成）作为 G3 复核的必交证据，把「AI ON = AI OFF」从主张钉成一项可执行、可复核的测试设计；R-03、R-04、R-07、R-08 的替代路径、运维和参与条件未闭合时不得升级，不得以内部演示替代参与者证据。第 61—90 天进入 **G3 独立复核闸门**：由未参与制作的专业人员与参与者代表复核，逐项检查 R-01—R-12 全部十二项风险条目的停止条件和恢复证据 [metric:risk_item_count]，结论按基准 v0.4 四选一：通过、有条件通过、退回整改，或关闭 AI 层保留服务；四道闸门缺一不可，未通过 G3 不进入开放空间 [depth:phasing_implementation] [metric:gate_count]。
...
**为什么是这个节点。** OP-04 是全案唯一一个被自己的证据链退回的节点：按提交几何复算，AI 关闭状态下不存在一条不依赖 AI 的连续路径把它与同区 OP-08、OP-09 连起来，裁决按本文《首批试点点位选取准则》既有规定将其保持在审计状态 [data:visual/assets/seb-op04-chain-data.json]。对这样一个节点做深化，检验的不是画图能力，而是一套治理框架在最不利条件下是否仍然自洽。深化图版给出 1:500 总平、1:200 缝合区、1:50 断面与八要素卡（净宽、纵坡、过街、材料、排水、触觉、声环境、维护边界），全部尺寸为设计目标值，每张卡带「待现场审计」状态，依据框架为《无障碍环境建设法》与 GB 55019-2021 [source:GB-55019-2021-ACCESSIBILITY-CODE] [standard:BARRIER-FREE-ENVIRONMENT-LAW]。

**交接是一份双联单，不是一句口头移交。** 人工接管在运行层落为双联交接结构：交出方与接收方必须是不同角色并各自署名；交接单上的未决项不得在交接时清空，只能随单移交；接收方对每一单在「接收 / 拒收 / 暂缓」三项中选一并署名，拒收与暂缓必须写明理由并触发上一责任方继续在岗。该结构在本节点先行试运行；将其升格为服务等价基准 node_schema 判据的提议，按版本治理登记为 v0.4 之后的待议项，本版不改动基准条文。**独立接管演练**按 SEB v0.4 L4a 就绪证据展开为六项可核对清单：无供应商在场完成开机与关机、执行一次完整双联交接、按维护卡处置一项标准故障、演示 AI 关闭态下的全部人工服务路径、签发一份退出回执、调取并解释最近一次演练记录——两名非原团队人员逐项完成并签收，培训签收单与服务等级承诺随演练记录同卷归档。
...
**听得见的断点。** 连通性档案的两组读数被翻译为声音：一秒等于二十米，AI 开启段十二点二秒行进音走完约二百四十四米到达；AI 关闭段按接口固定口径没有一米可走——**声音在起点即归于静默，静默的位置就是数据里接口缺失的位置**（静默区实测 RMS 为零）。中英旁白两版随包，映射规则逐项可由档案读数复算（映射说明 `assets/media/audible-break.md`）[data:visual/assets/seb-op04-chain-data.json]。
...
测试协议先于目标值。每项任务必须固定起点、终点、成功判据、合理便利、人工接管条件和退出方式；同一参与者按平衡顺序完成 AI 开启与关闭两轮，分别记录理解、选择、完成、错误恢复、求助和退出。分母须同时报告有效任务、撤回、技术故障与非完成原因，不能删除“失败样本”抬高结果。涉及身份、健康、障碍类别、位置或行为的数据默认不采集；确有必要时另行取得明确同意、限定保留期并由人工复核。当前不设置虚假的数值达标线，G2 只判断证据是否完整，G3 再由参与者代表和独立专业人员按基准 v0.4 四选一记录结论：通过、有条件通过、退回整改，或关闭 AI 层保留服务 [metric:independent_task_completion_rate]。
...
项目依据包括官方资格预审公告、经清权的智能体任务书、场地资料包、允许设计空间、来源登记、临时边界依据与专业标准快照 [self_check:PROFESSIONAL_EVIDENCE]，其中经处理的事实包只用于交叉核对，不作为独立事实来源 [source:PROCESSED-FACT-PACK]。通用设计依据包括《中华人民共和国无障碍环境建设法》、W3C Accessibility Principles 和 WHO Disability and health；它们支持平等参与、障碍移除与多模态界面原则 [assumption:A-CASE-TRANSFER-001]，不用于宣称场地合规、把服务等价概念解释为具体法定义务，或生成本地人口结论 [source:ACCESSIBILITY-LAW-CN]。
