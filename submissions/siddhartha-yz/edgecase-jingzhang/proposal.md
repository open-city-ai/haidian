...
| 当前等级 | 允许开展 | 放行或继续运行前必须证明 | 不满足时 |
| --- | --- | --- | --- |
| T1 受控试验 | 受控重放、设备与传感冲突、急停演练 | 失败条件可复现；停止或断电有效；记录可供复盘 | 不进入社区；修复后重新受控测试 |
| T2 社区共测 | 知情参与的无障碍、跨语言、公共服务与机器人共测 | 退出和非AI路径同等可达；人工接管在岗；数据最小化 | 切回柜台、纸面导视或人工引导；暂停共测 |
| T3 日常开放 | 日常导航、拥挤运营和智能体交接试点 | 责任主体明确；容量、低置信度等触发条件可执行；人工售后可达 | 切换为人工运营或下线；记录问题并回到 T1/T2 |

这张表是责任主体逐项核对的最小证据清单，不是自动升级器，也不构成部署、审批或安全认证。T3 没有未定义的“下一等级”；第三列在该等级表示继续运行的门槛。
