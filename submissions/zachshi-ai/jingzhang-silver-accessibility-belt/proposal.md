...
### 核心机制：银龄接力（Silver Relay，本方案原创）

AI 适老服务的通病是"机器处理老人、人际连接消失"——老人被变成被动接收者。本方案提出原创的**银龄接力机制**：每个 AI 适老服务点在服务边界必须把接力棒交给具名人工，老人始终是主动方 [E:SILVER-RELAY-MECHANISM] [standard:BARRIER-FREE-ENVIRONMENT-LAW]。

**五条接力规则：**
- **R1 人工交接**：AI 处理到边界必须交棒给具名人工（志愿者/家属/社区工作者）——就医导航结束即转导诊陪同，办事清单结束即转窗口人工核对。
- **R2 签收确认**：交接有签收——老人或其代理确认收到服务，未确认即视为服务未完成。
- **R3 回访闭环**：服务后 24-72 小时内必须回访，确认服务真正完成而非"已答复"。
- **R4 无AI等价**：老人可全程走人工通道，不受任何 AI 环节影响——AI 是加法不是前提。
- **R5 代际通道**：每个服务点配置代际志愿者通道（青年帮扶机制），让数字帮扶成为代际连接而非技术负担。

**10 个适老场景的接力合同**（完整见 `visual/assets/silver-relay-contracts.json`）：

| 场景 | AI 做什么 | 接棒人（R1） | 回访（R3） | 无AI路径（R4） |
| --- | --- | --- | --- | --- |
| 银龄就医导航 | 挂号/科室/取药导航 | 健康服务台志愿者/导诊 | 24小时回访确认就诊 | 社区健康热线全程人工 |
| 银龄办事导航 | 社保/医保/民政材料清单 | 街道办窗口值班员 | 48小时回访确认受理 | 直接到窗口全程人工 |
| 银龄代取代办 | 重物/药品/证件下单 | 社区志愿者/代取专员 | 24小时回访确认送达 | 志愿者上门全程人工 |
| 银龄无障碍出行 | 无障碍路线导航 | 无障碍专员/志愿者陪同 | 24小时回访确认安全到达 | 电话预约陪同 |
| 银龄数字帮扶 | 手机使用教程 | 代际志愿者（青年学生） | 72小时回访确认学会 | 社区数字帮扶站人工教学 |

**规则闭合验证。** `run_silver_relay.js` 对 10 场景 × 6 条规则分支（完整/缺接棒人/无同意/无签收/缺回访/缺无AI路径）共 60 个合成案例全部正确分类（50 阻断/10 接力），证明接力规则逻辑闭合——但仅证明分类正确，不构成现场适老服务、合规或授权证据，现场绩效仍为 null、状态 `not_authorized_not_run` [data:visual/assets/relay-tabletop-evidence.json#blocked]。

**与"老人是主动方"的关系。** 银龄接力的核心不是让 AI 更懂老人，而是**让每个 AI 服务都必须把接力棒还给一个具名的人**。签收、回访、无AI等价三条规则共同保证：AI 可以帮忙，但永远不会"替老人决定"——老人始终是服务的主动方，机器始终是助手 [standard:ELDERLY-SMART-TECH-PLAN-2020-45]。

### 核心机制：代际连接站（Intergenerational Connection Points，本方案原创，v5）

银龄接力解决了"AI 必须交棒给人"，但还缺一个更根本的追问。本方案提出反直觉事实：**AI 适老服务越多，老人越孤独**——技术替代了人际接触，服务"更高效"了，连接更少了。银龄接力的核心问题不是"办得多快"，而是**"老人与社会的连接是否增强"**——以"连接"为透镜（正如 human-hours 以时间为透镜，本方案以连接为透镜）[E:SILVER-CONNECTION-POINTS]。

**设计原则：每个 AI 服务点不是"服务终点"，而是"连接起点"。** AI 处理事务，但空间设计的目标是让老人留下来说话——办完事之后，空间引导老人与志愿者、家属、同龄人产生连接 [data:visual/assets/connection-points.json#design-principle]。

**5 座代际连接站（沿走廊布设，完整规格见 `visual/assets/connection-points.json`）：**

| 站 | 位置 | 连接设计 | 无AI等价 |
| --- | --- | --- | --- |
| CP-01 原点社区站 | 银龄服务驿站 | 对坐式连接长椅（非排排坐面向屏幕）+ 代际留言墙（手写便签，AI 不参与） | 社区活动室：茶水/长椅/值班志愿者 |
| CP-02 众智园站 | 银龄健康驿站旁 | 代际结对角：青年志愿者轮值，老人可预约陪聊/陪走/陪学 | 志愿者轮值角 |
| CP-03 公园中段站 | 中段驿站 | 对坐长椅+公园活动公告板（纸面） | 公园休憩点+公告板 |
| CP-04 大钟寺站 | 银龄服务港 | 代际文化角：老手艺/老故事分享台，老人与年轻人互教 | 文化角，互教活动照常 |
| CP-05 南端站 | 南端服务节点 | 对坐长椅+社区照片墙 | 社区休憩点+照片墙 |

**连接回执（服务的真正成功指标）。** 每次服务除接力回执外，并行生成**连接回执**：这次服务是否产生了连接、与谁、是否有回访、连接是否保持。接力回执证明"有人负责"，连接回执证明"有人连接"——**服务办成了但老人依然孤独，不算成功**。10 张连接回执完整见 `visual/assets/connection-receipt.json` [data:visual/assets/connection-receipt.json#receipts]：

| 场景 | 连接设计 | 回访 |
| --- | --- | --- |
| 银龄就医导航 | 导诊陪同转为陪同交流 | 24h：是否认识了新的人 |
| 银龄数字帮扶 | 代际志愿者教学即连接（固定结对） | 72h：是否保持结对联系 |
| 银龄助餐服务 | 助餐点同桌用餐（鼓励拼桌） | 72h：是否与同桌人再次相约 |

**负空间基线。** 无任何 AI 服务时，传统社区网络（志愿者/家庭/社区活动室）的连接功能完整——AI 关闭后连接不中断 [data:visual/assets/connection-points.json#negative-baseline]。
