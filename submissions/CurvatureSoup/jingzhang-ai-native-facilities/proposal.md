...
本次概念方案把“AI-native, not AI-dependent”作为设施级设计约束：AI、机器人和自动驾驶被视为会直接进入、停靠、排队、装卸或占用容量的城市使用者；城市空间因此可能发生真实的空间、尺度、结构、接口或运营重构，但 AI 退出后仍保留基本服务。[source:AGENT-TASKBOOK] [depth:overall_spatial_structure]

### AI 原生设施方法 v0.1

六类首批设施采用统一记录矩阵，依次回答 Who、非 AI 基准 B0、Why change、Physical change、Compatibility、AI-Native、AI-Degraded、AI-Off、基础服务 S_min、Second Life、ATS、R-Score 及三未来压力测试。反向设计先作 `retain / adapt / reconstruct` 判断；仅增加摄像头、传感器或后台算法的对象不满足 AI-native 门槛。ATS（AI Transformation Score，AI 转型分）与 R-Score（Urban AI Reversibility Score，城市 AI 可逆性分）分别衡量本质改变和物理—运营可逆性，不相加；当前六条记录因缺少现场容量、恢复时间目标、资本重置价值和独立复核，均保持待校准状态，不宣称进入 High–High。[metric:ai_native_facility_count] [data:visual/assets/ai-native-facility-matrix.json#FAC-ROAD_INTERSECTION-001]

每个设施均保留 AI-Native、AI-Degraded、AI-Off 三态路径。Degraded 状态逐项考虑网络、定位、模型、传感、供电和厂商接口故障；Off 状态只保证基础城市服务，不承诺继续提供 AI 专用服务。AI 专用部件必须有 Second Life，例如机器人停靠站转为自行车维修或社区物流点，边缘节点转为应急通信或公共网络节点。AI Boom、AI Fragmentation、AI Retreat 三种未来均在矩阵中登记；任何 FAIL 或 unknown 都会阻断 High–High。[metric:ai_transformation_score] [metric:urban_ai_reversibility_score]

![AI 原生设施方法链：从城市使用者、反向设计和双轴评价，到三态运行、Second Life 与三未来压力测试](assets/figures/ai-native-method-chain.png)
...
本次加入的 L2 公开试跑层位于 `visual/assets/osm-trial-layers.json`，内容为 GeoJSON FeatureCollection，由 OpenStreetMap Overpass 提取并按 ODbL 记录。它只用于建筑、道路、蓝绿空间和公共设施的视觉肌理与概念网络；Microsoft Global ML Building Footprints 已核查公开索引，但临时范围对应 quadkey 没有可下载分片，因此没有用其他地区数据替代。两者均不改变官方边界、控规或 ATS/R-Score 的未知状态 [source:DATA-SRC-OSM-L2-TRIAL-20260828] [source:DATA-SRC-MICROSOFT-ML-BUILDINGS-20260828]。
