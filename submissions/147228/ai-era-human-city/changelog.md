...
- 增加四条普通人路线的中英文 G0 回执图板与机器可读路线图谱，把代际共学、城市 API、夜间人工服务和技能再造放入同一套普通服务—人工接管—申诉撤回—独立复演阅读面。
- 增加引用解析、路线覆盖、五步顺序与 G0/HOLD 边界的确定性测试；继续保持零授权、零现场数据、`unknown` baseline、`not_run` 结果和空绩效。
- 本轮只增加合成回放的可审阅表达，不新增 geometry、路线长度、服务承诺、许可、部署、居民体验、AI 绩效或官方评分主张。
