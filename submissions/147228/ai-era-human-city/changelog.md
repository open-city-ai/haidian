...
- 将 AI 原点普通服务节点的相对平面写成 9 个低置信 formal features，分别表达 30×18 米包络、普通路径、人工服务、低刺激等候、可撤回服务岛、净空环、维护带和两个退出口。
- 以临时重点区内已有的 `SCN-03` 为概念中心，要求包络同时落在 `PUBLIC-03` 与 `PROV-KEY-002` 内；`PUBLIC-02/SCN-02` 因位于临时重点区外，只保留为跨区技能情境，不作本节点选址证据。
- 扩展既有 runner，验证 9 个引用、宿主包含关系、投影后多边形包含、目标尺寸、普通路径与服务岛分离、净空环和区外情境排除；负例由 12 个增至 16 个。
- 对齐中英正文、网页、图板、假设、空间索引和详细设计证据行；全部坐标仍为参赛者提出的低置信概念绑定，0 授权、0 现场观察、HOLD。

## v2.8 - 2026-08-22

- 将 ALT-C 深化为 AI 原点一个可追问试点：新增 `1:500` 参数化平面、`1:100` 界面剖面与 `1:50` 交接细部，明确普通路径、人工/纸面服务、可撤回 AI 服务岛、停用与维护关系；所有数字保持低置信度设计目标并要求专业重算。
- 新增六段条件式交付路径、五类资产 RACI、两阶段竞争性采购、透明 CAPEX/OPEX 公式、五条未观察 SLA、保险/验收/退役闸门；机构、预算、供应商、现场结果和启用决定均未确认。
- 新增确定性 runner 与 12 个 fail-closed 负例，拒绝伪造官方边界、现场观察、容量、业主、成本、SLA 或公众走查；四份 PDF 改用同一 16 页双语 storyboard、同方向和同页序生成。
- 保留 0 授权、0 现场观察、HOLD、`release_capacity=null` 和 `field_acceptance=not_run`；本轮不主张许可、工程可行性、采购承诺、实施绩效或官方评分。
