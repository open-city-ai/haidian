...
## One Spatial Decision: Keep the City Human Before Locating AI

The ordinary-person journey and continuity receipt still need an explicit spatial choice. Three organizations of the same human commons spine are reviewed side by side: **ALT-A AI showcase spine** makes the device frontage the public default and demotes ordinary, screen-free and staffed routes, so it is rejected; **ALT-B three isolated AI rooms** contain test risk but split cross-node maintenance, appeal and public-asset retention, so it must be revised; **ALT-C human commons spine plus reversible service islands** keeps ordinary passage, paper/staffed equivalence, accessible continuity and public assets first, while AI components remain pausable and removable, so it advances to design review. Selecting ALT-C is not authorization; the decision remains HOLD [data:visual/assets/ai-era-spatial-decision.json].
...
The drawings let a reviewer reject a specific dimension, route or rights relation without treating the node as built. `run-ai-era-pilot-dossier.js` validates nine formal refs, host containment, target dimensions, separation between the ordinary route and service island, the clear ring and exclusion of outside-area context. Sixteen negative fixtures reject fabricated official geometry, field observations, capacity, ownership, costs, SLA, public walkthroughs or spatial bindings. The node remains at zero authorisations, zero field observations and HOLD [data:visual/assets/run-ai-era-pilot-dossier.js] [data:visual/assets/test-ai-era-pilot-dossier.js] [depth:three_key_area_detailed_design].
...
The previous section answers which public and reversible relationships to compare in the three key areas. This section lowers the reading scale again to one node sequence: **ordinary arrival → human explanation/service → bounded state → freeze/replay or exit**. `visual/assets/ai-era-spatial-interface-plans.json` registers the v1.3 provisional-geometry context anchors, functional bands, entry evidence, and stop conditions for each area. AI Origin's `PUBLIC-02/SCN-02` pair remains cross-area skills context only and is not v2.9 pilot-siting evidence. It adds no dimensions, changes no geometry, and does not upgrade a concept interface into an existing facility or engineering proposal. [data:visual/assets/ai-era-spatial-interface-plans.json] [depth:overall_spatial_structure]

| Node | v1.3 context anchors (not v2.9 pilot siting) | Functional bands (no scale; state and order only) | Spatial action on failure |
|---|---|---|---|
...
| AI Origin Community (cross-area skills context) | `MOBILITY-04` → `PUBLIC-02` → `SCN-02` → `GREEN-03` | Daily arrival/dwell → paper, phone, staffed service → reskilling/intergenerational learning → screen-free recovery/exit | Return to human service when the human entry is unavailable or the ordinary route is cut; keep outcomes `unknown` |
