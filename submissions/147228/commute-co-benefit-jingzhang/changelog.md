...
- 将任务书六项要求从“交通侧局部接口”补成六份可独立复核的概念交付：3 个定位、5 项功能、3 区 2 翼，6 个有来源边界的全球机制案例、8 类资源和 5 个未确认区域接口，10 张场景卡，3 个责任地标，公共回程文化与五层导视，以及四季活动和 P0 至 P2 闭环。
- 新增 AI 原点社区照护环最小 P0 证据包，登记 4 个时间窗、7 条证据流、5 类未指派责任与 5 道 HOLD 门；当前仍为 0 条现场记录、未授权、未运行，缺群体分母或路线版本变化即停止。
- 修复任务书交叉图、服务等价图、空间裁决图和企业—居民合同图的文字对比度、换行与状态标签；同步中英正文、离线索引、合规矩阵和逐资产权利台账。
- 重建中英双语 A3/A0 图册，按任务书 → P0 → 空间裁决 → 服务等价 → 责任合同 → 活动链 → 场景卡 → 空间系统的顺序交付；不改变合成模型输入，也不声称合作、落位、排期、许可、现场绩效或官方提分。

## v1.7 - 2026-08-21

- 新增“空间裁决”合同：把企业优先路缘、独立接驳窗口、公共脊加可逆路缘窗口三种替代方案按 REJECT / REVISE / ADVANCE_TO_DESIGN_REVIEW 分开记录，避免把单一模型偏好写成空间结论。
- 新增 1:5000、1:2000、1:500、1:200、1:50 五级尺度、三处节点、五项公共权利、五道共享门、3 个正向控制和 5 个负向夹具；当前仍为 HOLD、临时边界、0 授权、0 现场观察、0 现场声明。
- 新增中英双语空间裁决图板、离线 runner、负例回归及 visual index 入口；不改变既有合成模型、现状分数、现场未知状态或高水位。

## v1.6 - 2026-08-21

- 新增中英双语“全日活动链服务等价回执”，把六个活动交接点分别绑定现场分母、非 AI 等价路径、拒绝/回退动作和维护回写字段。
- 新增离线 checker，验证六个节点、四次交接、五道扩展门及 0 授权/0 现场观察/unknown 基线/HOLD 边界。
- 双语 proposal 的 iteration 同步为 v1.6；不改变既有合成模型、现状分数、现场未知状态或高水位。

- 加固服务等价 checker：现在逐项核对四条交接、五道门、九个回写字段和五个负向夹具；新增三条离线负向回归，防止仅靠数量通过。
