...
const source = fs.readFileSync(path.join(__dirname, 'commute-spatial-decision.json'), 'utf8');
const runner = path.join(__dirname, 'run-commute-spatial-decision.js');
const original = JSON.parse(source);
const run = (payload) => {
  const dir = fs.mkdtempSync(path.join(require('os').tmpdir(), 'commute-spatial-decision-'));
  const json = path.join(dir, 'commute-spatial-decision.json');
  fs.writeFileSync(json, JSON.stringify(payload));
  const patched = fs.readFileSync(runner, 'utf8').replace("path.join(__dirname, 'commute-spatial-decision.json')", JSON.stringify(json));
  try {
