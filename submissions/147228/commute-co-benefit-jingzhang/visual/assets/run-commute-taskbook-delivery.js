...
const atlas = JSON.parse(fs.readFileSync(path.join(here, 'commute-taskbook-delivery.json'), 'utf8'));
const pilot = JSON.parse(fs.readFileSync(path.join(here, 'commute-p0-pilot-evidence-pack.json'), 'utf8'));

const fail = (message) => { throw new Error(`commute taskbook delivery: ${message}`); };
...
if (atlas.identity.short_mark !== 'C↔C') fail('identity mark mismatch');
...
if (!Array.isArray(atlas.regional_synergy) || atlas.regional_synergy.length !== 5 || atlas.regional_synergy.some((item) => item.state !== 'unconfirmed_conceptual_interface')) fail('regional synergy interfaces must remain five unconfirmed concepts');
...
function taskbookBoard(language) {
  ...
  s += text(60,50,'COMMUTE COMMONS / TASKBOOK DELIVERY / v2.35',18,'#77E7D3',800);
  ...
  s += text(205,252,zh ? atlas.identity.name_zh : 'Commute Commons',zh ? 24 : 20,C.navy,800);
  ...
  s += text(88,890,zh ? '区域接口 · 北纬社区 · 未来科学城 · 怀柔科学城 · 经开区 · 京津冀（均待验证）' : 'Regional interfaces · Beiwei · Future Science City · Huairou · E-Town · Jing-Jin-Ji · all unconfirmed',11,C.muted,650);
  ...
  atlas.task_outputs.forEach((item,i) => {
    ...
    const outputs=item.outputs.slice(0,3);
...
function pilotBoard(language) {
  ...
  s += text(60,50,'COMMUTE COMMONS / MINIMUM P0 EVIDENCE / v1.0',18,'#77E7D3',800);
  s += text(60,100,zh?pilot.title_zh:pilot.title_en,zh ? 38 : 30,C.white,800);
...
const outputs = {
  'commute-taskbook-delivery.svg': taskbookBoard('zh'),
  'commute-taskbook-delivery.en.svg': taskbookBoard('en'),
  'commute-p0-pilot.svg': pilotBoard('zh'),
  'commute-p0-pilot.en.svg': pilotBoard('en')
};
...
console.log(JSON.stringify({
  status: 'COMMUTE_TASKBOOK_DELIVERY_PASS',
  ...
  regional_interfaces: atlas.regional_synergy.length,
