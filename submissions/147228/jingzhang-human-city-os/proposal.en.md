---
title: "Jingzhang Human City OS: From AI Showcase to a City for People in the AI Era"
...
summary: "Release the human minimum before calling the machine. One public spine, six spatial units, three focus areas, four progression gates, and a twelve-week minimum-pilot method make ordinary access, staffed service, screen-free recovery, and replayable exit prerequisites for any AI scenario. Provisional geometry, recomputable design quantities, and unknown outcomes remain visibly separate."
...
![Figure 03 | Six spatial units: area, share, everyday capacity, and reversible boundary on one page](assets/figures/land-use-structure.en.png)

The six units are not a statutory land-use proposal. They expose a choice: retain community-support and reversible urban capacity before allowing machine-oriented R&D into removable interfaces. FAR, building height, ownership, demolition quantities, and formal roads remain `unknown`. [metric:community_retention_support_area_ratio] [metric:reversible_space_ratio] [depth:development_intensity_controls]

## Transport, Rail, Municipal Infrastructure, and Public Services | Four bounded corridors

The four conceptual corridors are not engineering alignments. They draw operating rules into space. The 5.919 km work-transition corridor starts with continuous walking and staffed career advice. The 4.586 km silicon-right-of-way test corridor permits only declared time windows, minimum permissions, and human takeover. The 2.015 km low-altitude concept corridor must preserve the ordinary ground route. The 8.366 km sponge-resilience inspection line begins with static refuge, staffed inspection, and missing hydrological evidence. [metric:skill_transition_corridor_length_m] [metric:silicon_right_of_way_length_m] [metric:low_altitude_concept_corridor_length_m]

![Figure 04 | Human–machine movement and blue-green resilience: four bounded conceptual corridors and stop conditions](assets/figures/mobility-bluegreen.en.png)
...
![Figure 05 | Three focus areas: priority users, five-stage path, G0–G3, and minimum evidence](assets/figures/key-areas.en.png)

![Figure 06 | Spatial action rooms: arrive, explain, bound, pause without a screen, exit and replay](assets/figures/spatial-action-rooms-v21.en.png)

Package G0 uses four synthetic public-service contracts to check fields and rejection logic. All 4/4 positive fixtures replay, 8/8 negative fixtures missing consent, a staffed entrance, minimum data, scope, appeal, low-impact boundaries, bilingual review, or a spatial anchor are rejected, and 4/4 bilingual field sets agree. This proves only that the contract structure is replayable. No real users, field staffing, personal data, or external systems were involved; G1 remains HOLD. [data:visual/assets/human-city-public-service-tabletop-v1-evidence.json]

![Figure 07 | Spatial action atlas for the three focus areas: scenarios, human takeover, and exit evidence](assets/figures/spatial-action-atlas.en.png)

## AI Innovation Ecosystem, Personas, and AI+ Scenarios | Nine groups hold distinct veto points

Six base personas cover residents and older people, transitioning workers, night-shift AI workers, small teams and OPCs, public-service staff, and people with mobility or low-digital barriers. Three extensions cover young entrants, developers and researchers, and first-time or international visitors. Each group is tied to an ordinary entrance, spatial node, human alternative, minimum data, and stop condition. Representative paths do not substitute for demographic research or real participation. [data:visual/assets/personas-and-fairness.json] [data:visual/assets/public-interest-coverage-v26.json]

![Figure 08 | Human City acceptance atlas: users, scenarios, spatial anchors, and stop conditions](assets/figures/human-city-acceptance-atlas.en.png)

The 17 scenario cards are not an event list. Each asks five questions: who benefits, where it lands, what AI alone may do, who can take over, and what must stop it. The staffed entrance in `SC-A03`, the silicon-right-of-way test in `SC-B02`, the incident review in `SC-C03`, and multilingual service in `SC-D03` represent the human foundation, machine boundary, hard constraint, and outward value. [data:geometry/constraints.geojson#SC-A03] [data:geometry/constraints.geojson#SC-C03]

![Figure 09 | Human–machine interface prototypes: putting human stop actions and ordinary access into the spatial layer](assets/figures/human-machine-interface-section.en.png)

## Overall Design Area: Urban Renewal and Regulatory-Plan-Level Urban Design | Use AI to expose trade-offs

The parametric study generates 128 candidates over the same provisional site area, compares four proxy lenses—community retention, reversible reserve, machine callability, and climate buffer—and identifies a non-dominated set. It allows a reviewer to see how shares change derived areas without presenting proxy values as resident outcomes, AI capability, or a recommended plan. [data:visual/assets/parametric-search.json] [depth:metrics_recalculation]

Three readable candidates then compress the difference onto one board. Relative to the baseline, `people_first` raises community-support and reversible-reserve shares and lowers the city-API/embodied-R&D share; `machine_ready` moves in the opposite direction. Without formal boundaries, statutory controls, and public baselines, the candidates remain conceptual comparisons and do not overwrite formal geometry or metrics. [data:visual/assets/parametric-tradeoff-study.json] [data:visual/assets/parametric-tradeoff-study-evidence.json]

![Figure 10 | Three conceptual spatial trade-offs: baseline, people-first, and machine-ready](assets/figures/parametric-tradeoff-study.en.png)

The city API follows six steps: catalogue, authorize, call, log, audit, and exit. A machine operates only for a named purpose, minimum fields, and revocable permission. Paper, phone, staffed explanation, and appeal remain parallel. The sequence is replayable offline, but this does not mean an interface has been deployed or authorized. [data:visual/assets/city-api-sequence-v23.json] [depth:municipal_new_infrastructure]

## Blue-Green Network, Public Space, and Urban Character | Return culture, learning, and version governance to daily life

Three concept landmarks hold different public actions. The Human Version Hall publishes issues, changes, and withdrawals. The Rail Interface Clock turns Jingzhang railway time into a legible renewal rhythm. The Screen-Free Recovery Beacon places shade, sound, night safety, and non-compulsory interaction beside technology experiences. They are not corporate showrooms or construction commitments. [source:AGENT-TASKBOOK] [depth:renewal_project_list]

![Figure 11 | Public space, culture, and annual operation: three concept landmarks and five project families](assets/figures/public-culture-operations-atlas.en.png)

The annual rhythm organizes questions and evidence only: register public problems in spring, review bounded interfaces and embodied tests in summer, inspect work transition and OPC co-creation in autumn, and publish version differences and exit records in winter. Without a site, operator, budget, permit, and public process, none of this becomes an operating calendar. [data:visual/assets/public-culture-operations-atlas-v20.json]

Regional collaboration exchanges methods, not unauthorized data or conclusions. The Zhongguancun technology-service wing receives legal, standards, compute, and translation questions. The Xiaoyue River scenario wing receives low-risk public problems and ecological feedback. Toward Beiwei Community, Future Science City, Huairou Science City, the Economic-Technological Development Area, and Jing-Jin-Ji, the package offers only de-located schemas, failure packages, and version differences; it does not invent partners. [data:visual/assets/regional-interface-ledger.json]

![Figure 12 | Regional interfaces: inputs and outputs for technology, standards, knowledge, ecology, and international service](assets/figures/regional-interface-ledger.en.png)

The identity system uses rail, interface, and human-confirmation marks. Color distinguishes the human foundation, machine interface, hard constraint, and exit/replay. It supports wayfinding, service cards, and version records; it is not an official mark or government endorsement. [data:visual/assets/brand-identity.json]

![Figure 13 | Concept identity: rail, interface, and human confirmation](assets/figures/brand-identity.en.png)
...
![Figure 14 | City release gates: entry evidence, stop actions, and exit records](assets/figures/release-gates.en.png)

One twelve-week minimum-pilot contract carries the four gates into work that a professional team can inspect. Weeks 1 and 2 freeze tasks, evidence boundaries, and responsibility slots. Weeks 3 to 6 run the human-only baseline, accessibility, and multilingual paths. Weeks 7 and 8 permit shadow calls with synthetic or separately authorized data. Weeks 9 and 10 rehearse failure, appeal, and deletion. The final two weeks require an independent choice among HOLD, revise, bounded pilot, and retire. ISO human-centred design, the NIST AI RMF, GOV.UK inclusive-service guidance, and the UN-Habitat people-centred smart-city method define process only; they do not prove that Jing-Zhang conforms to those references. [source:METHOD-ISO-HCD-2019] [source:METHOD-NIST-AI-RMF-2023] [source:METHOD-UK-INCLUSIVE-SERVICE]

| Weeks | Work limited to this phase | Test for entering the next phase |
| --- | --- | --- |
| 1 to 2 | Freeze four critical public tasks, prohibited data, human entry, and stop responsibility | Tasks, sources, rights, and responsibility are recorded |
| 3 to 4 | Plan 24 ordinary-route checks across three focus areas, four tasks, and day/evening periods | Every critical task has at least one usable non-AI route |
| 5 to 6 | Plan 12 independent accessibility walk-throughs and 24 channel-language equivalence cases | No critical accessibility blocker remains open on a declared route |
| 7 to 8 | Use reversible shadow calls only; people continue to decide high-impact actions | Refusing AI does not reduce basic service; purpose and permission do not drift |
| 9 to 10 | Replay eight fail codes and verify receipt, appeal, correction, and deletion evidence | Every failure stops safely; every case can appeal and delete on schedule |
| 11 to 12 | Review coverage, failure, dissent, labour, and resources before a retain-or-retire choice | One independent written decision discloses conflicts and open issues |

The counts 24, 24, 12, and 8 are minimum coverage-matrix quantities that prevent a convenient single-path test. They are not statistically representative and do not claim recruitment or completed fieldwork. Real participation requires separate authorization and informed consent. Disabled and low-digital-capability users cannot be replaced by project staff acting as proxies. The default decision is HOLD if any critical task lacks a staffed, telephone, or paper route; if a declared route retains a critical accessibility blocker; or if refusal, appeal, or deletion evidence is incomplete. [data:visual/assets/human-minimum-release-pilot-v29.json] [metric:planned_human_route_attempt_count] [metric:planned_service_equivalence_case_count]

Six unfilled accountability slots cover public service, accessibility, data, safety, public liaison, and independent review. Resources have three classes. R1 uses existing staff, telephone, paper, and portable signs. R2 adds a temporary staffed point, rented accessible components, translation review, and independent audit. Construction, live city APIs, safety-critical machinery, and low-altitude operation are R3 work and remain outside the twelve-week pilot. Current G0 retains no real case record. In a future authorized pilot, coded raw observations are proposed to remain for no more than 30 days and non-personal aggregates for no more than one year; the authorized data steward must approve the actual schedule. [data:visual/assets/human-minimum-release-pilot-v29.json] [metric:planned_accessibility_walkthrough_count] [metric:planned_failure_appeal_drill_count]

Five project families cover the human buffer; city API and reversible components; human–machine, climate, and compute-energy work; data authorization and version governance; and ecological/regional outward transfer. The twelve-week contract connects six action packages, coverage scale, resource class, responsibility slot, and exit evidence to those families, while site, operator, funding, and recruitment remain authorization-pending. It is a start method that professional teams can revise, not an appointed operator or a budget commitment. [data:visual/assets/implementation-operation-matrix.json] [data:visual/assets/human-minimum-release-pilot-v29.json] [depth:phasing_implementation]

Evidence gaps enter the gates as well. Formal boundaries, ownership, roads/airspace, existing buildings, energy and heat, hydrology, resident and work baselines, and service authorization each bind to affected layers, metrics, and return actions. A material input change triggers a coordinated recomputation of geometry, metrics, figures, and release decisions. [data:visual/assets/data-readiness-register.json]

![Figure 15 | Data readiness: gaps, responsibility slots, entry gates, and recomputation triggers](assets/figures/data-readiness.en.png)
...
Resident retention, sustained employment transition, human-service equivalence, PUE, green-electricity share, and recovered heat remain `unknown/null`. Each item has minimum evidence requirements. Spatial shares, training counts, webpage accessibility, or policy-reference targets cannot fill real-world outcomes. [metric:resident_retention_rate] [metric:sustained_employment_transition_rate] [metric:manual_service_equivalence_rate]

![Figure 16 | Metrics and evidence: recomputable geometry and unknown outcomes in separate lanes](assets/figures/metrics-evidence.en.png)

## Coordinated Research Area: Industry and Future City Research | Make methods and failure records reusable

The coordinated research area does not invent companies, partners, or events to complete an industry narrative. It frames university origination, small-team translation, technology services, public problems, and regional cooperation as interfaces that still need recipients, and it releases only de-located schemas, failure packages, and version differences. The seven repository review questions and the taskbook requirements return to short evidence paths. Brief alignment uses the three scales and three-area/two-wing structure. Originality uses the human-foundation, machine-plug-in, four-gate consistency. AI planning innovation uses parametric trade-offs and the bounded city API. Feasibility uses the twelve-week action packages, responsibility slots, resource classes, and stop actions. Public interest uses nine personas and human equivalence. Risk and compliance use evidence gates and unknowns. Expression completeness uses bilingual figures, PDFs, and offline HTML. [data:visual/assets/reviewer-navigation-index.json]

![Figure 17 | Seven-dimension reviewer evidence map: question, shortest path, evidence boundary, and next check](assets/figures/reviewer-scorecard-map.en.png)

![Figure 18 | Taskbook dimensions and workflow questions return to the same package evidence](assets/figures/reviewer-navigation.en.png)

The figures, A3/A0 sets, HTML, and structured evidence share one manifest hash chain. An offline PASS proves only file, reference, bilingual-mapping, and constraint consistency. It is not an official score, award, publication, permit, or implementation result. [data:manifest.json] [depth:risk_missing_data]
