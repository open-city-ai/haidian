...
const data = JSON.parse(fs.readFileSync(path.join(__dirname, 'human-city-spatial-decision.json'), 'utf8'));
const fail = (message) => { throw new Error(`Human City spatial decision: ${message}`); };
const exact = (actual, expected, label) => { if (JSON.stringify(actual) !== JSON.stringify(expected)) fail(`${label} mismatch`); };
...
console.log(JSON.stringify({status: 'PASS', scales: data.scales.length, alternatives: data.alternatives.length, selected: data.selected_alternative, nodes: data.nodes.length, rights: data.rights_matrix.length, gates: data.shared_gates.length, field_claims: data.field_claims.length}, null, 2));
