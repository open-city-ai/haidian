#!/usr/bin/env node
/* Read-only deterministic checks for the v1.5-v3.0 spatial, search, delivery, scorecard, brief-alignment, public-culture, city-API, connector, conversion, pilot-node, and evidence-crosswalk assets. */
const fs = require('fs');
...
const pilotDossierV30 = read('human-city-pilot-dossier.json');
...
const v30BilingualRefs = proposalZh.includes('visual/assets/human-city-pilot-dossier.json') && proposalEn.includes('visual/assets/human-city-pilot-dossier.json') && proposalZh.includes('图 30') && proposalEn.includes('Figure 30') && proposalZh.includes('图 31') && proposalEn.includes('Figure 31') && proposalZh.includes('assets/figures/human-city-pilot-node-dossier.png') && proposalEn.includes('assets/figures/human-city-pilot-node-dossier.en.png') && proposalZh.includes('assets/figures/human-city-pilot-delivery-receipt.png') && proposalEn.includes('assets/figures/human-city-pilot-delivery-receipt.en.png');
...
check('BILINGUAL_AUDIT_ITERATION', bilingualAudit.package_iteration === 'v3.0', `audit=${bilingualAudit.package_iteration}`);
check('BILINGUAL_AUDIT_SCOPE', ['图 16', '图 17', '图 18', '图 19', '图 20', '图 21', '图 22', '图 23', '图 26', '图 27', '图 30', '图 31'].every((label) => bilingualAudit.scope.join(' ').includes(label)) && ['human-city-mainline', 'parametric-search', 'human-city-delivery-spine', 'spatial-proof-v16', 'spatial-proof-v17', 'formal-scorecard-readback-v18', 'reviewer-scorecard-map', 'brief-alignment-atlas-v19', 'public-culture-operations-atlas-v20', 'city-api-sequence-v23', 'public-space-operations-v25', 'human-city-pilot-dossier'].every((name) => bilingualAudit.scope.join(' ').includes(name)), 'v1.5-v2.5 evidence and v3.0 pilot-node/delivery boards are named in the audit scope');
...
check('PILOT_DOSSIER_V30_SCHEMA', pilotDossierV30.iteration === 'v3.0' && pilotDossierV30.decision === 'HOLD' && pilotDossierV30.official_boundary === false && pilotDossierV30.authorization_count === 0 && pilotDossierV30.field_observation_count === 0 && pilotDossierV30.field_acceptance.status === 'not_run' && pilotDossierV30.spatial_dossier.scale_chain.length === 3 && pilotDossierV30.spatial_dossier.scale_representation === 'schematic_level_not_print_scale' && pilotDossierV30.spatial_dossier.site_fit.status === 'unlocated_parameter_envelope' && pilotDossierV30.spatial_dossier.site_fit.verified_anchor_count === 0 && pilotDossierV30.pre_field_walkthrough_protocol.status === 'authored_not_authorized_not_run' && pilotDossierV30.pre_field_walkthrough_protocol.participant_count === 0 && pilotDossierV30.pre_field_walkthrough_protocol.tasks.length === 5 && pilotDossierV30.delivery_contract.raci.every((row) => row.status === 'role_unconfirmed'), `iteration=${pilotDossierV30.iteration}; decision=${pilotDossierV30.decision}; scale=${pilotDossierV30.spatial_dossier.scale_representation}; site_fit=${pilotDossierV30.spatial_dossier.site_fit.status}; walkthrough=${pilotDossierV30.pre_field_walkthrough_protocol.status}; field=${pilotDossierV30.field_acceptance.status}`);
check('PILOT_DOSSIER_V30_CALIBRATION', pilotDossierV30.dimension_rule.calibration.accessible_route_clear_width_min_m === 1.2 && pilotDossierV30.dimension_rule.calibration.crowded_public_accessible_route_clear_width_min_m === 1.8 && pilotDossierV30.dimension_rule.calibration.new_or_extended_manual_door_clear_width_min_m === 0.9 && pilotDossierV30.dimension_rule.calibration.automatic_door_clear_width_min_m === 1.0 && JSON.stringify(pilotDossierV30.dimension_rule.calibration_source_ids) === JSON.stringify(['GB-55019-2021', 'ZJ-ACCESSIBLE-RESIDENTIAL-2022']), `sources=${pilotDossierV30.dimension_rule.calibration_source_ids.join(',')}`);
check('PILOT_DOSSIER_V30_REFS', v30BilingualRefs && ['assets/figures/human-city-pilot-node-dossier.png', 'assets/figures/human-city-pilot-node-dossier.en.png', 'assets/figures/human-city-pilot-delivery-receipt.png', 'assets/figures/human-city-pilot-delivery-receipt.en.png'].every(exists), 'v3.0 pilot-node and delivery boards occur in both proposal languages');
...
const evidence = {
  ...
  status: ok ? 'PASS' : 'FAIL',
  checks,
  interpretation_zh: 'PASS 只证明 v1.5-v2.5 既有资产与 v3.0 节点/交付合同的结构、来源分工、非打印比例声明、未落位状态、预走查边界和双语回接可读；尺寸、成本、角色、SLA 与现场验收仍保持设计目标、低置信、未确认、未观测或 not_run，不证明官方评分、现场绩效、批准或实施。',
  interpretation_en: 'PASS proves only that the v1.5-v2.5 assets and v3.0 pilot-node/delivery contract resolve structurally with source separation, a non-print-scale disclaimer, unlocated status, walkthrough boundary, and bilingual linkage. Dimensions, costs, roles, SLAs, and field acceptance remain design-target, low-confidence, unconfirmed, unobserved, or not-run; this does not prove official scoring, field performance, approval, or implementation.'
};
