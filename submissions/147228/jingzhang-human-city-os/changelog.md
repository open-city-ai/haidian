...
## v2.9-candidate - 2026-08-21

- 新增十二周人的最低版本试点合同，把四道推进门展开为六段工作。每段登记进入条件、去留判断尺和停止动作，并把普通路径、无障碍、多语、影子调用、申诉、删除证明与独立复核接到同一流程。
- 给出覆盖矩阵的最低计划量。24 次普通路径检查、24 个渠道语言等价案例、12 次独立无障碍走读和 8 个失败申诉演练均为待授权的方法参数，不具有人口代表性，也不表示已招募或已完成现场工作。
- 将公共服务、无障碍、数据、安全、公众联络和独立复核拆成六个未指定责任槽位；把资源分成 R1 便携工具、R2 临时服务点与独立审计、R3 另立项深化三档。施工、真实城市 API、安全关键机械与低空运行不进入十二周试点。
- 登记 ISO 人本设计、NIST AI 风险管理、GOV.UK 包容服务和 UN-Habitat 人本智慧城市方法来源，只用于试点过程设计，不据此声称京张现场符合标准或已取得授权。
- 新增确定性 runner，核对 1 至 12 周是否无重叠、覆盖量公式、六项门槛、六个责任槽位、六个行动包、资源档位、留存上限和禁入字段。PASS 只证明合同内部闭合，现场结果继续为 `null`。
