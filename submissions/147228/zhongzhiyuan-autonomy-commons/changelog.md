...
- 将区域生态双语图重做为实质等价交付：五个待确认接口逐一说明带入、返回、八类资源机制与共同证据边界。
- 新增“公共记忆—荣誉展示—AI 回馈”双语系统，用三处实体载体、六项回执字段、五层导视和一个可撤回年度循环连接 agent.4、agent.5 与 agent.6。
- 将自动驾驶证据板的主读数改为 `7 / 11 项等待现场证据`，并显著标注不是质量、成熟度、安全性、准备度或绩效分；不再以 63.64% 作为主视觉。
- 修复英文节点图标题遮挡和资源决策板断词，补充确定性双语渲染证据；未改变现场结果、伙伴、许可、官方边界、评分或实施结论。
- 从 v4.0 正文和 12 张本地双语图全量重建中英 A3 图册与 A0 展板，旧 v3.9 封面入口改为只读兼容检查，避免旧脚本再次覆盖新交付物。
- 补齐外部区域接口进入京张带后的公开内回路：分别登记众智园、AI 原点社区、待组织方确认的北纬社区接口和大钟寺的输入、返回与责任边界；不把 AI 原点与北纬社区写成同一实体，也不推定伙伴、居民代表权或运营授权。
