...
function textBlock(x, y, value, options = {}) {
  const lines = wrap(value, options.max || 22, Boolean(options.english));
...
function render(lang) {
  ...
  const rows = data.resource_classes.map((item, index) => {
    ...
    return `<g><rect x="40" y="${y - 18}" width="1520" height="${rowH - 8}" rx="16" fill="#0B2738" stroke="#24556B"/><rect x="40" y="${y - 18}" width="10" height="${rowH - 8}" rx="5" fill="${colors[index]}"/>${textBlock(x[0], y + 12, name, { max: zh ? 12 : 20, size: 17, fill: '#F7FBFF', weight: 800, english: !zh })}${textBlock(x[1], y + 8, holds, { max: zh ? 22 : 34, size: 14, english: !zh })}${textBlock(x[2], y + 8, route, { max: zh ? 22 : 33, size: 14, fill: '#B7D4DE', english: !zh })}${textBlock(x[3], y + 8, veto, { max: zh ? 32 : 50, size: 14, fill: '#FFD8DE', english: !zh })}</g>`;
  }).join('');
  ...
  const stages = data.decision_stages.map((stage, index) => {
    ...
    return `<g><rect x="${sx}" y="${stageY}" width="${stageW}" height="200" rx="18" fill="#102F43" stroke="${colors[index]}" stroke-width="2"/>${textBlock(sx + 18, stageY + 28, `${index + 1}  ${name}`, { max: zh ? 22 : 36, size: 15, fill: colors[index], weight: 800, english: !zh })}${textBlock(sx + 18, stageY + 82, decision, { max: zh ? 24 : 37, size: 13, fill: '#E6F4F8', english: !zh })}${textBlock(sx + 18, stageY + 140, stop, { max: zh ? 24 : 37, size: 13, fill: '#FFD8DE', english: !zh })}</g>`;
  }).join('');
  return `<svg xmlns="http://www.w3.org/2000/svg" width="1600" height="1180" viewBox="0 0 1600 1180" role="img" aria-labelledby="title desc">
  ...
  <rect width="1600" height="1180" fill="url(#bg)"/><circle cx="1450" cy="90" r="260" fill="#2A9D8F" opacity=".16"/><circle cx="80" cy="1120" r="260" fill="#6EA5FF" opacity=".12"/>
  ...
  <rect x="40" y="155" width="1520" height="45" rx="12" fill="#153D4E" stroke="#2A9D8F"/><text x="58" y="184" font-family="Arial, PingFang SC, Microsoft YaHei, sans-serif" font-size="15" font-weight="800" fill="#66E3CA">${esc(zh ? '公共底线先于自动化 · 提案不能自我放行 · 资源不购买公共性' : 'PUBLIC FLOOR BEFORE AUTOMATION · PROPOSER CANNOT SELF-RELEASE · RESOURCES DO NOT BUY PUBLICNESS')}</text>
  ${header.map((label, i) => `<text x="${x[i]}" y="${rowY - 28}" font-family="Arial, PingFang SC, Microsoft YaHei, sans-serif" font-size="14" font-weight="800" fill="#66E3CA">${esc(label)}</text>`).join('')}
  ${rows}
  <text x="45" y="890" font-family="Arial, PingFang SC, Microsoft YaHei, sans-serif" font-size="19" font-weight="900" fill="#F7FBFF">${esc(stageTitle)}</text>
  ${stages}
  <text x="58" y="1160" font-family="Arial, PingFang SC, Microsoft YaHei, sans-serif" font-size="12" fill="#9FC0CF">${esc(zh ? '数据来源：包内 contract、场景卡、准备度台账与公共利益审计 · provisional / not authorized / not a score' : 'Source: package contracts, scenario cards, readiness register, and public-interest audit · provisional / not authorized / not a score')}</text>
</svg>`;
}
