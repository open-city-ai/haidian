...
为让“场景卡”可以被复核而不是只被阅读，本包使用 `visual/assets/open-pulse-test-window-record.schema.json` 与 `visual/assets/example-s02-embodied-test-window.json`。该记录为 Open Pulse 独立设计的概念性、可逆测试窗口：它围绕一个尚未执行的 S02 合成低速配送排练，分别记录公共问题、普通服务替代、时间/空间边界、公众界面、人工停止权、最小数据、观察方法、是否放行以及复原路径。示例的 `result_status` 为 `not_run`、release decision 为 `hold`；它不证明机器人性能、现场验收、公众同意、法律合规或部署许可。任何真实试点都必须以当时的场地、无障碍、安全、维护与公众审查重新建档。
...
`report/copyright_statement.md` 从短声明升级为可审计协议，`visual/assets/copyright-ledger.json` 逐项记录路径、作者、生成方式、输入来源、第三方材料、许可、归属、字体处理和 SHA-256。文本、几何、图件、离线 HTML 和测试窗口记录由许丙南 / Codex 在提交工作树中创作或派生；官方统计资料保留来源署名；不嵌入第三方图片、地图、远程字体或运行时外链。未来新增素材必须先登记授权和哈希；该台账是提交证据，不是法律意见。
