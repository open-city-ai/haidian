...
## v1.9 首 168 小时与首 12 周：把“发布 AI”变成可退回的公共合同

高分方案不能只回答“最后想建成什么”，还要让普通人和评审者看见第一天如何不受伤、第一周谁负责、三个月后何时停止。`visual/assets/open-pulse-release-chain.json` 把这条链固定为四种状态：`BASE` 是不依赖 AI 的普通服务，`BOOST` 是有边界的可撤回辅助，`BLACKOUT` 是 AI 层熔断但普通服务继续，`BEQUEST` 是去标识化学习的可继承记录。当前四种状态均为设计合同，授权为 0、现场观察为 0、baseline 为 `unknown`、运行状态为 `not_authorized_not_run`，不代表现场部署或绩效结果。[data:visual/assets/open-pulse-release-chain.json] [source:NIST-HUMAN-CENTERED-AI]

![首168小时与首12周发布链](assets/figures/open-pulse-release-chain.svg)

### v2.2 三处公共版本接口：让时间链落到可进入的空间节点

时间合同必须有可进入、可解释、可回退的空间接口，才不会停留在运营口号。`visual/assets/open-pulse-release-interface-prototypes.json` 将三处关键区写成同一组四状态原型：众智园是公共版本发布前厅，AI 原点是同意—照护回退室，大钟寺是公共版本与申诉厅。每处均先显示 `BASE` 普通服务，再允许 `BOOST` 有界辅助；`BLACKOUT` 冻结 AI 层并返回普通服务，`BEQUEST` 只留下已清权的去标识化学习记录。

![三处公共版本接口原型](assets/figures/open-pulse-release-interfaces.svg)

原型按概念性 1:500 场地关系与 1:50 公共接口关系表达，不代表施工尺寸、容量、许可或现场观察。三处都保留步入、人工柜台/接待、纸面或语音说明、公开状态板、申诉和维护回执；AI 仅负责解释版本差异、准备通俗说明或整理有界学习。四道共同闸门是：`BASE` 必须先可见；发布前必须有人工接收和维护角色；公众必须能读到状态、拒绝理由、申诉路径与回退动作；`BLACKOUT` 后必须回到 `BASE`，`BEQUEST` 不得保存未清权的个体痕迹。当前授权、现场观察、baseline、绩效和具体责任人仍是 0/0/`unknown`/空值/待确认，方案不把接口原型写成落地结果。`run-open-pulse-release-interface-prototypes.js` 与负例测试对三处节点、四状态顺序、普通服务优先、人工责任、停止条件和零现场声称做 fail-closed 检查。

### 首 168 小时：先证明普通服务没有被 AI 挤掉

| 窗口 | 负责人 | 必须留下的证据 | 停止条件 | 对外回执 |
| --- | --- | --- | --- | --- |
| H0，0—24 小时 | 普通服务负责人 | 路线、人工排班、清权清单 | 无人工责任人或无障碍断点未处理 | 普通服务卡 |
| H1，24—72 小时 | 安全与无障碍负责人 | 配对走查、人工接管演练、资产与同意台账 | 接管失败、越权数据或路线冲突 | 路线—回退凭证 |
| H2，72—120 小时 | 场景负责人 | 一项有限辅助、最小数据、维护窗口 | 辅助改变服务资格、出现未登记数据或无法维护 | 有限增益卡 |
| H3，120—168 小时 | 复核与申诉负责人 | 公众反馈、近失事件、发布/退休决定 | 伤害无法解释、申诉无人负责或状态无法公开 | 发布或退休凭证 |

### 首 12 周：每两周过一扇门，不满足就退回 BASE

W01—02 只确认普通服务、清权和无 App 路径；W03—04 做日/夜无障碍与安全接管演练；W05—07 只允许一项不改变服务资格的公共辅助；W08—09 核对资产巡检、维修、备件和退出；W10—11 公开投诉、分歧和双语证据卡；W12 由多方复核决定扩容、保持、退休或只留下 `BEQUEST` 学习记录。两次维护窗口逾期、责任人消失、人工接管失败或没有比 `BASE` 更好的证据时，不得扩容。`visual/assets/run-open-pulse-release-chain.js` 和 `test-open-pulse-release-chain.js` 对缺窗口、非 HOLD、越权授权和现场声称做 fail-closed 回归；它们是离线合同检查器，不是现场验收器。

这条链补足了方案原有的长期 RACI/SLA：它把“未来要运营”压缩成评审者可以逐项追问的时间顺序，也把撤回做成比扩容更短的动作。普通步行、人工服务、手推车配送、纸面/语音/多语种服务始终保留；AI 不得成为获得公共服务的前置条件。所有现场数据、专业责任、许可、真实投诉和绩效结果仍须后续补证，不能用本地合成回放替代。[source:BEIJING-ACCESSIBILITY-REGULATION] [source:RESILIENT-CITY-INFRASTRUCTURE-2024]
