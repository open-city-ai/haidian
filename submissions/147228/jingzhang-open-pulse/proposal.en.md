...
## v3.0 One Component, One Proof-Mile: a maintainable handoff
...
## v3.0 Review-gap repair: make state, space and proof visible on the first read
...
**Review copy:** English translation of `proposal.md`, v3.0
...
To make a scenario card reproducible rather than merely readable, this package uses the independently designed `visual/assets/open-pulse-test-window-record.schema.json` and `visual/assets/example-s02-embodied-test-window.json` for S02. The record is organized around the bounded window, spatial boundary, public interface, human control, data contract, observation state, release decision and restoration of ordinary use; it does not reuse the #426 Relay Receipt schema skeleton. It remains a fully synthetic concept-only review interface, not robot performance, field acceptance, deployment permission or adaptation permission.
...
The package includes a path-level `visual/assets/copyright-ledger.json` and an expanded `report/copyright_statement.md`. Text, geometry, diagrams, offline HTML, figures and the independent test-window record are authored or derived in this worktree by 许丙南 / Codex. Official public data retain their source attribution. No third-party images, maps or remote runtime assets are embedded; local system fonts are used only for rendering and are not redistributed. The identity mark is a concept direction, not a registered trademark.
...
The implementation handoff is deliberately inspectable: `operations-matrix.json` gives each package an owner, accountable reviewer, dependency, acceptance test, stop condition, maintenance rule and relative cost band; `resource-accounts.json` states which evidence and capacity must exist before a package is released. Together with the scenario matrix and independent test-window record, these files form a reviewable chain from a public question to a bounded window, a human handoff, an evidence record and a scale/redesign/retire decision.
