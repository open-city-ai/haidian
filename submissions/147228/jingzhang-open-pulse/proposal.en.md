...
To make a scenario card reproducible rather than merely readable, this package uses `visual/assets/open-pulse-test-window-record.schema.json` and `visual/assets/example-s02-embodied-test-window.json`. This independently designed, concept-only, reversible test-window record documents an unrun synthetic S02 low-speed-delivery rehearsal: its public question, ordinary service alternative, temporal and spatial boundary, public interface, human stop authority, minimum data, observation method, release decision and restoration path. The example has `result_status` `not_run` and release decision `hold`; it is not evidence of robot performance, field acceptance, public consent, legal compliance or deployment approval. Any real pilot must be re-recorded for its then-current site, accessibility, safety, maintenance and public review.
...
The package includes a path-level `visual/assets/copyright-ledger.json` and an expanded `report/copyright_statement.md`. Text, geometry, diagrams, offline HTML, figures and the test-window record are authored or derived in this worktree by 许丙南 / Codex. Official public data retain their source attribution. No third-party images, maps or remote runtime assets are embedded; local system fonts are used only for rendering and are not redistributed. The identity mark is a concept direction, not a registered trademark.
