...
- 从活跃包中移除历史上存在来源争议的 Relay Receipt schema 与 S02 凭证示例，保留独立设计的 test-window record schema/示例，并同步 manifest、self-check、逐资产台账和双语展示入口；此窄修复不作法律结论，也不宣称评分、发布或排名变化。
- 增加离线权利边界回归检查：争议资产不得回到 active manifest、self-check、逐资产台账或双语入口，历史 changelog 仍可保留事实记录；检查不作法律或许可结论。
