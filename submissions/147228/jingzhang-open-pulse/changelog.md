...
- 将三处版本站从概念节点推进为可复核的参数平面、接口剖面、四状态前沿、流线、权属接口和容量公式；新增 12 个平面模块、12 段剖面和 9 处待确认权属接口。
- 新增三站交付合同，把六类责任原型、五步采购、成本测算方法、最低岗位、9 项未观测服务目标、验收门与回退写入同一回执；单位、金额、容量、许可和现场实绩继续保持 unconfirmed/null/HOLD。
- 新增双语空间交接与交付运维图板，并把 A3/A0 双语图册从 5 页扩展为 7 页；关键证据同步回到中英文正文和离线视觉入口，减少评审者跨台账拼接。
- 新增离线 runner 与 10 个负向夹具，拒绝虚构观察、固定不可重算尺寸、容量/成本冒填、权属冒认、缺失 BLACKOUT/CLOSEOUT 或回退路径。

## v3.1 - 2026-08-22

- 在 v3.0 普通服务等价性合同前置一个系统选项决策回执：OP-A `REJECT`、OP-B `REVISE`、OP-C `ADVANCE_TO_DESIGN_REVIEW`。
- 增加 `1:5000 → 1:2000 → 1:500 → 1:100 → 1:50` 尺度链、五项居民权利、五道共享闸门、中英文图板和离线 runner/6 个负例。
- 保留主干 40 道证据门禁、服务等价性、S-02 HOLD 和所有 provisional/unknown 边界；不新增现场事实、施工尺寸、授权、部署或排名主张。
