...
- 响应 #706 对结构性来源与许可边界的复核，移除此前带有 peer-derived 结构的 Relay Receipt schema 与 S02 样例；署名不被当作改编许可的替代物。
- 以 Open Pulse 独立设计的 `open-pulse-test-window-record.schema.json` 和合成 S02 测试窗口记录替代；新文件围绕有界窗口、普通路径、公共接口、人类控制、数据合同、观察、释放决定和恢复普通使用组织。
- 同步更新中英正文、离线 HTML、资源账、风险证据、逐资产清权台账与 manifest；#706 中的历史权利人回复仍然待定，不由本次替换宣告解决。
