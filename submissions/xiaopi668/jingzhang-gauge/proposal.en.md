---
title: "THE JING-ZHANG GAUGE: Fit the Gauge, Join the Line"
...
**Main name: JING-ZHANG GAUGE (京张限界).** The name comes from the hardest engineering institution of the Jing-Zhang railway — the structure gauge: every locomotive, vehicle, and piece of equipment must fit inside a fixed clearance envelope to run; tunnel profiles, platform edges, and signal positions are all locked by it. According to public historical records, the Guanggou section completed under Zhan Tianyou around 1909 was renowned for exact surveying and strict discipline, delivering China's first self-designed mainline railway [source:BG-HIST-JINGZHANG-RAILWAY]. A century later AI is the new train, and the city must answer the same question: **what may join the line, in what posture, and what happens when it exceeds the envelope.**
...
On this basis the package proposes the **Coupler Standard** as the ecosystem mechanism: the automatic Janney coupler adopted in Zhan Tianyou's era, as public records note, let cars from different makers couple safely [source:BG-HIST-JINGZHANG-RAILWAY]; today's AI innovation needs standardized hooks — a unified data-interface catalogue, model registration fields, safety-test report formats, and exit procedures. Any team, any model, once it passes the gate's envelope check, can couple onto the urban train; uncoupling exits cleanly without disturbing the mainline [source:SOURCE-REGISTRY]. Land, space, capital, talent, compute, data, and scenarios all organize around this standard: industrial space prioritizes G2 test zones, compute pools through the open compute center, data enters under minimal-necessity through gates, and the annual open scenario list is renewed by recalibration results [depth:overall_spatial_structure].

**The Gauge Operating Manual (auditable admission and exit):** every AI service goes on-line in seven steps — (1) registration (model, accountable entity, data inventory); (2) envelope self-check; (3) gate sampling; (4) 14-day public disclosure; (5) registered on-line - an internal governance mechanism, not an administrative licence and never exempting statutory construction, safety, accessibility, network or data checks; (6) annual recalibration; (7) downgrade or exit. Auditability rules: G2 test reports are retained for 90 days; appeals go through third-party re-inspection arranged by the industry alliance; two consecutive failed recalibrations trigger automatic demotion back to the depot; annual aggregates are all published in the Urban Gauge White Paper. Tiered inspection cadence — G0 limited to zero-collection facilities (exempt from data testing only, not from other statutory checks), G1 annually, G2 quarterly — turns "fit the gauge, join the line" into a checkable procedure rather than a slogan [data:geometry/constraints.geojson#SN-005].
...
**Data operations spec ("minimal necessary" made executable):** each card's boundary resolves into five fields — (1) data category (location / image / voice / transaction); (2) retention (G1 transactions ≤90 days and de-identified at capture; G2 imagery deleted immediately); (3) access (least-privilege operator roles; cross-party pulls logged and approved); (4) notice and consent (conspicuous entry-point labeling + one-tap revocation); (5) incident response (suspected leaks double-notified to the street office and affected parties within 72 hours with related flows frozen). Independent review rotates between the community co-governance committee and the industry association; review records enter the annual white paper.**Usability-testing ethics (conceptual protocol):** participants sign informed consent and may withdraw unconditionally at any time; reasonable accommodations (companions, mobility support, alternative task paths) are provided for disabled participants; minors require guardian consent with on-site supervision; modest transport subsidies avoid inducement; testing applies data minimization and erases personal records within 30 days; discovered defects enter a public defect list with remediation deadlines, closing the feedback loop.
...
**Scenario-level clarifications (avoiding operational misreading):** S11's "session content never leaves the room" means voice and screen content is processed only by in-room local devices with no cloud caching; system logs record only power/fault codes, never content; third parties (including maintenance contractors) cannot access in-room capture links, and repairs require two-person escort with logged entry. The "G2 test reports retained 90 days" rule covers report text and metadata only; raw imagery still follows immediate deletion - neither overrides the other. Cache, backup and deletion verification follow the table above.
...
![Scenario data-flow and privacy audit swimlanes](assets/figures/privacy-audit-flow.en.png)
...
**Responsibility & acceptance ledger (conceptual RACI):** every package fixes four roles - initiator (local street office or platform company), implementer (design-build-operation consortium), acceptor (third-party inspector co-signing with the community committee), supervisor (district authority and public representatives); named units follow statutory procedures at deepening stage. KPI definitions: testing lead time counts all services submitted in the quarter; fallback answer rate counts all calls and visits to human counters; remediation success rate counts all services judged out-of-gauge in the period; self-set default values are proposed so pilots can start directly (non-official, subject to hearing confirmation): complaint-rate threshold suggested at ≤2 per 1,000 service interactions; gate-testing lead time target ≤10 working days; disclosure-update lag ≤24 hours; remediation grace period suggested at 30 days; fallback rule - any indicator missing target for two consecutive quarters pauses the package and triggers third-party review; fallback rule - any indicator missing target for two consecutive quarters pauses the package and triggers third-party review.

**P1 minimum viable pilot acceptance checklist & participatory usability protocol (conceptual):** seven acceptance items, each bound to an owner and evidence: (1) southern spine length and barrier-free continuity (implementer / as-built survey plus wheelchair-and-cane field records); (2) gate testing function works (acceptor / sample inspection reports); (3) disclosure-screen completeness at 100% (supervisor / weekly screenshots archived); (4) human-fallback counter answers (community committee / call-and-visit ledger); (5) zero-collection verified (third party / packet audit on the G0 section); (6) deletion verification (third party / proof of data erasure after sampled service retirement); (7) complete operations handover documents (initiator / signed checklist). Usability protocol: quarterly, at least twelve volunteers covering all six personas - including blind and senior participants - complete defined task paths; any task failure is logged as a usability defect with a remediation deadline; any accessibility failure (missing braille post, broken ramp) fails that acceptance item and triggers suspension until re-inspection passes. Framework only; concrete values set at deepening stage.
