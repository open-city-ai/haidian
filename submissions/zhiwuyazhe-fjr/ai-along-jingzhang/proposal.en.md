...
The City Question Station provides an offline tabletop rehearsal of the public acceptance contract. It uses only explicitly labelled synthetic questions, writes no files, makes no network requests, processes no personal information, and does not claim that a field trial occurred. The positive rehearsal checks whether one question exposes status, responsibility, a human route, a stop condition, and a revision record. Negative cases model a missing responsible person, data beyond the ceiling, a broken human route, an unavailable pause, and an unpublished revision. The rehearsal fails if any negative case is incorrectly released.

The unit lets an operating team rehearse rights and responsibility before installing devices. Its state is fixed as `not_authorized_not_run`, and real-world performance remains `null`. The structured contract and synthetic fixtures are stored in `visual/assets/public-acceptance-tabletop/`, while the overview presents the acceptance protocol. They prove only that rules can be checked, not that a service works [metric:public_acceptance_tabletop_negative_case_count] [depth:renewal_project_list].
