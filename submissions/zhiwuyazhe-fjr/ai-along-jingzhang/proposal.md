...
summary: "以京张铁路遗产为公共主线，把公众问题依次送入AI原点社区转译、众智园验证和大钟寺采用，再由小月河与沿线公众反馈决定继续、修改或退役。中关村科技服务翼支撑每一环，城市问题护照、五道证据门和十二条同任务人工路径让AI创新可进入、可复核、可申诉、可退出。"
...
这条创新带首先处理一件具体的事。一个居民、学生、企业或游客提出的城市问题，怎样经过转译和验证，变成可用的服务，并在出现错误时回到原处修改。方案把三区两翼组织成一条可以检查的工作路径。
...
| 证据门 | 最少证据 | 人类决定 | 未通过的处理 |
| --- | --- | --- | --- |
| G0问题登记 | 地点、受影响人群、现行做法和问题来源 | 是否值得进入转译 | 记录后关闭或补充材料 |
| G1基线与权利 | 非AI基线、数据上限、产权许可、责任角色和恢复资金 | 是否允许形成试点方案 | 不占用场地，不采集数据 |
| G2受控试点 | 安全审查、人工接管、容量、告知、申诉和结束日期 | 是否允许短时开放 | 保持日常模式 |
| G3独立复测 | 原始记录、失败记录、同任务比较、受影响人群意见和场地恢复 | 是否支持扩大或修改 | 修正后重测或退出 |
| G4继续或退役 | 复测结论、持续预算、责任续期、来源有效性和年度公开说明 | 继续、缩小、修改或退役 | 撤除服务并完成数据与场地恢复 |
