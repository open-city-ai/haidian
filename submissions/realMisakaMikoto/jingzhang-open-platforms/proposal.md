...
设计判断首先服从正式征集公告规定的三层尺度与三处重点区，不用宣传口径替代任务口径。公园二期配套完工和全线开放改变了设计起点：本方案把京张铁路遗址公园视为已经投入公共使用的连续基底，不重复提出大拆大建或重新造景，而把精力放在公园与地铁、校园、社区、园区之间尚待核实的横向接口、公共服务和长期运营上。完工与开放只能证明公园状态，不能证明任何接口改造、设备安装或场景试验已经获批。[E:PARK-PHASE2-OPEN-20260806] [source:PARK-PHASE2-OPEN-20260806] [data:geometry/site_boundary.geojson#SITE-001]；对应标准与深度门见 [standard:PROJECT-OFFICIAL-ANNOUNCEMENT] [depth:existing_conditions_diagnosis]。
...
总体结构概括为“一条公园基底、九个相对候选接口、三座公共学习站、两翼服务网络”。为消除“位置”和“功能”混用，本版规定：`OI-xx` 只表示北—南相对候选位置，`IT-xx` 只表示可迁移的接口类型；道路、空间条目、场景与图件均沿用这一双层编号。九个 OI 不是已选址工程，而是正式边界和现场核验前的相对索引。[data:geometry/roads.geojson#ROAD-IF-01] [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:three_level_scope_framework]
...
本方案先准确承接任务书的三大定位——**百年京张文化带、都市 AI 生活体验带、AI 融合创新带**——以及五大功能——**AI 全栈自主创新体系、世界级 AI 创新生态、AI+场景赋能新范式、智能化 AI 活力城市、AI 治理全球话语权**。在此正式框架下，“开放站台”把众智园 AI 自主创新加速区定义为标准与全栈验证入口，把 AI 原点社区定义为开源、转化与共享入口，把大钟寺 AI 产业集聚区定义为公共服务与智能原生新业态入口；中关村科技服务翼提供专业、知识产权、资本和国际服务，小月河场景赋能翼承接受控测试、公共体验与城市反馈。三区两翼不是五块封闭园区，而是一条“提出问题—小规模试验—公开证据—人工复核—停止或扩展”的协作回路。[E:THREE-AREAS-TWO-WINGS] [source:THREE-AREAS-TWO-WINGS] [data:geometry/land_use.geojson#LU-OP-01]；对应标准与深度门见 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:overall_spatial_structure]。
...
任何案例的容积率、治理权限、投资模型或绩效数值都不移植到京张。[A:A-CASES-001] [source:CASE-TORONTO-QUAYSIDE]

名称“京张开放站台”把铁路站台从“等车的边缘”转译为“共同检验城市服务的公共界面”。本版已形成可见标志：一条公园绿竖向主线被五条公共蓝横线打开，其中责任节点用铁路氧化红、学习节点用金色；这套四色情况码延展为站牌、责任牌、停止牌、纸面回执和活动证。中文主名与英文 `JINGZHANG OPEN PLATFORMS` 并列，不使用大脑、电路、机器人头像、企业商标或外部素材。这里的“开放”是开放问题、规则、证据和申诉入口，而不是无条件开放数据或场地。标志、色板和三片区原型已进入双语图、HTML 与 PDF，并由资产级权利清单复核。[E:AGENT-TASKBOOK] [source:AGENT-TASKBOOK]

## 总体设计范围城市更新与控规深度城市设计

总体设计不把公园画成一条孤立绿带，而把每个候选接口做成可组合的“开放站台剖面”。构件目录为 C01 连续无障碍通行带、C02 轮椅并坐与照护位、C03 遮阴避雨顶、C04 饮水/卫生间/急救信息、C05 有人责任柜台、C06 纸面/电话/语音入口、C07 可断开设备位、C08 清晰绕行与停止牌。九个 OI 采用同一责任语法，却允许根据对应 IT 类型改变组合；官方 polygon 更新后迁移关系，而不保留错误坐标。[data:geometry/public_space.geojson#PS-IF-01] [standard:MOHURD-URBAN-DESIGN-MEASURES] [depth:overall_spatial_structure]
...
三座站同时是 AI 荣誉展示节点，但荣誉对象不是“最大模型”或企业排名，而是可复演的公共贡献：一次被修复的无障碍断点、一项被公众叫停后改正的服务、一套开放标准、一次经授权验证的能源改进或一份可追责的事故复盘。展示使用来源、版本、责任人与有效期，不采用未清权肖像和企业标志。站点之间通过同一导视、季度问题清单和年度开放站台周联结；若未来官方边界改变，功能可整体迁移而不保留伪精确点位。[data:geometry/key_areas.geojson#PROV-KEY-001] [standard:MOHURD-URBAN-DESIGN-MEASURES] [depth:three_key_area_detailed_design]
...
十二张场景卡把用户、地点、运营、许可、最小数据、人工替代、基线、验收、隐私、停止与恢复写在同一行；“基线”是未来获准试验开始时应采集的比较条件，不是本方案已经取得的现场数据。[data:visual/assets/t1-public-service-tabletop/contract.json#T1-PUBLIC-SERVICE-AGENT] [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:renewal_project_list]
...
默认禁止人脸识别和持续性个体追踪，采用数据最小化、边缘处理、主动退出、非数字通道、人工升级和事故留痕。T1 已形成一套零网络、零个人数据、零文件写入的离线合成桌面包：4 个正例覆盖 S06/S08/S09/S10，10 个负例逐项触发拒绝、停止与回滚；`receipt.example.json` 明确只是预期输出，不是现场执行证据。现场状态固定为 `not_authorized_not_run`，真实性能固定为 `null`，放行决定固定为 `do_not_advance`。其结构化通过只证明合同和拒绝路径可以离线复演，不证明服务有效、合作成立或现场安全。四个最小试验中的“4—8 周”仍是获准后的建议周期，并非正在运行。[A:A-DATA-PRIVACY-001] [source:SOURCE-REGISTRY]

## 用地、建筑规模与拆改留方案

用地策略以公园和现有城市功能为基底，不为显示完整而虚构精确现状比例。法定或现状 `building_density`、绿地率与公共空间率仍未知；但投稿者控制的九处微气候原型和九处公共接口包络已经按临时研究索引复算，因此 `green_ratio` 与 `public_space_ratio` 以“低置信概念原型覆盖率”登记为 known。它们不能当作现状、产权、规划指标或建设承诺。[M:building_density] [metric:building_density] [data:geometry/buildings.geojson#BLDG-OP-01]；对应标准与深度门见 [standard:MNR-LAND-USE-CLASSIFICATION-GUIDE] [depth:retain_renovate_demolish]。
...
交通策略不是增加一条宏观“智慧交通轴”，而是逐个验证九个 OI 对应的 IT 横向联系。IT-04 负责轨道、公交与公园之间最后 500 米的可理解换乘；IT-02 关注校园社区出入口对不同年龄和能力的公平；IT-03 在遗产保护前提下安排慢行绕行；IT-05 在暑热和暴雨时提供替代路线；IT-07 将低速物流限定在时段、速度、急停和人工观察内。所有路线先有普通标识和人工服务，再叠加数字提示；设备故障不能使人失去基本出行权。[data:geometry/roads.geojson#ROAD-SPINE-01] [standard:MOHURD-URBAN-DESIGN-MEASURES] [depth:traffic_rail_slow_parking]
...
市政与新型基础设施遵循“传统保障优先、数字设施可断开”。IT-09 的边缘计算和能源试验必须使用独立计量、温控、消防、网安、设备退出和人工旁路，不以公园照明、应急通信或无障碍设施的基本供电为代价。饮水、卫生间、排水、照明、垃圾清运、急救与通信的真实容量尚未取得，只列入核验清单，不以概念图估算。端侧计算只处理完成场景所需的最小数据，不建立跨场景个人档案。[depth:municipal_new_infrastructure]
...
公园已经开放，因此蓝绿设计的首要任务是保护连续性并补足接口体验，而不是用新景观覆盖既有成果。IT-05 把高温、暴雨、积水、树荫、饮水和替代路线组织为可更新安全层；IT-08 用安静时段、低刺激引导、活动减容和居民反馈保护夜间生活。法定/现状绿地率与公共空间率仍未知；投稿几何中的 `green_ratio` 与 `public_space_ratio` 仅是低置信概念原型覆盖率，已经复算但不得冒充现状或法定控制。[M:green_ratio] [metric:green_ratio] [data:geometry/green_space.geojson#GS-CLIMATE-01]；公共接口数据及专业门见 [data:geometry/public_space.geojson#PS-IF-01] [standard:MOHURD-URBAN-DESIGN-MEASURES] [depth:blue_green_public_space]。
...
T1 合同、正反例、预期回执、资产权利与双语 QA 及只读运行器均位于 `visual/assets/t1-public-service-tabletop/`，能够第三方离线复演；它只验证“规则能拒绝并回滚”，不验证现场绩效。[data:geometry/phasing.geojson#PHASE-GATE-01] [data:visual/assets/t1-public-service-tabletop/contract.json#T1-PUBLIC-SERVICE-AGENT]；对应标准与深度门见 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [depth:phasing_implementation]。
...
合规矩阵负责公告 1.3、1.4、1.5 和 agent.1—agent.6 的完整覆盖；专业标准矩阵记录城市设计、控规深度、无障碍和生成式 AI 服务边界及缺失正式文件；设计深度矩阵把每个判断链接到正文、GeoJSON、指标、T1 桌面包或待补资料。每次边界、来源或场景变化都按“源文件—几何—指标—五图—中英文本—HTML/PDF—manifest 哈希”顺序重算，禁止只改数字而不更新证据链。[E:SITE-PACKAGE] [source:SITE-PACKAGE] [data:geometry/phasing.geojson#PHASE-GATE-01]；对应标准与深度门见 [standard:MOHURD-CONTROL-DETAILED-PLANNING] [depth:metrics_recalculation]。
...
以下材料真正影响了设计判断；这里只给人类可读线索，完整 URL、用途、许可、时间和限制以 `sources.json` 为准。资料列入清单不意味着其全部内容都能支撑正式空间结论。[E:PROCESSED-FACT-PACK] [source:PROCESSED-FACT-PACK] [data:geometry/constraints.geojson#CONSTRAINT-METADATA]；对应标准与深度门见 [standard:PROJECT-OFFICIAL-ANNOUNCEMENT] [depth:risk_missing_data]。
