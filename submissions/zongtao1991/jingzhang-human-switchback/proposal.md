...
总体结构为“**一条遗产公共脊、三个差异化试点区、两类横向服务连接**”。遗产公共脊优先在地面连续，连接铁路记忆、日常通勤、公共服务和蓝绿空间；横向连接分别面向中关村科技服务与小月河社区场景。十三个面要素按十类国土空间分类代码形成当前概念模型的无缝覆盖，但不是法定用地调整。[data:geometry/land_use.geojson#LU-001] [metric:land_use_coverage_ratio]
...
法定开发强度、建筑高度、密度、退界和停车指标继续标记为待正式数据补齐。当前 `buildings.geojson` 含三个临时重点区内的 127 个低置信度概念体量，可复算方案基底、层数和设计模型建筑面积，但不能被解释为现状建筑普查或建设量承诺。[data:geometry/buildings.geojson#BLDG-0001] [metric:building_feature_count]
...
建筑规模只使用新空间模型的可复算输出：127 个概念体量的总基底约 475,392 平方米，设计模型总建筑面积约 2,525,728 平方米，三重点区合计设计模型容积率为 0.683943。它们服务于形态比较，均为低置信度、非现状、非法定指标；正式 `floor_area_ratio`、建筑密度和建筑高度继续为空值。原稿中无法由权威几何复算的建筑数量、高度序列和运营比例已全部撤出。[metric:design_model_total_floor_area_sqm] [metric:design_model_key_area_floor_area_ratio]
...
交通策略按“地面慢行先贯通、公共交通接口再校核、机动车组织服从安全与服务”的顺序推进。当前 `roads.geojson` 含一条连续绿道、两条南北向概念街、八条东西向共享支路中心线及一个合并道路面，用于表达设计网络，不能解释为现状道路红线、轨道线位、停车供给或工程线形。正式深化需要客流、路网、交叉口、公交站点、轨道保护、消防和停车调查。[data:geometry/roads.geojson#ROAD-CL-001] [metric:road_centerline_length_m]
...
当前模型把连续公园绿地、十二处公共空间面和十二个场景节点作为可复算方案图层：绿地比例约 10.77%，公共空间面比例约 0.92%。这些是临时边界上的设计模型值，不是法定绿地或公共空间指标；正式边界或空间方案改变时必须重新投影计算。[data:geometry/green_space.geojson#GREEN-001] [metric:green_ratio]
...
长期运营建议包括季度场景复盘、年度公开目录、开发者驻留、遗产讲述和“明德季”公共活动。活动名称、日期、主办方和资源均未确定；其价值在于形成“申请—试点—评估—记忆”的持续机制，而非把一次性节庆写成政府承诺。[standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK] [source:DATA-SRC-AGENT-TASKBOOK-20260518]

## 指标体系、面积复算与合规矩阵

三项核心空间指标由提交几何在 EPSG:4548 中复算：场地面积约 11,412,825 平方米，绿地比例 0.107678，公共空间面比例 0.009234。用地和分期均实现临时边界内无缝、无重叠的完整覆盖；这些仍是低置信度设计模型值，但公式、来源和重算触发可核查。[metric:site_area_sqm] [metric:land_use_coverage_ratio] [metric:phasing_coverage_ratio]

| 指标族 | 当前可用状态 | 评审用法 |
|---|---|---|
| 场地、绿地、公共空间 | 几何可复算，边界为临时 | 比较方案内部空间分配；不得当作法定指标 |
| 重点区、地标与场景节点 | 结构化要素可计数 | 检查空间任务覆盖；不表示落地数量 [metric:scenario_node_count] |
| 建筑体量 | 127 个低置信度概念体量可复算 | 比较三重点区形态；不代表现状或法定容量 [metric:building_feature_count] |
