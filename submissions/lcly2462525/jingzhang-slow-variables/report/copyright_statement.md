...
工具链与统一核验方式：全部图件由本地 matplotlib 脚本（`/Users/clara/jingzhang-viz/`，含 `fig01`–`fig06`、`jzmap.py`、`style.py`）程序化生成，坐标经 EPSG:4326→EPSG:4548 投影（pyproj）。脚本 import 语句可复核：未导入 openstreetmap、contextily、cartopy 或任何瓦片/底图库，即全部地图为无底图、几何来自本包自绘 GeoJSON 的原创表达，不涉及 © OpenStreetMap contributors 署名义务。图内文字使用 macOS 系统字体 Songti SC / PingFang SC / Menlo 渲染（Apple 系统字体随 macOS 许可使用，不单独再分发字体文件，详见第 7 节）。各文件哈希见 `manifest.json`。
...
字体说明：当前版本使用系统字体栈（Songti SC / PingFang SC / Menlo，经 `font-family` 声明调用，无 `@font-face`）。交付版本将内嵌 Noto Sans SC 子集字体（SIL Open Font License 1.1，来自 notofonts/noto-cjk 开源项目）；OFL 1.1 允许子集化与文档嵌入，该内嵌符合许可条款，详见第 7 节。
...
| Noto Sans SC（子集） | 交付版本 HTML 内嵌显示字体 | SIL Open Font License 1.1（notofonts/noto-cjk 开源项目） | OFL 1.1 明确允许子集化与文档内嵌入；仅嵌入字形子集，随 HTML 一同分发符合条款 | 字体来源项目公开可查；当前 HTML 尚无 `@font-face`（可 grep 复核现状），内嵌后可在文件头注释中核对子集声明 |

### 8. 代码依赖许可

| 依赖 | 用途 | 许可 | 可核验方式 |
| --- | --- | --- | --- |
| matplotlib | 图件绘制与 PDF 排版 | PSF 风格许可（matplotlib license） | `/Users/clara/jingzhang-viz/style.py`、`make_drawings.py` 等脚本 import 语句 |
