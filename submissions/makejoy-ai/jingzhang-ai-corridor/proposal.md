...
summary: "京张AI创新带城市设计正式方案:以京张遗址公园为主轴的记忆与进化城市智能体带;基于 provisional boundary 生成,保留精度警示与复算要求,组织方数据缺口不阻断内容评分。"
...
方案按照公告确定的三个层次组织工作：统筹研究范围关注 43.6 平方公里的AI产业生态、战略定位、创新链和未来城市形态；总体设计范围关注 11.4 平方公里京张遗址公园周边 1-2 公里城市地区和产业区，要求形成城市更新总体框架、产业空间布局、交通市政支撑和城市风貌控制；重点区域范围关注 369.3 公顷三处详细设计地区，要求明确功能业态、建筑规模、拆改留分类、公共空间连通和交通组织。三层范围在 `compliance_matrix.json` 中逐条映射，保证公告 1.3、1.4、1.5 与 agent.1-agent.6 的必选任务都有章节、图层、指标、图纸和 HTML 证据。
...
**量化设计说明（用地与建筑）**：按重算后的 `metrics.json`，用地覆盖面积为 11,412,833 平方米（land_use_coverage_sqm，覆盖提交边界全域、无缝无重叠），建筑基底面积为 567,581 平方米（building_footprint_area_sqm，为 30 栋真多边形建筑基底并集）。建筑基底约占总体设计范围的 4.97%，体现“低密度、高绿量、强场景”的产业园区基调：三处重点片区以占总体设计范围约 32% 的用地承载约 56% 的建筑基底（30 栋中 17 栋位于片区内；片区外 13 栋为 mixed_use 更新建筑，用于沿走廊的既有更新与站点周边织补），片区内建筑密度约为片区外的 2.7 倍，体现“集中投放、沿廊织补”的布局逻辑，为蓝绿空间、公共空间和慢行网络留出连续场地。容积率（floor_area_ratio）与建筑密度（building_density）因官方控规条件缺失而保持 `status=unknown`（见 assumptions A-FAR-001、A-DENSITY-001），正文不给出伪精确数值；正式控规条件到位后按 `总建筑面积/官方用地面积` 与 `建筑基底/官方用地面积` 复算。用地分类依据 [standard:MNR-LAND-USE-CLASSIFICATION-GUIDE]，拆改留方法由 [depth:retain_renovate_demolish] 管理。
## 交通、轨道、市政与公共服务设施
...
**量化设计说明（交通与道路）**：道路中心线按实际路网走向加密为 5 条、共 50 个折点（每条 9-11 个折点），反映北五环辅路、京张走廊主轴、清河路、知春路及五道口-大钟寺联络线的转折与走向；按 24 米缓冲带裁剪到提交边界后，道路面积为 1,251,569 平方米（road_area_sqm，confidence=low，见 assumptions A-ROAD-001），约占总体设计范围的 11.0%。这一比例支撑“轨道站点一体化+道路微循环+慢行断点缝合”的交通策略：主干路网只承担结构性联系，站点周边通过加密支路和慢行优先的街道断面实现高可达性。道路红线、管线与市政条件缺失时以 [depth:traffic_rail_slow_parking] 与 assumptions 说明待补，不冒充审定条件。
## 蓝绿空间、公共空间与城市风貌
...
**量化设计说明（蓝绿与公共空间）**：绿地为 1,764,847 平方米（green_space_area_sqm），绿地率 15.46%（green_ratio）；公共空间为 132,868 平方米（public_space_area_sqm），公共空间比例 1.16%（public_space_ratio）。两者与道路 24 米缓冲带和建筑基底互相裁齐，边界不穿越（空间复核冲突为 0）。绿地率反映“以京张遗址公园活力带为骨架、清河/小月河为廊道”的蓝绿结构：15% 以上的连续绿地支撑开放测试、低碳算力展示与日常交往；公共空间以三处重点片区入口广场为主，占比不高但紧邻轨道站点与产业节点，保证可及性。若与《城市居住区规划设计标准》等规范的绿地率下限比较，本方案作为产业园区+更新地区采用差异化标准，正式实施时以控规绿地率为准（见 assumptions A-BOUNDARY-001）。
## 更新项目清单、实施政策与分期计划
...
**量化设计说明（总体指标与合规）**：总体设计范围面积 11,412,825 平方米（site_area_sqm，约 11.4 平方公里，复算自 `geometry/site_boundary.geojson`），三处重点区域合计 3,692,893 平方米（key_area_area_sqm，约 369.3 公顷，key_area_count=3），分期总面积 11,412,833 平方米（phasing_area_sqm，与用地覆盖一致）。以上数值均按几何精度取整到平方米：临时粗略边界约 10 米坐标分辨率，面积精度只到百平方米量级，故不在小数位制造精确感（见 assumptions A-BOUNDARY-001）；官方边界到位后，几何、指标与图件由确定性脚本整体复算；脚本随工作区保留，不随提交包分发。全部 known 指标均可从 `geometry/` 图层复算，unknown 指标（容积率、建筑密度）的原因与复算路径见 `assumptions.json`。
## 风险、版权与合规说明
