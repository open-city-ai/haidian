...
**A 类决策映射的性质声明（Q1/Q2 执行判断明示）**：本方案的 Q1（「园作为可引用原型」定位）与 Q2（园规制度载体的差异化主张）所对应的 A 类决策选项（Q1=10A-A1「公共性基线」、Q2=10A-A1「园规可达扩展」——前缀 10A- 为用户 2026-08-28 01:48 批准的「10 A 决策」清单条目编号，与原创性 A.1/A.2/A.4 表标签不混用），系**本节点（haidian-base 项目总管）在执行层面的判断映射**，非用户决策原文——用户「10 A 决策」清单未逐一枚举 Q1/Q2 的三选一选项。此声明用于原创性终验的「A 类落地双验」口径：决策原文缺环由本声明补足，评级如因该映射存疑而影响上调，责任归属明确为本节点执行判断而非用户授权范围外主张。
...
**四类被避开母题的转译代价自认（v2 Q3 兑现）**——本方案有意避开「轴脉脊 / 铁路图解 / 口号化 / 缝合」四类流行母题，各自的转译代价诚实自认如下：

| 被避开母题 | 本方案改用 | 转译代价（自认） | 代价的承担方式 |
|---|---|---|---|
| 轴 / 脉 / 脊（空间主轴叙事） | 三园两翼 + 园径主脊（结构复合体，§5.2） | **命名差异非结构突破**——「翼 + 径 + 园」三件结构替换「主 + 副 + 节点」范式，是命名替换与功能分化的复合（§5.2 明示否认句）；代价 = 失去「单轴统领」的即时可读性 | 以「三层范围 + 六项目族」的分层可读性补偿 |
| 铁路图解（铁轨/车厢/站台的形态直译） | 节气驿站序列 + 园径铺装语汇（谱系呼应，§11.2） | 代价 = 放弃铁路形态符号的直接可识别性，站台/铁轨意象仅存于叙事层（京张老线作为谱系背景引用，§11.1） | 以「百年京张叙事」的文化谱系补偿，不复制形式语言 |
| 口号化（把治理写成标语） | 园规 R1–R5 + 三件空间载体（园门契约/园丁岗/养护日志，§12.2） | 代价 = 失去口号的高传播效率，治理主张需经制度—空间载体才能被感知 | 以「治理作为基础设施」的可感知性补偿 |
| 缝合（把更新写成「缝合」动作） | 两翼的界面组织 + 园径贯通（§6.2 西翼/东翼界面节点） | **语义同源**——「缝合」的本意（连接断裂处）与两翼界面组织同源，但本方案不以「缝合」为关键词（§3.4 措辞纪律），改用「界面节点」；代价 = 失去缝合一词的批判性张力 | 以界面节点机制的明确性补偿 |

**Q1 最小单位并列表（原创性终验 A.1 兑现）**——以官方公告的原始功能表述为最小单位（A1 行），逐行列出「去掉园字后不可还原为官方输入的增量」，证明「园」的差异化主张可还原：

| 官方原始表述（A1 最小单位） | 去掉「园」字后（可还原为官方输入） | 本方案的「园」增量（不可还原部分） |
|---|---|---|
| AI 创新生态体系 | AI 创新生态体系 | 以「园」为生态体系的空间原型——造园/学园/市园各承载一段要素循环，使生态体系有可漫步的实体形态 |
| 建设适配 AI 新质生产力的新型城市形态 | 建设适配 AI 新质生产力的新型城市形态 | 以「园」为城市形态原型——三园两翼 + 园径主脊，把「适配 AI」落为可感知的空间结构 |
| 打造全球 AI 创新人才向往的高品质城区 | 打造全球 AI 创新人才向往的高品质城区 | 以「园」为人才社区原型——学园近校转化、园丁岗职业化、荣誉市集展示，把人才向往落为可参与的公共界面 |
| 统筹研究范围 | 统筹研究范围 | 无增量（该条为范围界定，不承载差异化主张） |
| 总体设计范围 | 总体设计范围 | 无增量（范围界定） |
| 重点区域范围 | 重点区域范围 | 无增量（范围界定） |
| 众智园 AI 自主创新加速区 | 众智园 AI 自主创新加速区 | 以「园」为自主创新原型——造园试制温室/测试田/果实平台构成「育种—试制—发布」闭环 |
| 北京 AI 原点社区 | 北京 AI 原点社区 | 以「园」为原点社区原型——学园课间走廊/公共讲台/移栽机制构成「育苗—转化—共生」闭环 |
| 大钟寺 AI 产业聚集区 | 大钟寺 AI 产业聚集区 | 以「园」为产业聚集原型——市园开市钟/摊档/荣誉市集构成「收成—展示—交易」闭环 |

**Q2 三层差异表（原创性终验 A.2 兑现）**——把「园规制度载体」的差异化主张按原则/操作/空间三级，与 ≥4 个国际 AI 治理框架逐项对位（证明差异可还原、可比较、非口号）：

| 层级 | 本方案主张（园规 R1–R5 + 三件套） | EU AI Act | NIST AI RMF | UNESCO AI Ethics Rec. | OECD AI Principles |
|---|---|---|---|---|---|
| **原则级** | 园规五则：可见 / 可退 / 可停 / 可问责 / 可复核（R1–R5） | 风险分级（不可接受/高风险/低风险） | 治理/映射/度量/管理四功能 | 比例性、问责、透明、人类监督 | 以人为本、透明、问责、稳健 |
| **操作级** | 园门契约注册制（责任链路三方公示）、园丁岗人工并行位（R2）、停机协议（R3）、养护日志（R5） | 高风险系统需 CE 标识 + 人工监督 + 日志 | AI 风险映射表 + 人工监督 + 事件记录 | 影响评估 + 人工介入 + 问责链 | 国家政策框架 + 人类代理 + 存证 |
| **空间级** | 园门契约 = 园门物理界面；园丁岗 = 驿站/摊档/门厅实体位；养护日志 = 公示墙（S-12） | 无空间对应（法规文本层） | 无空间对应（组织流程层） | 无空间对应（伦理原则层） | 无空间对应（政策原则层） |
| **对照结论** | 园规把 AI 治理落为**可漫步、可感知、可参与**的空间制度——这是国际框架在原则/操作级之外的**空间级增量** | — | — | — | — |

**官方三区职能原文对照（原创性终验 A.4 兑现）**——本段为「三区职能转译为三园原型」主张的对照核验。**对照基座**：「官方原始表述」列以 **§6.1 / §6.2 / §6.3 官方职能锚内嵌的 direct_public 引文为逐字基座**（§17.3 来源等级表将三区官方定位文本列为 direct_public 档、可逐字核对；引文均带 `[standard:PROJECT-OFFICIAL-ANNOUNCEMENT §1.5(3)-N)]` 锚），不以转述替代。边界披露：官方公告**全文条款**不在本工作区（标准矩阵存引用标记与结论，未逐字转载），故完整职能条款级的逐字对照暂不可达——下表逐字基座列仅收录正文已内嵌、可直接核对的官方引文片段，其余以方向性对照承担；全文到达后的升级路径已登记为 §14 触发表 **T-9**：

| 官方三区（area_id） | 官方原始表述（逐字基座，§6.x 职能锚内嵌引文） | 本方案三园原型 | 转译方式（可核验） |
|---|---|---|---|
| 众智园 AI 自主创新加速区 | 「更具智慧型与未来感的花园型人工智能创新街区」「抓好国家人工智能平台契机，推进标准制定与安全治理」（§6.1 职能锚，[standard:PROJECT-OFFICIAL-ANNOUNCEMENT §1.5(3)-1)]） | 造园（Making Garden） | 「育种—试制—发布」要素循环（§6.1）：试制温室 / 测试田 / 果实平台 |
| 北京 AI 原点社区 | 「围绕清华、北大、中科院等高校原始创新策源成果，规划孵化区与转化区」「建设人才特区、成果转化、开源体系、品牌活动」（§6.2 职能锚，[standard:PROJECT-OFFICIAL-ANNOUNCEMENT §1.5(3)-2)]） | 学园（Learning Garden） | 「育苗—转化—共生」要素循环（§6.2）：课间走廊 / 公共讲台 / 移栽机制 |
| 大钟寺 AI 产业集聚区 | 「发挥领军企业牵引，围绕智能体、智能终端、内容消费等 AI 原生与 AI+ 融合赋能新业态」「探索数据要素与数字资产流通机制」（§6.3 职能锚，[standard:PROJECT-OFFICIAL-ANNOUNCEMENT §1.5(3)-3)]） | 市园（Market Garden） | 「收成—展示—交易」要素循环（§6.3）：开市钟 / 摊档 / 荣誉市集 |

对照结论：三区命名与核心职能的官方原始表述已在 §6.x 职能锚内嵌（direct_public 档、逐字可核），本对照表以此为**逐字基座**完成可核验的方向性对照；官方公告**全文条款**到达后，本表按逐字口径升级（触发器 §14 **T-9** 已登记，见 §17.2 假设—触发联动）。
...
**留白用地管控条目（LU-002 南门户战略留白 + LU-026 北过渡战略留白，合计约 4.3% 场地）**：两处留白地按「自然资发〔2023〕234 号」语境的战略留白机制管控——**保留远期决策弹性、不做早期承诺**。管控三则（概念建议）：①**禁建清单**——留白地内不落永久性构筑物与不可逆工程（可拆卸铺装、无地下构筑、可复原设计除外）；②**触发条件**——留白地启动正式开发须经 A-TRIGGER-001 升版联动（官方边界发布 + 存量权属核查后由专业团队评估）；③**复核节点**——留白地状态纳入 §14 触发表与 R5 年度复核。位于 LU-002 内的「南门户·迎客广场」（PUBLIC-008）按可逆性设计落位（见 public_space.geojson `reversibility_note`），不构成对留白地的早期承诺锁定。
...
- **S-13 智能体沙盒**（任务书重点场景）：智能体（含 B 系列业态所涉购物 / 服务 / 运营智能体）进入公共场景前的受控隔离验证单元——常设单元设于造园测试田围合空间：计算资源配额与网络隔离、数据流入流出白名单、外部智能体协议版本以园门契约登记（版本变更重注册）、危险操作回滚与误用兜底机制；**B 系列智能体类业态（B-01 等）须先经沙盒验证合格方可入园部署**——「先立规、后进场」纪律对智能体同样适用；边界：沙盒内测试数据默认脱敏回流 S-03（R5），沙盒运行状态接入养护日志（R3 可停适用于沙盒自身）。**一期治理过渡舱（分期衔接，2027–2028）**：因造园测试田属三期（PHASE-003，2029 起），一期（PHASE-001，市园+南门户）的 B 系列智能体业态须先在**南门户服务基座内设临时沙盒过渡舱**完成验证方可部署——过渡舱按 S-13 同套隔离/白名单/登记/回滚纪律运行，明示为「治理过渡舱」（临时载体，不替代造园常设沙盒），造园测试田建成后验证职能迁移、过渡舱转为常规设施或拆除。
...
|---|---|---|
| 北京历史文化名城保护条例——总体保护范围与程序 | 园径南段（大钟寺/西直门外大街一带）位于北京历史文化名城保护范围；与三山五园核心区（香山/颐和园一带）相距约 5 km、中间隔中关村建成区——园径不穿越三山五园历史文化保护区，亦不主张直接接触其缓冲 | 园径任何干预服从名城保护总体程序；景观与视廊变化为概念建议，深化阶段由保护区规划团队校核 |
...
| 三山五园历史文化保护区保护规划 | **不直接接触**——三山五园核心区位于场地西侧约 5 km，中间隔中关村建成区；本方案不主张园径穿越其东缘缓冲、不主张朝向颐和园/香山的视廊主张 | 三山五园相关引用仅作**谱系呼应**（11.2 三道防线），不涉及保护区缓冲的物理接触或视廊主张；深化阶段若涉及保护区缓冲评估，以官方公布的保护区边界及缓冲区图为准 |
| 国家文物局重要工业遗产名录——铁路遗产类 | 京张铁路老线地上段；清华园站 | 展示利用概念建议与铁路遗产保管机构的策展规划协同；任何路基物理改造为纯概念，须独立开展遗产铁路影响评估 |
| **觉生寺（大钟寺）——全国重点文物保护单位** | **紧邻市园·大钟寺片区**（本方案精神标识「开市钟」的来源）；钟楼本体及建控地带 | 觉生寺为带内最直接的国家级文保锚点：方案对钟楼仅作**场景转译**（开市钟声 × AI 启动装置，§10.3），不主张对其本体的任何干预；深化阶段须将觉生寺建控地带核查列入 A-TRIGGER-003（与 §11.1 高度与材质纪律联动）——钟楼周边新建载体（含 BLDG-009/010 概念塔楼）的限高与退距以官方公布的建控地带为准，概念阶段按「待文保核查封顶」处理 |
...
**R6 可达——*任何一条智能路开启之前，已经有一条人可以走的路。*** 适用对象：每项面向人的智能服务，其**入口门槛**预设了数据主体未必具备的 AI 能力（个人偏好智能体、智能手机、读写能力、数字支付账户、中文流畅度、与界面匹配的感觉-认知画像）。机制设计：每张携带面向人 AI 服务的 B/S 卡，在智能路径旁列一条**并行入口路径**，在**不依赖预设 AI 能力**的前提下完成同等服务——同价、同队、同尊严；① **B-01 智能体零售**：在「个人偏好智能体比价」入口旁，店员打印价目表+触觉样品盘；② **B-02 以旧换新**：在智能体参考价旁，每周更新一次的纸质参考表张贴于摊位；③ **B-03 机器人配送**：在机器人低速路径旁，等价的人工路径由机器人让行（B-03 的人优先条款已在 §7.2）；④ **B-04 策展展陈**：在 AI 策展墙旁，「当日人工策展人」轮值；⑤ **S-01~S-07 全带卡**：每张卡的「默认入口」重排为**非 AI 路径默认**，AI 路径为**附加**层（S-04~S-07 按 §7.4 包容性画像重排）。空间载体：每驿站的园丁岗即并行入口路径的物理锚点；无在岗园丁的驿站不得承载 AI-only 服务。**概念级人力测算区间（约束缺口诚实披露）**：按十二驿「四主八辅」+ 摊档群 + 门厅全覆盖的并行位网络，折算的在岗需求为**数十 FTE 量级的常设队伍**——参考区间：每主驿 2 岗 × 双班、每辅驿 1 岗 × 双班、市集摊档群与门厅按客流峰值 4–8 岗轮值，合计约 **36–72 FTE 常设**（不含认证爬坡期在培人员的非独立值守工时）；认证爬坡按 4 级分阶（40/80/120/200 学时）满编需约 **1–3 年**。该区间为**概念量级**（非人力编制承诺），但使 R2/R6 的「受训在岗人工」前置具备可核验的量级底座——园丁岗到位率 / R2 并行位覆盖率列入 R5 年度复核与 §14 三段流程③成活判定指标（后者已有「人工并行位覆盖率」，对齐即可）。边界与例外：R6 管**入口侧**，R2 管出口侧；R6 不要求每项服务都为每种可想到的访问壁垒复制——它要求**非 AI 默认**，当 AI 服务预设了数据主体可能缺乏的入口能力时。设计意图对位：把 R2 的「不抛弃任何人」从**出口**延伸到**入口**——园径是公共空间，不窄化为 AI 业内人的地盘。
...
**原型主张的可验证化（引用追踪表）**：对「园」原型的可引用性，本方案以**经验追踪**代替理论断言——随提交包公开一张原型引用追踪表（`report/archetype-citation-tracking.csv`），字段为 {日期, 引用方, 引用语境, 引用层级}，其中引用层级区分学术引用 / 方案借用 / 媒体转述三类；采用年度回填、每三年首次复盘的节奏，复盘前不对外主张任何引用数量。此路径不依赖任何特定原型理论语境——「原型成立与否」由后续实践中的引用记录回答，不由本方案自证。
...
**活动强度分层与场地纪律（运营机制设计）**：全年日历按空间影响分三层强度——**旗舰层**（三园节与全径走，动用全带公共界面）、**常态层**（月度 Demo Day、节气市集，限于园区与驿站界面）、**微活动层**（驿站轮值微活动，单点界面）。强度分层对应空间分层，使园径不必全年满负荷运转——公共空间的成熟在于平日与节庆两种状态都成立；「旗舰少而重、微活动多而轻」的塔形结构，也让运营强度与园丁计划志愿者体系的人力供给曲线相匹配。**人流分流机制（Q6.3 补全）**：①展示人流经两侧园门进出、**不经主径**——常态展示与市集活动限于园区与驿站界面（§13.1 常态层），主径保持通行；②**大事件两翼分流**——旗舰层活动（三园节、全径走）时，人流主力经西翼（中关村要素服务带）与东翼（小月河场景体验路径）两翼分流，主径仅承担连接与微活动，避免单一动线过载；③日夜错峰——日间常态运行、夜间旗舰活动错峰（§6.3 双时序运营）。三条场地纪律：①**声环境自查**——毗邻居住段（如小月河翼沿线）的活动概念方案须含声环境自查条目，夜间与清晨为概念性静音时段方向，具体时段边界留待深化阶段与属地协调（不设定管理权属结论）；②**还原交接**——每场活动结束执行「还原交接单」概念流程：设施撤场、绿地恢复、垃圾清运逐项核验，交接记录入养护日志活动栏——场地是向城市借来的，不是消耗品；③**活动可退**——日程全公开（R1），遇极端天气或安全考量可即时中止并公告（R3 人工叫停在活动层的对应），主承办方经园门契约界面登记（R4 可问责）。活动体系由此与园规同构：热闹也在治理之内。**断面图校核的下阶段归属（Q6.4 兑现）**：§10.1 配比区间（40-30-20-10 ±10）锚定的「深化阶段断面图校核」，其下阶段交付条目登记于 §14 触发表 T-8（断面图校核交付）——深化启动时按触发表推进，使配比承诺有明确的下阶段归属而非悬空。
...
- **园规试点工具**回答「规则从哪开始」：建议以众智园测试田为首个适用范围，入园即签园门契约，试点满一年由闭园复盘（13.1）出具修订建议——**规则的迭代周期与活动的年度周期同步**，使制度修订有固定的公共时刻，而非随机行政动作；**一期衔接（2027–2028）**：因测试田属三期（PHASE-003，2029 起），一期（PHASE-001，市园+南门户）的园规试点与 B 系列智能体业态验证暂以**南门户服务基座内治理过渡舱**为临时载体（按 §7.3 S-13 同套纪律运行），待造园测试田建成后迁移——治理试点的「先立规、后进场」在首期即可启动，不因常设沙盒时序而空转；
...
| T-3.5 | **治理试点启动**（一期过渡舱投用，B 系列首个智能体业态通过沙盒验证） | 过渡舱验证记录入养护日志 → 与 T-3 场景开放申报季解耦、独立推进 → 造园常设沙盒建成后迁移验证职能 | §7.3 S-13（过渡舱衔接）+ §14 园规试点工具（一期衔接） |
...
| T-8 | **断面图校核交付**（深化阶段首轮断面图：园径典型段、市集段、驿站段） | §10.1 配比区间（40-30-20-10 ±10）以断面图校核为锚——深化启动时交付首轮断面图，逐段校核配比与无障碍标准层 | §10.1（配比区间 + 断面图锚定） |
| T-9 | **官方三区职能公告原文到达**（采纳方提供公告全文，或开放官方渠道读取权限） | §3.4 A.4 对照表按逐字口径升级——「官方原始表述」列由 §6.x 职能锚内嵌引文片段扩展为全条款逐字对照 + 标准矩阵对应行同步更新 | §3.4（A.4 对照表）+ §17.3 来源等级表 |
...
**指标重叠口径声明**：两率共享同一园脊分子 **1,650,353 ㎡**（public_space 的 7 段线性公园 PUBLIC-001..007 与 green_space 的园脊段 GREEN-001/003/004/006/008/009/011 同坐标、同声明面积；占 public_space_ratio 分子的 94.5%、green_ratio 分子的 54.3%）。因此「绿地率 26.6% + 公共空间率 15.3%」不可读作两项独立成就——两指标的**并集率 ≈ 28.1%**（绿地 ∪ 公共空间 ≈ 3,213,681 ㎡），**硬质广场净率 ≈ 0.81%**（4 处 civic_plaza 合计 92,202.7 ㎡）。重叠值已机读入 metrics.json（`overlap_note_sqm` / `overlap_note` 字段），visual/index.html data-value 同步口径。

**配套值**（正文引用同一口径）：building_footprint_area_sqm = **382,190.394**（按提交几何坐标复算；不采用各面声明值合计 382,138.351，差 52 ㎡ 为顶点 5 位小数舍入）；园径主脊 GARDEN LINE ROAD-001 = **9,715.87 m**；FAR 维持 status=unknown、value=null（依赖未公开控规指标，不填占位）。**分期声明合计与场地差异披露**：phasing.geojson 三期声明面积合计 11,414,216.03 ㎡，超出场地面积 1,390.6 ㎡（+0.012%），属三期多边形顶点舍入累计（与 buildings 52 ㎡ 差异同型，按同一「先几何、后数字」纪律披露）；land_use 36 面声明合计 11,412,656.95 ㎡，低于场地 168 ㎡（−0.0015%，舍入级，切分声明成立）。**先几何、后数字、再对账**的纪律至此走完三步——上表数值以 metrics.json 为机读载体，与 visual/index.html 的 data-value 逐位一致；若官方边界正式发布（A-1 触发，17.2），全部数值随正式层重算并升版记录触发原因。
...
| A-8b | 园径慢行贯通对外部工程时序的依赖 | §10.1 冲突解决（13 号线扩能桥下空间 / 园径外环 B 案） | 首选依赖 13 号线扩能桥下空间；若不可得，启用园径外环 B 案（西侧市政路侧绕行，约 1.5–2.5 km，见 §10.1）。「连续行走」承诺线（§3.3）以外环 B 案兜底，不因外部工程时序悬空 |
...
| A-10 | 可引用史实锚点核对状态跟踪 | §11.2 谱系呼应锚点（汪菊渊 / 陈从周）引用核对 | 终稿如需实引，先核对原文与观点归属（含「造园境界」表述通行归属歧义）；本条目跟踪该核对状态——未核对前两条锚点仅作背景引用（§11.2 引用纪律） |
...
- **运维机制**：①**园丁岗常设机制**——园丁岗作为 R1–R5 的日常执行载体，由运营主体常设，编制与待遇由运营主体年度预算保障；**园丁岗编制登记为治理硬基础设施的资金科目**（与园径运维分列——概念量级见 §12.1.1 R6 人力测算区间，约 36–72 FTE 常设）；②**园门契约年度复核机制**——R5 年度复核由园丁岗主导、公众参与、第三方观察，输出年度偏差分析与下一年度升版清单。
- 概念方案只登记机制存在性与责任归属 + 概念量级区间，不对具体金额、编制数、待遇标准作承诺（依赖财政与人事专项）。
