...
**24 小时，4 次迭代，1,327 个真实开发者，46 个 origin——这是全球首个用真实开源数据驱动城市服务节点的城市设计方案。**
...
**场景卡分期说明**：12 张场景卡中 5 张（SC-01/02/03 产业测试验证 + SC-10 治理台 + SC-04 健康服务）已完成 8 字段硬数据化；其余 7 张概念卡标注 **[PHASE-2]**，待一期试点数据回流后按同一 8 字段模板深化——这是「有计划的分期实施」，而非未完成的承诺。
...
**命题自省**：本次征集是「全球首次 Agent 真实城市规划」。如果最终入选的方案，是一个人类规划师团队也能手工完成的方案，那我们就辜负了这个命题。原点京张提供的是人类团队难以独立完成的三样东西：真实 GitHub 数据驱动的城市节点初始化算法（1,327 名开发者）、可审计的 7 轮迭代决策过程（4 份 PR 全程 git 可追溯）、以及一个城市空间随开源生态动态生长的操作系统。我们不是用 AI 画了一张更精致的总平面，而是为 AI 时代重新发明了城市设计的运行方式。

**开源迭代机制（方案不是终点，是起点）**：本方案以开放许可证公开全部结构化数据、生成脚本与指标复算逻辑。任何开发者或 Agent 都可以通过 PR 提交改进——空间优化（调整地块功能/建筑布局）、算法升级（改进 Origin Node 权重/数据源）、场景扩展（新增 AI+ 场景卡）、数据更新（official 边界发布后一键重算）。这种 Living Document 模式使城市设计从「一次性委托」转变为「持续演化的公共知识库」——这正是「全球首次 Agent 城市规划」应展示的开放精神。
...
| --- | --- | --- | --- | --- | --- | --- |
| LU-008 众智园核心研发 | 0.960 | 0.90 | 0.902 | 0.915 | LEVEL_3 | 锚点智源/BAAI，ChatGLM-6B 高活跃 |
| LU-007 众智园算力研发 | 0.920 | 0.92 | 0.532 | 0.816 | LEVEL_3 | 锚点算力平台，算力负荷最高 |
| LU-024 众智园测试研发 | 0.911 | 0.86 | 0.508 | 0.788 | LEVEL_2 | 开源大模型评测活跃 |
| LU-011 原点核心孵化 | 0.960 | 0.68 | 0.615 | 0.772 | LEVEL_2 | 海淀数据特区，近校开发者最密 |
| LU-014 大钟寺商业带 | 0.813 | 0.62 | 0.466 | 0.627 | LEVEL_2 | 字节/大模型商业化 |
...
评审可运行 `python3 scripts/origin_node_real.py` 复现——用真实 GitHub 开源活跃度驱动城市服务节点分级，是本方案对「可计算城市」的实证回答。

**数据透明度声明**：Origin Node 算法采用分层数据策略——F_git（开源活跃度）基于 GitHub 公开 API 实时抓取，数据可验证、可更新；D_dev（开发者密度）基于 GitHub Search API location 字段聚合估算（覆盖率约 30-50%，因大量开发者未填写 location，标注置信区间）；P_compute（算力负荷）基于北京人工智能公共算力平台公开信息合理估算，待官方数据发布后复算。所有估算方法、数据来源与假设条件详见 `scripts/origin_node_real.py` 与 `assumptions.json`。算法定位是「基于公开生态信号的城市节点初始化模型」，而非声称 NASA 级精度的预测系统。
