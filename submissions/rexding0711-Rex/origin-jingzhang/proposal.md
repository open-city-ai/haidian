...
**24 小时，4 次迭代，1,327 个真实开发者，46 个 origin——这是基于真实开源数据驱动城市服务节点分级的城市设计方案。**
...
| 评审可能的问题 | 原点京张的回答 | 可核验成果 |
| --- | --- | --- |
| AI 如何参与城市设计？ | Agent 不是画图工具，而是城市演化的共同设计者 | 6 次 PR 迭代记录、审计白皮书、Origin Node 算法 |
| 空间策略是什么？ | 一公里 AI 创新服务圈，46 个可计算地块 | 46 地块 GeoJSON、119 建筑体量、服务圈覆盖 56% |
| 数据从哪来？ | 真实 GitHub API：1,327 名海淀开发者、41k★ 仓库 | `scripts/origin_node_real.py` 可一键复现 |
| 谁负责实施？ | 8 个行动包，每个含 RACI+KPI+SLO+停止条件 | 行动包清单、证据门机制（见实施章节） |
| 场景如何治理？ | 六字段协议：服务对象/最小数据/人工复核/退出申诉/停止条件 | 场景卡六字段协议（SC-10/SC-04 完整示例） |
| 失败怎么办？ | 原点·被拒绝提交碑：公开记录被 AI 建议但被人类否决的决策版本 | 被拒绝提交碑设计（见朝圣地标章节） |
| 算法会失控吗？ | Origin Node 是生成层建议，证据门+RACI 是治理层决策 | Origin Node × 证据门双层结构 |
| 边界是否可靠？ | 全部基于 provisional 边界，official 发布后一键重算 | `scripts/gen_origin_geometry.py` |
...
| 原点·被拒绝提交碑 Origin Rejected Commit Memorial | 原点碑南侧 | 城市也会产生错误版本。每一份被 AI 建议、被专家否决的方案，都记录：Commit ID / AI 建议 / 拒绝原因 / 人工裁决 / 未来启用条件。这不是展示失败，而是展示 **AI 如何被人类纠正**——「被拒绝的决策版本」与「被采纳的决策版本」同样构成城市的版本控制历史。呼应京张铁路（詹天佑原始方案中也有被放弃的备选线路）与软件版本控制（rejected commit）。原点碑记录「建成的」，被拒绝提交碑记录「被正确否决的」，两者合在一起才是完整的城市记忆 |
