---
title: "JINGZHANG SWITCHBACK LINE: Give Urban AI Signals, Switches, and Room to Turn Around"
...
The overall concept is the **Jingzhang Switchback Line**: in 1909, Zhan Tianyou's zigzag (switchback) alignment at Qinglongqiao let trains climb the steep Nankou slope by shunting back and forth, gaining momentum level by level. A century later, AI innovation is likewise not a straight upward curve — it needs the switchback loop of trial, feedback, and correction [source:AGENT-TASKBOOK] [depth:overall_spatial_structure].

The spatial "signal system" has three layers:

1. **Main Line**: the Jingzhang Heritage Park vitality corridor, the artery of history and public space, and the first "lit" demonstration segment of the signal system;
2. **Three Stations**: Zhongzhiyuan = Marshalling Yard (training/testing/compute marshalling); Beijing AI Origin Community = Departure Station (university incubation/open-source origin); Dazhongsi = Terminal Station (application deployment/industry agglomeration);
3. **Switchback Hubs**: zigzag innovation stops along the Main Line, the "legal stopping place for failure" where projects complete trial, feedback, and correction before climbing again.
...
1. **Qinglongqiao Switchback Monument**: a zigzag memorial node at the northern Main Line inscribing the engineering wisdom of the Jingzhang Railway and the names of the first Agent contributors to real urban design — the spatial origin of the "switchback is honor" narrative;
2. **Jingzhang Signal Tower**: a digital-twin observation tower at a Main Line core node whose body reads the three-state signal language (green/yellow/red), topped by the public display interface of the civic-agent dispatch console;
3. **Open-source Achievement Gallery and Agent Honor Wall**: a gallery along the Main Line continuously presenting open-source outcomes, Agent iteration records, and honor lists, updating in sync with the memorial system.

Landmarks, wayfinding, logos, fonts, images, portraits, and enterprise marks must be rights-cleared; landmarks are conceptual suggestions, not approved construction [source:AGENT-TASKBOOK].
...
For formal deepening, indicators are split into three classes: (1) spatial indicators directly recomputed from submitted geometry (boundary area, green ratio, public-space ratio, building footprint, phasing area); (2) control indicators requiring official regulatory conditions or brief attachments (FAR, height, density, setbacks, road redlines, facility standards); (3) performance indicators requiring ongoing operational or industry data (AI innovation index, talent density, slow-mobility accessibility, event participation, scenario usage). The three classes enter `metrics.json`, `assumptions.json`, and `compliance_matrix.json` respectively [standard:PROJECT-OFFICIAL-ANNOUNCEMENT].
