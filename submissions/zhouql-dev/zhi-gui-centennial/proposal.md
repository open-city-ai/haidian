...
**AI 生成内容披露（v0.3，多模态护栏）**：assets/media/ 中 7 张概念图由图像生成模型（Gemini 图像模型，经 ListenHub API）生成，均为设计意图示意，**不构成现场实据或现状记录**；图面不含真实人物肖像与受版权保护素材；生成提示词与工具链已在 report/narrative.md 记录；全部图配 alt 文本。正文引用的所有事实均来自公开信源，与生成图像严格区分。**v0.3 新增 3D 交互模型**：visual/index.html 内嵌离线 Three.js 场景（一带三站六道口），几何数据由提交的 GeoJSON 构建期内嵌（运行时零远程请求、零 CDN、无追踪），Three.js r160 以 MIT 许可本地打包于 visual/assets/；支持旋转缩放与点击三站/道口查看说明；场景为概念示意，非现场实据。
