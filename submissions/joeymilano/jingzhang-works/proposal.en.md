...
This proposal treats the approximate 11.4-square-kilometre overall design area as a public-service test corridor. Zhongzhiyuan in the north contains failure under controlled conditions. AI Origin in the middle puts public information into the hands of staff and users. Dazhongsi in the south tests whether a service can survive an ordinary day of commuting, resting, and seeking help. Products move from the lab to the street. Problems return through a second chain to the person responsible.[depth:overall_spatial_structure] [data:geometry/roads.geojson#ROAD-001]

A reviewer can follow an older visitor who does not use a smartphone. Static numbering shows the direction. At Ask First, the visitor explains the task and receives a paper sheet carrying its source and date. While rereading it, the visitor finds one missing item and returns to the counter. The paper that leaves the room contains a materials list, offline destination, expiry date, and issue ID. If one of the six steps fails, the service remains on paper with staff.[source:V13-COMPLETE-TASK-RECEIPT]

Works Bay fits this journey into a reversible room. During assisted operation, AI helps staff organize dated public material. When the network goes down, the blue interfaces switch off while the counter continues to issue paper. The 1.8 m through-route and quiet recheck place remain available. Two concept views use the same camera position so that the change can be read in staff position, paper handover, and passage clearance. Their role is limited to design illustration. Site photographs, construction results, and evidence of opening remain absent.[source:V18-GENERATED-MEDIA] [source:V18-WORKS-BAY-MODES]

The trial envelope measures 6.0 m by 4.8 m. Counter prototypes test 0.75 m and 1.05 m heights. The quiet recheck position reserves a 1.5 m turning envelope. Human takeover and the public change record sit before the exit. Every dimension belongs to the trial design. Site measurement, accessibility, fire, egress, and operating approvals remain incomplete. A missing approval prevents installation and opening.[source:V17-WORKS-BAY] [metric:works_bay_design_area_sqm]

The Urban Usability Compiler performs a release check. Each of twelve services must name an owner, no-app equivalent, minimum-data rule, human takeover, stop condition, and validation metric. The Pre-opening Rehearsal keeps twelve complete fixtures, removes each of the six fields in turn, and adds six fault cards for ninety offline decisions. All ninety match their expected outcomes. Field results remain zero. The result only shows that the rule blocks the declared synthetic faults.[source:V16-PREOPENING-REHEARSAL] [metric:preopening_expected_match_count]
...
![Concept land use and public-service supply](assets/figures/land-use-structure.en.png)
...
![Mobility, blue-green space, and human takeover network](assets/figures/mobility-bluegreen.en.png)
