...
本方案把约 11.4 平方公里的总体设计范围当作一条公共服务试验走廊。北段众智园先在受控环境里找出故障，中段 AI 原点把公开信息交到工作人员和使用者手里，南段大钟寺检验一项服务能否经得住通勤、休息和办事的普通一天。产品从实验走向街道，遇到的问题沿另一条服务链回到责任人手里。[depth:overall_spatial_structure] [data:geometry/roads.geojson#ROAD-001]

评审可以跟着一位不用智能手机的老年访客走一遍。他在静态编号前看懂方向，到先问台把事情说清，从工作人员手里接过一张写有来源和日期的纸。坐下重看时，他发现缺了一项材料，回到柜台补问。离开时，这张纸带着材料清单、线下地点、信息到期日和问题编号。六步少了一步，整项服务就留在纸质和人工状态。[source:V13-COMPLETE-TASK-RECEIPT]

“好用服务间”把这段路装进一间可撤回的房间。正常协助时，AI 只帮工作人员整理有日期的公开资料。网络断开后，蓝色界面熄灭，柜台继续交付纸张，1.8 米通行带和安静复核位照常可用。两张同视角概念图把变化落到人的站位、纸张和通行路径上。它们只用于说明设计状态，现场照片、施工效果和开放证明仍为空。[source:V18-GENERATED-MEDIA] [source:V18-WORKS-BAY-MODES]

试装范围为 6.0 米乘 4.8 米。柜台测试 0.75 米与 1.05 米两种高度，安静复核位预留 1.5 米回转范围，出口前放置人工接手和公开改进记录。所有数字都来自设计试装。场地测量、无障碍、消防、疏散和运营许可尚未完成，任何一项缺失都不能搭建开放。[source:V17-WORKS-BAY] [metric:works_bay_design_area_sqm]

城市可用性编译器只做放行检查。十二项服务都要写清责任主体、无 App 等价路径、最少数据、人工接手、停止条件和验证指标。开门前排演器保留十二个完整样例，再逐项拿掉六个字段，并加入六张故障卡，共做九十次离线检查。九十项判断与预期相符，现场结果仍为零。这个结果只说明规则挡住了预设的合成错误。[source:V16-PREOPENING-REHEARSAL] [metric:preopening_expected_match_count]

九十天开工账列出六项费用对象。现场基线、纸质服务、可移动界面、受控测试、值守运行和永久工程都有计量方式、重复费用、询价门槛与撤场责任。`planned_quantity` 和金额保持空值，因为场地、开放时段、责任采购人和可比报价尚未确认。六项费用均已登记，空值会阻止合计和采购。[source:V11-IMPLEMENTATION-PLAYBOOK] [metric:startup_cost_line_count]
...
开工账先算计量方法，再进入报价。现场基线按六条完整路线和十二个定点观察计量。纸质服务按三个设点纸包和一份无障碍母版计量。可移动界面以一套 6.0 米乘 4.8 米可撤回试装为单位。受控测试一次只放一个产业测试进入窗口。值守成本以八小时参考开放班次的 21.5 个功能工时比较，实际人数、班表和劳动规则留给获授权运营方确认。永久建设在九十天内计为零，待正式项目另行测绘、设计、审批和投资决策。[source:V19-WORKS-BAY-SHIFT-BOOK] [metric:quote_ready_cost_line_count]

| 开工成本行 | 数量怎样确定 | 一次投入或持续成本 | 允许询价的前置证据 |
| --- | --- | --- | --- |
| C01 现场基线 | 六条任务路线加十二个定点观察 | 一次投入 | 路线清单、隐私边界、证据验收单 |
| C02 纸质服务 | 三个设点纸包加一份无障碍母版 | 持续成本 | 信息责任人、到期规则、印制规格、每周替换办法 |
| C03 可移动公共界面 | 一套 6.0 米乘 4.8 米可撤回试装 | 一次投入 | 场地授权、无障碍实测、消防复核、撤场办法 |
| C04 受控测试窗口 | 同一时段只容纳一项产业测试 | 一次投入 | 测试边界、具名接手、实体急停、恢复押金 |
| C05 值守运行 | 每个八小时参考班次 21.5 个功能工时 | 持续成本 | 实际人数、班表、替岗人员、劳动规则、停用权 |
...
参考班次从开门前六十分钟开始，到八小时公共开放结束，再留四十五分钟核对问题和删除到期测试数据。值班负责人走完整条路线。纸质服务人员先用纸办完同一件事。无障碍复演人员在开场和班中各走一遍。安全故障人员试一次实体停止。信息数据人员核对来源、到期日和关场删除。五类工作合计 21.5 个功能工时，运营方仍需按真实劳动规则决定由几个人、怎样轮换和怎样休息。[source:V19-WORKS-BAY-SHIFT-BOOK]
