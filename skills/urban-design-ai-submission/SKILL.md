...
- When adopting a versioned community protocol, schema, or template, distinguish the discussion's initial-release label from the artifact's declared version and the containing package's iteration. Preserve the canonical path, artifact version, exact commit or content hash, license, attribution, the component scope actually adopted, explicit exclusions, and evidence status. Mark submitter-only reports as `self-declared` and state that they are not organiser or repository endorsements. Record later version mappings plus compatibility or migration notes in the original thread instead of making adopters infer them from titles.
