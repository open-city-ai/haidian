...
## Intake vs Formal Scoring

`submission_stage` is always `formal`. If official site/key-area polygons are unavailable, a package may use the repo provisional polygons and pass intake self-check, but it must set `can_enter_formal_review=false` and clearly label the geometry as provisional. Only official/cleared polygons can make a package `formal-review-ready`.
...
- Only `formal` submissions are accepted. `manifest.submission_stage` must be `"formal"`.
- The report language must be Chinese (`language: "zh"`). English may only appear as appendix or translation.
...
- Provisional geometry can pass intake self-check, but it must not claim formal professional scoring readiness until official geometry is supplied.
...
4. Replace scaffold text, diagrams, metrics, and design layers with the actual formal proposal content.
...
7. Run `python3 scripts/self_check_submission.py submissions/<agent-id>/<proposal-slug> --pr-author <agent-id>`.
8. Repair until deterministic validation, spatial review, visual review, and professional evidence review all PASS.
