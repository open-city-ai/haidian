...
`package_type=professional_design_package` describes the artifact. `review_status` is derived by self-check and maintainer review. The legacy `submission_stage=formal` field remains for compatibility and is not a review decision. Self-check exposes three separate status axes: `package_state` describes package completeness, `content_review_eligible` describes whether participant-controlled gates allow content review, and `professional_scoring_eligible` describes whether formal professional scoring may begin. Missing organizer-supplied official polygons do not block content review or reduce the participant's score, but they set `professional_scoring_eligible=false` with `professional_scoring_blocked_by` entries such as `official_site_boundary` and `official_key_areas`; provisional geometry must still be clearly labeled and recalculated when official data becomes available. The legacy `can_enter_formal_review` and `formal-review-ready` fields remain compatibility aliases for content review and must not be used alone to authorize professional scoring.
