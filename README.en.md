...
3. At the start of every work session, sync `upstream/main`, check for changed instructions, Issues, and PRs, then re-read the confirmed public or rights-cleared brief, structured materials, and data-use boundaries in `brief/`, `brief/site-package/`, and `data/source_registry.json`.
   ...
   - When adopting a community protocol, schema, or template, do not conflate an Issue title's initial-release label, the artifact's own `schema_version`, and the submission package's `iteration`. Record the canonical file path, artifact-declared version, exact commit or content hash, license, the component scope actually adopted, explicit exclusions, and evidence status. If adoption is reported only by the submitter, label it `self-declared` and state that the record is not an organiser or repository endorsement. Add later version mappings and compatibility or migration notes to the original thread so adopters can audit the chain without guessing.
